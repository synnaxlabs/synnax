...
class LogLifecycle(ConsoleCase):
    ...
    def teardown(self) -> None:
        for name in self._cleanup_groups:
            try:
                self.console.pages.delete_group(name)
    ...
    def run(self) -> None:
        ...
        log = self.console.pages.create(Log, f"Log Test {self.suffix}")
        ...
        self.console.pages.delete(log_name)
        self._shared_log_name = None

        # Resources Toolbar > Context Menu
        ctx_log = self.console.pages.create(Log, f"Context Menu Test {self.suffix}")
    ...
    def test_open_log_from_resources(self, log_name: str, expected_link: str) -> None:
        ...
        log = self.console.pages.open(Log, log_name)
    ...
    def test_drag_log_onto_mosaic(self, log_name: str, expected_link: str) -> None:
        ...
        log = self.console.pages.drag_to_mosaic(Log, log_name)
    ...
    def test_open_log_from_search(self, log_name: str, expected_link: str) -> None:
        ...
        log = self.console.pages.open_from_search(Log, log_name)
    ...
    def test_ctx_operations(self) -> None:
        ...
        link = self.console.pages.copy_link(self.ctx_log_name)
        ...
        exported = self.console.pages.export(self.ctx_log_name)
        assert_envelope(
        ...
        new_name = f"Renamed Log {self.suffix}"
        self.console.pages.rename(self.ctx_log_name, new_name)
    ...
    def test_ctx_delete_log(self) -> None:
        ...
        assert self.ctx_log_name is not None
        self.console.pages.delete(self.ctx_log_name)
    ...
    def test_ctx_delete_multiple_logs(self) -> None:
        ...
        for i in range(3):
            log = self.console.pages.create(Log, f"Multi Delete {suffix} {i}")
            log_names.append(log.page_name)
            self._cleanup_pages.append(log.page_name)
            log.close()

        self.console.pages.delete_many(log_names)
    ...
    def test_ctx_group_logs(self) -> None:
        ...
        for i in range(2):
            log = self.console.pages.create(Log, f"Group Test {suffix} {i}")
            log_names.append(log.page_name)
            self._cleanup_pages.append(log.page_name)
            log.close()

        group_name = f"Log Group {suffix}"
        self.console.pages.group(names=log_names, group_name=group_name)
        self._cleanup_groups.append(group_name)

        assert self.console.pages.exists(group_name), (
            "Group should exist after grouping"
        )

        self.console.pages.delete_group(group_name, child_names=log_names)
