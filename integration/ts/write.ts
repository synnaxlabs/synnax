...
async function writeTest(tc: TestConfig): Promise<void> {
...
function parseInput(argv: string[]): TestConfig {
  ...
  const numWriters = parseInt(argv[argvCounter++], 10);
  const domains = parseInt(argv[argvCounter++], 10);
  const samplesPerDomain = parseInt(argv[argvCounter++], 10);
  const timeRangeStart = parseInt(argv[argvCounter++], 10);
  const timeRangeEnd = parseInt(argv[argvCounter++], 10);
  const autoCommit = argv[argvCounter++] === "true";
  const indexPersistInterval = new TimeSpan(parseInt(argv[argvCounter++], 10));
  const writerMode = parseInt(argv[argvCounter++], 10) as framer.WriterMode;
  const numberOfChannelGroups = parseInt(argv[argvCounter++], 10);
  const channelGroups: IndexWriterGroup[] = [];

  for (let i = 0; i < numberOfChannelGroups; i++) {
    const numberOfIndex = parseInt(argv[argvCounter++], 10);
    const indexChannels = argv.slice(argvCounter, argvCounter + numberOfIndex);
    argvCounter += numberOfIndex;
    const numberOfData = parseInt(argv[argvCounter++], 10);
...
async function main(): Promise<void> {
  ...
  await writeTest(tc).catch(error => {
    console.error(error)
    client.close()
    exit(1)
  })
