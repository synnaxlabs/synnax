...
describe("Frame", () => {
  describe("construction", () => {
    describe("from keys and arrays", () => {
      test("valid", () => {
        const f = new Frame([new TArray(new Float32Array([1, 2, 3]))], ["a"]);
      ...
      test("invalid", () => {
        expect(
          () => new Frame([new TArray(new Float32Array([1, 2, 3]))], [])
        ).toThrow();
    ...
    test("from record", () => {
      const f = new Frame({ a: [new TArray(new Float32Array([1, 2, 3]))] });
      expect(f.size.valueOf()).toEqual(12);
      expect(f.keys.length).toEqual(1);
      expect(f.arrays.length).toEqual(1);
    });
  ...
  describe("toPayload", () => {
    it("should return the frame as FramePayload", () => {
      const f = new Frame({
        a: [
          new TArray(new Float32Array([1, 2, 3]), undefined, new TimeRange(40, 50000)),
        ],
        b: [
          new TArray(new Float32Array([1, 2, 3]), undefined, new TimeRange(500, 50001)),
        ],
