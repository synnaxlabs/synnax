...
const createListeners = (
  ...
  schema: z.ZodType,
  onChange: Mock,
): query.Listener[] => [{ channel, schema, onChange }];
...
describe("openStreamer", () => {
  ...
  describe("Error Handling & Recovery", () => {
    ...
    it("should continue streaming after a listener throws an error", async () => {
      ...
      const listeners: query.Listener[] = [
  ...
  describe("Channel Name Sorting (Delete Operations)", () => {
    ...
    it("should handle multiple delete channels before non-delete channels", async () => {
      ...
      const schema = z.object({ id: z.number() });
      const channelListeners: query.Listener[] = [];
  ...
  describe("demand", () => {
    ...
    it("resolves a close that outraces a failing open", async () => {
      ...
      const demanded = streamer.demand().catch((e: unknown) => e);
      const closing = streamer.close();
      rejectOpen(new Error("cluster unreachable"));
      // The open's failure belongs to the demander alone; the close has
      // nothing to tear down and must not re-report it.
      await expect(closing).resolves.toBeUndefined();
      expect(await demanded).toBeInstanceOf(Error);
    });
  });
});
