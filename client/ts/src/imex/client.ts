 ...
 * payload server-side, so a single import or export is bounded by the server's available
 * memory.
 *
 ...
 *
 * - Node: `import(Readable.toWeb(createReadStream(path)), opts)` and
 *   `(await export(id, opts)).pipeTo(Writable.toWeb(createWriteStream(path)))`.
...
export class Client {
   ...
   * Blob is streamed to the Core without buffering it in client memory; an ArrayBufferView
