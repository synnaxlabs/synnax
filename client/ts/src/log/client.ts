...
export class Client {
  ...
  private readonly store_?: cache.Store<Key, Log>;
  ...
  /**
   * Read surface of the log cache.
   * @throws when the cache was disabled at client construction.
   */
  get store(): cache.Store<Key, Log> {
    if (this.store_ == null)
  ...
  /**
   * Action-dispatch controller over the log cache.
   * @throws when the cache was disabled at client construction.
   */
  get dispatcher(): dispatch.Controller<Key, Log, Action> {
