...
import { cellZ, type Table } from "@/table/types.gen";
...
export const createReduceAll = (handlers: Handlers) => {
  const reduce = (state: Draft<Table>, action: Action): HandlerResult => {
    switch (action.type) {
      case "rename":
  ...
  return (state: Table, actions: Action[]): ReduceAllResult => {
