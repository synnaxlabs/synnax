...
import { caseconv, color, notation, spatial, text, uuid } from "@synnaxlabs/x";
import { z } from "zod";
...
export type CellConfig = TextCellConfig | ValueCellConfig;

export const CELL_CONFIG_SCHEMAS: {
  [K in CellConfigType]: z.ZodType<Extract<CellConfig, { variant: K }>>;
} = {
  text: textCellConfigZ,
  value: valueCellConfigZ,
};
...
export const tableZ = z.object({
   ...
   */
  cells: caseconv.preserveKeys(z.record(z.string(), cellConfigZ).default(() => ({}))),
});
