...
np.random.seed(42)
latency_times = generate_times(latency_base_time, lower_bound_latency, upper_bound_latency, n_samples)
execution_times = generate_times(execution_base_time, lower_bound_exec, upper_bound_exec, n_samples, shift=10, scale=20)
...
def create_plot(times_ms, stats, title_prefix):
    ...
    plt.figtext(0.1, 0.92, f"NI-CRIO 9041 (1.30GHz Atom, 2GB RAM) | {n_samples} Samples | Click Looback" , fontsize=10, ha='left')
    
    # Top plot: Times over time with percentiles
    ax1.plot(times_ms, label=title_prefix, alpha=0.6)
    ax1.axhline(y=stats['p90'], color='r', linestyle='--', label=f'P90: {stats["p90"]:.2f}ms')
    ax1.axhline(y=stats['p95'], color='g', linestyle='--', label=f'P95: {stats["p95"]:.2f}ms')
    ax1.axhline(y=stats['p99'], color='b', linestyle='--', label=f'P99: {stats["p99"]:.2f}ms')
    ...
    ax2.axhline(y=stats['avg_jitter'], color='r', linestyle='--',
