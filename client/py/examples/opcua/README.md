...
- **Array variables** (my_array_0, my_array_1): Arrays of sine wave values (5 elements
  each)
...
**What you'll see**: Commands sent to the OPC UA server. Check the server terminal to
...
**Performance**: Array mode is more efficient for high-rate data when the server
provides arrays.
...
**Stream Rate**: Can match sample rate or be lower. For example, 10 Hz sampling with 10
Hz streaming sends every sample immediately.

**Subscription vs. Polling**: OPC UA typically uses subscriptions with server-side
sampling, which is more efficient than polling.
