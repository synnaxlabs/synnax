...
class TestNumpy:
    def test_basic_write(self, channel: Channel, client: arya.Client):
    ...
    def test_invalid_data_type(self, channel: Channel, client: arya.Client):
        writer = client.data.new_writer([channel.key])
        try:
    ...
    def test_invalid_data_shape(self, channel: Channel, client: arya.Client):
        writer = client.data.new_writer([channel.key])
        try:
    ...
    def test_non_contiguous_segments(self, channel: Channel, client: arya.Client):
        writer = client.data.new_writer([channel.key])
        try:
            data = np.random.rand(10).astype(np.float64)
            writer.write(to=channel.key, data=data, start=0)
    ...
    def test_multi_segment_write(self, channel: Channel, client: arya.Client):
        writer = client.data.new_writer([channel.key])
        n_samples = 1000
        n_writes = 100
        try:
    ...
    def test_segment_split(self, channel: Channel, client: arya.Client):
