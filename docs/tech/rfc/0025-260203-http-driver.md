...
channels. All endpoints are polled at the same rate on each cycle.
...
- `strict`: Task-level setting. If true, lossy numeric conversions (e.g., float 3.7 →
  int64 truncated to 3) are treated as errors/warnings depending on
  `error_on_bad_field`. If false (default), truncation proceeds silently.
- `error_on_bad_field`: If true, any bad field (wrong type, unparseable) puts task in
  error state and no values are written for that cycle. If false, bad fields log warnings
  but good values still get written.
...
- `endpoints[].channels`: Array of channel mappings for this endpoint's response
- `endpoints[].channels[].pointer`: JSON Pointer (RFC 6901) path to extract value (e.g.,
  `/data/sensors/0/value`)
- `endpoints[].channels[].type`: **Required.** Expected JSON type: `"number"`,
  `"string"`, or `"boolean"`. An error or warning is raised if the response value does
  not match this type.
- `endpoints[].channels[].optional`: If false (default), a missing field causes an error
  or warning depending on `error_on_bad_field`. If true, missing fields are silently
  skipped.
- `endpoints[].timestamp`: Optional timestamp extraction config for this endpoint. If
  omitted, software timing (request midpoint) is used.
- `endpoints[].timestamp.pointer`: JSON Pointer to extract timestamp from response
- `endpoints[].timestamp.format`: Timestamp format (see below)

**Timestamp Formats:**

- `"iso8601"` - ISO 8601 string (e.g., `"2024-01-15T10:30:00Z"`)
- `"unix_sec"` - Unix timestamp in seconds (float64)
- `"unix_ms"` - Unix timestamp in milliseconds (int64)
- `"unix_us"` - Unix timestamp in microseconds (int64)
- `"unix_ns"` - Unix timestamp in nanoseconds (int64, Synnax native)

If no `timestamp` is configured for an endpoint, the driver uses software timing:
midpoint of `(request_start + response_received) / 2`.

**Type Conversion for Read Tasks (JSON → Synnax):**

| JSON Type | → Synnax Numeric                        | → Synnax String   |
| --------- | --------------------------------------- | ----------------- |
| `number`  | ⚠️ Direct (strict: error on truncation) | ✅ String repr    |
| `string`  | ❌ Error                                | ✅ Direct         |
...
- JSON `number` → Synnax Numeric with `strict=true`: if the JSON value is a float
  (e.g., 3.7) but the Synnax channel is an integer type (e.g., int64), this is treated
  as an error/warning because the truncation to 3 is lossy.
- JSON `boolean` → Synnax Numeric: `true` becomes 1, `false` becomes 0 (as the
  channel's numeric type).

**Behavior on conversion failure:**

- `error_on_bad_field=true`: Task enters error state, no values written for that cycle
- `error_on_bad_field=false`: Log warning, skip bad value, write good values
- `strict=true`: Lossy numeric truncation treated same as a conversion failure
...
3. **Required fields**: `type` is required for every channel mapping.
...
| ------------------- | --------------- | ----------------- | ----------------------- |
| Numeric (int/float) | ✅ Direct       | ✅ String repr    | ✅ 0→false, else→true   |
...
2. **Pointer uniqueness**: Within each endpoint, no two fields may have the same pointer.
...
- `"timestamp"` - Unix timestamp in seconds (written as a JSON number)
- `"timestamp_ms"` - Unix timestamp in milliseconds (written as a JSON number)
- `"timestamp_iso"` - ISO 8601 formatted string (written as a JSON string)
...
This difference exists because HTTP endpoints are explicitly configured by users (unlike
hardware devices that can be discovered), and health check requirements vary
significantly between APIs.
...
Reusable utilities for converting between JSON and Synnax telemetry types. Only supports
the three JSON types (number, string, boolean) and two Synnax type families (numeric,
string).
...
struct ConvertOptions {
    // Fail on lossy conversions (e.g., float→int truncation)
    bool strict = false;
    // Return error vs warning on conversion failure
...
// Supported JSON types: number, string, boolean.
// Supported Synnax types: numeric (int8..float64), string.
std::pair<telem::SampleValue, xerrors::Error>
to_sample(
    const nlohmann::json& value,
    telem::DataType target_type,
...
// Supported formats: "number", "string", "boolean".
std::pair<nlohmann::json, xerrors::Error>
from_sample(
...
// Check whether a conversion is supported at configuration time.
// Returns an error if the conversion is impossible (❌ in the tables).
xerrors::Error validate_read_conversion(
    const std::string& json_type,
    telem::DataType synnax_type
);

xerrors::Error validate_write_conversion(
    telem::DataType synnax_type,
...
class Client {
    CURLM* multi_;  // libcurl multi handle
...
Default behavior (per endpoint, when no `timestamp` is configured):
...
If `timestamp` is configured on an endpoint:

1. Extract value from response JSON via the pointer
2. Parse according to the specified format:
   - `"iso8601"` → Parse ISO 8601 string (e.g., `"2024-01-15T10:30:00.123Z"`)
   - `"unix_sec"` → Interpret as float64 seconds since epoch
   - `"unix_ms"` → Interpret as int64 milliseconds since epoch
   - `"unix_us"` → Interpret as int64 microseconds since epoch
   - `"unix_ns"` → Interpret as int64 nanoseconds since epoch (Synnax native)
...
The `xjson_convert` functions are called once per channel per poll cycle. For a task
polling 10 endpoints with 20 channels each at 10Hz, this is ~2000 conversions/second.

- **Avoid heap allocations in the conversion hot path.** Numeric conversions should be
  pure arithmetic with no string parsing or memory allocation. String conversions should
  write directly into pre-allocated Series buffers where possible.
- **Pre-validate at configuration time.** The `validate_read_conversion` and
  `validate_write_conversion` functions are called once during task setup. The runtime
  `to_sample`/`from_sample` functions should not re-check whether the conversion is
  supported; they can assume the caller has already validated.
- **Branch on DataType enum, not string comparison.** Store the Synnax DataType as an
  enum value at configuration time rather than comparing type name strings on every
  sample.
...
1. Fire HTTP requests to all endpoints (sequentially per device client, since libcurl
   easy handles are not thread-safe; multiple endpoints on the same device share a
   client)
2. Parse each response body with `nlohmann::json::parse`
3. Extract fields via JSON Pointer
4. Convert values and write to Synnax
...
times/second, sending up to 30 HTTP requests/second (3 per iteration, fired in
parallel via libcurl multi).
...
from synnax.http import (
    HTTPDevice, ReadChannel, ReadEndpoint,
...
pressure_ch = client.channels.create(name="pressure", data_type=sy.DataType.FLOAT64)
...
class MockHTTPServer:
    ...
    def set_response(
        self, path: str, method: str, response: dict, status: int = 200
    ):
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
            target=self.app.run, kwargs={"port": self.port}
        )
...
import pytest
import synnax as sy
from synnax.http import ReadChannel, ReadEndpoint
...
def test_read_task_polls_endpoint(client, mock_server, http_device):
    ...
    temp_ch = client.channels.create(
        name="temp", data_type=sy.DataType.FLOAT64
    )
...
The first two PRs establish the reusable x/cpp utilities that the driver depends on.
Then each feature goes through all layers: C++ → Console → Python → Tests → Docs.

---

### Reusable Utilities

#### PR 1: JSON Pointer Utility

**Scope:** RFC 6901 JSON Pointer extraction and set operations

- `x/cpp/json_pointer/BUILD.bazel`
- `x/cpp/json_pointer/json_pointer.h` - `extract` and `set` functions
- `x/cpp/json_pointer/json_pointer.cpp`
- `x/cpp/json_pointer/json_pointer_test.cpp` - Tests for nested paths, array indices,
  escaping (`~0`, `~1`), error cases (missing keys, out-of-bounds), root pointer

#### PR 2: JSON ↔ Synnax Conversion Utility

**Scope:** Type conversion between JSON values and Synnax telemetry types

- `x/cpp/json_convert/BUILD.bazel`
- `x/cpp/json_convert/json_convert.h` - `to_sample`, `from_sample`,
  `validate_read_conversion`, `validate_write_conversion`, `zero_value`
- `x/cpp/json_convert/json_convert.cpp`
- `x/cpp/json_convert/json_convert_test.cpp` - Tests for all supported conversions,
  strict mode, error cases, validation functions, zero values

---

### Device Connection Feature

#### PR 3: C++ Device Foundation
...
#### PR 4: Console Device Connection
...
#### PR 6: Documentation - Get Started
...
#### PR 10: Integration Tests - Read Task
...
- `integration/tests/driver/http_read.py`
  ...
  - Timestamp extraction with different formats (iso8601, unix_sec, unix_ns)
  - Configuration-time validation errors (incompatible types, duplicate channels)

#### PR 11: Documentation - Read Task
...
#### PR 14: Python Client - Write Task
...
#### PR 15: Integration Tests - Write Task
...
#### PR 16: Documentation - Write Task
...
#### PR 20: Integration Tests - Scan Task
...
10. **String-to-Number Parsing**: Parse numeric strings if demand warrants the complexity
...
| JSON Convert Location | `x/cpp/json_convert/`                           | Reusable utility, not HTTP-specific                                |
...
| PR Structure          | 21 PRs (Utilities → Device → Read → Write → Scan) | Utilities first, then incremental delivery by feature           |
