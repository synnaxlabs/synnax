...
├── convert_test.cpp
├── pointer.h/.cpp                # JSON Pointer (RFC 6901) extraction (NEW)
└── pointer_test.cpp
...
- `error_on_bad_field`: If true, any bad field (wrong type, unparseable) puts task in
  error state and no values are written for that cycle. If false, bad fields log
  warnings but good values still get written.
...
- `"unix_ns"` - Unix timestamp in nanoseconds (`int64`)
...
- JSON `number` → Synnax Numeric with `strict=true`: if the JSON value is a float (e.g.,
  ...
  error/warning because the truncation to 3 is lossy. If the JSON value is too large for
  the Synnax channel type, (e.g. 344 -> )
...
- `` - top level of the JSON for non-object or array JSONs
- `/` - root
- `/foo` - field "foo"
...
three JSON types (number, string, boolean) and two Synnax type families (numeric,
string).

```cpp
namespace xjson::convert {

// Enum for JSON types/formats supported in conversion
enum class JsonType {
    Number,
    String,
    Boolean
...
// 'strict' controls whether lossy numeric conversions are allowed.
...
// Supported formats: JsonType::Number, JsonType::String, JsonType::Boolean.
// 'strict' controls whether lossy numeric conversions are allowed.
std::pair<nlohmann::json, xerrors::Error>
from_sample(
...
xerrors::Error validate_write_conversion(
    telem::DataType synnax_type,
    JsonType json_format
);

// Get zero value for a JSON format type.
// JsonType::Number → 0, JsonType::String → "", JsonType::Boolean → false
nlohmann::json zero_value(JsonType format);
