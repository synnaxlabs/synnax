...
capture the exact type combination and `strict` flag. The runtime hot path calls only the
resolved converter with no branching on `DataType` or `JsonType`.
...
resolve call itself returns an error. If it succeeds, the returned function is guaranteed
to handle the conversion correctly. Tests exercise the same code path as production.
...
avoids `telem::SampleValue` (a `std::variant` requiring `std::visit` dispatch) and
works uniformly for both fixed-size and variable-length types (`telem::Series` does not
support appending variable-length data like strings to a pre-allocated series). The
resolved function captures the exact C++ type at config time, so the returned Series
is constructed directly (e.g., `Series(double_value)`) with no runtime branching.
...
- **Read converters return a single-sample Series.** The `ReadConvertFn` constructs
  a `telem::Series` directly from the converted value (e.g., `Series(double_value)`),
