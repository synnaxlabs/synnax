...
- `endpoints[].fields[].type`: **Required.** Expected JSON type: `"number"`, `"string"`,
  or `"boolean"`. An error or warning is raised if the response value does not match
  this type.
- `endpoints[].fields[].optional`: If false (default), a missing field causes an error
...
- `"unix_sec"` - Unix timestamp in seconds (`float`)
- `"unix_ms"` - Unix timestamp in milliseconds (`int`)
- `"unix_us"` - Unix timestamp in microseconds (`int`)
- `"unix_ns"` - Unix timestamp in nanoseconds (`int`)
...
- JSON `number` → Synnax Numeric with `strict=true`: if the JSON value is a float (e.g.,
  ...
  the Synnax channel type, (e.g. 344 -> uint8(344) is lossy and will be treated as an
  error)
...
- `"timestamp"` - Timestamp value. Requires a `format` field with one of the same five
  ...
  - `"unix_sec"` - Unix timestamp in seconds (written as a JSON number, `float`)
  - `"unix_ms"` - Unix timestamp in milliseconds (written as a JSON number, `int`)
  - `"unix_us"` - Unix timestamp in microseconds (written as a JSON number, `int`)
  - `"unix_ns"` - Unix timestamp in nanoseconds (written as a JSON number, `int`)
...
polling 10 endpoints with 20 channels each at 10 Hz, this is ~2000 conversions/second.
