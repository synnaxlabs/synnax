...
**Timestamp Formats (same six options used across read and write tasks):**

- `"iso8601"` - ISO 8601 string (e.g., `"2024-01-15T10:30:00Z"`)
- `"unix_sec_float"` - Unix timestamp in seconds (`float`, preserves sub-second
  precision)
- `"unix_sec_int"` - Unix timestamp in seconds (`int`, truncates sub-second precision)
...
- `time_format`: Required when `time_pointer` is set. Timestamp format, using the same
  ...
  - `"unix_sec_float"` - Unix timestamp in seconds (`float`)
  - `"unix_sec_int"` - Unix timestamp in seconds (`int`)
  - `"unix_ms"` - Unix timestamp in milliseconds (`int`)
  - `"unix_us"` - Unix timestamp in microseconds (`int`)
  - `"unix_ns"` - Unix timestamp in nanoseconds (`int`)
...
- `"timestamp"` - Timestamp value. Requires a `format` field with one of the same five
  ...
  - `"unix_sec_float"` - Unix timestamp in seconds (written as a JSON number, `float`)
  - `"unix_sec_int"` - Unix timestamp in seconds (written as a JSON number, `int`)
...
### JSON ↔ Synnax Conversion (`x/cpp/xjson/convert.h`)
...
#### Design: Resolved Converters

The API is built around **resolved converters** — functions returned at config time that
capture the exact type combination and `strict` flag. The runtime hot path calls only
the resolved converter with no branching on `DataType` or `JsonType`.

There are no generic `to_sample`/`from_sample` functions. Validation and conversion are
both done through the `resolve_*` functions: if the conversion is unsupported, the
resolve call itself returns an error. If it succeeds, the returned function is
guaranteed to handle the conversion correctly. Tests exercise the same code path as
production.

This avoids:

- Branching on `JsonType` every sample
- Branching on `telem::DataType` (which wraps a `std::string`) every sample
- Passing `strict` every call
- Any re-validation of type compatibility at runtime
- Dead API surface that production code never calls
...
/// Resolved converters — called once at config time, returns a function
/// that captures the exact conversion path with no runtime branching.

// Signature of the resolved read converter. Takes a JSON value extracted
// from the response and returns a single-sample Series containing
// the converted value.
using ReadConvertFn = std::function<
    std::pair<telem::Series, xerrors::Error>(
        const nlohmann::json& value
    )
>;

// Resolve a read converter for a specific (JsonType, DataType, strict)
// combination. The returned function captures the exact types — no branching
// on DataType at runtime. Returns an error if the conversion is unsupported.
std::pair<ReadConvertFn, xerrors::Error>
...
// Signature of the resolved write converter. Takes a SampleValue from a
// Synnax channel and returns the JSON value to insert into the request body.
using WriteConvertFn = std::function<
    std::pair<nlohmann::json, xerrors::Error>(
        const telem::SampleValue& value
    )
>;

// Resolve a write converter for a specific (DataType, JsonType, strict)
// combination. Returns an error if the conversion is unsupported.
std::pair<WriteConvertFn, xerrors::Error>
resolve_write_converter(
    const telem::DataType& synnax_type,
...
**Why resolved converters return Series:**

The `ReadConvertFn` returns a `telem::Series` containing the converted value. This
avoids `telem::SampleValue` (a `std::variant` requiring `std::visit` dispatch) and works
uniformly for both fixed-size and variable-length types (`telem::Series` does not
support appending variable-length data like strings to a pre-allocated series). The
resolved function captures the exact C++ type at config time, so the returned Series is
constructed directly (e.g., `Series(double_value)`) with no runtime branching.

The `WriteConvertFn` returns `nlohmann::json` because the write task needs a JSON value
to insert into the request body object. This allocation is unavoidable since we're
building a `nlohmann::json` tree for serialization.
...
2. Parse according to the specified `time_format`:
   ...
   - `"unix_sec_float"` → Interpret as float seconds since epoch
   - `"unix_sec_int"` → Interpret as int seconds since epoch
...
- `integration/tests/driver/http_read.py`
  ...
  - Timestamp extraction with different formats (iso8601, unix_sec_float, unix_sec_int,
    unix_ns)
