...
The `xjson::convert` functions are called once per channel per poll cycle. For a task
...
3. Extract fields via pre-parsed `nlohmann::json_pointer`
...
- `x/cpp/xjson/convert.h` - `to_sample`, `from_sample`, `validate_read_conversion`,
  `validate_write_conversion`, `zero_value`
- `x/cpp/xjson/convert.cpp`
- `x/cpp/xjson/convert_test.cpp` - Tests for all supported conversions, strict mode,
  error cases, validation functions, zero values
