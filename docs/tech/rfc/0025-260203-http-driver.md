...
with 20 fields each at 10 Hz, this is ~2000 conversions/second — negligible compared to
HTTP I/O (milliseconds) and JSON parsing (microseconds).

- **Read side** (`to_sample_value`): Branches on JSON value type and DataType at
  runtime. No config-time resolution needed since the JSON type is inferred from the
  response data.
- **Write side** (`from_sample_value`): Uses `std::visit` dispatch on the SampleValue
  variant. `check_from_sample_value` validates type compatibility at config time so
  runtime errors are avoided for unsupported combinations.
