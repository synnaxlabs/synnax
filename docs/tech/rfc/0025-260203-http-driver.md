...
2. Parse according to the specified `time_format`:
   ...
   - `"unix_sec"` → Interpret as float seconds since epoch
   - `"unix_ms"` → Interpret as int milliseconds since epoch
   - `"unix_us"` → Interpret as int microseconds since epoch
   - `"unix_ns"` → Interpret as int nanoseconds since epoch (Synnax native)
