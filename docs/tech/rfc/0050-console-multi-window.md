...
- **Window**: An OS window managed by Drift. The **main window** owns persistence; every
  other window is an **aux window**.
- **Panel**: A project-owned document holding a tab tree; shared by everyone with access
  to the project.
...
as the main one (auth, connection, project), so RFC 0049's takeover and login regimes
