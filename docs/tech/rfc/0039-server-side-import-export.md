...
- **Author**: Emiliano Bonilla, Patrick Dotson
- **Date**: 2026-05-07
...
(`@go version`) are one sequence. Oracle generates the `imex.Version` constant from a
bare `@go imex` marker on the resource's root struct. The constant always equals the
schema version, so the two can never drift. The consequence is accepted looseness: a
storage-only bump advances the wire number without a change to the portable shape, so
adjacent numbers can share a parser in the per-resource dispatch. Where legacy Console
semver majors had outrun a resource's schema version (Arc), the schema version moves
past them.
...
**Amended (SY-4233)**: the per-schema sequence is now the resource's `@go version`, with
the constant generated by Oracle (see §4.3.0). Legacy Console semver majors map into the
same sequence through the existing `"N.0.0"` → `N` conversion; the per-resource dispatch
gives each historical number its frozen parser.
