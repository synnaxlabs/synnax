...
record becomes the task's ontology parent. Both `type` and `config` become resolved
...
4. **A closed set, loudly enforced**: every task type has a schema. Task types are
   ...
   type is a bug and the Core says so.
...
No new relationship type is needed. The existing tree walk, delete cascade, and
`parent_of` index all apply unchanged. Cardinality is exactly one config record per
...
A decode failure on retrieve is not possible: the record was validated before storage,
and the migration chain covers older stored versions.
...
resource type. An envelope naming an unknown type fails the import with a clear error.
...
2. **Schema authorship**: the per-integration `.oracle` files and their generated
   ...
   schema plus inert generated code. Every task type in §4.9 is covered before Phase 4
...
4. **A dedicated relationship type — rejected** in favor of `parent_of`. The config
   ...
   existing type inherits the tree walk, the delete cascade, and the index.
5. **A `config` ontology ID field on the task — rejected** in favor of the relationship,
   which gives the config record ontology standing for later per-type endpoints and
   policies. A field would only serve composition.
