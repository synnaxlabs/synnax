...
(RFC 0048 §4.0). This RFC deletes the snapshot system and replaces it with explicit
...
these files, and the two-tier baseline of RFC 0048 §4.0 collapses to one mechanism.
...
5. **Whole-tree granularity answers no question anyone asks.** Versions are
   per-resource and dense (RFC 0042); a per-release copy of 35 files to record a
...
- **Define-vs-alias is read, not computed.** The version file enumerates the split;
  ...
  disappear. Go emission keeps the RFC 0048 §6.1 predecessor-chain form
...
- **`migrate_auto.gen.go` → `migrate.gen.go`**, now a pure function of the
  ...
  helpers. Hand-written transforms stay in `migrate.go`, and the RFC 0048 §4.3
...
- **RFC 0042** §5.6 is superseded. The snapshot folder as compatibility group dies
...
- **RFC 0048** §4.0's two-tier baseline collapses to one: the version-file chain.
  ...
  Everything else in RFC 0048 (backward aliasing, predecessor chain, migration
...
**6.4 - Frozen Go regenerated from version files over immutable-on-disk.** RFC 0048's
...
whole history, not just future versions. Supersedes RFC 0048 §6.2a.
...
3. **Chain compaction** (RFC 0048 §7.1) — unchanged by this RFC, still deferred.
...
- TypeScript, Python, C++, and protobuf outputs: the `versions/vN` layout is Go-only
  (RFC 0048 §8), and version files feed only the Go generator.
