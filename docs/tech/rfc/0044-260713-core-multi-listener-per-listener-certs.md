# 44 - Serving Core on Multiple Listeners with Per-Listener Certificates

**Feature Name**: Multi-Listener Core with Independent Per-Listener TLS Identity <br />
**Status**: Draft <br /> **Start Date**: 2026-07-13 <br /> **Authors**: Emiliano Bonilla
<br />

**Related:**
[RFC 0041 - Core Structure Refactor](./0041-260527-core-structure-refactor.md),
...
## 1.2 - Why the current architecture cannot do this
...
- one bind **address** (`host:port`), where `host` may be `0.0.0.0`, a specific
  ...
  control),
- one **certificate source** (Section 4.2) backing its own `*tls.Config`,
- its own `cmux` tree, reusing the existing secure/insecure multiplexing,
- optionally a **name** (for logs) and an **advertise** marker (Section 4.5).
...
listener serves the full surface (Section 2.2), the branch set is identical; only the
...
## 4.3 - Security package: boundaries and responsibilities
...
`secureProvider` and named (Section 4.2); `getCert` becomes the `file` source.
...
(`newSecureProvider` / `newInsecureProvider`) stays node-wide (Section 4.6).
...
## 4.5 - Cluster advertise address

Gossip is served on every listener (Section 2.2), but peers must dial one agreed
...
`file` hot-reload (Section 4.2). The `file` source loads once in Phases 1 and 2,
...
5. **Polymorphic parsing.** The scalar-or-list `listen` needs a hand-written unmarshaler
   with clear validation errors, bounded by the single polymorphic axis (Section 4.4).
