...
`Color.Swatch` gains an unset state that shows no color, plus a clear action that
returns the field to unset. A fallback color per call site cannot express a role, which
is why the resolver lives in the theme (RFC 0061 §3.1) and not in the form.
