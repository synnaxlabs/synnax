...
While logical operators are the same, certain runtimes may not support specific
statements and may benefit from additional statements. For example, fixed-point iteration
...
common specification format has yet to be solved effectively, and is an open problem
as the effect system matures.
...
A node is the basic building block of the Slate specification, and defines an 
isolated execution unit that can take in values through inputs and produce values 
through outputs.
...
_An aside on state_ - The `Config` field is used to pass static parameters to the node, 
and is not modified during runtime. Nodes that must maintain state across 
events/iterations must do so through their runtime implementation.
...
3. Validate that all edges have valid source and sink nodes that are compatible with the 
