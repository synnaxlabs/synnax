...
between them was the standing failure mode, and the read path that connected them had
fragmented across three read substrates and two selector systems. This RFC restructures
the whole seam:
...
1. All caching and cache invalidation moves out of flux (pluto) into `client/ts` as the
   ...
   write-through. Every client consumer, console or customer, inherits the same cache.
2. Every domain client exposes one read surface, three members total: `retrieve`,
   `onChange`, and `getCached`. Flux becomes a React binding layer over that surface; it
...
3. Session state converges to document truth through dispatched actions, never through
   ...
   and per-cluster-per-project scopes swapped on context switch.

# 1 - Motivation

Deletion was the sharpest edge of the split brain. A remote delete reached a suspense
view only as a thrown 404 after cache invalidation, indistinguishable from a network
failure; non-suspense reads never observed deletion at all, because a listener yielding
null kept the previous value. Delivery was lossy even while running: the hardened
streamer silently reopened on failure, frames lost in the gap were never replayed, and
nothing reconciled on reconnect, so a delete missed in a gap left a live-looking
document forever. Session repair was hand-wired per domain and incomplete, and persisted
session state reloaded stale keys unreconciled.

The read path had the same patchwork character. Reads flowed through flux stores,
per-domain listener wiring repeated across ~25 queries files, and a store-subscribing
selector system with no lifecycle: a selector on an unfetched record returned undefined
unless a parent remembered to pre-warm the cache. Dozens of event callbacks bypassed
caching entirely with direct network retrieves. Each hole was patched locally as found;
this RFC replaces the patchwork with a structure in which the holes cannot exist.
...
- **Scope**: the persistence partition a slice belongs to: `global`, `cluster`, or
  `project`.
...
banner; JetBrains eagerly closes on any delete and collects complaints for it. We align
with VS Code and Notion (tombstone remote, close local, no modals): the corpse in the
cache plays the role of VS Code's dirty buffer. Per-scope state swap follows VS Code's
per-workspace window state. Server-side trash, the industry substrate, is deliberately
deferred.
...
Firestore's `get`/`onSnapshot`/cache triple is the closest analog to the domain read
surface here. We follow that consensus and keep the cache below the framework: the
binding subscribes, it never owns.
...
4. **One read substrate.** Every read flows through the domain client's three read
   members. Tables remain canonical value plumbing; they are not a read API.
5. **Answers hold keys, not copies.** A space's entry records which rows answer the
   ...
   table. Content split brain is unrepresentable.
6. **Session state converges.** What Redux holds is what the UI renders; the action log
...
read behavior is fully described by the three read members; caching is invisible in
their signatures.

## 5.1 - The query package

`client/ts/src/query/` owns the mechanism. Its pieces:

- **`Cache`** is the registry and liveness owner: it creates tables and spaces, runs the
  lazy streamer, counts epochs, and schedules reconciliation. Constructed with an
  `openStreamer` (or `null` for a detached, local-only cache) and an `onError` sink.
- **`Table<K, V>`** is the single source of truth for one resource's records: a rows map
  ...
  equality, so server echoes of local writes announce nothing. Writes return rollback
  destructors. A table may declare a `fetch` primitive ("fetch current records for these
  keys"), which powers keyed reads, listener backfill, and reconciliation, and a hydrate
  mode: `set` (default) or `if-absent` for dispatch-backed document domains whose
  fetches must never clobber locally replayed edits.
- **Spaces** are created per question kind via `cache.queries()` and surfaced only as
  `query.Retrieves`. A space's entries hold `{ status, keys }`; content is composed from
  tables at read time, so an answer can never disagree with the record cache.
- **Channel listeners** are declared on the table as mirror specs built by three
  factories: `createSetListener`, `createDeleteListener`, and `createFetchListener`
  (key-announce channels that resolve through the fetch primitive). Non-mirror reactions
  (e.g. the dispatch controller wire) use `cache.listen`.
- **Derived tables** (`cache.derive`) materialize composition: domains whose answers
  compose fields from sibling stores (status, labels, aliases) maintain a composed-row
  table recomputed on source-table events, replacing rows wholesale so row identity
  remains the freshness signal. Seven domains use one; the rest compose at read time.
...
- **`retrieve`** hashes the query, dedupes against an in-flight promise, resolves
  ...
  the entry. A deleted key rejects with a plain `NotFoundError`: mount-after-delete is
  "not found". Errors never flow through `onChange`.
...
entries that changed. Diff, not nuke-and-refetch: full invalidation would re-suspend
every open view on any blip and cannot distinguish deleted from evicted. Maintained
spaces refetch on the same bump. Reads during a gap serve the cache; the epoch pass
repairs on reconnect. Cluster replacement resets the cache and returns the epoch to 0.
...
- **`createList`** stays imperative (suspending an already-rendered list on requery is a
  UX regression) but sources purely from the client: pages subscribe per query,
  `retrieveByKey`/`subscribeByKey` give item-level liveness, and cached answers seed the
  list synchronously.
...
session mutation (Principle 3). An operator mid-run never has a surface yanked from
under them. Tab names render the corpse name through the same guard, and a
`DeletedError` bubbling out of a view tab that reads someone else's resource falls back
generically: only a resource tab's own deletion gets the tombstone treatment.
...
each epoch. A synchronizer is `{ reconcile, listen }`: `reconcile` is an idempotent
boundary repair, `listen` mounts steady-state subscriptions. Most domains build theirs
from a factory taking `{ onDelete, retrieveExisting, selectKeys, remove }`, keeping
...
workspace can crash: repair must outlive what it repairs. Ten domains register: the
...
converging stored selection into Redux would mirror client-owned structure,
manufacturing the split brain this RFC removes. The recency list also gives
most-recently-used focus repair on tab close for free. Tab selection is the pattern's
only member; new uses require the same justification in review.
...
2. **Server-side trash, deferred; local trash, rejected.** Restore-from-corpse covers
   ...
   the core-owns-persistence rule.
3. **Reactive derivation as the session default, rejected.** Self-healing reads are
   real, but the costs decided it: two truths, subtle read-path resolution, no
   convergence of persisted garbage. Convergence with epoch reconciliation keeps one
   truth and a causal action log. Projection survives only as the fenced exception in
   §5.7.
...
6. **Opt-in caching, rejected.** Cache is the transparent default with construction-time
...
7. **Moving the cache without moving the responsibility, rejected mid-review.** The
   ...
   leaking store getters and cache internals so flux could keep operating them: a
   shallow module over a client that was just a bag of stores. The domain client owns
   the lifecycle end to end and exposes the three read members only.
8. **Domain-global `onChange`, rejected as the primitive.** "Which changes affect this
   query" is domain knowledge; a global feed pushes that judgment onto every subscriber.
   Query-scoped subscription is the primitive.
9. **A tagged `Cached` envelope, built then replaced.** The first contract wrapped every
   answer in `{ variant, data }`. Even held-state answers then allocated a fresh wrapper
   per read, breaking referential stability under `useSyncExternalStore`. The landed
   encoding is the bare union `D | Deleted<D>`: the tag moved off the hot path onto the
   rare case, and pull and push still share one contract. Deletion stays first-class;
   future restore metadata rides the `Deleted` class without taxing the live path.
...
13. **Lists stay imperative.** Suspense-on-requery would blow away rendered rows under a
    boundary fallback on every keystroke. Suspended records and imperative collections
    are two real read shapes, not an incidental split.
14. **Disconnection and deletion throw typed, no `Data | undefined`.** Threading
    `undefined` through every signature re-imports the null-checking the design kills; a
    typed throw to an aware boundary is honest and centralized.
15. **Pure-data reconciler declarations, amended.** A declarative keys-to-resource map
    was proposed; per-domain synchronizer hooks built on a shared factory won: each
    domain mounts what it needs while the forgot-a-wire failure stays structural.
16. **A pluto-visible dispatch seam, rejected.** Undo stacks and coalescing live in the
    client's actions controller; flux only subscribes. Writes are domain methods with
    options, and undo/redo are per-domain methods.
