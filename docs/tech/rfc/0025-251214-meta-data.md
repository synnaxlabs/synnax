...
The `EntryManager[K, E]` owns and maintains indexes for a particular entry type. Indexes
are **local materialized views** - they are not replicated through Aspen and are rebuilt
on node startup from primary data.
...
2. **EntryManager is the index owner**: Public API surface (`RegisterIndex`, `ByIndex`)
   becomes a contract. Changing it is breaking.
