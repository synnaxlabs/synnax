...
- Unsupported conversion (e.g., JSON `string` value for a numeric Synnax channel): Task
  enters error state, no values written for that cycle.
...
- **`Client`**: Wraps libcurl's multi interface. Supports single requests
  ...
  Multiple endpoints hitting the same host reuse keep-alive connections. RAII via
  `unique_ptr<CURLM>`.
- **`Manager`**: Connection pool keyed by device. Tasks acquire a shared `Client` via
  `acquire(ConnectionConfig)`.
- **`ConnectionConfig`/`AuthConfig`**: Parsed from device properties JSON via
  `x::json::Parser`.
...
| -------------- | --------- | ----------------------------------- |
| `CLIENT_ERROR` | CRITICAL  | HTTP 4xx (configuration issue)      |
| `SERVER_ERROR` | TEMPORARY | HTTP 5xx (retry with backoff)       |
| `TIMEOUT`      | TEMPORARY | Connection timeout                  |
| `UNREACHABLE`  | TEMPORARY | Connection refused                  |
| `PARSE_ERROR`  | CRITICAL  | JSON parse failure or missing field |

CRITICAL errors indicate configuration issues (don't retry). TEMPORARY errors use the
breaker pattern for automatic retry with exponential backoff.
...
4. For each field: call `to_sample_value` to convert JSON → SampleValue
5. Collect all values into one `Frame` and write to Synnax in a single batch
...
`integration/driver/devices.py` and runs in a separate process.

- **`HTTPTaskCase`** (`integration/tests/driver/http/task.py`): Base class extending
  `SimulatorTaskCase`. Subclasses implement `create_fields()` and `create()` for
...
| JSON Convert Location | `x/cpp/json/convert.h`                               | Reusable utility, not HTTP-specific                                |
...
| Config Validation     | Channel type + uniqueness at configuration time      | Fail fast rather than runtime surprises                            |
