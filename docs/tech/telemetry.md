...
#### 4.0.1 Tightly constrained types
...
#### 4.1.1 Highly-variable emission rates
...
hardware. PID control is also a good example of this. In a supervisory control system, a
command would be sent over the network containing the P, I, and D parameters for
...
example involves pushing a configuration file to a flight computer or a calibration
sheet to a data acquisition device. Supervisory control is as important as its real-time
counterpart, yet has very different requirements.

#### 4.2.0 Low, highly variable frequencies

Supervisory commands are issued at much lower, far more variable frequencies than
...
#### 4.2.1 Large, variably sized data types
...
#### 4.2.2 Latency and jitter tolerance
...
analysis on the data without any human interaction. It's easy to see how this pattern
can be extended to other use cases, making for a very powerful system that can automate
many manual tasks.
...
#### 4.3.0 Highly variable frequencies
...
The main differences between these variants can be broken down into:
