...
quite. Synnax nodes not only need to serve RPCs from other nodes but also __issue__ RPCs
to them. Certificates issued by a public CA are only signed for server side use, and cannot
be used to authenticate a client. There are ways around this, but they are complex and we don't
