...
instructions, see the [installation guide](/components/cluster/installation).

#### Install the Synnax Console

This guide assumes you've installed the [Synnax Console](/components/console/get-started) 
on your local machine. You can find the latest release [here](/components/console/get-started).
...
This command will start a Synnax node that listens on port `9090`, stores
its data in the `/usr/local/synnax/data` directory, and automatically generates self-signed
...
be valid for an IP address. Instead, stick with `synnax.example.com` or something similar. 
We'll configure a local host alias for this domain on the local machine in a [later step](#configuring-a-local-host-alias).
...
We'll start off by copying the CA certificate from the remote machine to your local machine.
...
Next, you'll need to add the CA certificate to your local machine's trust store. To do this,
first move the CA certificate to the linux certificate directory:
...
Next, you'll need to add the CA certificate to your local machine's trust store. You can do
...
certificate you installed on your local machine will only be valid for the host name
you used when generating it.
