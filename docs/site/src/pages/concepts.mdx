...
5. Node Key - This is the Key of the node that holds the lease on the channel. This node
   is known as the leaseholder, and is the only node that can write _new_ channel data to disk.
...
3. Data - A collection of sample _values_ represented as an ordered byte array. For a `float64`
