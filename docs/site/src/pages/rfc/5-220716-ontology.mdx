...
// Key is a cluster-unique identifier for the resource, regardless of its type (this
// is why it's called an Key, and not just a key).
type Key struct {
...
type Service interface {
	// Retrieve returns data for the resource with the given Key.
	Retrieve(Key) (interface{}, error)
}
```
...
type Resource struct {
	Key Key
	// Data can hold a struct, map, slice, etc. that represents the concrete resource Key
	// refers to.
	Data interface{}
}
```
...
channel depending on its `Channel.NodeKey` field. The only way to access this
...
func Enforce(resource Resource) error {
	...
	nodeKey, ok := resource.Data["nodeKey"]
	if !ok {
