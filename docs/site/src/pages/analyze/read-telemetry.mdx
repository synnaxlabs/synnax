...
<Header>
    <Header.Title>Read Telemetry</Header.Title>
</Header>
...
```python
iter = client.new_iterator(start, end, "my-precise-tc", "time", chunk_size=100)
```
...
It’s important to close an iterator after you’re done with it, so we reccommend wrapping it’s
