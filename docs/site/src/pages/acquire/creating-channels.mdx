...
using the Synnax CLI, Delta UI, or any ofthe Synnax client libraries.
...
2. Familiarized yourself with channels and their types.

## Channel Parameters

Channels have a few important parameters that need to be set when creating them. The following list summarizes each one. If you’d like deeper descriptions on each, see here.

1. **Name** - A human readable name for the channel. We reccomend keeping channel names unique, although this isn’t a strict requirement by the database.
...
3. **Rate** - The fixed rate at which samples are recorded. If this parameter is specified and index and is index are not, the channel is considered a fixed rate channel.
4. **Index** - The key of the channel whose timestamp values will be used to index the
   ...
   channel. If this parameter is specified, the channel is considered an indexed channel, and Is Index should not be set to true.
5. **Is Index** - A flag that indicates whether the values in a channel are indexed i.e.
   ...
   and must have a data type of timestamp. If this parameter is set to true, the channel is considered an index and ‘index’ should not be specified.
...
Creating variable rate channels requires us to create first create an index channel to store timestamps and then create our data channel.
...
Createing variable rate channels requires us to first create an indx channel to store
...
We can also create multiple channels by passing an array to the `create` method. The important thing to keep in mind is that we need to create index channels before creating their indexes.
