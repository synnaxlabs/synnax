...
<Header>
    <Header.Title>Writing Telemetry</Header.Title>
</Header>
...
Writing telemetry to a variable rate channel is slightly more complicated. First, we
need to create two channels: an index channel to store timestamps and another to store
thermocouple values.
...
# Introduce a litle bit of random noise to illustrate irregularly spaced #
# timestamps
...
sensor data. Synnax ensures that we to write data to our index before we’re allowed to
write to the channels indexed by it. If we do the opposite, the database will let us know t
hat our index is discontinuous at the point we’re writing to it. So, first timestamps,
then values.
...
### Live Writing Several Frames to an Index Channel and its Index
