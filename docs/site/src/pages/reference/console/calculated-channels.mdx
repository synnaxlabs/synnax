    ...
    <tr>
      <td>Operation (Optional)</td>
      {
        <td>
          ...
          <a href="#operations">running aggregations</a>, and <code>derivative</code>{" "}
          for rate of change.
        </td>
      }
    </tr>
...
```arc
return pressure > 100 // Returns true or false (bool)
```
...
This outputs the rate of pressure change in units per second. For example, if pressure
increases from 100 to 150 over 2 seconds, the output is 25.0.
...
**Best Practice** -> When creating channels that will be used in Arc calculated
channels, use underscores instead of special characters:
