    ...
    <tr>
      <td>Expression</td>
       <td>The resulting python expression that represents the value written to the calculated channel. The
        the value written to the channel must be assigned to the `result` variable.</td>
    </tr>
    <tr>
      <td> Requires </td>
      <td>The existing channels necessary involved in the calculation expressed in the expression. A calculated channel must
        use **at least one channel**.</td>
    </tr>
  </tbody>
</Table>
...
for i = 1, #readings do
    if readings[i] > max_pressure then
        max_pressure = readings[i]
    end
end
```
...
Here is an example expression that applies specific scaling to 4 channels and finds the max out of them:
...
local scale_params = {
    ...
    {slope = 3.0, offset = 0},     -- for fuel_tc_1
    {slope = 0.5, offset = 15}     -- for fuel_tc_2
}
