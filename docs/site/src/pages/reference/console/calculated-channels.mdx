...
<Note.Note variant="warning">
  **Legacy Lua-based calculated channels are deprecated.** Calculated channels now use
  the Arc language. Existing Lua-based channels will continue to work but should be
  migrated to Arc. See the [Migration from Lua](#migration-from-lua) section for
  guidance.
</Note.Note>
...
<Table>
  ...
  <tbody>
    ...
    <tr>
      <td>Data Type</td>
      <td>The type of data the channel stores. This is automatically inferred from the Arc expression.</td>
    </tr>
    <tr>
      <td>Expression</td>
      {
    ...
    <tr>
      <td>Operation (Optional)</td>
      {
        <td>
          ...
          <code>min</code>, <code>max</code>, and <code>avg</code> for running
          aggregations. See <a href="#operations">Operations</a> for details.
        </td>
      }
    </tr>
    <tr>
    ...
    <tr>
      ...
      {
        <td>
          A <code>uint8</code> channel that triggers operation reset when its value
          equals 1. Only applies when an operation is selected. See{" "}
          <a href="#reset-channels">Reset Channels</a> for details.
        </td>
      }
    </tr>
  </tbody>
...
Reference channels directly by name—no prefix or special syntax needed:
...
Reset channels provide signal-based control over operation state. When a reset channel
receives a value of `1`, the associated operation clears its accumulated state and
restarts.

### Requirements

- Reset channel must have data type `uint8`
- Triggers when channel value equals `1`
- Can detect fast pulses (1→0 transitions in the same data series)
...
3. **C-style operators** - Use `&&`, `||`, `!` instead of `and`, `or`, `not`
4. **Auto-detected dependencies** - No need to manually specify `requires` field

### Channel Naming for Arc

**Important**: Arc identifiers can only contain letters, digits, and underscores, and must
start with a letter or underscore. Channel names with special characters (like dashes,
spaces, or dots) **cannot be directly referenced** in Arc expressions.
...
calculated channels or using Arc's full capabilities. See the
[Arc documentation](https://docs.synnax.com/reference/arc) for more advanced features.
