...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      {
        <td>
          The number of seconds without data from the channel before the symbol
          registers as stale.
        </td>
      }
    </tr>
...
Staleness measures arrival. It tells you if the channel still sends data. It does not
tell you if the newest sample is recent. A channel that writes on change can pass its
stale timeout while it is healthy. Give such a channel a timeout longer than the longest
gap you expect between changes. A symbol registers as stale only after data arrives and
then stops.
