...
["Frame"](/reference/client/working-with-data/series-and-frames#frame) class.

<Client.Tabs client:load exclude={["console", "cpp"]}>

<Fragment slot="python">
  Access the `Series` object using the `[]` operator. The `Frame` can also be converted
  to a `pandas.DataFrame` by calling the `to_df` method.
