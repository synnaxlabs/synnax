...
my_sensor = client.channels.create( name="my_sensor", data_type=sy.DataType.FLOAT32,
...
Mutliple channels can be created in a single call using the `create` method. This is
...
Keep in mind that the index channel(s) must be still be creeated first.
...
<Note.Note variant="info" className="compact">
  ...
  Channels](/reference/client/working-with-data/ranges#working-with-channels).
</Note.Note>
...
<Fragment slot="python">
  ```python sensor_channels = client.channels.retrieve(["^sensor"]) ```
</Fragment>
...
Renam channels using the `channels.rename` method. Currently, renaming must be done by
...
To delete a channel, we use the `channels.delete` method.
