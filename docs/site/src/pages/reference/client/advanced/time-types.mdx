...
Synnax stores timestamps with nanosecond precision, which exceeds what native types
in most languages can represent. The client libraries provide utility classes for
working with high-precision timestamps, durations, and time ranges.
...
`write`, `open_iterator`/`openIterator`, `open_streamer`/`openStreamer`, and
...
preserve this precision, but there are limitations to be aware of when converting
to native types.
