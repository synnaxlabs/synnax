...
To start streaming data, call the `open_streamer`/`openStreamer` method on the client
and provide a list of channels to stream.
...
method on the streamer. This will _replace_ the current list of channels, not add to it.
