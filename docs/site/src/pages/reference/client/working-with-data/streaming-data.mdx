...
To start streaming data, call the `open_streamer`/`openStreamer` method on the client and provide a
...
To stream data at a lower rate, use the `downsample_factor` / `downsampleFactor`
parameter. The streamer will skip the specified number of samples in each series before
returning the next frame. Note that this does not average values—it skips samples.
...
TypeScript doesn't have built-in timeout support. Instead, you can use `Promise.race()` to achieve the equivalent functionality.
...
To update the list of channels being streamed, call the `update_channels` / `update`
method on the streamer. This will *replace* the current list of channels, not add to it.
