...
and Frames (multi-channel containers). These are core types returned by read
...
A series is a strongly typed array of data samples. A series can contain any data you want it
to, but in the context of Synnax, it represents a set of contiguous samples
...
# the series will automatically be of type string.
...
// Construct a series from an array of objects. In this case, the series
// will automatically be of type json.
...
The `Series` class is directly compatible with numpy, and supports all of
...
Whenever you read a series from Synnax, it will have a `time_range`/`timeRange` property that
represents the time range occupied by the samples in the Series. This property can be
...
method. Both return a `MultiSeries` object, which wraps multiple `Series` instances
but behaves like a single series:
...
returns a `MultiSeries` object, which wraps multiple `Series` instances but behaves
...
The `at` method returns a `TelemValue` type, which is a union of all possible data
types a series can contain. This can make type-safe code difficult:
