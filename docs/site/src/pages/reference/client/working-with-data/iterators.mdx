...
To open an iterator, use the `open_iterator` / `openIterator` method with a time range
...
size, use the `chunk_size` / `chunkSize` parameter with the desired number of samples
per iteration.
...
Use a `for` / `for await` loop to iterate through data in chunks. Each iteration yields
a frame containing a portion of the data.
