...
| <Client.Var client:load py="is_index" ts="isIndex" />   | <Client.Var client:load py="bool" ts="boolean" /> | `false`  | Set to `true` if the channel is an index channel, `false` otherwise. Index channels must have a data type of `DataType.TIMESTAMP`.    |
...
| `virtual`                                               | <Client.Var client:load py="bool" ts="boolean" /> | `false`  | Set to `true` if the channel is virtual. Virtual channels do not store data in the database and are used for streaming purposes only. |
| `retrieve_if_name_exists`                               | <Client.Var client:load py="bool" ts="boolean" /> | `false`  | If `true`, retrieves the channel if one with the same name already exists instead of creating a duplicate.                            |
...
<Note.Note variant="info" className="compact">
  To retrieve channels using ranges see, [Ranges - Working with
  Channels](/reference/client/ranges#working-with-channels).
</Note.Note>
...
Channels can be aliased to give them a more descriptive name for a specific test or
operation. Aliasing is a feature of ranges, and is covered in
[Ranges - Working with Channels](/reference/client/ranges#working-with-channels).
