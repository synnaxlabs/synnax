...
| `virtual`                                                                        | <Client.Var client:load py="bool" ts="boolean" /> | <Client.Var client:load py="False" ts="false" /> | Set to <Client.Var client:load py="True" ts="true" /> if the channel is virtual. Virtual channels do not store data in the database and are used for streaming purposes only. To persist string, JSON, or bytes data, leave this <Client.Var client:load py="False" ts="false" /> and create an indexed data channel with the appropriate data type. |
...
<Note.Note variant="info" className="compact">
  ...
  [ranges](/reference/client/ranges#channel-aliases).
</Note.Note>
...
Channels can be [aliased](/reference/client/ranges#channel-aliases) to give them a more
descriptive name for a specific test or operation.
