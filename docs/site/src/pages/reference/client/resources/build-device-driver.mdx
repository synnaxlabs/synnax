...
building a reliable, performant driver using the client libraries. We'll use an Arduino
as an example, but the patterns apply to any serial-based device.
...
- **Arduino IDE** ([download](https://www.arduino.cc/en/software)) - any Arduino board
  works (we use an Arduino Mega 2560)

<Client.Tabs client:load exclude={["console", "cpp"]}>

<Fragment slot="python">
- **Python 3.12+** with the Synnax client: `pip install synnax`
...
Create a driver file. Find your Arduino's port in the Arduino IDE (top-right corner
...
<Video
  client:only="react"
  id="client/resources/build-device-driver/line-plot"
/>
...
Now we'll receive commands from Synnax to control digital outputs on the Arduino.
...
<Video
  client:only="react"
  id="client/resources/build-device-driver/schematic"
/>
...
You may have noticed we set both "State" and "Command" to the same channel. This works,
but has a problem: if the driver stops, clicking the switch still appears to work (it
toggles visually) even though nothing happens on the hardware.
...
<Video
  client:only="react"
  id="client/resources/build-device-driver/console-setup"
/>
...
For production-grade hardware integrations, see the [C++ Driver
documentation](/reference/driver). The C++ driver offers:
