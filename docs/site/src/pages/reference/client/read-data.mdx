...
This call will block until a new frame is available. A frame may not contain data for
every channel, so be ready to [handle partial frames](#handling-partial-frames).
