...
We can create a channel using the `channels.create` client method. You must first create
an "index" channel to
...
We can then write data to `timeChannel` and `tempChannel`. First, we must write to the
index channel before writing to the data channel.
...
`Int32Array`, etc.). We can convert the returned data to a JavaScript array easily.
