...
Pluto leverages **Web Workers** and the
[`OffscreenCanvas` API](https://developer.mozilla.org/en-US/docs/Web/API/OffscreenCanvas#browser_compatibility).
These features are generally available on the latest versions of all major browsers, but
...
```typescript
import { Button as PButton } from "@synnaxlabs/charon/button";
