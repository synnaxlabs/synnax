...
The returned data is a [`MultiSeries`](series-and-frames#series), which maintains a very
similar interface to a JavaScript typed array (e.g. `Float32Array`, `Int32Array`, etc.).
We can convert the returned data to a JavaScript array easily:
...
The returned data is a [`Frame`](series-and-frames#frames) object, which contains a
`MultiSeries` for each channel. To access the data for a specific channel, we can use
the `get` method:
