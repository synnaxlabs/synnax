...
title: "Delete Data"
description: "Use the TypeScript client to delete data from a Synnax cluster."
---
...
deletion operation is complete, all future reads will no longer include the deleted data.
However, it may take a while before the underlying file sizes decrease – this allows
deletion operations to be served in a rapid manner and only actually collect the unwanted
data when the load on the server is low.
...
write over that time range with new data or even delete some more data.
Even if an entire channel's data is deleted, the channel is still in the database,
albeit empty.
...
In some situations, `delete` returns an error. If some channel keys or names do not exist
in the database, the entirety of the `delete` operation fails, no data is deleted, and a
`NotFound` error is returned:

```typescript
// Suppose 111 and 112 are keys to channels that do exist, since 113 does not exist, none
// of these channels' data get deleted.
...
on data in the requested time range, an error is returned:

```typescript
// If my_precise_tc is indexed by my_index_timestamps from 1 second to 3 seconds,
// we cannot delete my_index_timestamps. This call returns an error.
await client.delete(
    ["my_index_timestamps"],
    TimeStamp.seconds(1).range(TimeStamp.seconds(3))
)
...
before the deleting time range return an error. This is to ensure that the writer and the
...
// error returned since writer start 00:10 is before deleting time range [00:12 - 00:30)
await client.delete(
    ["my_precise_tc"],
