...
In addition, if a `delete` call is made to an **index channel** that other channels
depend on data in the requested time range, an error is returned:
...
before the deleting time range return an error. This is to ensure that the writer and
the deleter do not contend over data in the same region.
