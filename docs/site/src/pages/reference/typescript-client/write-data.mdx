...
Then, we'll make sure to write timestamps to the index before we write to the data
channel, note that to write data to an individual channel, you must provide a typed
array:
...
roughly 100ms apart, and will commit all writes when finished. Note that instead of
passing in series to the frame, you may also pass in singular samples if you wish to
only write one sample.
