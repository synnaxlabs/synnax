...
try {
    for (let i = 0; i < 100; i++) {
        ...
        if (!await writer.write(new Frame({
...
The Synnax writer is optimized for performance, and as such, does not synchronously check for errors on every call to `write`. Instead, calls to `write` return a boolean indicating whether the writer has accumulated any errors on the current call or on any previous calls.

Once a writer has accumulated an error, all future calls to `write` will fail until the writer is closed and a new writer is opened. When calls to `write` return `false`, you should check for the accumulated error by calling `writer.error()` or simply close the writer, at which point the error will be raised.

#### Error Handling Example - Out of Order Timestamps

Let's say we're writing a time series to a channel, but we accidentally write the timestamps out of order. This is a common mistake that can be hard to debug. Here's how we can use the writer to handle this:

```typescript
import { TimeStamp, TimeSpan, DataType } from "@synnaxlabs/client";
...
try {
  for (let i = 0; i < 100; i++) {
    // This will cause an error because timestamps are decreasing
    if (
      !(await writer.write({
        [timeChannel.key]: TimeStamp.now().subtract(TimeSpan.seconds(i)),
        [dataChannel.key]: i,
      }))
    ) {
      console.error("Write failed:", await writer.error());
      break;
    }
  }
} finally {
...
This will break out of the loop as soon as the writer encounters an out-of-order timestamp. The error can be retrieved using `writer.error()`, and the writer will be properly closed in the `finally` block.
