...
of [channels concepts guide](/reference/concepts/channels).
...
Notice how we set the `isIndex` property to `true` on the `"time"` index channel. This tells
Synnax that this channel will be used to store timestamps for other channels. We then
pass the `index` property to the temperature sensor channel so it uses the `"time"` channel
as its index.
...
<Note variant="info">
When creating variable rate channels, keep in mind that we need to create
...
If more than one channel matches the query, the client will raise a `MultipleFoundError`.
...
Synnax will not raise a `NotFoundError` if no channels match the query. Instead,
...
Deleting a channel will also delete all of the data stored in that channel.
...
The TypeScript client provides interfaces for creating, retrieving and deleting
the channels in a Synnax cluster. In this guide, we'll cover the details of each
of these operations.
...
Notice how we set the `isIndex` property to `true` on the `"time"` index
channel. This tells Synnax that this channel will be used to store timestamps
for other channels. We then pass the `index` property to the temperature sensor
channel so it uses the `"time"` channel as its index.

### Creating Multiple Channels

We can also create multiple channels efficiently by providing a list of channels
to the `create` method. This is far more efficient than creating channels
...
`NotFoundError`. If more than one channel matches the query, the client will
raise a `MultipleFoundError`. If you'd like to accept no or multiple results,
provide a list to the `retrieve` method as shown in the next section.
