...
and can be installed with your package manager of choice. We'll be using `npm` in our 
documentation, but feel free to use `yarn`, `pnpm`, or any other package manager you prefer.
...
| `secure` | Whether to use TLS encryption. This should be set to `true` if the cluster is using TLS. See [production](/reference/cluster/production) for more information. | `false` |
...
We recommend starting with the [Channels](channels) section to learn
