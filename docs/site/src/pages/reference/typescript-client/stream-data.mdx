...
Streaming data is useful for real-time processing, visualization, and monitoring.
This page guides you through using the TypeScript client to stream data from a 
Synnax cluster.
...
To start streaming data, call the `openStreamer` method on the client and provide
...
that only contains data for the first channel, followed by a frame that only contains data
