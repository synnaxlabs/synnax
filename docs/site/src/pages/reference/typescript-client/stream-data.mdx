...
For more details on how to work with the returned frames, see the [Series and
Frames](/reference/typescript-client/series-and-frames) page.

### Downsampling

If you are interested in streaming data at a lower rate, you can use the optional `downsampleFactor` parameter. This parameter will
cause the streamer to skip the specified number of samples in each series before
returning the next frame. Note that this parameter does not average values, and
will skip samples instead.
