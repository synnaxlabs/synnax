...
title: "Timestamps"
description: "Learn how to work effectively with timestamps in the Synnax TypeScript client."
next: "Examples"
...
Working with high-resolution timestamps in JavaScript is tricky. We provide
several utility classes to make it easier.
...
native `Date` object, which only supports millisecond precision.

Numbers in JavaScript are represented as 64-bit floating point numbers, which
means that they can only represent integers up to 2^53 accurately. This means
that you may lose precision for certain timestamps. Also, doing arithmetic with
timestamps expressed as floating point numbers can lead to rounding errors.

<Divider.Divider direction="x" />

## TimeStamp

Synnax provides the `TimeStamp` utility class to effectively work with
nanosecond timestamps in JavaScript. It wraps a `bigint` to store a unix
...
client. The union of these formats is called a `CrudeTimeStamp`. Examples
include `read`, `write`, `openIterator`, `openStreamer`, and `openWriter`.
...
`TimeSpan` is a utility class that represents a duration of time. It wraps a
`bigint` to store a duration in nanoseconds.
...
`TimeRange` is another utility class that represents a range of time. It
consists of two `TimeStamp` objects called `start` and `end`.
