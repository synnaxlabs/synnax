...
in your Core's data. Ranges are the primary means for organizing and accessing the data
stored within Synnax, and can be used to correlate metadata with channel telemetry.
...
[task](/reference/driver). When you create a snapshot of a task, you're capturing
critical information such as calibrations, scaling values, and mappings to physical
ports and lines. The snapshot provides a permanent way to refer to the configuration of
the task at the time the range was created.
...
Create, access, and modify ranges from the Console or the
[client libraries](/reference/client/ranges).
