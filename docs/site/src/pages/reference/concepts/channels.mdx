...
use them in your Core.

Create, retrieve, rename, and delete channels from the Console or the
[client libraries](/reference/client/channels).
...
recording samples 25 times per second (25 Hz) would write data to a channel in order to
store it permanently within Synnax, while at the same time streaming the data for live
consumption.
...
Every sample in a data channel has a single declared data type. Most data channels store
fixed-width samples, such as floating point numbers or integers. Data channels can also
store variable-length values such as strings, JSON objects, and raw bytes.
...
persisted to disk, and they do not have an associated index channel. Virtual channels
are useful when a value only needs to be observed by live consumers and does not need to
be queried by time range later.

If you need to persist strings, logs, JSON values, or arbitrary binary data, use a data
channel with a `string`, `json`, or `bytes` data type instead.

<Divider.Divider x />

## Calculated channels

Calculated channels compute values from other channels with an Arc expression. See
[calculated channels](/reference/client/calculated-channels) to create one.
...
All channels must have a consistent data type. Index channels must always have a data
type of `timestamp`. Data channels can have any of the following types:
...
| `uuid`      | [Universally Unique IDentifier](https://www.rfc-editor.org/rfc/rfc9562.html) | 16 bytes |
| `string`    | UTF-8 string                                                                 | variable |
| `json`      | JSON value                                                                   | variable |
...
[calculated channels](/reference/client/calculated-channels). This expression is an Arc
