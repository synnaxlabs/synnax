...
batches are called [frames](/reference/concepts/frames-and-series). The primary interface
for writing these frames is, unsurprisingly, a **writer**. Writers are analogous to pipes
that continuously feed a cluster with frames of data for one or more channels.
...
Internally, [schematics](/reference/console/schematics) within the Synnax [console](/reference/console/get-started)
use writers to send commands to actuators when a user clicks on a valve or other control element.
...
When opened in **stream-and-persist** mode, the writer will both persist the
...
for the time channel in the same frame as the temperature channel. This frame is <b style="color: var(--pluto-secondary-z);">valid</b>.
...
`time` channel, we violate Rule 1. This frame is <b style="color: var(--pluto-error-z);">invalid</b>.
...
writing to the `time` channel. Now, we open a writer on the `temperature` channel, marking
its `start` timestamp as `1677433720770863800`. This frame would be <b style="color: var(--pluto-secondary-z);">valid</b>
because we've already written timestamps to the `time` channel.
...
`temperature` are indexed by the `time` channel. Here's pseudocode for the fields
of each channel:
...
When we open a writer on all three channels, we must write frames that contain the
same number of samples for each channel. For example, this frame is <b style="color: var(--pluto-secondary-z);">valid</b>.
...
Omitting a sample from the `pressure` channel would violate Rule 2. This frame is <b style="color: var(--pluto-error-z);">invalid</b>.
...
Entirely omitting a channel from the frame would also violate Rule 2. This frame is <b style="color: var(--pluto-error-z);">invalid</b>.
...
As [virtual channels](/reference/concepts/channels#virtual-channels)
do not store data, they have far more flexibility in how they can be
written to. They do not require timestamps, and as a result do not need to be
associated with an index. Write to them whenever and however you like!
...
If you have a temperature and pressure sensor that are recorded by different
devices. The first device records temperature at 10Hz, while the second records pressure
at 25Hz. Pressure and temperature samples will not always share the same timestamp, and
by consequence cannot share the same index.
...
Channels that are used to send commands to actuators are typically used quite differently
to channels that record from sensors. It's common for a single device to sample from many
different sensors at the same rate, meaning that these sensor channels can share an index.
