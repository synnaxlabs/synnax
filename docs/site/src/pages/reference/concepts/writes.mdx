...
Writing data to Synnax revolves around sending batches of telemetry samples to a group
of [channels](/reference/concepts/channels). When ingesting historical data, it's common
to write large amounts of data in chunks. For real-time data, we can write samples in
a streaming fashion, where each sample is written and processed by Synnax as it gets
recorded.


Writers are the primary interface for writing data to a Synnax cluster. A writer
can be thought of as a pipe that continuously feeds data into one or more channels. 
Once written, the data is available for [real-time streaming](/reference/concepts/streams)
or for [historical access](/reference/concepts/queries).
