...
transaction, a caller opens a new writer and then begins writing data frames using a 
file like interface. After they're done writing, the user commits the write to the
...
2. The timestamp marking the time at or before the first written sample was recorded.
