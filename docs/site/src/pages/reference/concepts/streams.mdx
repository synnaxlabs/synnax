...
`Streamer` object, which is available in both the [Python](/reference/python-client/stream-data) and
[Typescript](/reference/typescript-client/stream-data) client libraries.
...
To open a streamer, you'll need to provide a list of channels that you'd like to 
stream data from. The streamer will validate that the channels exists and are readable
before starting the stream.

### Reading Data

Once the streamer is open, you can read data from the stream using the read method. This
will block until _any_ of the channels have new data available, at which point the
