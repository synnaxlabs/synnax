...
[NI Analog Read Task](/reference/driver/ni/analog-read-task) that samples the pressure
channels at 100 Hz, then we'll create an
[NI Digital Write Task](/reference/driver/ni/digital-write-task) to write to the outputs
of the device. We won't start any of them, so we're not acquiring data or controlling
outputs yet.
