...
title: "Embedded Sequence Keywords and Functions"
...
value from the channel. The sequence editor's auto-complete will make sure to add
the channel to the "Read From" list of channels for the sequence. Make sure every
channel you'd like to read from is in the list by checking the dialog at the bottom
of the editor.
...
list of channels for the sequence. Make sure every channel you'd like to write to is
in the list by checking the dialog at the bottom of the editor.
...
seize control of a set channels from one or more separate sequences.
...
rate` rate. So `elapsed_time` for a sequence that runs at 1Hz and is at iteration 10
will be very close to 10 seconds, but not exactly 10 seconds.

### Elapsed Time Within

The `time_elapsed_within` function returns true if the the current `elapsed_time` is
...
```lua
if time_elapsed_within(10, 11) then
    -- do something
end
```
