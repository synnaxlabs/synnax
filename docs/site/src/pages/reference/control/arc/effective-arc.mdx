...
This page covers what works well in Arc and what to avoid. These patterns
come from building real control systems.
...
Line order determines priority for one-shot transitions (`=>`). Always list
abort conditions before normal operations:
...
Continuous edges (`->`) run every cycle. One-shot edges (`=>`) fire once
and stop until the stage is re-entered.
...
Stateful variables (`$=`) persist across invocations. Consider whether your
initial value makes sense:
...
When an Arc automation doesn't behave as expected, check its status in
Console. Runtime errors (division by zero, out-of-bounds access) stop the
task and report the error.
