...
### Using Loops for Reactive Work

[`for` loops](/reference/control/arc/reference/loops) are available inside function
bodies for processing data that's already in hand. They're not a substitute for the
reactive model when you need to respond to data as it arrives:

```arc
// Wrong: trying to poll with a loop
// for { value := sensor; ... }

// Right: let the reactive model trigger your function
sensor -> process{} -> output

// Also right: use a loop to process a batch inside a function
func process_batch(data series f64) f64 {
    sum f64 := 0.0
    for x := data {
        sum = sum + x
    }
