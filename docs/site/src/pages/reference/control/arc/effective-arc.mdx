...
nextURL: "/reference/control/arc/reference"
---
...
```arc channels="ox_pt_1, fuel_pt_1, abort_btn, press_vlv_cmd", bodies="abort"
stage pressurize {
...
For sequence and stage transitions, `=>` is what you want. `->` into a stage name also
...
Stages are for steps that need to watch multiple conditions at once: a success check
with safety monitoring, or a control loop that runs while a timeout counts down. If a
step is just a write or a wait, make it a sequence item directly:
...
A stage earns its place when it holds two or more concurrent flows. A stage that holds a
single write is noise.
...
```arc channels="sensor, output, emergency_condition, press_vlv_cmd, vent_vlv_cmd"
import control
...
automations. Starting below 255 makes this possible without stopping the program.
...
`->` into a stage name is legal syntax but transitions on the first value from the
...
```arc channels="valve_cmd"
stage example {
    false -> valve_cmd // Writes false
    true -> valve_cmd // Writes true (overwrites false)
}
```

This is usually a mistake. Use conditional logic instead:

```arc channels="pressure, valve_cmd"
pressure < 500 -> valve_cmd
```
...
```arc channels="some_condition"
stage wait_forever {
    ...
    0 > 1 => next // Stage is blocked from exiting
}
```

Add timeouts:

```arc channels="some_condition", bodies="timeout_stage"
stage wait_with_timeout {
    some_condition => next
    time.wait{duration=30s} => timeout_stage
}
```
...
The C++ driver runtime runs control loops at 1kHz. Deterministic cycle times need a
Linux real-time kernel.

For timing-critical applications:

- Use `time.interval{period=...}` for consistent timing
- Keep flow chains short
- Avoid complex calculations in hot paths

```arc
// 1kHz control loop
time.interval{period=1ms} -> fast_controller{}
```
...
// More efficient: use an input parameter
...
```arc channels="setpoint, gain, debug_error"
func debug_controller(value f64) f64 {
    error := value - setpoint

     // Write to a channel for visibiliy
