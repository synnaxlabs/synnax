...
| Config     | Type    | Description                       |
| ---------- | ------- | --------------------------------- |
| **Input**  | numeric | Value to differentiate            |
| **Output** | `f64`   | Rate of change per second (dx/dt) |

The derivative node tracks the previous value and timestamp across invocations. On each
...
// Alarm when the rate exceeds a threshold
pressure -> derivative{} -> pressure_rate
pressure_rate > 100.0 => alarm
```
...
ox_pt_1 > 500 -> set_status{
    status_key = "overpressure",
    name = "Over-Pressure Alarm",
    variant = "error",
...
| ------------ | ----- | -------- | ------------------------------------------------ |
| `status_key` | `str` | Yes      | Key of the status to update                      |
| `variant`    | `str` | Yes      | Severity: `"success"`, `"warning"`, or `"error"` |
| `message`    | `str` | Yes      | Notification message                             |
| `name`       | `str` | No       | Display name (defaults to `""`)                  |
| **Input**    | `u8`  |          | Trigger condition                                |

**Workflow:**

1. Create a status in Synnax Console
2. Reference that status in Arc using `status_key`
...
| `derivative`    | Statistics | Rate of change (dx/dt)     |
