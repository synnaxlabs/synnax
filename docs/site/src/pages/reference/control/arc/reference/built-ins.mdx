...
description: "Arc built-in function reference for timing, statistics, I/O, signal processing, and utility functions"
prev: "Sequences"
...
```arc
sensor -> max{duration=1m} -> max_display
...
<Note.Note variant="info">
In most cases, you can reference channels directly by name without using
`on` or `write` explicitly.
</Note.Note>
...
Outputs only when the input value has been stable for a specified duration.
Useful for debouncing noisy signals.
...
Arc does not currently include `abs`, `sqrt`, or trigonometric functions.
Use conditional logic for absolute value:
</Note.Note>
