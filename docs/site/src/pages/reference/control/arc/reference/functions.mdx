...
```arc channels="debug_channel"
func log_value(value f64) {
    debug_channel = str(value)
}
```
...
```arc channels="count1, count2"
// Two separate counters with independent state
time.interval{100ms} -> counter{} -> count1
...
```arc channels="ox_pt_1, alarm_status"
func monitor() {
    if ox_pt_1 > 500 {
        alarm_status = true
    }
...
```arc channels="sensor1, sensor2, avg_output"
