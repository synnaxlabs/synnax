...
Config values can be provided positionally instead of by name. Values map to parameters
...
This also works for multi-parameter functions:
...
// Anonymous (positional: min=0.0, max=100.0)
sensor -> clamp{0.0, 100.0} -> output
```

When trailing parameters have defaults, they can be omitted:

```arc
func controller{setpoint f64, gain f64 = 1.0} (x f64) f64 {
    return(x - setpoint) * gain
}
// Only provide setpoint, gain uses default
sensor -> controller{100.0} -> output
```
...
routed to its own destination at the call site. Use [routing tables]((/reference/control/arc/reference/flow#output-routing-tables)) to connect multiple outputs:
...
Use [routing tables](/reference/control/arc/reference/flow#output-routing-tables) to map multiple sources to input parameters:
```arc
...
Built-in function that does not require an `import` or module prefix. Returns the length
of a series or string.
