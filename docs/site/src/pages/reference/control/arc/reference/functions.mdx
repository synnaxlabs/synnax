...
Arc has two calling conventions depending on context. Both take input parameters
positionally or by name (`name=value`), not a mix of the two.
...
func outer(x f64) f64 {
    y := clamp(x, 0.0, 100.0)     // positional
    z := clamp(value=x, max=50.0) // by name
    return y + z
}
```
...
```arc
sensor -> filter{threshold=50.0} -> output // by name
sensor -> filter{50.0} -> output           // positional
```
