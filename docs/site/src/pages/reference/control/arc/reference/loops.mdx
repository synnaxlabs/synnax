...
The most common use case is walking through each element of a series:

```arc
func total_impulse(thrust series f64) f64 {
    sum f64 := 0.0
    for x := thrust {
        sum = sum + x
    }
...
```arc
func find_peak(data series f64) i32 {
    peak f64 := 0.0
    peak_idx i32 := 0
    for i, x := data {
        if x > peak {
...
```arc
func sum_until_negative(data series i32) i32 {
    sum i32 := 0
    for x := data {
        if x < 0 {
            break
...
```arc
func sum_positive(data series i32) i32 {
    sum i32 := 0
    for x := data {
        if x < 0 {
            continue
