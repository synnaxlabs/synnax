...
only inside `` `...` ``. The other character is a literal everywhere it is not the
delimiter, so no escape is needed. Any other backslash sequence (e.g., `\z`) is
...
character can appear in the body, but the backslash is preserved in the value
(`r"a\"b"` yields `a\"b`, four characters; `` r`a\`b` `` yields `` a\`b ``).
...
Inside a format string, `{` opens a placeholder. To write a literal `{`, escape it as
`\{`. A bare `}` outside a placeholder is always a literal `}`.
...
The `\{` escape is recognized in `f` and `rf` strings. In non-format strings, `{` is
already literal so no escape is needed.
