...
Only `{` and `` ` `` have syntactic weight inside a body, so they are the only
characters that need escaping. A bare `}` is always a literal character.
...
Modules must be imported before their qualified members (`time.now`, `authority.set`,
`status.set`, …) can be used. A single import uses the bare form; multiple imports go in
an `import ( … )` block.
