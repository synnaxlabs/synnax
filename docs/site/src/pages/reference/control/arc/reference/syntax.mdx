...
Strings are enclosed in double quotes. Two forms:

- `"..."` — single-line. Cannot contain unescaped newlines.
- `"""..."""` — multi-line. Newlines and single `"` characters are allowed in the body;
...
Any other backslash sequence (e.g., `\z`) is preserved as the two literal characters;
only the escapes listed above are interpreted.
...
In a single-line raw string, `\"` is accepted by the lexer so a `"` can appear in the
body, but the backslash is preserved in the value (`r"a\"b"` yields `a\"b`, four
characters). Triple-quoted raw strings have no need for this since `"""` is the only
sequence that ends them.
