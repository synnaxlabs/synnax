...
| Sequences       | `sequence` `stage` `next`                                                                   |
| Control         | `authority`                                                                                 |
| Literals        | `true` `false`                                                                              |
| Data types      | `i8` `i16` `i32` `i64` <br /> `u8` `u16` `u32` `u64` <br /> `f32` `f64` <br /> `str` `bool` |
| Composite types | `chan` `series`                                                                             |
...
```arc channels="pressure, pump_err_code"
...
default string form unless a format spec is given. A `bool` renders as `true` or
`false`. Format specs follow printf-style conventions and are validated against the
placeholder type at compile time.
...
Authority declarations set the
[control authority](/reference/control/arc/reference/standard-library/control#authority)
...
```arc channels="valve, sensor, pressure"
stage pressurize {
    true -> valve
    sensor -> controller{}
    pressure > 500 => next
