...
next: "Variables"
nextURL: "/reference/control/arc/reference/variables"
---
...
| `i64` | 64-bit signed   | -9.2×10<sup>18</sup> to 9.2×10<sup>18</sup> |
| `u8`  | 8-bit unsigned  | 0 to 255                                    |
| `u16` | 16-bit unsigned | 0 to 65,535                                 |
| `u32` | 32-bit unsigned | 0 to 4,294,967,295                          |
| `u64` | 64-bit unsigned | 0 to 1.8×10<sup>19</sup>                    |
...
| Type   | Description                           |
| ------ | ------------------------------------- |
| `bool` | `true` or `false`, stored as one byte |
...
chan bool // channel of boolean
```

Channel names reference channels that exist in your Synnax cluster. Binding one to a
name declares a channel read/write
[variable](/reference/control/arc/reference/variables) that reads and writes the
channel:
...
Types can carry a single unit suffix for dimensional tracking. The suffix is one
identifier drawn from the built-in registry (length, time, mass, pressure, frequency,
voltage, current, temperature, angle, data, count):
...
Only single-identifier units are accepted in source syntax. Compound units like `m/s` or
...
The dimensions of `speed` and `area` are tracked by the compiler and checked against any
...
Explicit casting converts between types.
...
active := n > 0
level u8 := u8(active) // bool to int: 1
state str := str(active) // bool to string: "true"
```

<Note.Note variant="info">
  Boolean conversions (`bool(x)`) are not supported. Use a comparison (`x != 0`) to
  derive a `bool`. Casting a string to a numeric or boolean is not supported either.
</Note.Note>
...
| `bool` to numeric (e.g., `u8(b)`)   | `1` or `0`                                   |
| `bool` to string (`str(b)`)         | `"true"` or `"false"`                        |

Float values render without trailing zeros (`str(1.0)` returns `"1"`), and special
values render as `"NaN"`, `"+Inf"`, `"-Inf"`. Casting from `str` back to a numeric or
...
To build strings from runtime values, use
[format strings](/reference/control/arc/reference/syntax#format-strings) with `{expr}`
placeholders instead of repeated concatenation.
