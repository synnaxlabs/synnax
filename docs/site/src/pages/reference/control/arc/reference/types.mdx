...
active := bool(n) // numeric to bool: n != 0
level u8 := u8(active) // bool to numeric: 1 or 0
```

<Note.Note variant="info">
  Casting a string to a numeric or boolean is not supported.
</Note.Note>
### Casting Rules
...
| Numeric to `bool` (e.g., `bool(x)`) | `false` if `x` is zero, otherwise `true`     |
| `bool` to numeric (e.g., `u8(b)`)   | `1` for `true`, `0` for `false`              |
