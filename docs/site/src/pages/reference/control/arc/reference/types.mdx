...
<Note.Note variant="info">
  Boolean conversions (`bool(x)`) and casts between `bool` and numeric types are not
  supported. Use a comparison (`x != 0`) to derive a `bool`. Casting a string to a
  numeric or boolean is not supported either.
</Note.Note>
