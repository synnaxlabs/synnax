...
A sequence is a state machine containing ordered stages. Only one stage is active at a
time per sequence.
...
Stage bodies contain flow statements separated by commas. All flow statements in a stage
run concurrently while the stage is active.
...
4. Channel triggers only react to new writes — values written before stage activation
   are ignored
