...
prev: "`time`"
prevURL: "/reference/control/arc/reference/standard-library/time"
---
...
| `"X has more than one parameter"`                      | Function needs a routing table for multiple inputs    |
...
| `"authority value must be an integer in range 0-255"`  | Authority value out of range                          |
| `"authority declaration must appear before ..."`       | Authority block after func/sequence declarations      |
| `"multiple default authority values"`                  | More than one bare numeric default in authority block |
| `"duplicate authority for 'X'"`                        | Same channel listed twice in authority block          |
| `"channel 'X' not found"`                              | Authority references a channel that doesn't exist     |
