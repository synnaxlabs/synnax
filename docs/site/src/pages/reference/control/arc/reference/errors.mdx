...
Arc errors fall into two categories: compile-time errors (caught before deployment)
...
A runtime error in one Arc task only stops that task. Other tasks and the
hardware driver continue running normally. The error status is reported to
Console where you can see what went wrong.
