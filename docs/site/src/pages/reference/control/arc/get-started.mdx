...
```arc channels="data_channel, data_scaled"
import time
...
For more details on channel types and creation options, see the
[channels page](/reference/client/channels).
...
[**stateful variable**](/reference/control/arc/reference/variables#stateful), a value
that persists across invocations. Regular variables (`:=`) reset every time the function
runs, but stateful variables remember their value.
...
```arc channels="tank_pressure, pressure_scaled"
...
`time.interval` fires on a schedule. Here it triggers every 200 milliseconds, which runs
`output_reading` five times per second.
...
- **Input parameter** (`{limit f64}`): A constant set when the function is instantiated.
...
- **[Reactive execution](/reference/control/arc/concepts/reactive-execution)** -> how
  Arc schedules computations and the difference between continuous (`->`) and
  conditional (`=>`) edges
- **[Channels and Series](/reference/control/arc/concepts/channels-and-series)**: Work
  with telemetry data and array operations
- **[Stateful variables](/reference/control/arc/reference/variables#stateful)**: Persist
