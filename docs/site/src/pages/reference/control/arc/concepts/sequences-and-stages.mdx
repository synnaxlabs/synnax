...
1. Open the pressurization valve with `1 -> press_vlv_cmd`.
2. Enter `pressurize`, a stage where all three transitions run in parallel. Arc watches
   for target pressure, for over-pressure, and for the abort button.
3. Close the valve with `0 -> press_vlv_cmd`.

The stage exits when one of its transitions fires. `=> next` advances to the next item
(closing the valve). `=> abort` jumps to the `abort` sequence, which vents the tank.
