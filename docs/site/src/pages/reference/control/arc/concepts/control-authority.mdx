...
inside stages to change authority during execution:

```arc channels="sensor, output, press_vlv_cmd, start_cmd", bodies="escalate, done"
import control
import time
...
`control.set_authority` and channel writes can safely coexist in the same stage.
...
<Note.Note variant="warning">
  ...
  You must explicitly call `control.set_authority&#123;value=0&#125;` to yield.
</Note.Note>
