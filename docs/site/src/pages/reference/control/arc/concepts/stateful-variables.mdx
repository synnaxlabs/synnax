...
Arc gives you two ways to work with repeated data, and they serve different purposes.

**[`for` loops](/reference/control/arc/reference/loops)** process data you already have.
If a function receives a series, a loop can walk through it in a single execution:

```arc
func batch_average(data series f64) f64 {
    sum f64 := 0.0
    for x := data {
        sum = sum + x
    }
...
Use `for` loops for batch processing within a single function call. Use stateful
variables for streaming computations that build up over time.
