...
prev: "Control Authority"
prevURL: "/reference/control/arc/concepts/control-authority"
...
```arc channels="tank_pressure, pressure_display"
...
```arc channels="tank_pressure, pressure_scaled, sensor_1, sensor_2, average_display"
// Scale sensor readings continuously
tank_pressure -> scale{factor=2.0} -> pressure_scaled

// Run a control loop every 50ms
time.interval{period=50ms} -> controller{}
...
propagates to its target. On ticks where it's falsy, it doesn't.

Edges whose target is a stage or sequence name are **transitions**. See
[Sequences and Stages](/reference/control/arc/concepts/sequences-and-stages#transitions)
for how transitions combine with stage activation.

Timing nodes (`time.interval` and `time.wait`) reset on stage re-entry. A
`time.interval` fires immediately on the first tick after re-entry, and a `time.wait`
restarts its countdown from zero.

<Note.Note variant="warning">
A common mistake is using `->` when you meant `=>`. `->` fires on every data arrival
from the source. `=>` fires only while the source is truthy. For most stage
transitions, `=>` is what you want.
</Note.Note>
...
```arc channels="valve_cmd, tank_pressure, throttle"
stage pressurize {
    1 -> valve_cmd
...
```arc channels="start_btn, stop_btn", bodies="idle"
sequence main {
    stage idle {
        start_btn => next
    }
