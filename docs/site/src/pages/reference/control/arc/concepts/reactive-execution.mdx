...
description: "How Arc executes programs using reactive dataflow and stratified scheduling"
...
Arc prevents this. Both functions see exactly the same value. Either both see 400 PSI
or both see 500 PSI, never a mix.

This is called **stratified execution**, and it's why Arc programs are predictable
and safe.
...
**reactive dataflow model** where computations run automatically in response to new
data arriving on channels.
