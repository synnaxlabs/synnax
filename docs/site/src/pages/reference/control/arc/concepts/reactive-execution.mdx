...
Timing nodes (`interval` and `wait`) reset on stage re-entry. An `interval` fires
immediately on the first tick after re-entry, and a `wait` restarts its countdown from
zero.
