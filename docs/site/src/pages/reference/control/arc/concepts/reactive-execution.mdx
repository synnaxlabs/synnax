...
No polling. No manual scheduling. When `tank_pressure` receives new data, Arc
automatically runs `scale` and updates `pressure_display`.
...
```arc
func find_max(readings series f64) f64 {
    peak f64 := 0.0
    for x := readings {
        if x > peak {
            peak = x
...
This separation is what keeps Arc programs predictable. The reactive layer handles
timing, ordering, and consistency. Functions handle the math.
