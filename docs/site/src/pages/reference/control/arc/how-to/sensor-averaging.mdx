...
Sensor data is often noisy. A pressure reading might fluctuate by several PSI even
when the actual pressure is stable. Averaging reduces this noise, giving you a more
accurate picture of the true value.
...
In test sequences, you often want to reset statistics at specific points. Use a
separate reset channel:
...
Wire a button or stage transition to the `stats_reset` channel to clear the
accumulated values.
...
Using `interval` ensures the average is calculated at a fixed rate regardless of
when individual sensor updates arrive.
...
The median rejects a single faulty sensor reading, making it useful for
