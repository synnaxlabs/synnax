...
Many useful values aren't measured directly but calculated from other measurements.
Mass flow rate depends on pressure and temperature. Thrust depends on chamber pressure
and throat area. Arc lets you compute these derived values in real-time.

<Divider.Divider x />

## Multi-Input Functions

When a calculation needs multiple sensor values, pass the channels as config
parameters:
