...
The [Variables](/reference/control/arc/reference/variables#stateful) reference page
explains persistence in more depth.
...
compile time, so a `.2f` against an integer is caught before the program runs. A `bool`
placeholder renders as `true` or `false`.
