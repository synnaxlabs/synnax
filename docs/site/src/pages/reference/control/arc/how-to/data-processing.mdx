...
Backtick format strings build human-readable status messages from live channel values.
Placeholders inside `{...}` accept any expression, and a `%spec` after the expression
controls formatting:

```arc
func status{sensor str} (value f64) str {
    return `{sensor}: {value%.2f} psi`
}

pressure -> status{sensor="ox_pt_1"} -> pressure_status
```

Each time `pressure` updates, `pressure_status` receives a string like
`"ox_pt_1: 523.70 psi"`. Format specs are validated against the placeholder type at
...
func alert(value f64) str {
    if value > 800.0 {
        return `HIGH PRESSURE: {value%.1f} psi`
    }
    return `pressure nominal: {value%.1f} psi`
}
...
See
[Format String Literals](/reference/control/arc/reference/syntax#format-string-literals)
for the full syntax and supported format specs.
