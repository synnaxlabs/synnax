...
```arc channels="temperature, temperature_celsius"
...
Input parameters make functions reusable. They're set when you instantiate the function,
not when data flows through:

```arc channels="pressure, pressure_doubled, temperature, temperature_halved"
...
Both flows use the same [function](/reference/control/arc/reference/functions#inputs)
...
```arc channels="pressure_raw, pressure_psi"
...
```arc channels="temperature, temperature_fahrenheit"
...
```arc channels="pressure, pressure_high"
...
```arc channels="pressure_1, pressure_2, pressure_avg"
...
The [Stateful Variables](/reference/control/arc/concepts/stateful-variables) concept
page explains persistence in more depth.
...
```arc channels="pressure, pressure_change"
...
```arc channels="pressure, pressure_smooth"
...
[Format strings](/reference/control/arc/reference/syntax#format-strings) build
human-readable status messages from live channel values. Placeholders inside `{...}`
accept any expression, and a `:spec` after the expression controls formatting:

```arc channels="pressure, pressure_status"
...
Each time `pressure` updates, `pressure_status` receives a string like
`"Pressure: 523.70 psi"`. Format specs are validated against the placeholder type at
compile time, so a `.2f` against an integer is caught before the program runs.

Format strings are useful for writing diagnostic outputs:

```arc channels="pressure, log"
