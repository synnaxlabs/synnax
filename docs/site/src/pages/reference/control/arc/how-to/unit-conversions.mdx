...
The config parameters (`v_min`, `v_max`, `psi_min`, `psi_max`) are set when the function
is instantiated, making it reusable across different sensors.
...
// Strain gauge: 1V = 100 microstrain, 5 microstrain offset
strain_raw -> scale{
    gain=100.0,
    offset=5.0
} -> strain_microstrain
...
tc_raw -> mv_to_volts{} -> volts_to_temp{
    v_ref=0.0,
    temp_coeff=0.041
} -> tc_temp
```
...
<Note.Note variant="info">
  ...
  debugging harder. If you have a complex multi-step conversion, consider combining the
  math into a single function.
</Note.Note>
...
The return type `f64 bar` documents that this function returns a value in bar units. The
compiler will catch if you accidentally use this where a PSI value is expected (in
functions that declare their parameters with units).

<Divider.Divider x />

## Reusable Conversion Patterns

For systems with many sensors of the same type, create a single conversion function and
instantiate it for each channel:
...
pressure_1_raw -> pt_convert{
    v_min=0.5,
    v_max=4.5,
    p_max=500.0
} -> pressure_1

pressure_2_raw -> pt_convert{
    v_min=0.5,
    v_max=4.5,
    p_max=500.0
} -> pressure_2

pressure_3_raw -> pt_convert{
    v_min=0.5,
    v_max=4.5,
    p_max=1000.0
} -> pressure_3
