...
to a channel, fetch a range and set its data, or leverage writers for writing large
volumes of data.
...
Writing to a channel requires us to write to it index before we write its data. We'll 
create the following channels to use as examples:
...
We'll make sure to write timestamps to the index before we write the data to the channel:
...
Synnax will raise a `ValidationError` if the index channel does not contain a corresponding 
timestamp for every sample in the data channel. After all, it wouldn't make sense to have
...
to write data in a streaming fashion for use in control sequences and live data processing.
The `Writer` class is designed for this use case (and is actually used under the
hood by the other methods).
...
properly closed after use, ensuring that resources have been freed and sockets are closed.

If you can't use a context manager, ensure you call `writer.close()` when you're done using it.