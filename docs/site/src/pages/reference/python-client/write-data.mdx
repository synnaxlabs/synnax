...
to write data in a streaming fashion for use in control sequences and live
...
Alternatively, you could initiate a writer in Auto-Commit mode by calling
`client.open_writer` with the additional configuration argument
`enable_auto_commit=True`. In Auto-Commit mode, a writer would
effectuate a commit after every write, thus persisting the data to the cluster.

Ensure that you call `writer.close()` in Auto-Commit mode as well.
