...
boolean indicating whether the writer has accumulated any errors on the call to write or
on any previous calls to write.
...
Let's say we're writing a time series to a channel, but we accidentally write the
timestamps out of order. This is a common mistake, and can be hard to debug. Here's how
we can use the writer to handle this:
...
timestamp, causing the context manager to complete, the writer to be closed, and a
...
successful. Failing to check this return value can result in the writer silently failing
to write data, causing hard to debug issues.
