...
If you are interested in streaming data at a lower rate than it is being written to the server, you can use the optional `downsample` to specify the number of frames to skip before returning the next frame:

```python
# setting downsample a factor of n means every nth sample will be retained
# e.g. for a downsample of 2, if channel being streamed receives [0.13, 0.12, 0.14, 0.13]
...
we're reading from two sensors, `temperature` and `pressure`, that are being sampled by 
