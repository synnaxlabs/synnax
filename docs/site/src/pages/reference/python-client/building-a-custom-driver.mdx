...
A common workflow is to have a single thread/program that:

1. Creates a writer for the data channels.
2. Poll the external system for data.
3. Generate timestamps for the data, if the device does not provide timestamps directly.
4. Write the data to Synnax using the writer.
5. Repeat.

How frequently you poll the external system and how much data you write at one time is
up to you. We recommend writing in larger batches if possible to improve efficiency.

We recommend you read the sections on [writing via the python
client](/reference/python-client/writing)
and the [general writing overview](/reference/concepts/writing) for more information.
