...
If you've installed Synnax in a virtual environment, you'll need to use `uv run` to run
commands within that environment. For example:

```bash
uv run sy --help
```

This ensures the command runs within the correct virtual environment.
