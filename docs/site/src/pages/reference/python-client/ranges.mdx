...
There are two ways to create child ranges: by creating a sub-range of a parent range, or 
specifying a parent range.

### Creating a Sub-Range of a Parent  

```python
child_range = parent_range.create_sub_range(
    name="My Child Range",
    time_range=sy.TimeRange(
        start = datetime.strptime("2023-2-12 12:30:00", time_format),
        end = datetime.strptime("2023-2-12 14:30:00", time_format),
...
### Defining a Parent by Ontology ID
...
We can retrieve child ranges using the `client.ontology.retrieve_children` method.

```python
child_ranges = client.ontology.retrieve_children(my_range.ontology_id)
```
