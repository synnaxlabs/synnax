...
If we only want to create a range if one with the same name doesn't already exist,
...
Synnax will raise a `NotFoundError` if the range does not exist, and a `MultipleFoundError`
