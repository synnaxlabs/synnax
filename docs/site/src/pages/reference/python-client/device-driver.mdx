...
While our [pre-built drivers](/reference/device-drivers) are great for the
...
The best way to get started with the Arduino is to download and run the [Arduino
IDE](https://www.arduino.cc/en/software).

We'll be using an [Arduino Mega 2560](https://store.arduino.cc/products/arduino-mega-2560-rev3)
for this guide, but any Arduino board will work.
...
Now that we have the cluster running, it's time to download and run the [Synnax
Console](https://docs.synnaxlabs.com/reference/console/get-started).
...
samples that we collect, and is used so that synnax can look up the samples in our
data channels at a later time.
...
to the `arduino_time` and `arduino_value` channels. Writers are the primary method for
writing data to Synnax, and are created by calling the `open_writer` method on the
client we created.
...
While the script is still running, we can switch back to the Synnax Console and set up
a line plot to visualize the incoming data. To do this, we'll:
...
of reading from the serial connection, we'll be reading from the `arduino_command` channel.
...
You may have found it strange that we set the "State" and "Command" fields on the
switch to the same channel. If that's the case, then why are there two separate fields?
That's because there's a more reliable way to set up command systems.

The **command** channel is what we send the enable/disable signal down when we click
on the switch, while the **state** channel is what we actually use to determine whether
the switch is on or off. For the sake of simplicity, we used the same channel for both.
...
Both the `arduino_state` and `arduino_value` channels will be indexed by the `arduino_time`
...
connection. To kick things off, we'll open a streamer on the arduino command and a writer
