...
And more. In this section, we'll cover how to install the client and
...
The `synnax` library requires Python 3.10 or higher, and is available on [PyPI](https://pypi.org/project/synnax/).
Install it directly using `pip` or define it as a requirement in your virtual
...
a troubleshooting guide that you can reference [here](troubleshooting).
</Note>
...
store your credentials, or by passing them directly to the client.
...
We highly recommend this method when using Synnax for data analysis, as it makes
...
<Note variant="info">
If you get the error `command not found`, you might not have Python scripts
...
Synnax! We recommend starting with the [Channels](channels) section
