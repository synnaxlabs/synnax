...
The easiest way to install the Driver is to use an SSH connection to the cRIO.
Once the driver is installed, SSH can be disabled, although we recommend keeping it
enabled to install updates and manage the driver.

#### Network Available Synnax Core

In order to function properly, the Driver must be able to connect to a Synnax Core
running on the same local network. Make sure you have a
[Core running](/reference/cluster/quick-start) and have the connection parameters on
hand.

### Install and Run the Driver

Installing the Driver is as simple as opening an SSH connection to the cRIO and
...
Download the latest Synnax Driver executable for Windows from the
...
To verify that the installation was successful, open a new **PowerShell** terminal and
run:
...
To connect to the Core, you'll need to know the Core's IP address, port, username, and
...
```json
{
  ...
  "connection": {
    ...
    // to the Synnax Core. This is only required when the Core
    ...
    // Synnax Core. This is only required when the Core is
...
# The path to the CA certificate file to use when connecting
# to the Synnax Core. This is only required if
# the Core is configured to use TLS.
