...
Every instance of the [Synnax server](/reference/core/quick-start) comes with a built-in
device driver. This driver will run on the same machine as the Synnax server, and starts
automatically when the server starts. The embedded driver is recommended as the default
