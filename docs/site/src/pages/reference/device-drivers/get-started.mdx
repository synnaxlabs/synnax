...
Synnax Driver embedded inside. This driver will run on the same machine as the Synnax server,
and starts automatically when the server starts. The embedded driver is recommended as
the default driver for most users.

### Driver Installation

The [Synnax Driver](/reference/device-drivers/installation) can also be installed as a separate binary
that can run on a different machine from the Synnax server. It is used to integrate data
acquisition devices across multiple machines. The Driver connects to a Synnax Core over the
network.
