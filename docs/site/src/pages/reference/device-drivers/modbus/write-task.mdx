...
heading: "Modbus Write Task"
description: "Write data to Modbus devices with write tasks."
prev: "Read Task"
...
Write tasks are the primary method for sending control commands to a Modbus device from
Synnax.
...
process that writes data to registers or coils on the Modbus device based on values
written to Synnax [command channels](#how-commands-work-in-synnax).

A Modbus write task can be started, stopped, and re-configured at any time. We
...
<Table>
  ...
  <tbody>
    ...
    <tr>
      <td>Modbus Device</td>
      <td>Which Modbus device the task will write to.</td>
    </tr>
    <tr>
      <td>Data Saving</td>
      {
