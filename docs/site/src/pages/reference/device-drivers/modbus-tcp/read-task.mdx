...
heading: "Modbus Read Task"
description: "Read data from Modbus devices with read tasks."
next: "Write Task"
nextURL: "/reference/device-drivers/modbus-tcp/write-task"
prev: "Connect Device"
prevURL: "/reference/device-drivers/modbus-tcp/connect-device"
---
...
Read tasks are the primary method for acquiring data from a Modbus device and storing it
in Synnax.
...
[connected to a Modbus device](/reference/device-drivers/modbus-tcp/connect-device).
...
defines a background process that reads data from your device at a specified sampling
rate.

A Modbus read task can be started, stopped, and re-configured at any time. We
...
Synnax uses software timing to read from your Modbus device, and typically delivers
...
<Video client:only="react" id="device-drivers/modbus-tcp/read-task/open" />

You can also open the dialog by clicking in the top-right corner of the window and
selecting the add <Icon.Add /> icon, or opening the context menu on a Modbus device from
...
<Table>
  ...
  <tbody>
    ...
    <tr>
      <td>Modbus Server</td>
      <td>Which Modbus device the task will read from.</td>
    </tr>
    <tr>
      <td>Auto Start </td>
    ...
    <tr>
      <td>Sample Rate</td>
      <td>How often, in Hz, Synnax polls data from the Modbus device.</td>
    </tr>
    <tr>
      <td>Stream Rate</td>
      {
...
stream rate. The sample rate defines how often Synnax will poll the Modbus device for
...
will poll the Modbus device every 100 ms, collect 10 samples, and then forward these
...
<Video client:only="react" id="device-drivers/modbus-tcp/read-task/channels" />
...
<Video client:only="react" id="device-drivers/modbus-tcp/read-task/start-stop" />
