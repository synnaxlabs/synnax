...
title: "Modbus TCP | Read Task"
heading: "Modbus TCP Read Task"
description: "Read data from Modbus TCP devices with read tasks."
next: "Write Task"
...
Read tasks are the primary method for acquiring data from a Modbus TCP device and
storing it in Synnax.
...
for both control and data acquisition purposes. In the context of Modbus TCP, a read
task defines a background process that reads data from your device at a specified
sampling rate.

A Modbus TCP read task can be started, stopped, and re-configured at any time. We
...
Synnax uses software timing to read from your Modbus TCP device, and typically delivers
...
<Table>
  ...
  <tbody>
    ...
    <tr>
      <td>Modbus Server</td>
      <td>Which Modbus TCP device the task will read from.</td>
    </tr>
    <tr>
      <td>Auto Start </td>
...
Modbus TCP supports four types of data storage areas, each with a specific purpose:
