...
device and created the relevant channels in Synnax. Check out the [device
configuration](/reference/device-drivers/labjack/configure-device) guide for more
information.
...
LabJack Driver, a read task defines a background process that reads data from the analog
or digital inputs on a particular LabJack device.
