...
description: "Learn how to read data from LabJack devices using Synnax."
...
The LabJack Read Task is used for acquiring analog and digital data from LabJack
devices. In this guide, we'll walk you through the steps to create a LabJack Read Task
in Synnax.
...
[connected and configured a LabJack device](/reference/device-drivers/labjack/configure-device)
and created the relevant channels in Synnax.
...
Synnax can only accept live data for a channel from one source at a time. If you have a
...
To create a read task from the Devices Toolbar, open the Synnax Console and click on the
device icon (<Icon.Device />) on the left-hand side of the screen. Find the device you'd
like to create the task for, right-click on it, and select "Create read task".
...
In command mode (enabled when the first character in the input is ">"), type "LabJack".
You'll see an option called "Create a LabJack Read Task". Select this option to open the
...
To create a read task from the layout selector, click on the add icon (<Icon.Add />) in
