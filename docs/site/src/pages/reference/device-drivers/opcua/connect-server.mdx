...
<Video client:only="react" id="device-drivers/opcua/opcua-connect-server" />
...
nodes and present them as a list along with their data types, paths, and browsable names.
...
has a special Index channel that is used to store the timetamps for the samples recorded
...
<Video client:only="react" id="device-drivers/opcua/opcua-configure-sampling-groups" />
...
<Video client:only="react" id="device-drivers/opcua/opcua-configure-remove-channels" />
...
<Video client:only="react" id="device-drivers/opcua/opcua-configure-channel-props" />
...
from the server, delete this channel, find the corect node you'd like to read 
...
`Int64` or `UInt64` data types, but you'll need to ensure that the server records the timstamps
in the correct format.

<Video client:only="react" id="device-drivers/opcua/opcua-configure-edit-index" />
...
request. We've successfully tested this configuration up to 10KHz. Correclty setting
...
<Image client:only="react" id="device-drivers/opcua/opcua-high-rate-example-config" />
