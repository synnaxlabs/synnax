...
controllers running NI Linux RT 2024 Q1 and later.

#### NI-DAQmx Programming Mode

Data acquisition hardware connected to the cRIO must be configured to use the
NI-DAQmx real-time programming mode.

To change the programming mode, use NI Measurement & Automation Explorer (MAX). For
detailed instructions, see the [NI CompactRIO User Manual](https://www.ni.com/docs/en-US/bundle/ni-compactrio/page/programming-modes.html?srsltid=AfmBOoqFu5wG9ATXqPpjwPAodfxzyjIfzWoli0P7_R7flU0G9c0WrQrN)
and the [NI-DAQmx on CompactRIO](https://knowledge.ni.com/KnowledgeArticleDetails?id=kA00Z0000019PUNSA2&l=en-US)
guide.
...
Installing the Synnax standalone driver is as simple as opening an SSH connection to
the cRIO and running the following command:
