...
The standalone driver can optionally read connection parameters from a JSON configuration
...
```json
{
  ...
  "connection": {
    ...
    // This is only required if the cluster is configured to use TLS.
    "ca_cert_file": "/path/to/ca.crt",
    // The path to the client certificate file to use when connecting to the Synnax cluster.
    // This is only required when the cluster is configured to use TLS and client certificates
    // for authentication.
