...
Before configuring the task, you'll need to have [connected to an OPC UA
server](/reference/device-drivers/opc-ua/connect-server).
...
for both control and data acquisition purposes. In the context of OPC UA, a write task
defines a background process that writes data to nodes on the OPC UA server based on values 
...
"OPC UA". You'll see an option called "Create a New Write Task". Select this
option to open the write task configuration dialog.
...
top right corner and select "OPC write Task".
...
<Table>
    ...
    <tbody>
        ...
        <tr>
            <td>Data Saving</td>
            <td>Data saving defines whether acquired data will be permanently stored in
            Synnax. When enabled, Synnax will both stream and store data. When disabled,
            Synnax will only stream data for real-time visualization. Historical data
            will not be available for review.</td>
        </tr>
    </tbody>
</Table>
...
Note that configuring a write task here will only create [command channels](#how-commands-work-in-synnax).
In order to create state channels, you must configure a [read task](/reference/device-drivers/opc-ua/read-task)
...
states of the nodes you are writing to. For more information on configuring a read task, see [the following section.](/reference/device-drivers/opc-ua/read-task)
{/* TODO: ADD VIDEO creating a read task and naming channels to be state*/}


## Using a Schematic to Control Write Tasks

Once the necessary tasks are configured, you can use the [console schematic](/reference/console/schematics) to write commands to the OPC UA server.

{/* TODO: ADD VIDEO of right after both tasks have been started and channels are created. In the video the schematic is created with a single valve and the command and state channels are plotted */}
