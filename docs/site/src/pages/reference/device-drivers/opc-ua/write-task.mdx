...
<Table>
  ...
  <tbody>
    ...
    <tr>
      <td>Data Saving</td>
      <td>Data saving defines whether acquired data will be permanently stored in Synnax.
        When enabled, Synnax will both stream and store data. When disabled, Synnax will
        only stream data for real-time visualization. Historical data will not be
...
Note that configuring a write task here will only create [command
channels](#how-commands-work-in-synnax). In order to create state channels, you must
configure a [read task](/reference/device-drivers/opc-ua/read-task).
...
task](/reference/device-drivers/opc-ua/read-task). You can then use the read task to
read back states of the nodes you are writing to.
