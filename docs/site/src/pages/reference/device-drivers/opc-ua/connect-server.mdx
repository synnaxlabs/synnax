...
1. The [Synnax Core](/reference/core/quick-start) running on your network.
2. The [Synnax Driver](/reference/device-drivers/get-started) running on your network.
   Note that the Core comes with an embedded Driver that could be used for this purpose.
3. The [Synnax Console](/reference/console/get-started) installed on your machine.
4. An OPC UA server running on your network. This OPC UA server must be reachable by the
   Synnax Driver.
...
You'll see an option called "Connect an OPC UA Server". Select this option to open the
...
Synnax Core. When you are ready to connect the server, click "Save".
