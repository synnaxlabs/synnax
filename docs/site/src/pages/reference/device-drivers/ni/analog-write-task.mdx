...
and created the relevant channels in Synnax. Check out the [Configure
Device](/reference/device-drivers/ni/configure-device) guide for more information.
...
outputs of your device based on values written to Synnax [command
channels](#how-commands-work-in-synnax).
...
you'd like to create the task for, right-click on it, and select "Create Analog Write Task".
...
"NI". You'll see an option called "Create a New Analog Write Task". Select this
...
<Image client:only="react" id="device-drivers/ni/analog-write-task/state-rate" />
