...
The NI Digital Read Task is used for acquiring data from digital inputs on your NI
devices. In this guide, we'll walk you through the steps to create a digital read task
in Synnax.
...
A digital read task can be started, stopped, and re-configured at any time. Synnax
...
To create a digital read task from the Devices Toolbar, open the Console and click on
the device icon (<Icon.Device />) on the left-hand side of the screen. Find the device
you'd like to create the task for, right-click on it, and select "Create digital read
task".
...
To create a digital read task from the command palette, open the Console and click on
the Search and Command Palette at the top. You can also open this palette with with
`Ctrl+Shift+P` on Windows and `Cmd+Shift+P` on macOS.

In command mode (enabled when the first character in the input is ">"), type "NI".
You'll see an option called "Create an NI Digital Read Task". Select this option to open
the digital read task configuration dialog.
...
To create a digital read task from the layout selector, click on the add icon (<Icon.Add
/>) in the top-right corner and select "NI Digital Read Task".
