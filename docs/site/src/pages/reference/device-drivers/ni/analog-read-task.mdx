...
The Analog Read Task is used for acquiring data from analog inputs on your NI devices. In
this guide, we'll walk you through the steps to create an Analog Read Task in Synnax.
...
All channels in an analog read task are sampled at the same rate. If you'd like to sample
...
To configure a read task, open the Synnax Console and click on the Quick Search & Command
Palette at the top. You can also open this palette with `Ctrl+Shift+P` on Windows/Linux
...
### <StepText level="h3" step={2}>Add Channel to the Task</StepText>
