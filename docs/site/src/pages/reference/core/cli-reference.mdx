  ...
  data={[
    {
      ...
      description: `A reachable address and port to listen on for incoming client connections and RPCs from other nodes. To bind
            multiple addresses, each with its own certificate, provide a list of listener objects in a configuration file.
            See <a href="/reference/core/production#serving-multiple-listeners">Serving Multiple Listeners</a>.`,
    },
    {
      option: "--peers/-p",
      default: "[]",
    ...
    },
    {
      ...
      default: '"synnax"',
      description:
        'Username for the cluster root user. Reconciled on every start — see <a href="#root-user-reconciliation">Root User Reconciliation</a> for the behavior when this changes between restarts.',
    },
    {
      option: "--password",
      default: '"seldon"',
      description:
        'Password for the cluster root user. The configured value is the source of truth: if the stored password diverges, it is rotated to match on the next start. See <a href="#root-user-reconciliation">Root User Reconciliation</a>.',
    },
    {
      option: "--license-key",
      default: '""',
    ...
    },
    {
      ...
      default: "[]",
      description:
        "List of device integrations to disable. This is useful when you don't want particular integrations. All integrations are enabled by default. Options are: arc, ethercat, labjack, modbus, ni, opc",
    },
    {
      option: "--enable-integrations",
      default: "[]",
      description:
        "List of device integrations to enable. This is useful when you only want particular integrations. All integrations are enabled by default. Options are: arc, ethercat, labjack, modbus, ni, opc",
    },
    {
...
The `listen` key takes either form. A single address (as above) binds one listener over
the node certificate paths. A list of objects binds one listener per entry, each with
its own address and certificate. The list form works in configuration files only, not as
a flag. See
[Serving Multiple Listeners](/reference/core/production#serving-multiple-listeners) for
...
Each listener object accepts the following fields:

<Table.Data
  client:only="react"
  columns={[
    { key: "field", name: "Field", type: "code", width: 130 },
    { key: "description", name: "Description", type: "html" },
  ]}
  data={[
    {
      field: "address",
      description:
        "The <code>host:port</code> the listener binds to. Required and unique across listeners.",
    },
    {
      field: "cert.source",
      description:
        'The certificate source: <code>file</code>, <code>auto</code>, or <code>tailscale</code>. See <a href="/reference/core/production#certificate-sources">Certificate Sources</a>.',
    },
    {
      field: "cert.cert",
      description: "For a <code>file</code> source, the path to the PEM certificate.",
    },
    {
      field: "cert.key",
      description: "For a <code>file</code> source, the path to the PEM private key.",
    },
    {
      field: "advertise",
      description:
        "Marks this listener's address as the one peers dial to join the cluster. At most one listener may set it; the first listener is advertised if none do.",
    },
    {
      field: "name",
      description: "An optional label for the listener, shown in logs.",
    },
  ]}
/>
...
Every time a Core starts, it reconciles the configured `--username` / `--password`
against the existing cluster state. The configured credentials are the source of truth:
the Core makes the state match config, rather than asking the operator to log in and
update things by hand.
...
<Table.Data
  client:only="react"
  columns={[
    { key: "scenario", name: "Scenario", type: "html", width: 260 },
    { key: "behavior", name: "Behavior", type: "html" },
  ]}
  data={[
    {
      scenario: "Fresh Core (no existing users)",
      behavior:
        "A new user is created with the configured username and marked as the root user. Credentials are registered and the user is granted the <code>Owner</code> role.",
    },
    {
      scenario: "Restart with unchanged credentials",
      behavior: "No-op — the existing root user is left alone.",
    },
    {
      scenario: "Restart with the same username but a different password",
      behavior:
        "The stored password is rotated to match the configured one. The old password no longer authenticates. Other users' passwords are untouched.",
    },
    {
      scenario: "Restart with a different username",
      behavior:
        "The previous root user is demoted (the <code>RootUser</code> flag is cleared) but retains their <code>Owner</code> role and credentials, so they can still log in with full permissions. A new user is created with the configured username, marked root, and granted <code>Owner</code>.",
    },
    {
      scenario: "Configured username matches an existing <em>non-root</em> user",
      behavior:
        "The existing user is promoted in place: their <code>RootUser</code> flag is set to true, their stored password is rotated to match the configured one, and any previous root user is demoted. The user keeps the same key, name, and role assignments. This lets you point <code>--username</code> at an already-provisioned account to make it the root, instead of creating a new one.",
    },
    {
      scenario: "Restart with no credentials configured (both flags omitted)",
      behavior:
        "Auth state is left untouched. If multiple users somehow have <code>RootUser=true</code>, all but the first returned by the retrieve are demoted (keeping their <code>Owner</code> role) and a warning is logged. A cluster with zero or one root user is left as-is, so test setups and other callers that do not need a root user can open the Core without supplying credentials.",
    },
  ]}
/>
...
Because the configured password is the source of truth, you can recover a forgotten root
password by editing the Core's configuration (config file, environment variable, or
`--password` flag) and restarting the Core. The stored password will be rotated on
startup. Non-root users rotate their passwords through the API as normal — only the root
user's password is governed by configuration.

#### Demoted users

When a username change demotes the previous root user, that user keeps their `Owner`
role and their original credentials. They can still log in with full permissions; they
just no longer hold the immutable root-user flag. To fully remove them, an `Owner` can
delete the demoted user through the API.
...
The `doctor` command inspects the data a Core stored on disk and reports problems with
it: corrupt or unreadable telemetry, storage the garbage collector never reclaimed,
implausible timestamps, and metadata that references entities which no longer exist.

The command never writes to the data directory. Stop the Core before you run it: a
running Core holds the key-value store open, and the command then checks the telemetry
alone.

### Examples

```cmd
synnax doctor --data /mnt/ssd1
```

Add `--verbose` for a per-channel table of domain counts, sample counts, disk use, and
garbage. Add `--json` to write the whole report as one JSON document instead of text.
...
<Table.Data
  client:only="react"
  columns={[
    { key: "code", name: "Code", type: "code", width: 100 },
    { key: "description", name: "Meaning", type: "html" },
  ]}
  data={[
    { code: "0", description: "The command ran. No errors were found." },
    { code: "1", description: "The command could not run." },
    {
      code: "2",
      description: "The command ran and found at least one error in the stored data.",
    },
  ]}
/>
  ...
  data={[
    {
      option: "--data/-d",
      default: '"synnax-data"',
      description: "The file path to the folder holding the data to inspect.",
    },
    {
      option: "--channels",
      default: "[]",
      description: `A list of comma-separated channel keys. Restricts the telemetry checks to those
            channels. The default inspects every channel.`,
    },
    {
      option: "--skip-deep",
      default: "false",
      description: `Skip the checks that read sample bytes. These checks are the slowest, and the only
            ones that read the telemetry itself.`,
    },
    {
      option: "--skip-kv",
      default: "false",
      description: "Skip every check on the key-value store.",
    },
    {
      option: "--skip-ts",
      default: "false",
      description: "Skip every check on the time-series store.",
    },
    {
      option: "--json",
      default: "false",
      description: "Write the report as one JSON document instead of text.",
    },
    {
      option: "--verbose/-v",
      default: "false",
      description: "Include notes and the per-channel statistics table.",
    },
  ]}
/>
