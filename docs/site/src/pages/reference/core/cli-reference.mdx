...
<Table.Data
  ...
  data={[
    ...
    },
    {
      ...
      behavior:
        "The Core refuses to start with an error. The operator must rename the conflicting user or pick a different root username — the Core will not silently promote an existing account.",
    },
    {
      scenario: "Restart with no credentials configured (both flags omitted)",
      behavior:
        "Auth state is left untouched. If multiple users somehow have <code>RootUser=true</code>, all but the lexicographically-first by username are demoted (keeping their <code>Owner</code> role) and a warning is logged. If <em>no</em> root user exists, the Core refuses to start — an empty cluster has no admin, and without credentials there is no way to create one.",
    },
  ]}
/>
