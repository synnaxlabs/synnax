...
description:
  "Hardware and software requirements for deploying a Synnax Core to production."
next: "CLI Reference"
...
For most production scenarios, we recommend running a recent glibc-based Linux
distribution, such as [Ubuntu](https://ubuntu.com/download/server) 24.04 LTS or later,
...
certain operating systems are supported. Minimum OS versions are listed in the
[driver installation guide](/reference/driver/installation).
...
<table>
  ...
  <tbody>
    <tr>
      <td>[National Instruments](/reference/driver/ni)</td>
      <td>✓</td>
      <td>✓</td>
      <td></td>
      <td></td>
    ...
    <tr>
      <td>[LabJack](/reference/driver/labjack)</td>
      <td>✓</td>
      <td>✓</td>
      <td>✓</td>
      <td></td>
    ...
    <tr>
      <td>[OPC UA](/reference/driver/opc-ua)</td>
      <td>✓</td>
      <td>✓</td>
      <td>✓</td>
      <td>✓</td>
    ...
    <tr>
      <td>[Modbus](/reference/driver/modbus)</td>
      <td>✓</td>
      <td>✓</td>
      <td>✓</td>
      <td>✓</td>
...
<Note.Note variant="error">
  **Critical Security and Performance Requirement** -> Never use public internet
...
- **Isolated Networks** -> Deploy Synnax Cores on dedicated, air-gapped networks
  separate from corporate IT infrastructure.
- **Network Segmentation** -> Use VLANs or physical separation to isolate control
  traffic from data acquisition traffic.
