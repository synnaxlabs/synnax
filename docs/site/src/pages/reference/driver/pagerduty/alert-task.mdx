...
The PagerDuty integration watches status keys and opens or resolves PagerDuty incidents
when they change. Error, warning, and info variants send an event with the matching
severity. Success resolves the open incident. Events are deduplicated by status key, so
repeated changes update the existing incident.
...
2. A PagerDuty service with an Events API v2 integration. In PagerDuty, open `Services`
   -> `Service Directory`, select the service, then `Integrations` -> `Add Integration`
   -> `Events API v2`.
