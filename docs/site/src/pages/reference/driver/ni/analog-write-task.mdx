...
<ChannelTypeDetails>
<Text.Text slot="summary" level="p"><Icon.Units.Voltage /> Voltage (`ao_voltage`)</Text.Text>
...
<ChannelTypeDetails>
<Text.Text slot="summary" level="p"><Icon.Units.Current /> Current (`ao_current`)</Text.Text>
...
task = client.hardware.tasks.retrieve(name="Analog Write")

# List all analog write tasks
analog_write_tasks = [t for t in all_tasks if t.type == "ni_analog_write"]
```
...
// Retrieve task by name
const task = await client.hardware.tasks.retrieve({ name: "Analog Write" });

// List all analog write tasks
const analogWriteTasks = allTasks.filter(t => t.type === "ni_analog_write");
```
...
// Retrieve task by name
auto [task, retrieve_err] = rack.tasks.retrieve("Analog Write");
if (retrieve_err) return retrieve_err;

// List all analog write tasks
std::vector<synnax::Task> analog_write_tasks;
for (const auto& t : tasks) {
    if (t.type == "ni_analog_write") {
        analog_write_tasks.push_back(t);
    }
}
```
...
// Retrieve task
auto [task, task_err] = rack.tasks.retrieve("Analog Write");
