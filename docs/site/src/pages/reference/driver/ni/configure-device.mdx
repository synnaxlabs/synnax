...
The Synnax Driver supports every [National Instruments](https://www.ni.com/en-us.html)
device that the
[NI-DAQmx driver](https://www.ni.com/en-us/support/downloads/drivers/download.ni-daqmx.html)
supports, including CompactDAQ, CompactRIO, and PXI systems. Chassis and their modules
are discovered automatically and shown hierarchically in the Console.
...
// Or retrieve by model
const [byModel] = await client.devices.retrieve({ models: ["NI 9205"] });

// Or retrieve by name
const [byName] = await client.devices.retrieve({ names: ["My NI Module"] });
