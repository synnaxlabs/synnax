...
| `rate`        | number  | Yes      | -       | Polling rate in Hz                                                                                     |
...
| `fields`       | array  | Yes      | -       | Fields to extract from the response. See [Field Extraction](#field-extraction-reference) below |
...
| `timestamp_format` | string  | No       | -           | Timestamp format (required when the channel data type is `TIMESTAMP`). See [Timestamp Handling](#timestamp-handling) below |
| `enum_values`      | object  | No       | -           | String-to-number mapping for enum fields (e.g., `{"OFF": 0, "ON": 1}`)                                                     |
...
{/* TODO: Add Console video for configuring an HTTP read task */}
