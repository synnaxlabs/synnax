...
<Note.Note variant="warning" className="compact">
  Setting `verify_ssl` to `false` disables certificate validation. This is useful for
  development and self-signed certificates, but should not be used in production.
</Note.Note>
...
<Details>
<Text.Text slot="summary" level="p"><Icon.Details /> Basic Auth</Text.Text>
...
| `send_as`   | string | No       | `"header"` | How to send the key: `"header"` or `"query_param"`                |
...
| Parameter           | Type    | Required | Default     | Description                                                     |
| ------------------- | ------- | -------- | ----------- | --------------------------------------------------------------- |
| `method`            | string  | No       | `"GET"`     | HTTP method for the health check (`GET` or `POST`)              |
| `path`              | string  | No       | `"/health"` | URL path to ping                                                |
...
| `validate_response` | boolean | No       | `false`     | Whether to validate the response body against an expected value |
...
When `validate_response` is `true`, you must also provide a `response` object:
...
| `expected_value`      | any    | Yes      | The expected value at that pointer                                |
