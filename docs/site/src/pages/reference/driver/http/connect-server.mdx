...
prev: "EtherCAT Write Task"
prevURL: "/reference/driver/ethercat/write-task"
next: "Read Task"
...
The Synnax Driver talks to any device or service with an HTTP or HTTPS JSON API: REST
endpoints on embedded devices, cloud telemetry APIs, monitoring endpoints, and IoT
gateways. Bearer token, basic auth, and API key authentication are supported.
...
| `auth`         | object  | No       | `none`  | The [configuration](#authentication-reference) used to authenticate with the HTTP server                |
| `health_check` | object  | Yes      | -       | The [configuration](#health-check-configuration) the Driver uses to track the health of the HTTP server |
...
<Details>
<Text.Text slot="summary" level="p"><Icon.Details /> Basic (Username and Password)</Text.Text>
...
<Details>
<Text.Text slot="summary" level="p" id="response-validation-parameters"><Icon.Details /> Response Validation Parameters</Text.Text>
...
<Video client:only="react" id="device-drivers/http/connect-server/connect-server" />
