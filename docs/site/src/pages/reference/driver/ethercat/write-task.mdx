...
| `state_rate`           | number  | No       | `1`     | Rate (Hz) at which state channels are updated       |
...
| `bit_length`    | number | Yes      | -       | Data width in bits (8, 16, 32, 64)                   |
| `data_type`     | string | Yes      | -       | Synnax data type (`uint8`, `int16`, `float32`, etc.) |
| `cmd_channel`   | number | Yes      | -       | Synnax command channel key                           |
| `state_channel` | number | Yes      | -       | Synnax state channel key                             |
