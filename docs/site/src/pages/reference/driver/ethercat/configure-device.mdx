...
| `vendor_id`    | number  | EtherCAT vendor ID                                                        |
| `product_code` | number  | Device model/product identifier                                           |
...
| `sub_index`  | number | CoE object subindex                             |
| `bit_length` | number | Data width in bits (e.g., 8, 16, 32)            |
| `data_type`  | string | Data type (e.g., `uint16`, `int32`, `float32`)  |
...
// Retrieve the device
const [device] = await client.devices.retrieve({ names: ["EL3102"] });
...
# Retrieve every EtherCAT device
ethercat_devices = client.devices.retrieve(makes=["EtherCAT"])
...
for dev in ethercat_devices:
    ...
    print(f"  Vendor ID: {dev.properties.get('vendor_id')}")
    print(f"  Product Code: {dev.properties.get('product_code')}")
...
for pdo in tx_pdos:
    print(f"  {pdo['name']}: index=0x{pdo['index']:04X}, "
          f"subindex={pdo['sub_index']}, type={pdo['data_type']}")
...
// Retrieve every EtherCAT device
const ethercatDevices = await client.devices.retrieve({ makes: ["EtherCAT"] });
...
// Retrieve a specific device by name
const [device] = await client.devices.retrieve({ names: ["EK1100"] });
...
const pdos = device.properties.pdos as {
  inputs?: PDOEntry[];
  outputs?: PDOEntry[];
};
