...
[OPC UA Read Service Specification](https://reference.opcfoundation.org/specs/OPC-10000-4/5.11.2)
...
<Note.Note variant="info">
  If timestamp channels are not added to the task, Synnax automatically generates
  timestamps with ~100 µs precision using software timing.
</Note.Note>
...
- **Array mode** -> Use array mode only above 500 Hz. It needs tuning to avoid under- or
  oversampling.
