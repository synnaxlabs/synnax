...
description: "Read data from OPC UA servers with the Synnax Driver."
next: "Write Task"
...
The OPC UA Read Task is used for acquiring data from an OPC UA server and storing it in
Synnax.
...
An OPC UA read task can be started, stopped, and re-configured at any time. Synnax
...
You'll see an option called "Create an OPC UA Read Task". Select this option to open the
...
selecting the add icon (<Icon.Add />), or opening the context menu on an OPC UA server
...
<Note.Note variant="info">
  ...
  Core](/reference/core/quick-start) with a memory-backed data store. This will allow
