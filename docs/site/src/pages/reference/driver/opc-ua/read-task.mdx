...
**Array Mode**: When `array_mode=true`, the task reads multiple samples in bulk from the OPC UA server. This is more efficient for high-rate tasks but requires careful configuration to avoid undersampling or oversampling.
</Note.Note>
...
```python
import synnax as sy
from synnax.hardware import opc
...
task = client.hardware.tasks.retrieve(name="OPC UA Read")

# Modify configuration
task.config.sample_rate = 20
task.config.stream_rate = 20
...
```typescript
// Retrieve existing task
const task = await client.hardware.tasks.retrieve({ name: "OPC UA Read" });
