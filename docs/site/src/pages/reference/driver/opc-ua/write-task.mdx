...
description: "Write data to OPC UA servers with the Synnax Driver."
prev: "Read Task"
...
The OPC UA Write Task is used for writing data to an OPC UA server. In this guide, we'll
walk you through the steps to create an OPC UA Write Task in Synnax.
...
[connected to an OPC UA server](/reference/driver/opc-ua/connect-server).
...
An OPC UA Write Task can be started, stopped, and re-configured at any time. Synnax
permanently store the configuration of the task, so it's easy to set up multiple tasks
...
To create a write task from the Devices Toolbar, open the Console and click on the
device icon (<Icon.Device />) on the left-hand side of the screen. Find the device you'd
...
You'll see an option called "Create an OPC UA Write Task". Select this option to open
...
To create a write task from the layout selector, click on the add icon (<Icon.Add />) in
the top-right corner and select "OPC UA Write Task".
...
must configure a [read task](/reference/driver/opc-ua/read-task).
...
[read task](/reference/driver/opc-ua/read-task). You can then use the read task to read
back states of the nodes you are writing to.
