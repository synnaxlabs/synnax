...
OPC UA write tasks do not create separate state channels. To read back the current
state, configure a [read task](/reference/driver/opc-ua/read-task) for the same node.
...
const task = await client.tasks.create({
  ...
  config: JSON.stringify({
    ...
    channels: [
      {
        cmd_channel: setpointCmd.key,
        node_id: "ns=2;s=TemperatureSetpoint",
      },
      {
        cmd_channel: valveCmd.key,
        node_id: "ns=2;s=ValveControl",
      },
    ],
  }),
