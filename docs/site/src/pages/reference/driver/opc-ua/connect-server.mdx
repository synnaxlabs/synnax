...
2. A [Synnax Driver](/reference/driver/get-started) running on your network. This Driver
   must be connected to the Core and running on the same machine or network as your OPC UA
   server.
...
You can use the "Test Connection" button to verify that the server is reachable. Once the
connection is successful, click "Save" to save the connection. You can now create read
