...
description: "Learn how to use Read Tasks for LabJack hardware."
...
Acquires data from analog and digital inputs on LabJack devices at a specified sample rate.
...
<Client.Tabs client:only="react" exclude={["cpp"]}>
  <Fragment slot="console">
    <Video
      client:only="react"
      id="device-drivers/labjack/read-task/select-device"
    />
  </Fragment>
...
with task.run():
    with client.open_streamer(["ai_0", "di_4"]) as streamer:
        for _ in range(10):
            frame = streamer.read()
            print(frame)
```
...
task = client.hardware.tasks.retrieve(name="LabJack Read")

# Modify configuration
task.config.sample_rate = 200
task.config.stream_rate = 50
...
// Read data
const streamer = await client.openStreamer(["ai_0", "di_4"]);
for (let i = 0; i < 10; i++) {
...
```typescript
// Retrieve existing task
const task = await client.hardware.tasks.retrieve({ name: "LabJack Read" });
