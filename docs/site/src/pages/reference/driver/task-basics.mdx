...
Before creating a task, connect and configure a device:
...
A task is a background process on the Driver that reads data from or writes data to a
device. Tasks are how Synnax acquires data from and controls hardware.

Tasks can be started, stopped, and reconfigured at any time. Synnax stores every task's
configuration, so you can keep several tasks and switch between them.
...
Click the device icon (<Icon.Device />) on the left-hand side of the screen. Right-click
the device you want to create the task for and select the task type.
...
Click the add icon (<Icon.Add />) in the top-right corner of the central mosaic and
...
keep the stream rate under 50 Hz. How samples are [timestamped](#timing) depends on
whether the task is hardware or software timed.
