...
Now that you've configured your hardware device, you're ready to create tasks to communicate
with it. This guide covers the fundamentals of working with tasks in the Synnax driver.

<Divider.Divider x />

## Prerequisites

Before creating any tasks, you'll need to have connected and configured a hardware device. See the specific device documentation for details:
...
and select the appropriate task type.
...
In command mode (first character in the input is `>`), type "Create".
You'll see options for creating different task types. Select the one you need to open the task configuration dialog.
...
```python
import synnax as sy
from synnax.hardware import modbus, ni, labjack, opc
...
// Modify the configuration
// Note: You'll need to parse and modify the config JSON string
const config = JSON.parse(copiedTask.config);
config.auto_start = true;
copiedTask.config = JSON.stringify(config);

// Apply the changes by creating/updating the task
await client.hardware.tasks.create(copiedTask);
...
- [Analog Read Task](/reference/driver/ni/analog-read-task) - Acquire data from analog inputs
- [Analog Write Task](/reference/driver/ni/analog-write-task) - Send commands to analog outputs
- [Counter Read Task](/reference/driver/ni/counter-read-task) - Acquire data from counter inputs
- [Counter Write Task](/reference/driver/ni/counter-write-task) - Generate pulse train outputs
- [Digital Read Task](/reference/driver/ni/digital-read-task) - Acquire data from digital inputs
- [Digital Write Task](/reference/driver/ni/digital-write-task) - Send commands to digital outputs
