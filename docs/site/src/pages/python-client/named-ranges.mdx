...
Python client ot read and write telemetry and attach metadata to these named
ranges.
...
for ch in my_named_range["^pressure"]:
    plt.plot(ch, my_named_range.timestamps, label=ch.name)
```
...
```python
tank_pressure = burst_test.tank_pressure
```
...
<div class="info">
<b>Note - </b> All meta-data values are stored as strings. It's up to you to correctly
...
Retrieving meta-dta is just as easy as setting it. 
