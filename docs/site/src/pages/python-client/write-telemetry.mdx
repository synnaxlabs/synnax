...
While the above methods are great for writing static, existing data, Synnax provides 
first-class support for writing large volumes of data over long periods of time. Writers 
are also essential for streaming telemetry to control sequences and live dashboards.

Writer's are a bit more complicated to operate that the above methods, so we recommend
reading the [concepts](/concepts/write-domains#transactions) page to learn more about
how they work.
...
We recommend using writer's within a context manager to automatically close the writer,
ensuring that resources have been freed and sockets are closed. To open the writer,
we use the `new_writer` method on the clinet and provde a starting timestamp for the 
...
The above example will write 100 samples to the `my_precise_tc` channel, each spaced
