...
import { Divider, Note, Text, Align, Icon } from "@synnaxlabs/pluto";
...
numbers of sensors (millions of channels) at very low data rates (1Hz or lower).
...
the counts increase past 1000, Synnax's performance begins to degrade, while Timescale
...
Inspired by the
[1 billion row challenge](https://www.morling.dev/blog/one-billion-row-challenge/),
we'll be inserting 1 billion rows of data into each database to see how they handle the
load.
...
16kb in size. These pages contain a certain number of rows. When accepting writes, the
