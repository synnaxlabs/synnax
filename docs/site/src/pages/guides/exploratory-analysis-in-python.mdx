...
We'll import the data set, explore it using the Synnax console, run analysis, and
attach post-processed results.
...
instructions [here](/deployment/get-started). You're more than welcome to use a remote
...
We're going to be using the Synnax console to do plot our data. To install it, follow 
the instructions [here](/console/get-started).
...
authenticated with your cluster. A guide for doing so is available [here](/python-client/get-started).
...
This question is asking if we want to import data for all columns in the file. We do,
so we'll hit enter. Next, Synnax will check if all of the columns in the CSV correspond
...
columns in the data set. In our case, it does, so we'll enter `y` and continue. Synnax
...
Again, we'll enter `Time` and continue. Then, Synnax will ask us about how we'd like
...
<div class="info">
<b>Note - </b> The next time we import a data set with the same channels, Synnax will
automatically detect the channels and skip this process.
</div>
...
Next, we'll retrieve the named range referencing the data we just imported:

```python
data = client.ranges.retrieve("April 9 Wetdress")
```

`ec_pressure_5` on our data set contains our flowmeters' pressure differential readings 
in psi. To make things easier to work with, we'll alias `ec_pressure_5` to `flow_dp`:

```python
data.ec_pressure_5.set_alias("flow_dp")
```
...
```python
elapsed_time = sy.elapsed_seconds(data.Time)
```
