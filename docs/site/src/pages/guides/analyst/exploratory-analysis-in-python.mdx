...
instructions [here](/reference/cluster/quick-start). You're more than welcome to use a remote
...
the instructions [here](/reference/console/get-started).
...
authenticated with your cluster. A guide for doing so is available [here](/reference/python-client/get-started).
...
<Note variant="info">
 The next time we import a data set with the same channels, Synnax will
...
<Image client:only="react" id="guides/analyst/exploratory-analysis-in-python/filter-results" themed={false} />
