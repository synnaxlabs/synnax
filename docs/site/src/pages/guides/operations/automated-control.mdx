...
[code](https://github.com/synnaxlabs/synnax/blob/main/client/py/examples/simulators/press.py)
...
<Note.Note variant="info">
    [Lambda functions](/reference/control/python/get-started#lambda-expressions) are
    short, anonymous functions in Python. This lambda function is equivalent to writing
...
```shell
synnax start --insecure --mem
```

Next, you will want to run the simulated DAQ. You can do this by cloning the
[Synnax repository](https://github.com/synnaxlabs/synnax) and running the simulator from
the `client/py` directory:
