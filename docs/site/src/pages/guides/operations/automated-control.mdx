...
control sequence. We recommend using a context manager to ensure that the `Controller` is
...
<Note variant="info">
    ...
    Write authorities determine which writer is allowed to write to a given channel.
    Every writer in Synnax has an authority between 0 and 255. If two writers are open
    on the same channel, the writer with the higher authority will be able to write but
...
This range is now saved `control_sequence.py` and can be referenced later to view this test.
...
If you want to look at more examples of control sequences, visit our [GitHub
