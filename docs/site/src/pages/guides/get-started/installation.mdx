...
To start a local cluster on Windows, use the following command in a **PowerShell** terminal
to download the latest Synnax cluster.
...
<Details>
<div slot="summary">
<small>Explanation of this command</small>
</div>
...
2. **Set Security Protocol**:

   - `[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12;`
   - Ensures that the TLS 1.2 protocol is used for secure connections, which is a modern and secure standard.

3. **Disable Progress Display**:

   - `$ProgressPreference='SilentlyContinue';`
   - Suppresses the display of progress bars during script execution.

4. **Create Directory for Synnax**:

   - `$null=New-Item -Type Directory -Force $env:appdata/synnax;`
   - Creates a directory named `synnax` in the `%APPDATA%` folder (user's application data folder). The `-Force` flag ensures the directory is created even if it already exists.

5. **Download the Synnax Executable**:
   ...
   - Downloads the `synnax-v0.37.0-windows` executable file from the specified GitHub URL and saves it as `synnax.exe` in the current directory.

6. **Copy Executable to AppData**:

   - `Copy-Item -Force "synnax.exe" -Destination $env:appdata/synnax;`
   - Copies the downloaded `synnax.exe` file to the newly created `%APPDATA%\synnax` directory. The `-Force` flag overwrites any existing file with the same name.

7. **Update the User PATH Variable**:
   ...
   - Checks if the `%APPDATA%\synnax` directory is already in the user's PATH environment variable. If not, appends it to the PATH.

8. **Update Current Session PATH**:
   - `$env:PATH += ";$env:appdatasynnax"`
   - Updates the current PowerShell session's PATH variable to include `%APPDATA%\synnax`, allowing the Synnax executable to be run without restarting the session.
