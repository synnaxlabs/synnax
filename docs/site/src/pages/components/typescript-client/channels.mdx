...
Note that Synnax will not raise a `NotFoundError` if no channels match the query. Instead,
...
data to them. Check out the [reading data](/typescript-client/read-data)
and [writing data](/typescript-client/write-data) guides to learn more.