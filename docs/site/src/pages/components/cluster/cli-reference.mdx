...
By default, Synnax will look for a configuration file at `~/.synnax.yaml`. You can specify
a different configuration file using the `--config` flag or the `SYNNAX_CONFIG` environment
variable.
...
The `version` command prints the version of the Synnax database CLI. 
...
You'll see output similar to the following (the version number may vary):

```cmd
Synnax version 0.16.5
```
