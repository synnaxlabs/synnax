...
read directly from a channel, fetch a range and access its data, or leverage server
...
the exact range of time we're interested in reading. [Ranges](/python-client/named-ranges)
are a useful way of categorizing important time ranges in a cluster's data. We can read
directly from these ranges.
