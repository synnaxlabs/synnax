...
title: "Ranges"
description: "Use ranges to categorize your cluster's data."
---
...
read and write data and attach metadata to these ranges.
...
Synnax will raise a `NotFoundError` if the range does not exist, and a `MultipleResultsError`
if multiple ranges with the given name exist. If you'd like to accept multiple or no
results, provide a list to the `retrieve` method as shown below.
...
When providing a list, Synnax will not raise an error if a range cannot be found. Instead,
it simply omits the missing range from the returned list.
...
a channel a more descriptive name in the context of a particular range. Ranges allow us to do just that.
...
<Note class="warn">
Aliases are only valid within the context of a particular range. If you try to access an 
aliased channel outside of the range, Synnax will not be able to find it.
</Note>
...
It's common to have non-data information we'd like to attach to a particular range,
...
<Note variant="info">
All metadata values are stored as strings. It's up to you to correctly
