...
try {
    for (let i = 0; i < 100; i++) {
        const timestamps = new Series({ data: new BigInt64Array([TimeStamp.now().valueOf()]) });
        const temperatures = new Series({ data: new Float32Array([20.0 + i]) });
        await writer.write(new Frame({
            [timeChannel.key]: timestamps,
            [temperatureChannel.key]: temperatures
        }));
