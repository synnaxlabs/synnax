...
Layer 1 holds cluster state in a node map `map[NodeKey]Node`. `NodeKey` is a
...
// NodeKey is a unique identifier for a node.
type NodeKey int16
...
type Digest struct {
	// NodeKey is the node's unique identifier
	Key NodeKey
...
is due to a desire to identify nodes using a unique `int16` value. The Key of a
...
channel `NodeKey + ChannelID`. This results in a sample that is 40 percent
...
second message assigning it an Key. If the peer rejects the request or doesn't
...
given Key. A **juror** tracks the results of all accepted proposals until the
...
with an incremented value. This process continues until an Key is accepted. If
the **responsible** node tries to contact an unresponsive peer, it will reselect
a quorum of peers and try again. Once an Key is selected, the **responsible**
node will send it to the **
pledge**.

### 3 - Disseminate New Node

Once the **pledge** receives an Key assignment from the **responsible** node, it
...
automatically assign itself an Key of 1.

### 6 - Implications of Algorithm

Using a quorum based approach to Key assignment means that we get a strong
...
type KV interface {
	// Set sets a key to a value. nodeKey specifies the node that holds the lease on the key.
	// If nodeKey is 0, the lease is assigned to the host node.
	Set(key []byte, leaseholder NodeKey, value []byte) error
...
If the node Key is non-zero, perform a layer 1 lookup for the leaseholder's
address. Forward the request to the leaseholder. If the node Key is zero,
...
type Update struct {
	...
	Value []byte
	// Leaseholder is the Key of the leaseholder node.
	Leaseholder NodeKey
...
```go
type HighWaterMarks map[node.Key]version.Counter
```
...
```go
type HighWaterMarks map[node.Key]struct{
