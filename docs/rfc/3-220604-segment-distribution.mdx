...
This query returns the address of the node with an Key of `1`. The DL uses this
...
5. Node Key - The Key of the node that owns the channel. This node is known as the
...
1. The Key of the leaseholder node for the channel.
2. An auto-incrementing counter for the Cesium DB on the leaseholder node where
   channel data is written to.

Together, these two elements guarantee uniqueness. By keeping the node Key in the
