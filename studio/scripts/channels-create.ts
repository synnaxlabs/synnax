...
/**
 * Docs `console/channels/create`: run "Create channel" from the palette, fill
 * the dialog (name, data type, index), and submit; the new channel appears in
 * the open Channels toolbar.
 */
export default async (session: capture.CaptureSession): Promise<void> => {
  ...
  try {
    ...
    await capture.commandPalette(session, "Create channel");
