...
- `now()` - Returns the current timestamp as `i64` with nanosecond time units. The value
  is constant within a cycle: every call in one cycle returns the same stamp
