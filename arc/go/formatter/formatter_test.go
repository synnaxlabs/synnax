...
var _ = Describe("Formatter", func() {
	...
	DescribeTable("Stage Body Formatting",
		...
		// Inline input stays inline, commas between items, no trailing comma.
		Entry("single-line stays inline with no trailing comma", "stage init{x:=0}", "stage init { x := 0 }\n"),
		Entry("inline multiple items keep commas", "stage init { x := 0, y := 1 }", "stage init { x := 0, y := 1 }\n"),
		Entry("inline drops trailing comma", "stage init { x := 0, y := 1, }", "stage init { x := 0, y := 1 }\n"),
		Entry("inline flow statement", "stage run{sensor->output}", "stage run { sensor -> output }\n"),
		Entry("empty stage body", "stage init{}", "stage init {}\n"),
		// Multi-line source stays multi-line, no commas at all.
	...
	DescribeTable("Stageless Sequence Body Formatting",
		...
		Entry("single-line stays inline", "sequence main { 1 -> a, 1 -> b }", "sequence main { 1 -> a, 1 -> b }\n"),
		Entry("single-line no commas stays inline and comma-less",
			"sequence main { 1 -> a  1 -> b }",
			"sequence main { 1 -> a 1 -> b }\n"),
		Entry("inline drops trailing comma",
			"sequence main { 1 -> a, 1 -> b, }",
			"sequence main { 1 -> a, 1 -> b }\n"),
		...
		Entry("multi-line drops user-written commas",
			"sequence main {\n    1 -> valve_a,\n    1 -> valve_b,\n}",
			"sequence main {\n    1 -> valve_a\n    1 -> valve_b\n}\n"),
		Entry("sequence containing stages stays multi-line",
			"sequence main { stage a {} stage b {} }",
			"sequence main {\n    stage a {}\n    stage b {}\n}\n"),
