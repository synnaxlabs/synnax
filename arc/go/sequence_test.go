...
var _ = Describe("Sequence", func() {
	...
	Describe("Inline routing case body lifetime", func() {
		...
		mk := func(ctx SpecContext, body string) *runtimeHarness {
			...
			return newRuntimeHarness(ctx, prog, resolver,
				channels.Digest{Key: 100, DataType: telem.Uint8T},
				channels.Digest{Key: 101, DataType: telem.BooleanT},
				channels.Digest{Key: 102, DataType: telem.Float64T},
				channels.Digest{Key: 103, DataType: telem.BooleanT},
			)
		}
	...
	Describe("Re-entry", func() {
		...
		It(
			"Resets a function's stateful variable when its stage is re-entered",
			func(ctx SpecContext) {
				resolver := channelSymbols(map[string]channelDef{
					"start_cmd": {types.U8(), 100},
					"go_b":      {types.U8(), 101},
				...
				Expect(
					lastU8(out, 104),
				).To(Equal(uint8(1)), "stateful variable should reset when stage a is re-entered")
			},
		)

		It(
			"Holds a function's stateful variable while its stage stays active",
			func(ctx SpecContext) {
				resolver := channelSymbols(map[string]channelDef{
					"start_cmd": {types.U8(), 100},
					"tick":      {types.U8(), 101},
				...
				for i := uint8(1); i <= 5; i++ {
					...
					Expect(
						lastU8(out, 102),
					).To(Equal(i), "stateful variable must accumulate while stage a stays active")
				}
			},
		)

		It(
			"Resets a function's stateful variable on every re-entry across repeated cycles",
			func(ctx SpecContext) {
				resolver := channelSymbols(map[string]channelDef{
					"start_cmd": {types.U8(), 100},
					"go_b":      {types.U8(), 101},
				...
				for cycle := range 4 {
					...
					Expect(
						lastU8(out, 104),
					).To(Equal(uint8(1)), "cycle %d: stateful variable should start from 1", cycle)
					...
					Expect(
						lastU8(out, 104),
					).To(Equal(uint8(2)), "cycle %d: stateful variable should accumulate within the stage", cycle)
		...
		It(
			"Resets every stateful variable in a function when its stage is re-entered",
			func(ctx SpecContext) {
				resolver := channelSymbols(map[string]channelDef{
					"start_cmd": {types.U8(), 100},
					"go_b":      {types.U8(), 101},
				...
				Expect(
					lastF32(out, 104),
				).To(Equal(float32(10)), "count and total must both reset when stage a is re-entered")
			},
		)

		It(
			"Resets a function's stateful string variable when its stage is re-entered",
			func(ctx SpecContext) {
				resolver := channelSymbols(map[string]channelDef{
					"start_cmd": {types.U8(), 100},
					"go_b":      {types.U8(), 101},
				...
				Expect(
					lastString(out, 104),
				).To(Equal("xx"), "stateful string should reset when stage a is re-entered")
			},
		)

		It(
			"Scopes the reset to the re-entered stage's own call site",
			func(ctx SpecContext) {
				resolver := channelSymbols(map[string]channelDef{
					"start_cmd":  {types.U8(), 100},
					"go_b":       {types.U8(), 101},
				...
				Expect(
					lastU8(out, 104),
				).To(Equal(uint8(1)), "the staged call site should reset on re-entry")
		...
		It(
			"Resets a stateful variable in a nested sequence's stage on inner re-entry",
			func(ctx SpecContext) {
				resolver := channelSymbols(map[string]channelDef{
					"start_cmd": {types.U8(), 100},
				...
				Expect(
					lastU8(out, 104),
				).To(Equal(uint8(1)), "stateful variable should reset when the inner stage is re-entered")
			},
		)
