...
tick{period=50ms} -> sim_daq`))
			ctx := context.CreateRoot(bCtx, ast, NewRoot(nil, intervalResolver...))
...
func consumer(v f64) {}
my_func -> consumer{}`))
				ctx := context.CreateRoot(bCtx, ast, NewRoot(nil, localResolver...))
				analyzer.AnalyzeProgram(ctx)
				Expect(ctx.Diagnostics.Ok()).To(BeFalse())
...
int_chan -> consumer{}`))
		...
		It("Should return an error when once of the tasks being called is not defined", func(bCtx SpecContext) {
			...
			once{} -> processor{}
			`))
			ctx := context.CreateRoot(bCtx, ast, NewRoot(nil))
			analyzer.AnalyzeProgram(ctx)
			Expect(ctx.Diagnostics.Ok()).To(BeFalse())
		...
		It("Using channel as source", func(bCtx SpecContext) {
			...
			ctx := context.CreateRoot(bCtx, ast, NewRoot(nil, noOnResolver...))
			analyzer.AnalyzeProgram(ctx)
			Expect(ctx.Diagnostics.Ok()).To(BeTrue(), ctx.Diagnostics.String())
		})
	...
	Describe("Literal Type Inference", func() {
		DescribeTable("Should infer literal types from target channel",
			func(bCtx SpecContext, source string, chanType types.Type) {
				...
				ctx := context.CreateRoot(bCtx, ast, NewRoot(nil, literalResolver...))
				analyzer.AnalyzeProgram(ctx)
				Expect(ctx.Diagnostics.Ok()).To(BeTrue(), ctx.Diagnostics.String())
			},
