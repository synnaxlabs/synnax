...
var _ = Describe("Flow Statements", func() {
	Describe("Function Without Input Braces", func() {
		It(
			"Should detect when function follows function invocation without input braces",
...
tick{period=50ms} -> sim_daq`))
				...
				).To(Equal("sim_daq is not a channel"))
				Expect((*ctx.Diagnostics)[0].Notes).To(HaveLen(1))
				Expect(
					(*ctx.Diagnostics)[0].Notes[0].Message,
				).To(Equal("use sim_daq{} to instantiate the function"))
			},
		)

		It(
			"Should detect when function follows channel without input braces",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
func sim_daq() {}
sensor_chan -> sim_daq`))
		...
		It(
			"Should detect when function follows expression without input braces",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
func sim_daq() {}
...
sensor_chan -> transform{2.5, 0.1} -> sink{}`))
		...
		It(
			"Should accept partial anonymous input when trailing params have defaults",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
...
sensor_chan -> controller{100.0}`))
		...
		It(
			"Should detect type mismatch in anonymous input values",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
...
sensor_chan -> filter{5.0, 20, 30} -> sink{}`))
		...
		It(
			"Should reject partial anonymous input missing required params",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
...
sensor_chan -> controller{100.0}`))
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(Equal("missing required argument for parameter 'gain' of func 'controller'"))
			},
		)
...
sensor_chan -> filter{5.0} -> sink{}`))
		...
		It(
			"Should accept a channel identifier as anonymous input value",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
...
sensor_chan -> controller{sensor_chan, 100.0}`))
		...
		It(
			"Should reject zero anonymous input values when params are required",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
func filter{threshold f64} (x f64) f64 {
    return x
...
sensor_chan -> filter{} -> sink{}`))
				...
				Expect((*ctx.Diagnostics)[0].Message).To(
					Equal(
						"missing required argument for parameter 'threshold' of func 'filter'",
					),
				)
			},
		)
		...
		It(
			"Should accept empty input when all params have defaults",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
...
sensor_chan -> controller{}`))
		...
		It(
			"Should detect type mismatch in second anonymous input value",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
func transform{scale f64, offset f64} (x f64) f64 {
    return x * scale + offset
...
sensor_chan -> transform{2.5, "bad"} -> sink{}`))
	...
	Describe("Positional arguments with a trigger-first param", func() {
		...
		It(
			"Should bind a positional value to the first non-trigger param on a standalone node",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
sequence main {
    stage done {
		...
		It(
			"Should bind a positional value while the upstream feeds the trigger",
			func(bCtx SpecContext) {
		...
		It(
			"Should count only non-trigger params when rejecting too many positional values",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
sequence main {
    stage done {
        set_auth{0, 1, 2}
				...
				).To(ContainSubstring("too many arguments"))
				Expect(
					ctx.Diagnostics.String(),
				).To(ContainSubstring("expected at most 2"))
			},
		)
	})
	...
	Describe("WASM functions used as flow nodes", func() {
		...
		It(
			"Should reject a WASM function wired as a flow node",
			func(bCtx SpecContext) {
		...
		It(
			"Should reject a WASM function invoked as a standalone stage node",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
sequence main {
    stage done {
	...
	Describe("Channel to func Flows", func() {
		Context("function to function connections", func() {
			It(
				"Should detect when func with no output connects to func expecting input",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
func source() {}
func sink(v u8) {}
source{} -> sink{}`))
					...
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(Equal("func 'source' has no return value but 'sink' expects an input parameter"))
				},
			)

			It(
				"Should detect type mismatch between func output and next func input",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
func producer() u8 {
    return 1
...
producer{} -> consumer{}`))
					...
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(ContainSubstring("does not match"))
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(ContainSubstring("trigger parameter"))
				},
			)

			It(
				"Should detect unit mismatch between func output and next func input",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
...
producer{} -> consumer{}`))
			...
			It(
				"Should detect when func with multiple params is used without routing table",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
func source() u8 {
    return 1
...
source{} -> multi{}`))
					...
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(Equal("missing required argument for parameter 'b' of func 'multi'"))
				},
			)

			It(
				"Should detect when func with named outputs is chained without routing table",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
...
splitter{} -> consumer{}`))
					...
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(ContainSubstring("has named outputs and requires a routing table"))
				},
			)

			It(
				"Should allow valid func-to-func connection with matching types",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
func producer() f64 {
    return 1.0
...
producer{} -> consumer{}`))
		...
		Context("channel to function type checking", func() {
			It(
				"Should detect when non-channel identifier is used as flow source",
				func(bCtx SpecContext) {
...
my_func -> consumer{}`))
					...
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(Equal("my_func is not a channel"))
				},
			)

			It(
				"Should detect channel value type mismatch with func parameter",
				func(bCtx SpecContext) {
					localResolver := []symbol.Symbol{
						{
							Name: "int_chan",
...
int_chan -> consumer{}`))
		...
		It(
			"Should return an error when once of the tasks being called is not defined",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			once{} -> processor{}
			`))
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(Equal("undefined symbol: once"))
				Expect(
					(*ctx.Diagnostics)[1].Message,
				).To(Equal("undefined symbol: processor"))
			},
		)

		It(
			"Should verify func input parameters match the expected signature types",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
		...
		It(
			"Should detect when func is invoked with missing required parameters",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			...
			sensor_chan -> filter{input=sensor_chan}`))
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(Equal("missing required argument for parameter 'threshold' of func 'filter'"))
				Expect(
					(*ctx.Diagnostics)[1].Message,
				).To(Equal("missing required argument for parameter 'output' of func 'filter'"))
			},
		)

		It(
			"Should allow omitting input param with default value",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func controller{setpoint f64, gain f64 = 1.0} () {}
		...
		It(
			"Should still require input params without defaults",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func controller{setpoint f64, gain f64 = 1.0} () {}
			...
			sensor_chan -> controller{gain=2.5}`))
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(Equal("missing required argument for parameter 'setpoint' of func 'controller'"))
			},
		)

		It(
			"Should detect when func is invoked with extra parameters not in signature",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			...
			sensor_chan -> simple{input=sensor_chan, extra=42.0}`))
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(Equal("unknown parameter 'extra' for func 'simple'"))
			},
		)

		It(
			"Should detect type mismatch in func input parameters",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
		...
		It(
			"Should accept correct types for func input parameters",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func typed_task{threshold f64, count u32, message str, input chan f64} () {
			    value := input
		...
		It(
			"Should allow channels as both sources and targets in flow statements",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
		...
		It(
			"Should understand channel pass-through triggers tasks on new values",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
		...
		It(
			"Should implicitly convert channel sources to on{channel} func invocations",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
		...
		It(
			"Should convert expressions in flow statements to anonymous tasks",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
func alarm{} () {}
func logger{} () {}
...
// Arithmetic expression
 (sensor_chan * 1.8) + 32.0 -> logger{}`))
		...
		It(
			"Should validate that expressions in flows only reference channels and literals",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
func alarm{} () {}
...
sensor_chan > threshold -> alarm{}`))
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(Equal("undefined symbol: threshold"))
			},
		)
	})
	...
	Describe("Multi-Output fns and Routing Tables", func() {
		Context("routing table structure validation", func() {
			It(
				"Should detect output routing table not following a func",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
func target(v f64) {}
sensor_chan -> {
    high: target{}
					...
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(ContainSubstring("output routing table must follow a func invocation"))
				},
			)
...
sensor_chan -> demux{} -> {
    high: some_var
			...
			It(
				"Should accept a stage name as a routing table target",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
...
sequence main {
    ...
    stage opt_1 {}
		...
		It("Should analyze func with multiple named outputs", func(bCtx SpecContext) {
			...
			hasNamedOutputs := len(demuxSymbol.Type.Outputs) > 1 ||
				(len(demuxSymbol.Type.Outputs) == 1 && !hasDefaultOutput)
		...
		It(
			"Should detect when routing table is used with func without named outputs",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			...
			sensor_chan -> simple{} -> {
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(ContainSubstring("does not have named outputs"))
			},
		)
		...
		It("Should detect when routing to non-existent output", func(bCtx SpecContext) {
			...
			Expect(
				(*ctx.Diagnostics)[0].Message,
			).To(ContainSubstring("does not have output 'medium'"))
		})
		...
		It(
			"Should analyze routing table with chained nodes via =>",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func demux{} (value f64) (high f64, low f64) {
			    if (value > 100.0) {
		...
		It(
			"Should type-check multi-node case body via chain, not select output",
			func(bCtx SpecContext) {
		...
		It(
			"Should reject type mismatch in a chained case body node",
			func(bCtx SpecContext) {
				customResolver := StaticResolver{
					{
						Name: "sensor_chan",
		...
		It(
			...
			sensor_chan -> demux{} -> {
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(ContainSubstring("named outputs and requires a routing table"))
			},
		)

		It(
			"Should accept select{} with boolean discriminator and string-literal chain bodies",
			func(bCtx SpecContext) {
				customResolver := StaticResolver{
					{
						Name: "flag",
		...
		It(
			"Should infer channel value type for a channel in a non-terminal chain position",
			func(bCtx SpecContext) {
				customResolver := StaticResolver{
					{
						Name: "source_chan",
		...
		It(
			"Should report undefined symbol for an unresolved func in a chain",
			func(bCtx SpecContext) {
				customResolver := StaticResolver{
					{
						Name: "source_chan",
		...
		It(
			"Should accept inline stage with multiple parallel flow statements per case body",
			func(bCtx SpecContext) {
		...
		It(
			"Should accept inline sequence with multiple sequential steps per case body",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should register inline stage case bodies under the lexically enclosing scope",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should register inline sequence case bodies under the lexically enclosing scope",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It("Should reject a named inline sequence case body", func(bCtx SpecContext) {
			...
			Expect(msgs).To(ContainElement(SatisfyAll(
				ContainSubstring(
					"inline routing case body sequences must be anonymous",
				),
		...
		It(
			"Should reject `=> name` from a peer stage targeting a named inline",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should accept a routing table mixing one inline case body and one chain case body",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should reject a type mismatch inside an inline stage case body",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should reject a type mismatch inside an inline sequence case body",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should report an undefined symbol referenced inside an inline stage case body",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
			...
			sequence main {
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(ContainSubstring("mystery_chan"))
			},
		)

		It(
			"Should accept an inline case body routed off a custom multi-output func",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "pressure",
		...
		It(
			"Should accept nested routing tables inside an inline stage case body",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "outer_flag",
		...
		It(
			"Should accept a top-level inline stage with multiple parallel flow statements",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should accept a top-level inline sequence with multiple sequential steps",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should register top-level inline stage flow targets under the root scope",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should register top-level inline sequence flow targets under the root scope",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should reject a named top-level inline stage flow target",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
			...
			flag -> stage my_step { "branch" -> log_str }`))
				...
				Expect(msgs).To(ContainElement(SatisfyAll(
					ContainSubstring(
						"inline routing case body stages must be anonymous",
					),
					ContainSubstring(`"my_step"`),
				)), "diagnostics: %v", msgs)
			},
		...
		It(
			"Should reject a named top-level inline sequence flow target",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should reject `=> name` from a peer stage targeting a named inline flow target",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should accept a top-level scope mixing one inline flow target and one plain flow chain",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should reject a type mismatch inside a top-level inline stage flow target",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should reject a type mismatch inside a top-level inline sequence flow target",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should report an undefined symbol referenced inside a top-level inline stage flow target",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "flag",
		...
		It(
			"Should accept nested top-level inline flow targets",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "outer_flag",
		...
		It(
			"Should accept a top-level inline flow body containing a routing table with its own inline body",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "outer_flag",
		...
		It(
			"Should accept a top-level routing case inline body containing an inline flow body",
			func(bCtx SpecContext) {
				customResolver := []symbol.Symbol{
					{
						Name: "outer_flag",
		...
		It(
			"Should validate input parameters in routing table targets",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func demux{} (value f64) (high f64, low f64) {
			    if (value > 100.0) {
		...
		It(
			"Should analyze routing table with parameter mapping",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func demux{} (value f64) (high f64, low f64) {
			    if (value > 100.0) {
		...
		It(
			"Should detect invalid parameter name in routing table",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func demux{} (value f64) (high f64, low f64) {
			    if (value > 100.0) {
			...
			sensor_chan -> demux{} -> {
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(ContainSubstring("does not have parameter 'invalid_param'"))
			},
		)
		...
		It(
			"Should analyze routing table with chained processing and parameter mapping",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func demux{} (value f64) (high f64, low f64) {
			    if (value > 100.0) {
		...
		It(
			"Should require next func when using parameter mapping",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func demux{} (value f64) (high f64, low f64) {
			    if (value > 100.0) {
			...
			sensor_chan -> demux{} -> {
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(ContainSubstring("parameter mapping requires a func after the routing table"))
			},
		)

		Context("Input routing tables", func() {
			It(
				"Should analyze input routing table mapping sources to parameters",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
				func combiner{} (a f64, b f64) f64 {
				    return a + b
			...
			It(
				"Should detect when input routing table has invalid parameter name",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
				func combiner{} (a f64, b f64) f64 {
				    return a + b
			...
			It(
				"Should detect when input routing table is not followed by a func",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
				...
				`))
					...
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(ContainSubstring("input routing table must precede a func invocation"))
				},
			)

			It(
				"Should detect when input routing entry does not end with identifier",
				func(bCtx SpecContext) {
					ast := MustSucceed(parser.Parse(`
				func combiner{} (a f64, b f64) f64 {
				    return a + b
				...
				func processor{} () f64 {
...

				 {
					...
					Expect(
						(*ctx.Diagnostics)[0].Message,
					).To(ContainSubstring("last element in input routing entry must be a parameter name"))
				},
			)
		})
	})
	...
	Describe("Single Invocations in Stages", func() {
		It(
			"Should allow standalone function invocation with no required inputs",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func setup{} () {
			// Do something
		...
		It(
			"Should detect when standalone function has required inputs without defaults",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			...
			sequence main {
				...
				Expect(
					(*ctx.Diagnostics)[0].Message,
				).To(ContainSubstring("missing required argument for parameter 'value'"))
			},
		)

		It(
			"Should allow standalone function with inputs that have default values",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
		...
		It(
			"Should allow mixing flow statements and single invocations",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
			func setup{} () {}
...
var _ = Describe("TriggerOnly upstream activation", func() {
	...
	It(
		"Should not type-check a channel upstream against a TriggerOnly func",
		func(bCtx SpecContext) {
			ast := MustSucceed(parser.Parse(`
		func sink{value f64} () {}
	...
	It(
		"Should still reject a void func feeding a func with a wire-fed input",
		func(bCtx SpecContext) {
			ast := MustSucceed(parser.Parse(`
		func void_src() {}
		func consumer(v f64) {}
...
var _ = Describe("Trigger call-site conflict", func() {
	It(
		"Should flag a param bound by both a named call-site argument and an upstream wire",
		func(bCtx SpecContext) {
			ast := MustSucceed(parser.Parse(`
		func consumer(v f64) {}
	...
	It(
		"Should reject a positional argument when the only param is the trigger",
		func(bCtx SpecContext) {
			ast := MustSucceed(parser.Parse(`
		func consumer(v f64) {}
...
var _ = Describe("Trigger in select routing branches", func() {
	It(
		"Should not type-check a select branch routing into a TriggerOnly func",
		func(bCtx SpecContext) {
			ast := MustSucceed(parser.Parse(`
	...
	It(
		"Should type-check a select branch routing into a func with a wire-fed input",
		func(bCtx SpecContext) {
			ast := MustSucceed(parser.Parse(`
		func sink(v str) {}
		start_cmd -> select{} -> {
...
var _ = Describe("Flow Sink Type Compatibility", func() {
	...
	DescribeTable(
		"Should report a located error when a source does not match the channel sink",
	...
	Describe("Writing to a channelRead variable", func() {
		It(
			"Should reject a flow that writes into a channelRead variable",
			func(bCtx SpecContext) {
				ast := MustSucceed(parser.Parse(`
r := sensor_chan + 1
sensor_chan -> r`))
