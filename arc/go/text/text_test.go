...
var _ = Describe("Text", func() {
	...
	Describe("Analyze", func() {
		...
		Context("Config Values", func() {
			...
			It("Should compile a non-literal brace input into a synthetic node and edges", func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{Name: "sensor", Kind: symbol.KindChannel, Type: types.Chan(types.I32()), ID: 10042},
				}
				source := `
				...
				sensor -> processor{threshold = (1 + 2)} -> print{}`
				parsedText := MustSucceed(text.Parse(text.Text{Raw: source}))
				inter, diagnostics := text.Analyze(ctx, parsedText, NewRoot(nil, resolver...))
				Expect(diagnostics.Ok()).To(BeTrue(), diagnostics.String())
				...
				_, ok := inter.Functions.Find(synth.Type)
				Expect(ok).To(BeTrue())
				...
				sensorNode := findNodeByKey(inter.Nodes, "on_sensor_0")
				triggerFanout := lo.Filter(inter.Edges, func(e ir.Edge, _ int) bool {
					return e.Target.Node == synth.Key
				})
				Expect(triggerFanout).To(HaveLen(1))
				Expect(triggerFanout[0].Source.Node).To(Equal(sensorNode.Key))
			})

			It("Should bind channels read by a brace input expression to the synthetic node", func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{Name: "sensor", Kind: symbol.KindChannel, Type: types.Chan(types.I64()), ID: 10042},
				}
				source := `
				func processor{threshold i64} () i64 {
				    return threshold
				...
				sensor -> processor{threshold = (sensor + 1)} -> print{}`
				parsedText := MustSucceed(text.Parse(text.Text{Raw: source}))
				inter, diagnostics := text.Analyze(ctx, parsedText, NewRoot(nil, resolver...))
				Expect(diagnostics.Ok()).To(BeTrue(), diagnostics.String())
