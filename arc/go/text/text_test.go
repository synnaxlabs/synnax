...
var _ = Describe("Text", func() {
	Describe("Variables", func() {
		...
		It(
			"Should lower a written variable to a node wired between channel reads and writes",
			...
			sequence main {
				...
				Expect(
					writeKey,
				).ToNot(BeEmpty(), "expected a write-node writing out_ch")
		...
		It(
			"Should reject a flow write to a channel-read variable",
			func(ctx SpecContext) {
				source := `
			sequence main {
				r i64 := count_ch + 1
				...
				Expect(
					diagnostics.String(),
				).To(ContainSubstring("channel-read variable"))
			},
		)

		It(
			"Should inline a never-reassigned constant flow read as a constant node",
			func(ctx SpecContext) {
				source := `
			sequence main {
				k i64 := 5
				...
				Expect(
					constKey,
				).ToNot(BeEmpty(), "expected a constant node initialized with 5")
				Expect(hasEdge(inter, constKey, writeKey)).To(BeTrue())
			},
		)

		It(
			"Should reject a top-level stateful variable declaration",
			func(ctx SpecContext) {
				source := `total i64 $= 0`
				parsedText := MustSucceed(text.Parse(text.Text{Raw: source}))
				_, diagnostics := text.Analyze(
		...
		It(
			"Should lower a written stateful variable to a stateful_variable node",
			func(ctx SpecContext) {
				source := `
			sequence main {
				total i64 $= 0
		...
		It(
			"Should bind a derivation's trigger read to the alias's register",
			func(ctx SpecContext) {
				source := `
			sequence main {
				p := count_ch
				...
				found := false
				for _, e := range inter.Edges {
					if e.Target.Param == "channel" &&
						strings.HasPrefix(e.Source.Node, "bind_p") {
						found = true
					}
				}
		...
		It(
			"Should accept a channel alias as a chained flow head",
			func(ctx SpecContext) {
		...
		It(
			"Should route a chained flow's head read through the alias register",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "sensor_f",
		...
		It(
			"Should stamp a var ref input for a reassigned variable",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "out_str",
		...
		It(
			"Should accept a reassigned variable as a native's input value",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "span_ch",
						Kind: symbol.KindChannel,
						Type: types.Chan(types.TimeSpan()),
						ID:   909,
					},
					{
						Name: "done_ch",
		...
		It(
			"Should inline a never-reassigned nested variable input value",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "out_str",
		...
		It(
			"Should stamp a var ref input for a stateful variable",
			func(ctx SpecContext) {
				source := `
		...
		It(
			"Should accept a stateful variable as a native's input value",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "span_ch",
		...
		It(
			"Should inline a never-written stateful variable input value",
			func(ctx SpecContext) {
				source := `
			func my_func{tag i64} (n u8) i64 {
				return tag
		...
		It(
			"Should stamp the folded cast initializer on a var ref",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "span_ch",
		...
		It(
			"Should not fold a cast of a non-literal initializer",
			func(ctx SpecContext) {
				source := `
			func my_func{tag i64} (n u8) i64 {
				return tag
		...
		It(
			"Should reject a reactive variable as an input value",
			func(ctx SpecContext) {
				source := `
			func my_func{tag i64} (n u8) i64 {
				return tag
		...
		It(
			"Should reject a reactive variable as a channel input value",
			func(ctx SpecContext) {
				source := `
		...
		It(
			"Should reject a reactive variable as a native's input value",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "span_ch",
		...
		Describe("Mid-chain variable reads", func() {
			...
			It(
				"Should lower a mid-chain read of a flow-written string variable to a live read",
				func(ctx SpecContext) {
					resolver := append([]symbol.Symbol{
						{
							Name: "str_ch",
			...
			It(
				"Should lower a mid-chain read of an assignment-written variable to a live read",
				func(ctx SpecContext) {
					source := `
				sequence main {
					k i64 := 5
			...
			It(
				"Should keep a chain-head read wired straight off the register",
				func(ctx SpecContext) {
					source := `
				sequence main {
					k i64 := 5
			...
			It(
				"Should register-back a mid-chain read with no other write site",
				func(ctx SpecContext) {
					resolver := append([]symbol.Symbol{
						{
							Name: "out_str",
			...
			It(
				"Should lower a mid-chain read of a stateful variable to a live read",
				func(ctx SpecContext) {
					source := `
				sequence main {
					acc i64 $= 7
			...
			It(
				"Should feed a downstream function from a mid-chain live read",
				func(ctx SpecContext) {
					source := `
		...
		Describe("Triggered expression variable reads", func() {
			...
			It(
				"Should bind a triggered format-string read of a reassigned variable as a var ref",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					k i64 := 5
					...
					Expect(vars).To(HaveLen(1))
					fmtNode := MustBeOk(
						findByTypePrefix(inter, compiler.FmtStrSyntheticPrefix),
					)
					...
					Expect(
						hasEdge(inter, nodeReading(inter, 903), fmtNode.Key),
					).To(BeTrue(),
			...
			It(
				"Should bind a triggered expression read of a reassigned variable as a var ref",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					k i64 := 5
			...
			It(
				"Should leave the synthesized function's params unstamped",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					k i64 := 5
					...
					synth := lo.Filter(
						inter.Functions,
						func(f ir.Function, _ int) bool {
							return strings.HasPrefix(
								f.Key,
								compiler.FmtStrSyntheticPrefix,
							)
						},
					)
			...
			It(
				"Should keep a chain-head format string fed from the register",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					k i64 := 5
			...
			It(
				"Should bind a triggered read of a stateful variable as a var ref",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					acc i64 $= 7
			...
			It(
				"Should keep a triggered read of a reactive variable edge-fed",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					r := count_ch + 1
			...
			It(
				"Should keep a triggered read of a rebound alias edge-fed",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					src := sensor_f
			...
			It(
				"Should stamp every variable in a multi-placeholder format string",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					a i64 := 1
			...
			It(
				"Should mix a var ref with a channel placeholder",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					k i64 := 5
			...
			It(
				"Should stamp a self-incrementing expression and keep its feeder edge",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					stage s1 {
			...
			It(
				"Should not lift a never-reassigned variable placeholder",
				func(ctx SpecContext) {
					inter := analyze(ctx, `
				sequence main {
					k i64 := 5
		...
		It(
			"Should reject a reassigned reactive variable as an input value",
			func(ctx SpecContext) {
				source := `
			func my_func{tag i64} (n u8) i64 {
				return tag
		...
		It(
			"Should reject a reassigned reactive variable as a channel input value",
			func(ctx SpecContext) {
				source := `
			func reader{channel chan i64} () i64 {
				return channel
		...
		It(
			"Should reject a reactive variable inside an input expression",
			func(ctx SpecContext) {
				source := `
			func my_func{tag i64} (n u8) i64 {
				return tag
		...
		It(
			"Should reject a write-channel alias as an input value",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "cmd_ch",
						Kind: symbol.KindChannel,
						Type: types.WriteChan(types.I64()),
						ID:   905,
					},
				}, varResolver...)
				source := `
		...
		It(
			"Should resolve a channel alias to its source channel as a channel input value",
			func(ctx SpecContext) {
				source := `
			func reader{channel chan i64} () i64 {
				return channel
			...
			sequence main {
				...
				Expect(
					found,
				).To(BeTrue(), "expected the alias registered as a read channel")
			},
		)

		It(
			"Should resolve a write-channel alias as a channel input value",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "cmd_ch",
			...
			sequence main {
				...
				Expect(
					found,
				).To(BeTrue(), "expected the alias registered as a write channel")
			},
		)

		It(
			"Should accept a rebound channel alias as a channel input value",
			func(ctx SpecContext) {
				source := `
			func reader{channel chan i64} () i64 {
				return channel
		...
		It(
			"Should register declared and rebound channels for a rebound alias chan input",
			func(ctx SpecContext) {
				source := `
			func reader{channel chan i64} () i64 {
				return channel
		...
		It(
			"Should register declared and rebound channels for a rebound alias chan write input",
			func(ctx SpecContext) {
				resolver := append([]symbol.Symbol{
					{
						Name: "cmd_ch",
		...
		It(
			"Should reject a literal variable as a channel input value",
			func(ctx SpecContext) {
				source := `
			func reader{channel chan i64} () i64 {
				return channel
		...
		It(
			"Should reject a stateful variable as a channel input value",
			func(ctx SpecContext) {
				source := `
			func reader{channel chan i64} () i64 {
				return channel
		...
		It(
			"Should lower a literal reassignment to a feeder into the variable node",
			func(ctx SpecContext) {
				source := `
			sequence main {
				msg := "hello"
				...
				Expect(
					msg,
				).ToNot(BeNil(), "expected msg's register initialized with its literal")
				fed := false
				for _, e := range inter.Edges {
				...
				Expect(
					fed,
				).To(BeTrue(), "expected the reassignment to feed the variable node")
			},
		)

		It(
			"Should fold a re-expressed variable's derivations into one dispatcher",
			func(ctx SpecContext) {
				source := `
			sequence main {
				r := count_ch + 1
				...
				Expect(
					branches,
				).To(HaveLen(2), "the dispatcher must fold both derivations")
				...
				Expect(
					feeders,
				).To(Equal(1), "the dispatcher is the deref's only feeder")
				Expect(trigFed).To(BeTrue(), "count_ch must trigger the dispatcher")
			},
		)

		It(
			"initializes the literal register and leaves the derivation reader edge-fed",
			func(ctx SpecContext) {
				source := `
			sequence main {
				k := 5
				...
				Expect(
					literalValues,
				).To(Equal(1), "only k's register carries a literal value")
				Expect(
					valueless,
				).To(Equal(1), "r's derivation reader is edge-fed, carrying no value")
			},
		)

		It(
			"Should register both channels as rebind candidates on the alias read",
			func(ctx SpecContext) {
				source := `
			sequence main {
				a := count_ch
		...
		It(
			"Should reject rebinding an alias to a nonexistent channel",
			func(ctx SpecContext) {
				source := `
			sequence main {
				a := count_ch
		...
		It(
			"Should reject rebinding an alias to a non-channel value",
			func(ctx SpecContext) {
				source := `
			sequence main {
				a := count_ch
		...
		It(
			"Should reject rebinding an alias to a channel of a different type",
			func(ctx SpecContext) {
				source := `
			sequence main {
				a := count_ch
				...
				Expect(
					diagnostics.String(),
				).To(ContainSubstring("cannot rebind channel read/write variable a of type"))
			},
		)

		It(
			"Should reject reassigning a variable at the top level",
			func(ctx SpecContext) {
				source := `
			a := count_ch
			a = out_ch`
		...
		It(
			"Should reject redeclaring a variable in the same scope",
			func(ctx SpecContext) {
				source := `
			sequence main {
				c i64 := 0
		...
		It(
			"Should reject reassigning a variable with an incompatible value type",
			func(ctx SpecContext) {
				source := `
			sequence main {
				c i64 := 0
		...
		It(
			"Should reject reading a variable outside its declaring scope",
			func(ctx SpecContext) {
				source := `
			sequence main {
				stage s1 {
		...
		It(
			"Should reject assigning a channel read/write to a stateful variable",
			func(ctx SpecContext) {
				source := `
			sequence main {
				c i64 $= count_ch
		...
		It(
			"Should reject assigning a reactive expression to a stateful variable",
			func(ctx SpecContext) {
				source := `
			sequence main {
				c i64 $= count_ch + 1
		...
		It(
			"Should reject a computed stateful variable initializer",
			func(ctx SpecContext) {
				source := `
			sequence main {
				c i64 $= 2 + 3
		...
		It(
			"Should reject initializing a ':=' variable from a stateful variable",
			func(ctx SpecContext) {
				source := `
			sequence main {
				s i64 $= 0
		...
		It(
			"Should assign distinct keys to variables in sibling sequences",
			func(ctx SpecContext) {
				source := `
			sequence a {
				x i64 := 0
		...
		It(
			"Should compile a variable read inside a transition condition",
			func(ctx SpecContext) {
				source := `
			sequence main {
				counter i64 := 0
		...
		It(
			"Should lower a stage-local variable to a node wired between channel reads and writes",
			func(ctx SpecContext) {
				source := `
			sequence main {
				stage s1 {
		...
		It(
			"Should assign distinct keys to stage-local variables in sibling stages",
			func(ctx SpecContext) {
				source := `
			sequence main {
				stage s1 {
	...
	Describe("Analyze", func() {
		...
		Context("Expression Flow Analysis", func() {
			...
			DescribeTable(
				...
				func(ctx SpecContext, source string, chans []symbol.Symbol, expectConstant bool, expectedType types.Type) {
					...
					if expectConstant {
						Expect(
							constantCount,
						).To(Equal(1), "expected exactly one constant node")
						...
						Expect(
							constantCount,
						).To(Equal(0), "expected no constant nodes for complex expressions")
					}
				},
			...
			It(
				"Should reject a negative constant that is out of range for its target",
				func(ctx SpecContext) {
					parsedText := MustSucceed(
						text.Parse(text.Text{Raw: `-5 -> output`}),
					)
					root := symbol.NewRoot(nil, stl.NewSymbols())
					...
					Expect(
						diagnostics.String(),
					).To(ContainSubstring("out of range for u8"))
				},
			)
		})
		...
		Context("Input Values", func() {
			...
			It(
				"Should handle simple input with multiple values",
				func(ctx SpecContext) {
					source := `
				...
				calculator{a=10, b=20, c=30} -> print{}`
					...
					for i, cfg := range node.Inputs {
						...
						Expect(
							cfg.Value,
						).To(Equal(inputValues[cfg.Name]), "input[%d] '%s' value mismatch", i, cfg.Name)
					}
				},
			)
			...
			It(
				"Should handle a type-minimum input value whose magnitude overflows the target",
				func(ctx SpecContext) {
					source := `
				func processor{
					threshold i8
			...
			It(
				"Should resolve channel name to channel ID in input parameter",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "temp_sensor",
			...
			It(
				"Should produce diagnostic error when channel input type mismatches",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "temp_sensor",
			...
			It(
				"Should produce diagnostic error when channel name is not found in resolver",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{}
					source := `
				func reader{channel chan f64} () f64 {
			...
			It(
				"Should reject read channel for input param requiring write channel",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "read_sensor",
			...
			It(
				"Should reject read channel for qualified control.set_authority input param",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "read_sensor",
			...
			It(
				"Should emit deprecation warning for deprecated bare function name",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "write_ch",
				...
				`
					...
					Expect(
						diags.Warnings()[0].Message,
					).To(ContainSubstring("deprecated"))
					Expect(
						diags.Warnings()[0].Message,
			...
			It(
				"Should not emit deprecation warning for qualified function name",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "write_ch",
			...
			It(
				"Should not emit deprecation warning for qualified math.avg",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor",
...
sensor -> math.avg{} -> output`
			...
			It(
				"Should not emit deprecation warning for bare select",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
			...
			It(
				"Should reject a flow node whose required input is never connected",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
					...
					Expect(
						diags.String(),
					).To(ContainSubstring("missing required argument"))
				},
			)

			It(
				"Should emit deprecation warning for bare stable_for",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor",
					...
					Expect(
						diags.Warnings()[0].Message,
					).To(ContainSubstring("stable.for"))
				},
			)

			It(
				"Should not emit deprecation warning for qualified stable.for",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor",
...
sensor -> stable.for{duration=1s} -> output`
			...
			It(
				...
				func(ctx SpecContext) {
					...
					parsedText := MustSucceed(text.Parse(text.Text{Raw: source}))
					_, diags := text.Analyze(
						ctx,
						parsedText,
						NewRoot(nil, statusResolver...),
					)
					Expect(diags.Ok()).To(BeTrue())
					Expect(diags.Warnings()).To(HaveLen(1))
					Expect(
					...
					Expect(
						diags.Warnings()[0].Message,
					).To(ContainSubstring("status.set"))
				},
			)

			It(
				"Should not emit deprecation warning for qualified status.set",
				func(ctx SpecContext) {
					statusFnType := types.Function(types.FunctionProperties{
						Inputs: types.Params{
							{Name: "status_key", Type: types.String()},
...
sensor -> status.set{status_key="alarm", variant="error", message="Bad"}`
			...
			It(
				"Should emit deprecation warning for bare derivative",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor",
					...
					Expect(
						diags.Warnings()[0].Message,
					).To(ContainSubstring("math.derivative"))
				},
			)

			It(
				"Should emit deprecation warning for bare interval",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "output",
					...
					Expect(
						diags.Warnings()[0].Message,
					).To(ContainSubstring("time.interval"))
				},
			)

			It(
				"Should not emit deprecation warning for qualified time.interval",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "output",
...
time.interval{period=100ms} -> output`
			...
			It(
				"Should not emit deprecation warning for qualified time.wait",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "output",
...
time.wait{duration=500ms} -> output`
			...
			It(
				"Should resolve channel name for write operations and add to Channels.Write",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "output_channel",
			...
			It(
				"Should register separate write channels when function with channel input is used multiple times",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "toggle_1",
				...
				toggle_2 -> count_rising{counter=counter_2}`
					...
					Expect(
						node1.Channels.Write,
					).To(HaveKey(uint32(10013)), "first node should write to counter_1")
					Expect(
						node2.Channels.Write,
					).To(HaveKey(uint32(10014)), "second node should write to counter_2")

					Expect(
						node1.Channels.Read,
					).To(HaveKey(uint32(10013)), "first node should read from counter_1")
					Expect(
						node2.Channels.Read,
					).To(HaveKey(uint32(10014)), "second node should read from counter_2")
				},
			)

			It(
				"Should not add stateful variable to write channels when initialized from global channel",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "toggle_1",
			...
			It(
				"Should resolve read-only input param channel in Channels.Read",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "do_0_state",
				...
				do_0_state -> count_rising_test{counter_ch=do_0_counter, max_ch=do_0_counter_max}`
					...
					Expect(
						node.Channels.Write,
					).To(HaveKey(uint32(10202)), "should write to do_0_counter")
					Expect(
						node.Channels.Read,
					).To(HaveKey(uint32(10203)), "should read from do_0_counter_max")
					Expect(node.Inputs).To(HaveLen(3))
			...
			It(
				"Should handle input values using global constants",
				func(ctx SpecContext) {
					source := `
				A := 10
				B := 20
			...
			It(
				"Should handle mixed literal and constant input values",
				func(ctx SpecContext) {
					source := `
				THRESHOLD := 100
			...
			It(
				"Should resolve anonymous input values by position into IR nodes",
				func(ctx SpecContext) {
					source := `
				func transform{scale f64, offset f64} (x f64) f64 {
				    return x * scale + offset
			...
			It(
				"Should resolve partial anonymous input with defaults into IR nodes",
				func(ctx SpecContext) {
					source := `
			...
			It(
				"Should resolve channel identifier as anonymous input into IR",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor_chan",
		...
		Context("Edge Parameter Validation", func() {
			It(
				"Should create edges with parameters that exist in node definitions",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor",
			...
			It(
				"Should handle functions with custom input parameter names",
				func(ctx SpecContext) {
					source := `
				func generator{} () i64 {
				    return 42
			...
			It(
				"Should reject a multi-input function used as a bare flow node",
				func(ctx SpecContext) {
		...
		Context("Output Routing Tables", func() {
			It(
				"Should analyze simple output routing with multiple targets",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "signal",
			...
			It(
				"Should generate a conditional edge for a => chained case body",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "signal",
			...
			It(
				"Should assign per-edge kinds in a case body mixing -> and =>",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "signal",
			...
			It(
				"Should not create phantom output edges for void functions in routing branches",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "counter",
			...
			It(
				"Should report error for non-existent output parameter",
				func(ctx SpecContext) {
					source := `
				func simple{} () (bob i64) {
				    bob = 42
		...
		Context("Stratification", func() {
			...
			It(
				"Should calculate strata for output routing tables",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "signal",
		...
		Context("Channel Sink Detection", func() {
			It(
				"Should create write node for channel at end of flow",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "input_chan",
		...
		Context("Sequence Targeting", func() {
			It(
				"Should connect one-shot edge to sequence's first stage entry node",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should handle sequence with multiple stages - activates first stage",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should error when targeting empty sequence (no stages)",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should emit an inline stage case body as a gated synth nested in its enclosing scope with Activation bound",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should emit an inline sequence case body as a sequential synth nested in its enclosing scope",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should emit two distinct synths for two inline case bodies in the same table",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should emit only one synth when one entry is inline and another is a chain",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
				...
				sequence main {
					...
					for _, stratum := range hold.Strata {
						for _, m := range stratum {
							if m.Scope != nil &&
								strings.HasPrefix(m.Scope.Key, ir.InlinePrefix) {
								inlineCount++
							}
						}
					}
			...
			It(
				"Should not wrap a top-level stage with inline routing in a synthetic Sequential scope",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
			...
			It(
				"Should not emit a duplicate transition when inline routing sits directly in a sequence body",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should not auto-advance the enclosing sequence when inline routing fires",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
				...
				sequence main {
					...
					for _, t := range main.Transitions {
						if strings.HasPrefix(t.On.Node, "select") {
							Fail(
								"inline routing must not register a transition on main; " +
			...
			It(
				"Should preserve activation on a named top-level stage with an inline case body",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should emit an inline stage flow target as a gated synth root sibling with Activation bound",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
				...
				trigger -> stage {
					...
					for _, stratum := range inter.Root.Strata {
						for _, m := range stratum {
							if m.Scope != nil &&
								strings.HasPrefix(m.Scope.Key, "__inline_") {
								synth = m.Scope
							}
						}
					}
			...
			It(
				"Should emit an inline sequence flow target as a sequential synth sibling",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should emit two distinct synth siblings for two inline flow targets in the same scope",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should emit only one synth sibling when one flow statement is inline and another is a plain chain",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should not wrap a top-level stage with inline flow targets in a synthetic Sequential scope",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
			...
			It(
				"Should not emit a duplicate transition when an inline flow target sits directly in a sequence body",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should not auto-advance the enclosing sequence when an inline flow target fires",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
			...
			It(
				"Should preserve activation on a named top-level stage with an inline flow target",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should emit a distinct gated synth for each body when inline flow targets are nested",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should reject `=> next` inside an inline routing case body",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
			...
			It(
				"Should reject `=> next` that escapes an inline sequence routing case body",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
			...
			It(
				"Should reject `=> next` inside an inline stage flow target",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
			...
			It(
				"Should reject `=> next` that escapes an inline sequence flow target",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "flag",
			...
			It(
				"Should handle sequence in routing table as sink",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "high_chan",
		...
		Context("Direct Stage Targeting", func() {
			It(
				"Should emit a member-key transition when targeting a sibling stage",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "input",
		...
		Context("Cross-scope Transitions", func() {
			...
			It(
				"Places => X on the enclosing sequence's frame when X is two levels up through a stage and a nested sequence",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Prefers the innermost sequence's member when the name exists at multiple levels",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Emits exactly one transition per flow step that ends in an explicit => X (no auto-wire duplicate)",
				func(ctx SpecContext) {
		...
		Context("Top-level anonymous scopes", func() {
			It(
				"Should compile an anonymous top-level stage and produce a root member with an auto-generated key",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "ch",
			...
			It(
				"Should compile an anonymous top-level sequence and produce a root member with an auto-generated key",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "ch",
			...
			It(
				"Should compile a named top-level sequence as LivenessGated with no activation when nothing targets it",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "ch",
			...
			It(
				"Should compile a named top-level stage as LivenessGated with no activation when nothing targets it",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "ch",
			...
			It(
				"Should compile a named top-level sequence as LivenessGated with an activation handle when `trigger => name` targets it",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "trigger",
			...
			It(
				"Should compile named nested scopes as LivenessGated at every depth",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "ch",
			...
			It(
				"Should compile anonymous nested scopes as LivenessAlways at every depth",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "ch",
				...
				sequence {
					...
					for _, stratum := range inter.Root.Strata {
						for _, m := range stratum {
							if m.Scope != nil &&
								strings.HasPrefix(m.Scope.Key, "seq_") {
								outer = m.Scope
							}
						}
					}
			...
			It(
				"Should compile mixed named/anonymous nesting per the uniform rule",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "ch",
				...
				sequence main {
					...
					for _, stratum := range first.Strata {
						for _, mem := range stratum {
							if mem.Scope != nil &&
								strings.HasPrefix(mem.Scope.Key, "seq_") {
								anonSeq = mem.Scope
							}
						}
					}
		...
		Context("next keyword", func() {
			It(
				"Should emit a member-key transition targeting the following stage",
				func(ctx SpecContext) {
					source := `
				sequence main {
				    stage first {
		...
		Context("Implicit Expression Triggers", func() {
			It(
				"Should inject implicit trigger for expression as first flow node",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor",
			...
			It(
				"Should not suppress a trigger when the flow writes a different channel",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "a",
			...
			It(
				"Should inject multiple triggers for multi-channel expression",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "temp",
			...
			It(
				"Should not inject trigger for constant expressions",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "output",
			...
			It(
				"Should not inject trigger when expression is not first node",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor",
			...
			It(
				"Should inject trigger for expression in sequence stage",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "sensor",
		...
		Context("Interval One-Shot Edge Generation", func() {
			It(
				"Should generate one-shot edge for interval triggering function",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "interval",
							...
							Type: types.Function(types.FunctionProperties{
								Inputs: types.Params{
									{Name: "period", Type: types.TimeSpan()},
								},
			...
			It(
				"Should generate continuous edge for interval with -> operator",
				func(ctx SpecContext) {
					resolver := []symbol.Symbol{
						{
							Name: "interval",
	...
	Describe("Synthesized Format-String Functions", func() {
		It(
			"Registers a fmt$ function for a flow-form format string with a single placeholder",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
				...
				Expect(
					synthNode.Channels.Read,
				).To(HaveKeyWithValue(uint32(100), "sensor"))
			},
		)

		It(
			"Registers a fmt$ function whose Channels.Read covers every placeholder channel",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Records the aliased channel key for a placeholder that reads a channel read/write",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Does not synthesize a fmt$ function for a literal format string with no placeholders",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "trig",
				...
				for _, f := range inter.Functions {
					Expect(
						strings.HasPrefix(f.Key, compiler.FmtStrSyntheticPrefix),
					).To(BeFalse(),
		...
		It(
			"Registers a fmt$ function for an rf-prefixed multi-line format string preserving backslashes across newlines",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Registers a fmt$ function for an rf-prefixed format string preserving backslashes",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Registers a fmt$ function for a multi-line format string with placeholders across newlines",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Surfaces analyzer diagnostics for an invalid format spec at this layer",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
	...
	Describe("Single Invocations in Stages", func() {
		It(
			"Should compile standalone function invocation to IR node",
			func(ctx SpecContext) {
				source := `
			func setup() {}
		...
		It(
			"Should reject void functions mid-chain in flow statements",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "counter",
		...
		It(
			"Should place a single-invocation step's node in its stage's first phase",
			func(ctx SpecContext) {
				source := `
			func initialize() u8 {
			    return 1
	...
	Describe("Authority Analysis", func() {
		It(
			"Should include authority input in IR with simple form",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "valve",
	...
	Describe("Compile", func() {
		...
		It(
			"Should reject reading a variable whose initializer did not fold",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "trig",
		...
		It(
			"Should reject a function body reading a variable whose initializer did not fold",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "trig",
		...
		It(
			"Should fold a never-reassigned top-level variable into a function body",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "trig",
		...
		It(
			"Should compile a function-only program with no flow statement",
			func(ctx SpecContext) {
		...
		It(
			"Should compile function with channel input param assigned to intermediate variable",
			func(ctx SpecContext) {
	...
	Describe("ExecContext", func() {
		It(
			"Should reject a WASM-only function in a flow statement",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
			...
			`
				...
				Expect(
					diagnostics.String(),
				).To(ContainSubstring("cannot be used as a flow statement"))
			},
		)
		...
		It(
			"Should allow an ExecBoth user function in a flow statement",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Should allow a flow-only STL function in a flow statement",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Should allow an ExecBoth STL function (time.now) in a flow statement",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Should omit inputs on an STL ExecBoth node whose upstream is a trigger",
			func(ctx SpecContext) {
...
sensor -> stub.both{key_or_name="x", message="y", variant="info"}`
		...
		It(
			"Should keep inputs on a user-defined function in a flow statement",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"Should keep inputs on an ExecFlow STL node in a flow statement",
			func(ctx SpecContext) {
...
sensor -> stub.sink{}`
		...
		It(
			"Should allow time.now{} to write into a plain i64 channel",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "int_out",
		...
		It(
			"Should reject a flow-only function called in a func body at analysis time",
			func(ctx SpecContext) {
				resolver := []symbol.Symbol{
					{
						Name: "sensor",
			...
			`
				...
				).To(ContainSubstring("cannot be called inside a func block"))
			},
		)
	})
})
