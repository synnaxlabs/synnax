...
time.wait{duration=500ms} -> output`
		...
		Context("Channel Sink Detection", func() {
			...
			It("Should create write node for channel mid-chain followed by ExecBoth consumer", func(ctx SpecContext) {
				...
				resolver := []symbol.Symbol{
					{Name: "tick", Kind: symbol.KindChannel, Type: types.Chan(types.TimeStamp()), ID: 10051},
					{Name: "log_b", Kind: symbol.KindChannel, Type: types.Chan(types.String()), ID: 10052},
					{Name: "emitKey", Kind: symbol.KindFunction, Exec: symbol.ExecBoth, Type: producerProps},
					{Name: "finish", Kind: symbol.KindFunction, Exec: symbol.ExecBoth, Type: consumerProps},
				}
			...
			It("Should wire edges from upstream producer into mid-chain channel write", func(ctx SpecContext) {
				...
				resolver := []symbol.Symbol{
					{Name: "tick", Kind: symbol.KindChannel, Type: types.Chan(types.TimeStamp()), ID: 10061},
					{Name: "log_b", Kind: symbol.KindChannel, Type: types.Chan(types.String()), ID: 10062},
					{Name: "emitKey", Kind: symbol.KindFunction, Exec: symbol.ExecBoth, Type: producerProps},
					{Name: "finish", Kind: symbol.KindFunction, Exec: symbol.ExecBoth, Type: consumerProps},
				}
				source := `tick -> emitKey{key="flow"} -> log_b -> finish{key_or_name="flow"}`
