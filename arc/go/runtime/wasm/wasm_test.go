...
func newHarness(
	...
	if resolver != nil {
		compileResolver = symbol.CompoundResolver{resolver, stdlib}
	} else {
		compileResolver = stdlib
	}
	opts = append(opts, arc.WithResolver(compileResolver))
...
func newTextHarness(
	...
	mod := MustSucceed(text.Compile(ctx, analyzed, compiler.WithHostSymbols(testutil.NewStdlibResolver())))
	cfg := state.Config{IR: analyzed}
	if len(channelDigests) > 0 {
		cfg.ChannelDigests = channelDigests
	}
...
var _ = Describe("WASM", func() {
	...
	Describe("String Operations Extended", func() {
		DescribeTable("string len() function",
			...
			Entry("variable concatenation", ctx, "len_var_concat", types.I32(), `{
				...
				b str := " world"
				return len(a + b)
			}`, series.SymbolResolver, int32(11)),
		)
	})
	...
	Describe("Imperative Channel Writes", func() {
		Describe("Writing to Non-Indexed Channels", func() {
			It("Should write only data channel when index is not configured", func() {
				...
				h.state.Series.Clear()
				h.state.Strings.Clear()
				fr, changed := h.state.Channel.Flush(telem.Frame[uint32]{})
		...
		Describe("Comparison with Declarative Writes", func() {
			It("Should produce same output structure as WriteChan for indexed channels", func() {
				...
				h.state.Series.Clear()
				h.state.Strings.Clear()
				fr, _ := h.state.Channel.Flush(telem.Frame[uint32]{})
	...
	Describe("Channel Config Parameter Arithmetic", func() {
		...
		It("Should read from channel config param and perform f32 arithmetic", func() {
			...
			h.state.Series.Clear()
			h.state.Strings.Clear()
			outFr, changed := h.state.Channel.Flush(telem.Frame[uint32]{})
			Expect(changed).To(BeTrue())
...
`
			...
			h.state.Series.Clear()
			h.state.Strings.Clear()
			outFr, _ := h.state.Channel.Flush(telem.Frame[uint32]{})
	...
	Describe("For Loops", func() {
		Describe("Range loops", func() {
			DescribeTable("should compute correct sums",
				expectOutput[int64],
				Entry("1-arg range", ctx, "range1", types.I64(), `{
					sum i64 := 0
					for i := range(5) {
						sum = sum + i
					}
				...
				Entry("2-arg range", ctx, "range2", types.I64(), `{
					sum i64 := 0
					for i := range(5, 10) {
						sum = sum + i
					}
				...
				Entry("3-arg range with step", ctx, "range3", types.I64(), `{
					sum i64 := 0
					for i := range(0, 10, 2) {
						sum = sum + i
					}
				...
				Entry("empty range", ctx, "range_empty", types.I64(), `{
					sum i64 := 99
					for i := range(0) {
						sum = sum + i
					}
				...
				Entry("reversed bounds", ctx, "range_rev", types.I64(), `{
					sum i64 := 99
					for i := range(10, 5) {
						sum = sum + i
					}
				...
				Entry("step of 3", ctx, "range_step3", types.I64(), `{
					sum i64 := 0
					for i := range(0, 10, 3) {
						sum = sum + i
					}
				...
				Entry("explicit step of 1", ctx, "range_step1", types.I64(), `{
					sum i64 := 0
					for i := range(0, 5, 1) {
						sum = sum + i
					}
				...
				Entry("step larger than range", ctx, "range_big_step", types.I64(), `{
					sum i64 := 0
					for i := range(0, 5, 10) {
						sum = sum + i
					}
				...
				Entry("step equals range", ctx, "range_step_eq", types.I64(), `{
					sum i64 := 0
					for i := range(0, 10, 10) {
						sum = sum + i
					}
				...
				Entry("negative step", ctx, "range_neg_step", types.I64(), `{
					sum i64 := 0
					for i := range(10, 0, -2) {
						sum = sum + i
					}
				...
				Entry("negative step of -1", ctx, "range_neg1", types.I64(), `{
					sum i64 := 0
					for i := range(10, 0, -1) {
						sum = sum + i
					}
				...
				Entry("negative step with large gap", ctx, "range_neg_big", types.I64(), `{
					sum i64 := 0
					for i := range(20, 0, -5) {
						sum = sum + i
					}
				...
				Entry("negative step empty", ctx, "range_neg_empty", types.I64(), `{
					sum i64 := 99
					for i := range(0, 10, -1) {
						sum = sum + i
					}
