...
var _ = Describe("Scheduler", func() {
	...
	Describe("Construction", func() {
		...
		It(
			"Should execute every member of a single-phase root scope",
			func(ctx SpecContext) {
				mock("A")
				mock("B")
				mock("C")
	...
	Describe("Phase-based execution", func() {
		...
		It(
			"Should skip phase-N members without an incoming change",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				prog := programOf(
		...
		It(
			"Should propagate continuous edges to downstream members",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
		...
		It(
			"Should gate conditional edges on source output truthiness",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeA.OnNext = markOnNext()
		...
		It(
			"Should replay a self-changed node on the next cycle",
			func(ctx SpecContext) {
				nodeA := mock("A")
	...
	Describe("Gated scope activation", func() {
		It(
			"Should not execute a gated scope before its activation fires",
			func(ctx SpecContext) {
				trigger := mock("trigger")
		...
		It(
			"Should activate a gated scope once its activation handle is truthy",
			func(ctx SpecContext) {
				mock("trigger", true)
				stage := mock("stage_node")
				act := ir.Handle{Node: "trigger", Param: "output"}
	...
	Describe("Sequential scope transitions", func() {
		...
		It(
			"Should honor source-order when multiple transitions become truthy simultaneously",
			func(ctx SpecContext) {
				mock("trigger", true)
				firstNode := mock("first_node")
		...
		It(
			"Should cascade multiple transitions within a single cycle",
			func(ctx SpecContext) {
	...
	Describe("Activation cascading & reset", func() {
		...
		It(
			"Should cascade reset into nested always-live scopes on activation",
			func(ctx SpecContext) {
				mock("trigger", true)
				inner := mock("inner")
		...
		It(
			"Should auto-activate an anonymous top-level always-live scope",
			func(ctx SpecContext) {
		...
		It(
			"Should leave a named top-level gated scope inert when it has no activation handle",
			func(ctx SpecContext) {
		...
		It(
			"Should cascade through nested always-live scopes at depth",
			func(ctx SpecContext) {
	...
	Describe("Change propagation", func() {
		It(
			"Should fire only the edge whose source param was marked",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeC := mock("C")
		...
		It(
			"Should fan out a single change to multiple downstream members",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeC := mock("C")
		...
		It(
			"Should run a join node once when several inputs fire in the same cycle",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeC := mock("C")
		...
		It(
			"Should propagate a change through a chain in a single cycle",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeC := mock("C")
		...
		It(
			"Should not propagate when no edge targets the source's changed param",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeA.OnNext = markOnNext()
	...
	Describe("Conditional edge semantics", func() {
		...
		It(
			"Should keep continuous edges unaffected by source truthiness",
			func(ctx SpecContext) {
		...
		It(
			"Should fire each conditional edge independently per param truthiness",
			func(ctx SpecContext) {
	...
	Describe("Self-changed replay", func() {
		It(
			"Should stop replaying once the node no longer marks itself",
			func(ctx SpecContext) {
				nodeA := mock("A")
				callCount := 0
	...
	Describe("NextDeadline", func() {
		It(
			"Should return TimeSpanMax when no node sets a deadline",
			func(ctx SpecContext) {
				mock("A")
				prog := programOf(
					[]ir.Node{irNode("A")},
		...
		It(
			"Should reset to TimeSpanMax between cycles when no node re-sets it",
			func(ctx SpecContext) {
				nodeA := mock("A")
				call := 0
				nodeA.OnNext = func(c node.Context) {
		...
		It(
			"Should track deadlines from nodes inside an active gated scope",
			func(ctx SpecContext) {
				mock("trigger", true)
	...
	Describe("Error handling", func() {
		It(
			"Should continue executing remaining members after a node reports an error",
			func(ctx SpecContext) {
		...
		It(
			"Should swallow errors silently when no handler is configured",
			func(ctx SpecContext) {
				nodeA := mock("A")
	...
	Describe("Complex graph and sequence interactions", func() {
		...
		It(
			"Should mix continuous and conditional edges in a single graph",
			func(ctx SpecContext) {
	...
	Describe("Transitions gated on fresh output marks", func() {
		...
		It(
			"Should not fire a transition when the source is truthy but never called MarkChanged",
			func(ctx SpecContext) {
		...
		It(
			"Should not re-fire a transition on a later cycle when the source stays truthy but only marked on the first cycle",
			func(ctx SpecContext) {
		...
		It(
			"Should fire a transition again when the source freshly marks changed on a later cycle",
			func(ctx SpecContext) {
	...
	Describe("Convergence bound", func() {
		It(
			"Should not infinite-loop if a transition's on-handle stays truthy and owner stays active",
			func(ctx SpecContext) {
	...
	Describe("Settle passes", func() {
		It(
			"Should re-run an already-visited node when a later node writes back to it",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
		...
		It(
			"Should not re-pass when a conditional backward edge stays falsy",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeA.OnNext = markOnNext()
		...
		It(
			"Should bound settle passes for a mutually-marking cycle",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeA.OnNext = markOnNext()
		...
		It(
			"Should run the next sequential step on the settle pass so it observes prior writes",
			func(ctx SpecContext) {
				var order []string
				trigger := mock("trigger", true)
				nodeV := mock("V")
	...
	Describe("Change-flag consumption", func() {
		...
		It(
			"Should dispatch a marked node once despite an unrelated re-pass",
			func(ctx SpecContext) {
				trigger := mock("trigger")
				worker := mock("worker")
				nodeA := mock("A")
		...
		It(
			"Should dispatch chain nodes once per mark across settle passes",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeC := mock("C")
		...
		It(
			"Should fire a stage's one-shot-triggered node once per activation",
			func(ctx SpecContext) {
				mock("trigger", true)
				entry := mock("entry")
		...
		It(
			"Should re-dispatch a node marked again after it already ran",
			func(ctx SpecContext) {
				nodeA := mock("A")
				nodeB := mock("B")
				nodeC := mock("C")
		...
		It(
			"Should never dispatch an unmarked node across settle passes",
			func(ctx SpecContext) {
				mock("quiet")
				silent := mock("silent")
				nodeA := mock("A")
		...
		It(
			"Should preserve a mark a node sets on itself while it runs",
			func(ctx SpecContext) {
		...
		It(
			"Should dispatch again on a fresh mark in the next cycle",
			func(ctx SpecContext) {
				nodeA := mock("A")
				worker := mock("worker")
				nodeA.OnNext = markOnNext()
	...
	Describe("Sequential strata variable members", func() {
		It(
			"Should reset a sequential scope's strata members on activation",
			func(ctx SpecContext) {
				mock("trigger", true)
				nodeV := mock("V")
				stageNode := mock("M")
		...
		It(
			"Should clear a pending self-change and re-reset on scope re-entry",
			func(ctx SpecContext) {
				trigger := mock("trigger", true)
				trigger.SuppressAutoMark = true
		...
		It(
			"Should ignore a strata variable member with no matching node",
			func(ctx SpecContext) {
				mock("trigger", true)
				mock("M")
				stage := parallelScope("stage", stratum(ir.NodeMember("M")))
