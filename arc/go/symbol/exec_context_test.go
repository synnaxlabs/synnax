...
var _ = Describe("ExecContext", func() {
	Describe("Compatible", func() {
		DescribeTable(
			...
			Entry(
				"untagged is never visible (WASM filter)",
				symbol.ExecContext(0),
				symbol.ExecWASM,
				false,
			),
			Entry(
				"untagged is never visible (Flow filter)",
				symbol.ExecContext(0),
				symbol.ExecFlow,
				false,
			),
			Entry(
				"untagged is never visible (no filter)",
				symbol.ExecContext(0),
				symbol.ExecContext(0),
				false,
			),
			Entry(
				"WASM visible when no filter",
				symbol.ExecWASM,
				symbol.ExecContext(0),
				true,
			),
			Entry(
				"Flow visible when no filter",
				symbol.ExecFlow,
				symbol.ExecContext(0),
				true,
			),
			...
			Entry(
				"WASM not compatible with Flow",
				symbol.ExecWASM,
				symbol.ExecFlow,
				false,
			),
			Entry(
				"Flow not compatible with WASM",
				symbol.ExecFlow,
				symbol.ExecWASM,
				false,
			),
