...
var _ = Describe("Scope", func() {
	Describe("Root", func() {
		...
		It(
			"Should resolve through the given global resolver on a miss",
		...
		It(
			"Should resolve lexically through a resolver set after construction",
			func(bCtx SpecContext) {
	...
	Describe("Add", func() {
		...
		It(
			"Should correctly increment IDs for variables within function scopes",
			func(bCtx SpecContext) {
				rootScope := symbol.NewRoot(nil, nil)
				funcScope := MustSucceed(rootScope.Add(
					bCtx,
		...
		It(
			"Should not return error when adding duplicate symbol that shadows a global",
			func(bCtx SpecContext) {
				rootScope := symbol.NewRoot(nil, nil)
				scope1 := MustSucceed(rootScope.Add(
					bCtx,
		...
		It(
			"Should allow shadowing global symbols from resolver",
			func(bCtx SpecContext) {
		...
		It(
			"Should resolve to local symbol when shadowing global",
			func(bCtx SpecContext) {
				globalResolver := StaticResolver{
					{Name: "x", Kind: symbol.KindInput, Type: types.F64()},
				}
				rootScope := symbol.NewRoot(globalResolver, nil)
				localScope := MustSucceed(
					rootScope.Add(
						bCtx,
						symbol.Symbol{
							Name: "x",
		...
		It(
			"Should resolve to local symbol when shadowing global in nested scope",
			func(bCtx SpecContext) {
				globalResolver := StaticResolver{
					{Name: "x", Kind: symbol.KindInput, Type: types.F64()},
				}
				rootScope := symbol.NewRoot(globalResolver, nil)
				funcScope := MustSucceed(
					rootScope.Add(
						bCtx,
						symbol.Symbol{Name: "f", Kind: symbol.KindFunction},
					),
		...
		It("Should allow symbols with empty names", func(bCtx SpecContext) {
			rootScope := symbol.NewRoot(nil, nil)
			child := MustSucceed(
				rootScope.Add(bCtx, symbol.Symbol{Name: "", Kind: symbol.KindBlock}),
			)
	...
	Describe("GetChildByParserRule", func() {
		...
		It("Should return error when parser rule not found", func(bCtx SpecContext) {
			...
			Expect(scope).ToNot(BeNil())
			Expect(
				rootScope.GetChildByParserRule(antlr.NewBaseParserRuleContext(nil, 0)),
			).Error().
				To(MatchError(ContainSubstring("could not find symbol matching parser rule")))
		})
	})
	...
	Describe("Resolve", func() {
		...
		It("Should skip internal symbols from global resolver", func(bCtx SpecContext) {
			...
			rootScope := symbol.NewRoot(globalResolver, nil)
			Expect(
				rootScope.Resolve(bCtx, "host_fn"),
			).Error().
				To(MatchError(ContainSubstring("undefined symbol: host_fn")))
		})
		It(
			"Should resolve non-internal symbols from global resolver alongside internal ones",
			func(bCtx SpecContext) {
				globalResolver := StaticResolver{
					{
						Name:     "host_fn",
		...
		It(
			"Should resolve internal symbols when IncludeInternal is passed",
			func(bCtx SpecContext) {
				globalResolver := StaticResolver{
					{
						Name:     "host_fn",
				...
				rootScope := symbol.NewRoot(globalResolver, nil)
				resolved := MustSucceed(
					rootScope.Resolve(bCtx, "host_fn", symbol.IncludeInternal),
				)
		...
		It(
			"Should match a numeric name by ID rather than by Name",
			func(bCtx SpecContext) {
				rootScope := symbol.NewRoot(nil, nil)
		...
		It(
			"Should report the origin scope on undefined-symbol error after a multi-level walk",
			func(bCtx SpecContext) {
				rootScope := symbol.NewRoot(nil, nil)
				funcScope := MustSucceed(rootScope.Add(
					bCtx,
		...
		Describe("Module aliases", func() {
			...
			It(
				"Should fail head lookup when no alias exists for the module",
				func(bCtx SpecContext) {
			...
			It(
				"Should resolve a qualified lookup through a same-name alias",
				func(bCtx SpecContext) {
					rootScope := buildAmbientRoot(bCtx)
					...
					resolved := MustSucceed(
						resolveQualified(bCtx, rootScope, "time", "now"),
					)
			...
			It(
				"Should resolve a qualified lookup through a renamed alias",
				func(bCtx SpecContext) {
					rootScope := buildAmbientRoot(bCtx)
					timeMod := rootScope.Parent.FindChild("time")
					MustSucceed(rootScope.Add(bCtx, symbol.Symbol{
					...
					resolved := MustSucceed(
						resolveQualified(bCtx, rootScope, "t", "now"),
					)
					Expect(resolved.Name).To(Equal("now"))
				},
			)
			...
			It(
				"Should mark the alias as used on head resolution",
				func(bCtx SpecContext) {
					rootScope := buildAmbientRoot(bCtx)
					timeMod := rootScope.Parent.FindChild("time")
			...
			It(
				"Should leave the alias unused when usage tracking is suppressed",
				func(bCtx SpecContext) {
					rootScope := buildAmbientRoot(bCtx)
					timeMod := rootScope.Parent.FindChild("time")
					alias := MustSucceed(rootScope.Add(bCtx, symbol.Symbol{
					...
					MustSucceed(
						rootScope.Resolve(bCtx, "t", symbol.WithoutUsageTracking),
					)
	...
	Describe("Search", func() {
		...
		It(
			"Should fuzzy-match a term within edit distance two",
			func(bCtx SpecContext) {
				rootScope := symbol.NewRoot(nil, nil)
				scope := MustSucceed(
					rootScope.Add(
		...
		It(
			"Should not fuzzy-match a term of two characters or fewer",
			func(bCtx SpecContext) {
				rootScope := symbol.NewRoot(nil, nil)
				scope := MustSucceed(
					rootScope.Add(
		...
		It(
			"Should skip all internal symbols when searching by prefix",
			func(bCtx SpecContext) {
				globalResolver := StaticResolver{
					{
						Name:     "element_add",
	...
	Describe("ClosestAncestorOfKind", func() {
		It("Should find closest ancestor of kind", func(bCtx SpecContext) {
			...
			blockScope := MustSucceed(
				funcScope.Add(
					bCtx,
					symbol.Symbol{Name: "block", Kind: symbol.KindBlock},
				),
			)
			varScope := MustSucceed(
				blockScope.Add(
		...
		It("Should return self if matching kind", func(bCtx SpecContext) {
			...
			ancestor := MustSucceed(
				funcScope.ClosestAncestorOfKind(symbol.KindFunction),
			)
			Expect(ancestor).To(Equal(funcScope))
		})
	...
	Describe("String", func() {
		It("Should format scope as string", func(bCtx SpecContext) {
			...
			funcScope := MustSucceed(
				rootScope.Add(
					bCtx,
					symbol.Symbol{Name: "myFunc", Kind: symbol.KindFunction},
				),
			)
			varScope := MustSucceed(
				funcScope.Add(
	...
	Describe("Channels", func() {
		...
		Describe("ResolveInputChannel", func() {
			It(
				"Should fall back to Read when fnSym has no children (built-in functions)",
				func() {
			...
			It(
				"Should replace internal Read ID with actual channel ID for user-defined functions",
				func(bCtx SpecContext) {
			...
			It(
				"Should replace internal Write ID with actual channel ID for user-defined functions",
				func(bCtx SpecContext) {
					fnSym := symbol.NewRoot(nil, nil)
					fnSym.Kind = symbol.KindFunction
					fnSym.Channels = types.NewChannels()
			...
			It(
				"Should handle param that is both read and written",
				func(bCtx SpecContext) {
					fnSym := symbol.NewRoot(nil, nil)
					fnSym.Kind = symbol.KindFunction
					fnSym.Channels = types.NewChannels()
			...
			It(
				"Should use WriteChan access for built-in with WriteChan input param",
				func() {
					fnSym := &symbol.Symbol{
						Name: "set_authority",
						Kind: symbol.KindFunction,
			...
			It(
				"Should use ReadChan access for built-in with ReadChan input param",
				func() {
					fnSym := &symbol.Symbol{
						Name: "on",
						Kind: symbol.KindFunction,
			...
			It(
				"Should default to Read for built-in with plain Chan input param",
				func() {
					fnSym := &symbol.Symbol{
						Name: "custom",
						Kind: symbol.KindFunction,
