...
var keywordDocs = map[string]string{
	...
	// Hardcoded until SY-4198 (#2346) migrates STL hover docs to Symbol.Doc.
	"status.set": doc.New(
		...
		doc.Divider(),
		arcCode("status_key := sensor -> status.set{key_or_name=\"ox_alarm\", variant=\"error\", message=\"Overpressure\"}"),
		doc.Divider(),
