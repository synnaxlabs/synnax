...
var _ = Describe("Semantic Tokens", func() {
	...
	Describe("appendTokenPerLine — raw string spans", func() {
		It("emits one token for a single-line raw literal", func(ctx SpecContext) {
			...
			tokens := filterByType(
				decodeSemanticTokens(SemanticTokens(server, ctx, uri).Data),
				tokenTypeString,
			)
		...
		It(
			"splits a multi-line literal with one mid-newline into two tokens",
		...
		It(
			"splits a three-line multi-line literal into three tokens",
			func(ctx SpecContext) {
				OpenArcDocument(server, ctx, uri, "x := `a\nb\nc`")
				tokens := filterByType(
					decodeSemanticTokens(SemanticTokens(server, ctx, uri).Data),
		...
		It(
			"emits a final token for a closing backtick on its own line",
			func(ctx SpecContext) {
		...
		It(
			"skips empty lines in a multi-line literal with consecutive newlines",
			func(ctx SpecContext) {
				OpenArcDocument(server, ctx, uri, "x := `a\n\nb`")
				tokens := filterByType(
					decodeSemanticTokens(SemanticTokens(server, ctx, uri).Data),
	...
	Describe("Token Type Routing", func() {
		It(
			"routes single-quoted literals to the string token type",
			func(ctx SpecContext) {
		...
		It(
			"routes raw-prefixed literals to the string token type",
			func(ctx SpecContext) {
		...
		It(
			"does not emit a string token when no string literal is present",
			func(ctx SpecContext) {
		...
		It(
			"routes raw triple-quoted literals to the string token type across newlines",
			func(ctx SpecContext) {
	...
	Describe("Numeric-literal unit suffixes", func() {
		...
		It(
			"does not classify a unit suffix as the builtin it collides with",
			func(ctx SpecContext) {
		...
		It(
			"still classifies a real call to that builtin as a function",
			func(ctx SpecContext) {
				OpenArcDocument(server, ctx, uri, src)
				fn := filterByType(
					decodeSemanticTokens(SemanticTokens(server, ctx, uri).Data),
	...
	Describe("Format-string placeholders", func() {
		It(
			"splits f\"val: {42}\" into prefix, string segments, placeholder braces, and a number",
			func(ctx SpecContext) {
				...
				Expect(
					inLit[0],
				).To(Equal(decodedToken{Line: 0, StartChar: 5, Length: 1, TokenType: tokenTypeFunction}))
				Expect(
					inLit[1],
				).To(Equal(decodedToken{Line: 0, StartChar: 6, Length: 1, TokenType: tokenTypeString}))
				Expect(
					inLit[2],
				).To(Equal(decodedToken{Line: 0, StartChar: 7, Length: 5, TokenType: tokenTypeString}))
				Expect(
					inLit[3],
				).To(Equal(decodedToken{Line: 0, StartChar: 12, Length: 1, TokenType: tokenTypeStringPlaceholder}))
				Expect(
					inLit[4],
				).To(Equal(decodedToken{Line: 0, StartChar: 13, Length: 2, TokenType: tokenTypeNumber}))
				Expect(
					inLit[5],
				).To(Equal(decodedToken{Line: 0, StartChar: 15, Length: 1, TokenType: tokenTypeStringPlaceholder}))
				Expect(
					inLit[6],
				).To(Equal(decodedToken{Line: 0, StartChar: 16, Length: 1, TokenType: tokenTypeString}))
			},
		)

		It(
			"classifies a placeholder identifier through the global resolver",
			func(ctx SpecContext) {
				...
				Expect(
					ch[0],
				).To(Equal(decodedToken{Line: 0, StartChar: 11, Length: 10, TokenType: tokenTypeChannel}))
			},
		)

		It(
			"treats {{ as a literal-brace escape and leaves bare } literal",
			func(ctx SpecContext) {
		...
		It(
			"recognizes a real placeholder while ignoring surrounding doubled braces",
			func(ctx SpecContext) {
		...
		It(
			"falls back to prefix + string tokens on a malformed placeholder",
			func(ctx SpecContext) {
				...
				Expect(
					str[0],
				).To(Equal(decodedToken{Line: 0, StartChar: 6, Length: 17, TokenType: tokenTypeString}))
		...
		It(
			"emits a placeholder span for a numeric format spec after the expression",
			func(ctx SpecContext) {
				...
				Expect(
					ph[0],
				).To(Equal(decodedToken{Line: 0, StartChar: 9, Length: 1, TokenType: tokenTypeStringPlaceholder}))
				Expect(
					ph[1],
				).To(Equal(decodedToken{Line: 0, StartChar: 12, Length: 4, TokenType: tokenTypeStringPlaceholder}))
				Expect(
					ph[2],
				).To(Equal(decodedToken{Line: 0, StartChar: 16, Length: 1, TokenType: tokenTypeStringPlaceholder}))
				Expect(filterByType(all, tokenTypeNumber)).To(HaveLen(1))
			},
		)

		It(
			"classifies multi-token placeholder expressions with prev/next context",
			func(ctx SpecContext) {
				channels := []symbol.Symbol{
					{
						Name: "sensor",
		...
		It(
			"skips inner placeholder tokens that classify to nil (parens)",
			func(ctx SpecContext) {
		...
		It(
			"classifies placeholders across newlines in a multi-line format string",
			func(ctx SpecContext) {
		...
		It(
			"classifies placeholders inside an rf-prefixed format string",
			func(ctx SpecContext) {
		...
		It(
			"classifies placeholders inside an rf-prefixed multi-line format string",
			func(ctx SpecContext) {
		...
		DescribeTable("emits the r/f/rf/fr prefix as a function-typed token",
			func(ctx SpecContext, source string, prefixLen uint32) {
				...
				Expect(
					fn[0],
				).To(Equal(decodedToken{Line: 0, StartChar: 5, Length: prefixLen, TokenType: tokenTypeFunction}))
			},
		...
		It(
			"does not emit a function token for an unprefixed string",
			func(ctx SpecContext) {
	...
	Describe("SemanticTokensFull", func() {
		It(
			"returns an empty token stream for an unknown document URI",
			func(ctx SpecContext) {
				result := MustSucceed(
					server.SemanticTokensFull(ctx, &protocol.SemanticTokensParams{
						TextDocument: protocol.TextDocumentIdentifier{
							URI: "file:///not-open.arc",
						},
					}),
				)
		...
		It(
			"routes module names in a bare import to the namespace token type",
			func(ctx SpecContext) {
		...
		It(
			"routes every module name in a block import to namespace",
			func(ctx SpecContext) {
		...
		It(
			"routes both the module name and its alias to namespace",
			func(ctx SpecContext) {
		...
		It(
			"routes import qualifiers at use sites to the namespace token type",
			func(ctx SpecContext) {
		...
		It(
			"does not route an unimported qualifier to namespace",
			func(ctx SpecContext) {
				OpenArcDocument(server, ctx, uri,
					"func cat() i64 { return time.now() }\n")
				tokens := decodeSemanticTokens(SemanticTokens(server, ctx, uri).Data)
		...
		It(
			"does not route an unimported qualifier's member to function",
			func(ctx SpecContext) {
		...
		It(
			"routes sequence and stage names to the function token type",
			func(ctx SpecContext) {
		...
		It(
			"routes import and as alongside func and authority to the keyword token type",
			func(ctx SpecContext) {
				OpenArcDocument(server, ctx, uri, `import time as t
...
`)
	...
	Describe("variable kinds", func() {
		It(
			"classifies literal, stateful, channel read/write, and channel-read variables distinctly",
			func(ctx SpecContext) {
				channels := []symbol.Symbol{
					{
						Name: "sensorData",
				...
				Expect(channelVariable).To(ContainElements(
					decodedToken{
						...
						StartChar: 0,
						Length:    3,
						TokenType: tokenTypeChannelVariable,
					},
					decodedToken{
						Line:      4,
						StartChar: 0,
		...
		It(
			"colors the declaration of a reassigned channel read/write variable",
			func(ctx SpecContext) {
				channels := []symbol.Symbol{
					{
						Name: "crw_a",
				...
				channelVariable := filterByType(
					decodeSemanticTokens(SemanticTokens(server, ctx, uri).Data),
					tokenTypeChannelVariable,
				)
		...
		It(
			"colors a constant reference with the constant token type",
			func(ctx SpecContext) {
