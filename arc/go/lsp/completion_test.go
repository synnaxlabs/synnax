...
var _ = Describe("Completion", func() {
	...
	BeforeEach(func() {
		server = MustSucceed(
			lsp.New(lsp.Config{NewRoot: func() *symbol.Symbol { return NewRoot(nil) }}),
		)
	...
	Describe("Context-Aware Completion", func() {
		It(
			"should return empty completions in single-line comment",
		...
		It(
			"should return empty completions in multi-line comment",
			func(ctx SpecContext) {
				content := "/* multi\nline\ncomment */"
				OpenArcDocument(server, ctx, docURI, content)
		...
		It(
			"should return only types in type annotation position",
			func(ctx SpecContext) {
				content := "func foo(x "
				OpenArcDocument(server, ctx, docURI, content)
				...
				Expect(
					HasCompletion(completions.Items, "func"),
				).To(BeFalse(), "Should not show 'func' keyword in type annotation context")
				Expect(
					HasCompletion(completions.Items, "if"),
				).To(BeFalse(), "Should not show 'if' keyword in type annotation context")
				Expect(
					HasCompletion(completions.Items, "sequence"),
				).To(BeFalse(), "Should not show 'sequence' keyword in type annotation context")
				Expect(
					HasCompletion(completions.Items, "stage"),
				).To(BeFalse(), "Should not show 'stage' keyword in type annotation context")
				Expect(
					HasCompletion(completions.Items, "next"),
				).To(BeFalse(), "Should not show 'next' keyword in type annotation context")
			},
		)

		It(
			"should return types matching prefix in type annotation position",
			func(ctx SpecContext) {
				...
				for _, item := range completions.Items {
					Expect(
						item.Label,
					).To(HavePrefix("i"), "Expected items with 'i' prefix, got: %s", item.Label)
				}
			},
		)
		...
		It("should not show keywords in expression context", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "func"),
			).To(BeFalse(), "Should not show 'func' keyword in expression context")
			Expect(
				HasCompletion(completions.Items, "if"),
			).To(BeFalse(), "Should not show 'if' keyword in expression context")
			Expect(
				HasCompletion(completions.Items, "sequence"),
			).To(BeFalse(), "Should not show 'sequence' keyword in expression context")
			Expect(
				HasCompletion(completions.Items, "stage"),
			).To(BeFalse(), "Should not show 'stage' keyword in expression context")
			Expect(
				HasCompletion(completions.Items, "next"),
			).To(BeFalse(), "Should not show 'next' keyword in expression context")
		})

		It(
			"should show functions and values in expression context",
			func(ctx SpecContext) {
				content := "x := "
				OpenArcDocument(server, ctx, docURI, content)
				...
				Expect(
					HasCompletion(completions.Items, "len"),
				).To(BeTrue(), "Should show 'len' function in expression context")
				Expect(
					HasCompletion(completions.Items, "time.now"),
				).To(BeTrue(), "Should show 'time.now' function in expression context")
			},
		)

		It(
			"should show function keywords at statement start inside func body",
			func(ctx SpecContext) {
				content := "func foo() {\n    \n}"
				OpenArcDocument(server, ctx, docURI, content)
				...
				Expect(
					HasCompletion(completions.Items, "if"),
				).To(BeTrue(), "Should show 'if' keyword at statement start in func body")
				Expect(
					HasCompletion(completions.Items, "return"),
				).To(BeTrue(), "Should show 'return' keyword at statement start in func body")
				Expect(
					HasCompletion(completions.Items, "func"),
				).To(BeFalse(), "Should not show 'func' inside func body")
				Expect(
					HasCompletion(completions.Items, "sequence"),
				).To(BeFalse(), "Should not show 'sequence' inside func body")
				Expect(
					HasCompletion(completions.Items, "stage"),
				).To(BeFalse(), "Should not show 'stage' inside func body")
				Expect(
					HasCompletion(completions.Items, "next"),
				).To(BeFalse(), "Should not show 'next' inside func body")
			},
		)
		...
		It("should show top-level keywords at top level", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "sequence"),
			).To(BeTrue(), "Should show 'sequence' keyword at top level")
			Expect(
				HasCompletion(completions.Items, "i32"),
			).To(BeFalse(), "Should not show 'i32' type at top level")
			Expect(
				HasCompletion(completions.Items, "f64"),
			).To(BeFalse(), "Should not show 'f64' type at top level")
			Expect(
				HasCompletion(completions.Items, "if"),
			).To(BeFalse(), "Should not show 'if' at top level")
			Expect(
				HasCompletion(completions.Items, "return"),
			).To(BeFalse(), "Should not show 'return' at top level")
			Expect(
				HasCompletion(completions.Items, "stage"),
			).To(BeFalse(), "Should not show 'stage' at top level")
			Expect(
				HasCompletion(completions.Items, "next"),
			).To(BeFalse(), "Should not show 'next' at top level")
		})

		It(
			"should insert a sequence snippet without a nested stage block",
			func(ctx SpecContext) {
		...
		It("should show func keyword at top level", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "func"),
			).To(BeTrue(), "Should show 'func' keyword at top level")
		})

		It(
			"should show only stage keyword inside a sequence body",
			func(ctx SpecContext) {
				...
				Expect(
					HasCompletion(completions.Items, "stage"),
				).To(BeTrue(), "Should show 'stage' keyword inside sequence body")
				Expect(
					HasCompletion(completions.Items, "next"),
				).To(BeFalse(), "Should not show 'next' inside sequence body")
				Expect(
					HasCompletion(completions.Items, "if"),
				).To(BeFalse(), "Should not show 'if' inside sequence body")
				Expect(
					HasCompletion(completions.Items, "return"),
				).To(BeFalse(), "Should not show 'return' inside sequence body")
				Expect(
					HasCompletion(completions.Items, "func"),
				).To(BeFalse(), "Should not show 'func' inside sequence body")
				Expect(
					HasCompletion(completions.Items, "sequence"),
				).To(BeTrue(), "Should show 'sequence' inside sequence body (nested sequences)")
				Expect(
					HasCompletion(completions.Items, "i32"),
				).To(BeFalse(), "Should not show 'i32' type inside sequence body")
				Expect(
					HasCompletion(completions.Items, "f64"),
				).To(BeFalse(), "Should not show 'f64' type inside sequence body")
			},
		)

		It(
			"should show channels and flow-compatible functions inside a sequence body",
			func(ctx SpecContext) {
				...
				completions := Completion(server, ctx, docURI, 1, 4)
				Expect(
					HasCompletion(completions.Items, "temperature_sensor"),
				).To(BeTrue(),
		...
		It(
			"should not offer completions at a sequence declaration name slot",
			func(ctx SpecContext) {
		...
		It(
			"should not offer completions at a stage declaration name slot",
			func(ctx SpecContext) {
		...
		It(
			"should suggest only modules after the 'import' keyword",
			func(ctx SpecContext) {
		...
		It(
			"should suggest only modules matching the prefix when partially typed after 'import'",
			func(ctx SpecContext) {
		...
		It(
			"should not surface internal modules in import-path completion",
			func(ctx SpecContext) {
		...
		It(
			"should not treat a new line after 'import math' as an import-path slot",
			func(ctx SpecContext) {
		...
		It(
			"should not offer completions at a func declaration name slot",
			func(ctx SpecContext) {
		...
		It("should show next keyword inside a stage body", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "next"),
			).To(BeTrue(), "Should show 'next' keyword inside stage body")
			Expect(
				HasCompletion(completions.Items, "stage"),
			).To(BeFalse(), "Should not show 'stage' inside stage body")
			Expect(
				HasCompletion(completions.Items, "if"),
			).To(BeFalse(), "Should not show 'if' inside stage body")
			Expect(
				HasCompletion(completions.Items, "return"),
			).To(BeFalse(), "Should not show 'return' inside stage body")
			Expect(
				HasCompletion(completions.Items, "func"),
			).To(BeFalse(), "Should not show 'func' inside stage body")
			Expect(
				HasCompletion(completions.Items, "sequence"),
			).To(BeTrue(), "Should show 'sequence' inside stage body (inline sequences)")
		})
		...
		It("should not show types at statement start", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "i32"),
			).To(BeFalse(), "Should not show 'i32' type at statement start")
		})
	})

	Describe("Nested If Inside Function", func() {
		It(
			"should show function keywords inside nested if block",
			func(ctx SpecContext) {
				content := "func foo() {\n    if x > 0 {\n        \n    }\n}"
				OpenArcDocument(server, ctx, docURI, content)
				...
				Expect(
					HasCompletion(completions.Items, "if"),
				).To(BeTrue(), "Should show 'if' inside nested if block")
				Expect(
					HasCompletion(completions.Items, "return"),
				).To(BeTrue(), "Should show 'return' inside nested if block")
				Expect(
					HasCompletion(completions.Items, "func"),
				).To(BeFalse(), "Should not show 'func' inside nested if block")
				Expect(
					HasCompletion(completions.Items, "sequence"),
				).To(BeFalse(), "Should not show 'sequence' inside nested if block")
			},
		)
	})
	...
	Describe("Block Expression Completion", func() {
		It("should show function keywords in block expression", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "if"),
			).To(BeTrue(), "Should show 'if' in block expression")
			Expect(
				HasCompletion(completions.Items, "return"),
			).To(BeTrue(), "Should show 'return' in block expression")
			Expect(
				HasCompletion(completions.Items, "func"),
			).To(BeFalse(), "Should not show 'func' in block expression")
			Expect(
				HasCompletion(completions.Items, "sequence"),
			).To(BeFalse(), "Should not show 'sequence' in block expression")
		})
	})

	Describe("GlobalResolver", func() {
		It(
			"should include global variables from GlobalResolver in completion",
			func(ctx SpecContext) {
				...
				Expect(
					found,
				).To(BeTrue(), "Expected to find 'myGlobal' in completion items")
		...
		It(
			"should not show GlobalResolver symbols when prefix doesn't match",
			func(ctx SpecContext) {
				globalResolver := []symbol.Symbol{{
					Name: "myGlobal",
					Type: types.I32(),
				...
				Expect(
					HasCompletion(completions.Items, "myGlobal"),
				).To(BeFalse(), "Expected NOT to find 'myGlobal' in completion items when prefix doesn't match")
			},
		)
	})
	...
	Describe("Parenthesized Expression Completion", func() {
		It(
			"should suggest channels inside parenthesized expression after return",
			func(ctx SpecContext) {
				globalResolver := []symbol.Symbol{{
					Name: "output_sensor",
					Kind: symbol.KindChannel,
	...
	Describe("Input Parameter Completion", func() {
		...
		It(
			...
			func(ctx SpecContext) {
				...
				).To(BeTrue(), "Should suggest 'threshold' parameter")
				Expect(
					HasCompletion(completions.Items, "timeout"),
				).To(BeTrue(), "Should suggest 'timeout' parameter")
				Expect(
					HasCompletion(completions.Items, "channel"),
				).To(BeTrue(), "Should suggest 'channel' parameter")
			},
		)
		...
		It("should filter out already-provided parameters", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "threshold"),
			).To(BeFalse(), "Should NOT suggest already-provided 'threshold' parameter")
			Expect(
				HasCompletion(completions.Items, "channel"),
			).To(BeTrue(), "Should still suggest 'channel' parameter")
		})
		...
		It("should filter by prefix when typing parameter name", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "threshold"),
			).To(BeTrue(), "Should suggest 'threshold' matching prefix 'th'")
			Expect(
				HasCompletion(completions.Items, "timeout"),
			).To(BeFalse(), "Should NOT suggest 'timeout' not matching prefix 'th'")
		})
		...
		It(
			...
			func(ctx SpecContext) {
				...
				Expect(
					HasCompletion(completions.Items, "sensorCh"),
	...
	Describe("Loop Keyword Completion", func() {
		It(
			"should show for keyword at statement start inside func body",
			func(ctx SpecContext) {
				content := "func foo() {\n    \n}"
				OpenArcDocument(server, ctx, docURI, content)
				...
				Expect(
					HasCompletion(completions.Items, "for"),
				).To(BeTrue(), "Should show 'for' keyword at statement start in func body")
				Expect(
					HasCompletion(completions.Items, "break"),
				).To(BeTrue(), "Should show 'break' keyword at statement start in func body")
				Expect(
					HasCompletion(completions.Items, "continue"),
				).To(BeTrue(), "Should show 'continue' keyword at statement start in func body")
			},
		)
		...
		It("should not show for keyword at top level", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "for"),
			).To(BeFalse(), "Should not show 'for' at top level")
			Expect(
				HasCompletion(completions.Items, "break"),
			).To(BeFalse(), "Should not show 'break' at top level")
			Expect(
				HasCompletion(completions.Items, "continue"),
			).To(BeFalse(), "Should not show 'continue' at top level")
		})
		...
		It("should not show loop keywords inside sequence body", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "for"),
			).To(BeFalse(), "Should not show 'for' inside sequence body")
			Expect(
				HasCompletion(completions.Items, "break"),
			).To(BeFalse(), "Should not show 'break' inside sequence body")
			Expect(
				HasCompletion(completions.Items, "continue"),
			).To(BeFalse(), "Should not show 'continue' inside sequence body")
		})

		It(
			"should not show loop keywords in expression context",
			func(ctx SpecContext) {
				content := "func foo() {\n    x := \n}"
				OpenArcDocument(server, ctx, docURI, content)
				...
				Expect(
					HasCompletion(completions.Items, "for"),
				).To(BeFalse(), "Should not show 'for' in expression context")
				Expect(
					HasCompletion(completions.Items, "break"),
				).To(BeFalse(), "Should not show 'break' in expression context")
				Expect(
					HasCompletion(completions.Items, "continue"),
				).To(BeFalse(), "Should not show 'continue' in expression context")
			},
		)
		...
		It("should show range function in expression context", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "range"),
			).To(BeTrue(), "Should show 'range' function in expression context")
		})
	})
	...
	Describe("Stage Body Completion", func() {
		...
		It("should suggest channels inside stage body", func(ctx SpecContext) {
			...
			Expect(
				HasCompletion(completions.Items, "vent_vlv_cmd"),
			).To(BeTrue(), "Should suggest 'vent_vlv_cmd' channel inside stage")
			Expect(
				HasCompletion(completions.Items, "press_vlv_cmd"),
			).To(BeTrue(), "Should suggest 'press_vlv_cmd' channel inside stage")
			Expect(
				HasCompletion(completions.Items, "press_pt"),
			).To(BeTrue(), "Should suggest 'press_pt' channel inside stage")
		})
		...
		It(
			...
			func(ctx SpecContext) {
				...
				Expect(
					HasCompletion(completions.Items, "vent_vlv_cmd"),
				).To(BeTrue(), "Should suggest 'vent_vlv_cmd' matching prefix 'v'")
				Expect(
					HasCompletion(completions.Items, "press_vlv_cmd"),
				).To(BeFalse(), "Should NOT suggest 'press_vlv_cmd' not matching prefix 'v'")
			},
		)
		...
		It(
			...
			func(ctx SpecContext) {
				...
				Expect(
					HasCompletion(completions.Items, "vent_vlv_cmd"),
				).To(BeTrue(), "Should suggest 'vent_vlv_cmd' channel")
				Expect(
					HasCompletion(completions.Items, "press_vlv_cmd"),
				).To(BeTrue(), "Should suggest 'press_vlv_cmd' channel")
			},
		)
	...
	Describe("Module Qualified Completion", func() {
		...
		It("Should return module members for 'math.a' prefix", func(ctx SpecContext) {
			server = MustSucceed(
				lsp.New(
					lsp.Config{NewRoot: func() *symbol.Symbol { return NewRoot(nil) }},
				),
			)
			server.SetClient(&MockClient{})
		...
		It(
			"Should return only WASM time members for 'time.' prefix in func block",
			func(ctx SpecContext) {
				server = MustSucceed(
					lsp.New(
						lsp.Config{
		...
		It(
			"Should return control.set_authority for 'control.' prefix",
			func(ctx SpecContext) {
				server = MustSucceed(
					lsp.New(
						lsp.Config{
		...
		It(
			"Should return control.set_authority for 'control.set_a' prefix",
			func(ctx SpecContext) {
				server = MustSucceed(
					lsp.New(
						lsp.Config{
				...
				Expect(
					item.FilterText,
				).To(Equal(protocol.NewOptional("control.set_authority")))
				Expect(ItemTextEdit(item)).ToNot(BeNil())
				Expect(
					ItemTextEdit(item).NewText,
				).To(Equal("control.set_authority{$0}"))
			},
		)

		It(
			"Should suggest module names at top-level when typing a partial module name",
			func(ctx SpecContext) {
				server = MustSucceed(
					lsp.New(
						lsp.Config{
		...
		It(
			"Should not surface internal members like 'panic' for 'error.' prefix",
			func(ctx SpecContext) {
		...
		It(
			"Should set TextEdit that replaces the full module prefix",
			func(ctx SpecContext) {
				server = MustSucceed(
					lsp.New(
						lsp.Config{
				...
				).To(Equal(protocol.InsertTextFormatSnippet))
		...
		It(
			"Should exclude channel symbols from module-qualified results",
			func(ctx SpecContext) {
				server = MustSucceed(
					lsp.New(
						lsp.Config{
							NewRoot: func() *symbol.Symbol { return NewRoot(nil, channelsWithChannels...) },
						},
					),
				)
				server.SetClient(&MockClient{})
				...
				Expect(HasCompletion(completions.Items, "now")).To(BeTrue())
				Expect(
					HasCompletion(completions.Items, "sy_node_1_metrics_time"),
				).To(BeFalse(),
		...
		It(
			"Should exclude channels even with partial member prefix",
			func(ctx SpecContext) {
				server = MustSucceed(
					lsp.New(
						lsp.Config{
	...
	Describe("ExecContext Filtering", func() {
		...
		It(
			"should not show unqualified flow-only functions in func block",
			func(ctx SpecContext) {
				server = MustSucceed(
					lsp.New(
						lsp.Config{
	...
	Describe("Module Completion Metadata", func() {
		...
		It(
			"labels an unimported module with Module kind and a non-'invalid' detail",
			func(ctx SpecContext) {
				...
				Expect(
					found,
				).To(BeTrue(), "math module should be suggested for prefix 'mat'")
		...
		It(
			"labels an already-imported module with Module kind and a non-'invalid' detail",
			func(ctx SpecContext) {
				OpenArcDocument(
					server,
					ctx,
		...
		It(
			"inserts a loose import statement when no imports exist",
			func(ctx SpecContext) {
				OpenArcDocument(server, ctx, docURI, "func test() {\n    mat\n}")
				completions := Completion(server, ctx, docURI, 1, 7)
				item, found := FindCompletion(completions.Items, "math")
		...
		It(
			"adds to an existing import block instead of creating a second block",
			func(ctx SpecContext) {
				OpenArcDocument(
					server,
					ctx,
		...
		It(
			"consolidates a loose import statement into a single block when adding a module",
			func(ctx SpecContext) {
				OpenArcDocument(
					server,
					ctx,
		...
		It(
			"consolidates multiple loose import statements into one block",
			func(ctx SpecContext) {
				OpenArcDocument(
					server,
					ctx,
				...
				Expect(
					replace.Range.Start,
				).To(Equal(protocol.Position{Line: 0, Character: 0}))
				Expect(
					replace.Range.End,
				).To(Equal(protocol.Position{Line: 0, Character: 11}))
				deleteOld := item.AdditionalTextEdits[1]
				Expect(deleteOld.NewText).To(BeEmpty())
				Expect(
					deleteOld.Range.Start,
				).To(Equal(protocol.Position{Line: 1, Character: 0}))
				Expect(
					deleteOld.Range.End,
				).To(Equal(protocol.Position{Line: 2, Character: 0}))
			},
		)

		It(
			"preserves comments sitting between two loose import statements",
			func(ctx SpecContext) {
				OpenArcDocument(
					server,
					ctx,
				...
				Expect(
					deleteOld.Range.End,
				).To(Equal(protocol.Position{Line: 3, Character: 0}))
			},
		)

		It(
			"does not attach an import edit when the module is already imported",
			func(ctx SpecContext) {
				OpenArcDocument(
					server,
					ctx,
	...
	Describe("Module Member Auto-Import", func() {
		...
		It(
			"attaches an import edit to a qualified member when the module is not imported",
			func(ctx SpecContext) {
				OpenArcDocument(server, ctx, docURI, "func test() {\n    time.no\n}")
				completions := Completion(server, ctx, docURI, 1, 11)
				...
				Expect(
					found,
				).To(BeTrue(), "time.now should be suggested for prefix 'time.no'")
				Expect(item.AdditionalTextEdits).To(HaveLen(1))
				edit := item.AdditionalTextEdits[0]
		...
		It(
			"does not attach an import edit to a qualified member when the module is already imported",
			func(ctx SpecContext) {
				OpenArcDocument(
					server,
					ctx,
		...
		It(
			"does not double-import when a dot-completion follows a freshly auto-imported module",
			func(ctx SpecContext) {
		...
		It(
			"surfaces module members under their qualified name for a bare-name prefix and qualifies on insert",
			func(ctx SpecContext) {
				OpenArcDocument(server, ctx, docURI, "func test() {\n    no\n}")
				completions := Completion(server, ctx, docURI, 1, 6)
		...
		It(
			"does not attach an import edit to a bare-name qualified suggestion when the module is already imported",
			func(ctx SpecContext) {
				OpenArcDocument(
					server,
					ctx,
		...
		It(
			"appends an input-block snippet when a function completes in a flow context",
			func(ctx SpecContext) {
		...
		It(
			"appends a call-parens snippet when a function completes in an imperative context",
			func(ctx SpecContext) {
		...
		It(
			"appends an input-block snippet for bare-name deep-search results in a flow context",
			func(ctx SpecContext) {
		...
		It(
			"does not append an invocation suffix to non-function symbols",
			func(ctx SpecContext) {
				...
				Expect(
					item.InsertTextFormat,
				).ToNot(Equal(protocol.InsertTextFormatSnippet))
			},
		)

		It(
			"filters bare-name qualified suggestions by execution context",
			func(ctx SpecContext) {
	...
	Describe("Same-Line After Opening Brace", func() {
		DescribeTable(
			"returns no completions when the cursor is on the same line as the opening brace",
		...
		DescribeTable(
			"still returns block-body completions on the next line",
