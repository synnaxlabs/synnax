...
var _ = Describe("Rename", func() {
    ...
    return x * y
		...
		It(
			"should not rename variables across different functions",
...
func main() {
    ...
    y := sensor * 2.0
		...
		It(
			"should invoke OnRename with the resolved symbol and names",
			func(ctx SpecContext) {
				var (
		...
		It(
			"should produce text edits for every channel reference in the document",
			func(ctx SpecContext) {
		...
		It(
			"should abort the rename when OnRename returns an error",
			func(ctx SpecContext) {
		...
		It(
			"should invoke OnRename for source-defined symbols too",
			func(ctx SpecContext) {
				callCount := 0
    ...
    y := x + 10
		...
		It(
			"should reject rename on symbols not marked Renameable",
			func(ctx SpecContext) {
				onRename := func(context.Context, *symbol.Symbol, string, string) error { return nil }
				server = MustSucceed(lsp.New(lsp.Config{
					NewRoot:  newRootWithChannels,
