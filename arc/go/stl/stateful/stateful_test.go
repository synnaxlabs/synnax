...
var _ = Describe("Vars", func() {
	var (
		...
		seriesS *seriesstate.State
		strS    *stringsstate.State
		mod     *stateful.Module
	)
	...
	BeforeEach(func() {
		...
		seriesS = seriesstate.New()
		strS = stringsstate.New()
	...
	Describe("i32 scalar types", func() {
		It("Should load initial value and persist stores", func() {
			...
			Expect(load(ctx, 0, 42)).To(Equal(uint32(42)))
			Expect(load(ctx, 0, 99)).To(Equal(uint32(42)))
			store(ctx, 0, 100)
			Expect(load(ctx, 0, 99)).To(Equal(uint32(100)))
		})
		...
		It("Should keep different varIDs independent", func() {
			...
			store(ctx, 0, 10)
			store(ctx, 1, 20)
			Expect(load(ctx, 0, 0)).To(Equal(uint32(10)))
			Expect(load(ctx, 1, 0)).To(Equal(uint32(20)))
		})
		...
		It("Should return latest value after overwrite", func() {
			...
			store(ctx, 0, 20)
			store(ctx, 0, 30)
			Expect(load(ctx, 0, 0)).To(Equal(uint32(30)))
		})
	...
	Describe("i64 scalar types", func() {
		It("Should load initial value and persist stores", func() {
			...
			Expect(load(ctx, 0, 1000)).To(Equal(uint64(1000)))
			store(ctx, 0, 2000)
			Expect(load(ctx, 0, 0)).To(Equal(uint64(2000)))
		})
		...
		It("Should create inner map on store to a fresh node key", func() {
			...
			store(ctx, 0, 999)
			Expect(load(ctx, 0, 0)).To(Equal(uint64(999)))
		})
	})
	...
	Describe("float types", func() {
		It("Should load and store f32", func() {
			...
			Expect(load(ctx, 0, float32(3.14))).To(Equal(float32(3.14)))
			store(ctx, 0, float32(2.718))
			Expect(load(ctx, 0, 0)).To(Equal(float32(2.718)))
		})
		...
		It("Should create inner map on f32 store to a fresh node key", func() {
			...
			load := testutil.Get[func(context.Context, uint32, float32) float32](rt, "state", "load_f32")
			mod.SetNodeKey("fresh")
			store(ctx, 0, float32(1.5))
			Expect(load(ctx, 0, 0)).To(Equal(float32(1.5)))
		})
		...
		It("Should load and store f64", func() {
			...
			Expect(load(ctx, 0, 3.14159)).To(Equal(3.14159))
			store(ctx, 0, 2.71828)
			Expect(load(ctx, 0, 0)).To(Equal(2.71828))
		})
		...
		It("Should create inner map on f64 store to a fresh node key", func() {
			...
			mod.SetNodeKey("fresh")
			store(ctx, 0, 9.99)
			Expect(load(ctx, 0, 0)).To(Equal(9.99))
		})
	})
	...
	Describe("node key isolation", func() {
		It("Should isolate state between different node keys", func() {
			...
			store(ctx, 0, 200)
			mod.SetNodeKey("node1")
			Expect(load(ctx, 0, 0)).To(Equal(uint32(100)))
			mod.SetNodeKey("node2")
			Expect(load(ctx, 0, 0)).To(Equal(uint32(200)))
		})
	})
	...
	Describe("string state", func() {
		...
		It("Should silently ignore store with invalid handle", func() {
			...
			Expect(MustBeOk(strS.Get(load(ctx, 0, fallbackH)))).To(Equal("fallback"))
		})
	})
	...
	Describe("series state", func() {
		It("Should load and store series via handles", func() {
			...
			store(ctx, 0, seriesS.Store(telem.NewSeriesV(10.0, 20.0)))
			ser2 := MustBeOk(seriesS.Get(load(ctx, 0, 0)))
		...
		It("Should deep copy on initial load so mutations don't corrupt state", func() {
			...
			load(ctx, 0, initH)
			initSer.Data[0] = 0xFF
			stored := MustBeOk(seriesS.Get(load(ctx, 0, 0)))
		...
		It("Should isolate series state between different node keys", func() {
			...
			store := testutil.Get[func(context.Context, uint32, uint32)](rt, "state", "store_series_f64")
			mod.SetNodeKey("node1")
			store(ctx, 0, seriesS.Store(telem.NewSeriesV(1.0, 2.0)))
			mod.SetNodeKey("node2")
			store(ctx, 0, seriesS.Store(telem.NewSeriesV(100.0, 200.0, 300.0)))
			mod.SetNodeKey("node1")
			Expect(MustBeOk(seriesS.Get(load(ctx, 0, 0))).Len()).To(Equal(int64(2)))
			mod.SetNodeKey("node2")
			Expect(MustBeOk(seriesS.Get(load(ctx, 0, 0))).Len()).To(Equal(int64(3)))
		})
	})
})
