...
import (
	...
	"github.com/synnaxlabs/arc/stl/channel/state"
...
var _ = Describe("State", func() {
	...
	Describe("New", func() {
		It("Should initialize index mappings from digests", func() {
			cs := state.New([]state.Digest{
				{Key: 10, Index: 11},
				{Key: 20, Index: 21},
			})
			cs.WriteSeries(10, telem.NewSeriesV[float32](1.0), telem.NewSeriesSecondsTSV(100))
		...
		It("Should ignore zero-value index in digests", func() {
			cs := state.New([]state.Digest{{Key: 10, Index: 0}})
	...
	Describe("ReadValue", func() {
		...
		It("Should return false for a channel with no data", func() {
			s = state.New([]state.Digest{{Key: 10}})
			_, ok := s.ReadValue(10)
			Expect(ok).To(BeFalse())
		})
