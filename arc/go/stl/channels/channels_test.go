	...
	Describe("WASM Bindings", func() {
		...
		Describe("float types", func() {
			...
			It("Should write and read back f64 values", func(ctx SpecContext) {
				...
				Expect(
					testutil.AsF64(result[0]),
				).To(BeNumerically("~", 2.718281828, 0.0001))
			})
		})

		Describe("string type", func() {
			It(
				"Should write and read back string values via handles",
		...
		Describe("read with no data", func() {
			...
			It(
				"Should report no value until a sample is ingested",
				func(ctx SpecContext) {
					Expect(cs.HasValue(1)).To(BeFalse())
					cs.Ingest(telem.UnaryFrame[uint32](1, telem.NewSeriesV(2.5)))
					Expect(cs.HasValue(1)).To(BeTrue())
				},
			)

			It("Should report no value for an empty series", func(ctx SpecContext) {
				cs.Ingest(telem.UnaryFrame[uint32](1, telem.NewSeriesV[float64]()))
				Expect(cs.HasValue(1)).To(BeFalse())
			})
		})
	})
			...
			It("Should create source node for on type", func() {
				cfg := rnode.Config{
					Node: ir.Node{
						Type: "on",
						Inputs: types.Params{
							{Name: "channel", Type: types.U32(), Value: uint32(42)},
						},
					},
			...
			It("Should parse channel from input", func() {
				cfg := rnode.Config{
					Node: ir.Node{
						Type: "on",
						Inputs: types.Params{
							{Name: "channel", Type: types.U32(), Value: uint32(123)},
						},
					},
					State: rtState.Node("test"),
				}
			...
			It("Should coerce channel to uint32", func() {
				cfg := rnode.Config{
					Node: ir.Node{
						Type: "on",
						Inputs: types.Params{
							{Name: "channel", Type: types.U32(), Value: uint32(99)},
						},
					},
					State: rtState.Node("test"),
				}
			...
			It("Should create sink node for write type", func() {
				cfg := rnode.Config{
					Node: ir.Node{
						Type: "write",
						Inputs: types.Params{
							{Name: "channel", Type: types.U32(), Value: uint32(10)},
						},
					},
			...
			It("Should return error for invalid input", func() {
				cfg := rnode.Config{
					Node: ir.Node{
						Type: "on",
						Inputs: types.Params{
							{Name: "invalid", Type: types.String(), Value: "field"},
						},
					},
					State: rtState.Node("test"),
				}
		...
		Describe("Data Reading", func() {
			It("Should read channel data after ingestion", func(ctx SpecContext) {
				...
				source.Next(
					rnode.Context{
						Context:     ctx,
						MarkChanged: func(int) { outputChanged = true },
					},
				)
				Expect(outputChanged).To(BeTrue())
				Expect(
					*progState.Node("source").Output(0),
				).To(telem.MatchSeries(telem.NewSeriesV[float32](1.5, 2.5, 3.5)))
				Expect(
					*progState.Node("source").OutputTime(0),
				).To(telem.MatchSeries(telem.NewSeriesSecondsTSV(100, 101, 102)))
			})
			...
			It("Should handle channel without index", func(ctx SpecContext) {
				source := MustSucceed(factory.Create(rnode.Config{
					Node: ir.Node{
						Type: "on",
						Inputs: types.Params{
							{Name: "channel", Type: types.U32(), Value: uint32(20)},
						},
					},
					State: progState.Node("source"),
				}))
			...
			It("Should not trigger on empty channel", func(ctx SpecContext) {
				source := MustSucceed(factory.Create(rnode.Config{
					Node: ir.Node{
						Type: "on",
						Inputs: types.Params{
							{Name: "channel", Type: types.U32(), Value: uint32(999)},
						},
					},
					State: progState.Node("source"),
				}))
			...
			It("Should handle multiple series in MultiSeries", func(ctx SpecContext) {
				...
				source.Next(
					rnode.Context{
						Context:     ctx,
						MarkChanged: func(int) { outputCount++ },
					},
				)
		...
		Describe("Reset", func() {
			It(
				...
				func(ctx SpecContext) {
					...
					Expect(
						triggered,
					).To(BeFalse(), "stale pre-reset data should not trigger the source")
					...
					Expect(
						triggered,
					).To(BeTrue(), "data written after reset should trigger the source")
				},
			)
			...
			It(
				...
				func(ctx SpecContext) {
					...
					source.Next(
						rnode.Context{
							Context:     ctx,
							MarkChanged: func(int) { callCount++ },
						},
					)
			...
			It("Should skip data when alignment mismatch", func(ctx SpecContext) {
				...
				source := MustSucceed(mod.Create(rnode.Config{
					Node: ir.Node{
						Type: "on",
						Inputs: types.Params{
							{Name: "channel", Type: types.U32(), Value: uint32(30)},
						},
					},
	...
	Describe("Index Pairing", func() {
		...
		newSource := func(nodeKey string, ch uint32) rnode.Node {
			return MustSucceed(factory.Create(rnode.Config{
				Node: ir.Node{
					Type: "on",
					Inputs: types.Params{
						{Name: "channel", Type: types.U32(), Value: ch},
					},
				},
			...
			DescribeTable("Should fire every channel sharing one index",
				func(ctx SpecContext, n int) {
					...
					for i := range n {
						Expect(
							firesOn(ctx, srcs[i]),
						).To(BeTrue(), "channel %d must fire", i)
		...
		Describe("Single write fires each source once", func() {
			fireCount := func(ctx SpecContext, src rnode.Node) int {
				count := 0
				src.Next(
					rnode.Context{Context: ctx, MarkChanged: func(int) { count++ }},
				)
				return count
			}

			DescribeTable(
				"Should fire each shared-index channel exactly once per write",
				func(ctx SpecContext, n int) {
					chans := []uint32{10, 12, 14, 16}
					srcs := make([]rnode.Node, n)
					for i := range n {
					...
					for i := range n {
						Expect(
							fireCount(ctx, srcs[i]),
						).To(Equal(1), "channel %d must fire once", i)
						Expect(
							(*progState.Node(keys[i]).Output(0)).Len(),
						).To(Equal(int64(1)),
							"channel %d must emit one sample, not the shared index buffer", i)
						Expect(
							fireCount(ctx, srcs[i]),
						).To(Equal(0), "channel %d must not re-fire", i)
					}
				},
				Entry("two channels", 2),
				Entry("three channels", 3),
			...
			It(
				...
				func(ctx SpecContext) {
					...
					Expect(
						fireCount(ctx, src),
					).To(Equal(1), "one data sample must fire once")
					Expect((*progState.Node("s0").Output(0)).Len()).To(Equal(int64(1)))
					Expect(emittedValue("s0")).To(Equal(float32(42)))
					Expect(
						fireCount(ctx, src),
					).To(Equal(0), "must not re-fire without new data")
				},
			)
		})
		...
		Describe("Sustained high-rate (permanent-stall regression)", func() {
			...
			It(
				"Should fire all four fridge sensors every cycle on a shared 50Hz index",
				func(ctx SpecContext) {
					chans := []uint32{10, 12, 14, 16}
		...
		Describe("High-water mark and ordering", func() {
			It("Should ignore data below the high-water mark", func(ctx SpecContext) {
				...
				Expect(
					firesOn(ctx, src),
				).To(BeFalse(), "below-watermark sample must not fire")
			})
		...
		Describe("Data Writing", func() {
			It("Should write channel data when input available", func(ctx SpecContext) {
				...
				inputData.TimeRange = telem.TimeRange{
					Start: 500 * telem.SecondTS,
					End:   501 * telem.SecondTS,
				}
			...
			It("Should flow data from source through sink", func(ctx SpecContext) {
				...
				sink := MustSucceed(mod.Create(rnode.Config{
					Node: ir.Node{
						Type: "write",
						Inputs: types.Params{
							{Name: "channel", Type: types.U32(), Value: uint32(3)},
						},
					},
					State: s.Node("write"),
				}))
				...
				).To(telem.MatchSeries(telem.NewSeriesV[int32](42, 99)))
				Expect(
					outputFr.Get(4).Series[0],
				).To(telem.MatchSeries(telem.NewSeriesSecondsTSV(10, 20)))
			})
		})
		Describe("Multiple Channels", func() {
			It(
				"Should handle multiple independent source-sink pairs",
				func(ctx SpecContext) {
					g := graph.Graph{
						Nodes: []graph.Node{
							{Key: "read1"},
					...
					sink2, _ := factory.Create(rnode.Config{
						Node: ir.Node{
							Type: "write",
							Inputs: types.Params{
								{Name: "channel", Type: types.U32(), Value: uint32(40)},
							},
						},
						State: s.Node("write2"),
					})
					...
					).To(telem.MatchSeries(telem.NewSeriesV[float32](1.1, 2.2)))
					Expect(
						outputFr.Get(40).Series[0],
					).To(telem.MatchSeries(telem.NewSeriesV(3.3, 4.4)))
				},
			)
		})
	})
...
var _ = Describe("Source Rebind", func() {
			...
			Nodes: ir.Nodes{
				{
					...
					Type: "variable",
					Inputs: types.Params{
						{Name: "f0", Type: types.U32(), Value: uint32(10)},
					},
					Outputs: types.Params{
	...
	It(
		"Should re-point at the key on the binding edge and skip buffered data",
		func(ctx SpecContext) {
			ingest(10, 0, 1.5)
			changed := false
			source.Next(
			...
			*progState.Node("bind").Output(0) = telem.NewSeriesV[uint32](20)
			...
			Expect(
				changed,
			).To(BeFalse(), "data buffered before the rebind must not fire")
	...
	It(
		"Should rebind on Reset and absorb data buffered on the new channel",
		func(ctx SpecContext) {
			*progState.Node("bind").Output(0) = telem.NewSeriesV[uint32](20)
			ingest(20, 0, 9.9)
			source.Reset()
