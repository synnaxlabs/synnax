...
func (fp FunctionProperties) EncodeOrc(w *orc.Writer) error {
	...
	w.Bool(fp.Config != nil)
	if fp.Config != nil {
		w.Uint32(uint32(len(fp.Config)))
		for i := range fp.Config {
			if err := fp.Config[i].EncodeOrc(w); err != nil {
				return err
			}
		}
	}
...
func (fp *FunctionProperties) DecodeOrc(r *orc.Reader) error {
	...
	}
	{
		...
		if present {
			...
			for i := range fp.Config {
				if err = fp.Config[i].DecodeOrc(r); err != nil {
					return err
				}
			}
		}
...
func (t Type) EncodeOrc(w *orc.Writer) error {
	...
	if t.Config != nil {
		...
		for i := range t.Config {
			if err := t.Config[i].EncodeOrc(w); err != nil {
				return err
			}
		}
	}
...
func (t *Type) DecodeOrc(r *orc.Reader) error {
	...
	}
	{
		...
		if present {
			...
			for i := range t.Config {
				if err = t.Config[i].DecodeOrc(r); err != nil {
					return err
				}
			}
		}
