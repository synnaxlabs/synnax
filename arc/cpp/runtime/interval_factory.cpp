...
#include "arc/cpp/runtime/interval_factory.h"
...
IntervalNodeFactory::create(const NodeFactoryConfig& cfg) {
    ...
    if (cfg.ir_node.config_values.count("period") == 0) {
        return {nullptr, xerrors::Error(
            ...
            "Interval node '" + cfg.ir_node.key + "' missing 'period' in config_values")};
    }
    ...
    if (cfg.ir_node.channels.write.count("output") == 0) {
        return {nullptr, xerrors::Error(
            ...
            "Interval node '" + cfg.ir_node.key + "' missing 'output' in channels.write")};
    }
