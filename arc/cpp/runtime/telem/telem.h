...
public:
    ...
    x::errors::Error next(node::Context &ctx) override {
        ...
        for (size_t i = 0; i < data.series.size(); i++) {
            ...
            state.output(0) = x::memory::make_local_shared<x::telem::Series>(
                ser.deep_copy()
            );
            state.output_time(0) = x::memory::make_local_shared<x::telem::Series>(
...
public:
    ...
    create(const node::Config &cfg) override {
        ...
        if (channel_param == nullptr)
            return {nullptr, x::errors::Error("telem node missing channel config")};

        auto channel_key = channel_param->value.get<types::ChannelKey>();

        if (cfg.node.type == "on") {
            return {std::make_unique<On>(cfg.state, channel_key), x::errors::NIL};
        }
        return {std::make_unique<Write>(cfg.state, channel_key), x::errors::NIL};
    }
};
