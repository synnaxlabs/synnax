...
TEST(TelemFactoryTest, CreateSourceNode) {
    ...
    state::Config cfg{
        .ir = ir,
        .channels = {{10, telem::FLOAT32_T, 11}}
    };
    state::State s(cfg);
...
TEST(OnNodeTest, ReadChannelData) {
    ...
    frame.series->push_back(telem::Series(std::vector<telem::TimeStamp>{
        telem::TimeStamp(100),
        telem::TimeStamp(101),
        telem::TimeStamp(102)
    }));
...
TEST(OnNodeTest, EmptyChannel) {
    ...
    state::Config cfg{
        .ir = ir,
        .channels = {{999, telem::FLOAT32_T, 0}}
    };
    state::State s(cfg);
...
TEST(WriteNodeTest, WriteChannelData) {
    ...
    state::Config cfg{
        .ir = ir,
        .channels = {{100, telem::FLOAT32_T, 101}}
    };
    state::State s(cfg);
