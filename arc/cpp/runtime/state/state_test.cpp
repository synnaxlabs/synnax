...
/// @brief Test that flush calls error handler when multiple frames arrive for same
/// channel.
TEST(StateTest, FlushReportsDataDroppedError) {
    arc::ir::Node ir_node;
    ir_node.key = "test";
    ir_node.type = "test";
    ...
    Config cfg{.ir = ir, .channels = {{42, telem::FLOAT32_T, 0}}};
    State s(cfg, error_handler);

    auto series1 = telem::Series(telem::FLOAT32_T, 1);
    series1.write(1.0f);
    auto series2 = telem::Series(telem::FLOAT32_T, 1);
    series2.write(2.0f);
    auto series3 = telem::Series(telem::FLOAT32_T, 1);
    series3.write(3.0f);

    s.ingest(telem::Frame(42, std::move(series1)));
    s.ingest(telem::Frame(42, std::move(series2)));
    s.ingest(telem::Frame(42, std::move(series3)));

    EXPECT_TRUE(reported_errors.empty());

    s.flush();

    ASSERT_EQ(reported_errors.size(), 1);
    ASSERT_MATCHES(reported_errors[0], arc::runtime::errors::DATA_DROPPED);
    EXPECT_NE(reported_errors[0].message().find("dropped 2 frames"), std::string::npos);
}

/// @brief Test that flush does not report error when only one frame arrives.
TEST(StateTest, FlushNoErrorOnSingleFrame) {
    arc::ir::Node ir_node;
    ir_node.key = "test";
    ir_node.type = "test";
