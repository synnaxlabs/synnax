...
class StableFor : public node::Node {
    ...
    x::telem::TimeSpan last_changed{0};

public:
    explicit StableFor(const StableForConfig &cfg, state::Node &&state):
    ...
    x::errors::Error next(node::Context &ctx) override {
        ...
        if (ctx.elapsed - this->last_changed >= this->cfg.duration) {
            if (!this->last_sent.has_value() || *this->last_sent != current_value) {
                ...
                *o_time = x::telem::Series(ctx.elapsed.nanoseconds());
    ...
    void reset() override {
        ...
        this->last_changed = x::telem::TimeSpan(0);
    }
