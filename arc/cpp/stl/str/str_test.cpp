...
(module
  ...
  (import "string" "format_i32" (func $format_i32 (param i32 i32 i32) (result i32)))
  (import "string" "format_u32" (func $format_u32 (param i32 i32 i32) (result i32)))
  (import "string" "format_i64" (func $format_i64 (param i64 i32 i32) (result i32)))
  (import "string" "format_u64" (func $format_u64 (param i64 i32 i32) (result i32)))
  (import "string" "format_f32" (func $format_f32 (param f32 i32 i32) (result i32)))
  (import "string" "format_f64" (func $format_f64 (param f64 i32 i32) (result i32)))
  (import "string" "format_string" (func $format_string (param i32 i32 i32) (result i32)))
  ...
  (func (export "call_format_string") (param i32 i32 i32) (result i32)
    (call $format_string (local.get 0) (local.get 1) (local.get 2)))
)
)wat";
...
static std::string
call_format_string(StrModuleFixture &f, int32_t handle, const std::string &spec) {
    f.write_spec(spec);
    auto fn = f.get_func("call_format_string");
    auto result = fn.call(
                        f.store,
                        {wasmtime::Val(handle),
                         wasmtime::Val(static_cast<int32_t>(SPEC_OFFSET)),
...
TEST(StrModule, FormatStringWithSVerbPassesThrough) {
    ...
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(handle), "s"), "hello");
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(handle), "10s"), "     hello");
    EXPECT_EQ(
        call_format_string(f, static_cast<int32_t>(handle), "-10s"),
        "hello     "
    );
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(handle), ".3s"), "hel");
}

TEST(StrModule, FormatStringWithQVerbQuotesGoStyle) {
    StrModuleFixture f;
    const auto handle = f.state->create("hello");
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(handle), "q"), "\"hello\"");

    const auto h_special = f.state->create("a\nb\tc\"d\\e");
    EXPECT_EQ(
        call_format_string(f, static_cast<int32_t>(h_special), "q"),
        R"("a\nb\tc\"d\\e")"
    );

    const auto h_ctrl = f.state->create(std::string("\x01\x7F", 2));
    EXPECT_EQ(
        call_format_string(f, static_cast<int32_t>(h_ctrl), "q"),
        "\"\\x01\\x7f\""
    );

    const auto h_empty = f.state->create("");
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(h_empty), "q"), "\"\"");
}

TEST(StrModule, FormatStringWithXVerbHexEncodesBytes) {
    StrModuleFixture f;
    const auto handle = f.state->create("abc");
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(handle), "x"), "616263");
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(handle), "X"), "616263");
    const auto h_full = f.state->create(std::string("\xDE\xAD", 2));
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(h_full), "x"), "dead");
    EXPECT_EQ(call_format_string(f, static_cast<int32_t>(h_full), "X"), "DEAD");
}
