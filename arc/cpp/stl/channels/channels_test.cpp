...
/// @brief a seeded digest pre-fills the read buffer so the value is readable
/// before any write occurs.
TEST(ChannelStateTest, SeedPrefillsReadBuffer) {
    auto seed = ::x::mem::make_local_shared<::x::telem::Series>(
        static_cast<int64_t>(42)
    );
    State channel_state(
        std::vector<Digest>{
            {.key = 7, .data_type = ::x::telem::INT64_T, .index = 0, .seed = seed}
        }
    );

    auto [ms, ok] = channel_state.read_value(7);
    ASSERT_TRUE(ok);
    ASSERT_FALSE(ms.series.empty());
    EXPECT_EQ(ms.series.back().at<int64_t>(-1), 42);
}

/// @brief a variable `on` node emits its seeded value on the first cycle.
TEST(OnTest, VarSourceEmitsSeed) {
    types::Param output_param;
    output_param.name = ir::default_output_param;
    output_param.type.kind = types::Kind::I64;
    ...
    channel_config.name = "channel";
    channel_config.type.kind = types::Kind::U32;
    channel_config.value = static_cast<uint32_t>(50);
    ir_node.inputs.push_back(channel_config);
    ...
    runtime::state::Config cfg{
        .ir = ir,
        .channels = {
            {.key = 50, .data_type = ::x::telem::INT64_T, .index = 0, .seed = seed}
        }
    };
    runtime::state::State s(cfg, runtime::errors::noop_handler);
    ...
    EXPECT_TRUE(changed);
    auto checker = ASSERT_NIL_P(s.node("source"));
    EXPECT_EQ(checker.output(0)->at<int64_t>(-1), 7);
}

/// @brief reset() re-arms a variable `on` node so it re-emits its current value
/// on stage re-entry, unlike a plain channel read.
TEST(OnTest, VarSourceResetReEmitsCurrentValue) {
    types::Param output_param;
    output_param.name = ir::default_output_param;
    output_param.type.kind = types::Kind::I64;
...
/// @brief a variable `write` node loops its input back into the read buffer
/// rather than the flushed frame, making it readable by an `on` node.
TEST(WriteTest, VarSinkLoopsBackIntoReads) {
    types::Param upstream_output;
    upstream_output.name = ir::default_output_param;
    upstream_output.type.kind = types::Kind::I64;
    ...
    types::Param sink_input;
    sink_input.name = ir::default_input_param;
    sink_input.type.kind = types::Kind::I64;
    ...
    sink_node.key = "sink";
    sink_node.type = "write";
    sink_node.is_var = true;
    sink_node.inputs.push_back(sink_input);
    sink_node.outputs.push_back(sink_output);

    types::Param sink_channel;
    sink_channel.name = "channel";
    sink_channel.type.kind = types::Kind::U32;
    sink_channel.value = static_cast<uint32_t>(60);
    sink_node.inputs.push_back(sink_channel);

    types::Param reader_output;
    reader_output.name = ir::default_output_param;
    reader_output.type.kind = types::Kind::I64;

    ir::Node reader_node;
    reader_node.key = "reader";
    reader_node.type = "on";
    reader_node.is_var = true;
    reader_node.outputs.push_back(reader_output);

    types::Param reader_channel;
    reader_channel.name = "channel";
    reader_channel.type.kind = types::Kind::U32;
    reader_channel.value = static_cast<uint32_t>(60);
    reader_node.inputs.push_back(reader_channel);
    ...
    ir.nodes.push_back(upstream_node);
    ir.nodes.push_back(sink_node);
    ir.nodes.push_back(reader_node);
    ir.edges.push_back(edge);
    ...
    auto reader_state = ASSERT_NIL_P(s.node("reader"));
    auto reader = ASSERT_NIL_P(
        module.create(runtime::node::Config(ir, reader_node, std::move(reader_state)))
    );
    ...
    x::telem::Frame out;
    s.flush_into(out);
    EXPECT_FALSE(out.contains(60));

    ASSERT_NIL(reader->next(ctx));
    auto reader_checker = ASSERT_NIL_P(s.node("reader"));
    EXPECT_EQ(reader_checker.output(0)->at<int64_t>(-1), 123);
}
