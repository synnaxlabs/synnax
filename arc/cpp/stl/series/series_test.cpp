...
#define TEST_CMP(suffix, op_name, a, b, expected)                                      \
    TEST(SeriesModule, op_name##_##suffix) {                                           \
        ...
        auto rh = result[0].i32();                                                     \
        EXPECT_GT(rh, 0);                                                              \
        EXPECT_EQ(f.state->get(rh)->data_type(), x::telem::BOOL_T);                    \
...
std::string build_bitwise_wat(const std::string &suffix) {
    std::string wt = wasm_type_str(suffix);
    std::ostringstream w;
    w << "(module\n";
    w << "  (import \"series\" \"create_empty_" << suffix
    ...
    const std::vector<std::string> ops =
        {"element_band", "element_bor", "element_bxor", "element_shl", "element_shr"};
    for (const auto &op: ops) {
        w << "  (import \"series\" \"" << op << "_" << suffix << "\" (func $" << op
          << " (param i32 " << wt << ") (result i32)))\n";
    }
    ...
    const std::vector<std::string> sops =
        {"series_band", "series_bor", "series_bxor", "series_shl", "series_shr"};
    for (const auto &op: sops) {
        w << "  (import \"series\" \"" << op << "_" << suffix << "\" (func $" << op
          << " (param i32 i32) (result i32)))\n";
    }
    w << "  (import \"series\" \"bnot_" << suffix
      << "\" (func $bnot (param i32) (result i32)))\n";
    ...
    w << "  (func (export \"bnot\") (param i32) (result i32)\n";
    w << "    (call $bnot (local.get 0)))\n";
    w << ")";
    return w.str();
}

// The fixture series is {12, 3}: element tests read index 0 (12 op scalar) and
// reverse tests read index 1 (scalar op 3) so every result fits in an i8.
#define SETUP_BITWISE_FIXTURE(suffix)                                                  \
    Fixture f(build_bitwise_wat(#suffix));                                             \
    auto h_r = f.get("create_empty")                                                   \
                   .call(f.store, {wasmtime::Val(int32_t{2})})                         \
                   .unwrap();                                                          \
    auto h = h_r[0].i32();                                                             \
    (void) f.get("set_element")                                                        \
        .call(                                                                         \
            f.store,                                                                   \
            {wasmtime::Val(h), wasmtime::Val(int32_t{0}), make_wasm_val(#suffix, 12)}  \
        )                                                                              \
        .unwrap();                                                                     \
    (void) f.get("set_element")                                                        \
        .call(                                                                         \
            f.store,                                                                   \
            {wasmtime::Val(h), wasmtime::Val(int32_t{1}), make_wasm_val(#suffix, 3)}   \
        )                                                                              \
        .unwrap();

#define TEST_BITWISE_ELEMENT(suffix, op_name, scalar, expected, get_val)               \
    TEST(SeriesModule, op_name##_##suffix) {                                           \
        SETUP_BITWISE_FIXTURE(suffix)                                                  \
        auto result = f.get(#op_name)                                                  \
                          .call(                                                       \
                              f.store,                                                 \
                              {wasmtime::Val(h), make_wasm_val(#suffix, scalar)}       \
...
#define TEST_BITWISE_REVERSE(suffix, op_name, scalar, expected, get_val)               \
    TEST(SeriesModule, op_name##_##suffix) {                                           \
        SETUP_BITWISE_FIXTURE(suffix)                                                  \
        auto result = f.get(#op_name)                                                  \
                          .call(                                                       \
...
#define SETUP_TWO_BITWISE(suffix, a0, a1, b0, b1)                                      \
    Fixture f(build_bitwise_wat(#suffix));                                             \
    auto ha_r = f.get("create_empty")                                                  \
                    .call(f.store, {wasmtime::Val(int32_t{2})})                        \
                    .unwrap();                                                         \
...
#define TEST_BITWISE_SERIES(suffix, op_name, a0, a1, b0, b1, exp0, get_val)            \
    TEST(SeriesModule, op_name##_##suffix) {                                           \
        SETUP_TWO_BITWISE(suffix, a0, a1, b0, b1)                                      \
        auto result = f.get(#op_name)                                                  \
                          .call(f.store, {wasmtime::Val(ha), wasmtime::Val(hb)})       \
                          .unwrap();                                                   \
        auto rh = result[0].i32();                                                     \
...
#define TEST_BNOT(suffix, val_in, expected, get_val)                                   \
    TEST(SeriesModule, Bnot_##suffix) {                                                \
        SETUP_BITWISE_FIXTURE(suffix)                                                  \
        (void) f.get("set_element")                                                    \
            .call(                                                                     \
                f.store,                                                               \
                {wasmtime::Val(h),                                                     \
                 wasmtime::Val(int32_t{0}),                                            \
                 make_wasm_val(#suffix, val_in)}                                       \
            )                                                                          \
            .unwrap();                                                                 \
        auto result = f.get("bnot").call(f.store, {wasmtime::Val(h)}).unwrap();        \
        auto rh = result[0].i32();                                                     \
        EXPECT_GT(rh, 0);                                                              \
        auto v = f.get("index")                                                        \
                     .call(f.store, {wasmtime::Val(rh), wasmtime::Val(int32_t{0})})    \
...
#define TEST_BITWISE_NULLS(suffix)                                                     \
    TEST(SeriesModule, BitwiseNullHandles_##suffix) {                                  \
        Fixture f(build_bitwise_wat(#suffix));                                         \
        auto null_h = wasmtime::Val(int32_t{999});                                     \
        auto scalar = make_wasm_val(#suffix, 1);                                       \
        EXPECT_EQ(                                                                     \
            f.get("element_band").call(f.store, {null_h, scalar}).unwrap()[0].i32(),   \
            0                                                                          \
        );                                                                             \
        EXPECT_EQ(                                                                     \
            f.get("element_rshl").call(f.store, {scalar, null_h}).unwrap()[0].i32(),   \
            0                                                                          \
        );                                                                             \
        EXPECT_EQ(                                                                     \
            f.get("series_band")                                                       \
                .call(f.store, {null_h, wasmtime::Val(int32_t{998})})                  \
                .unwrap()[0]                                                           \
                .i32(),                                                                \
            0                                                                          \
        );                                                                             \
        EXPECT_EQ(f.get("bnot").call(f.store, {null_h}).unwrap()[0].i32(), 0);         \
    }

#define ALL_BITWISE_TESTS(suffix, get_val)                                             \
    TEST_BITWISE_ELEMENT(suffix, element_band, 8, 8, get_val)                          \
    TEST_BITWISE_ELEMENT(suffix, element_bor, 1, 13, get_val)                          \
    TEST_BITWISE_ELEMENT(suffix, element_bxor, 15, 3, get_val)                         \
    TEST_BITWISE_ELEMENT(suffix, element_shl, 2, 48, get_val)                          \
    TEST_BITWISE_ELEMENT(suffix, element_shr, 2, 3, get_val)                           \
    TEST_BITWISE_REVERSE(suffix, element_rband, 10, 2, get_val)                        \
    TEST_BITWISE_REVERSE(suffix, element_rbor, 8, 11, get_val)                         \
    TEST_BITWISE_REVERSE(suffix, element_rbxor, 5, 6, get_val)                         \
    TEST_BITWISE_REVERSE(suffix, element_rshl, 2, 16, get_val)                         \
    TEST_BITWISE_REVERSE(suffix, element_rshr, 64, 8, get_val)                         \
    TEST_BITWISE_SERIES(suffix, series_band, 12, 3, 10, 2, 8, get_val)                 \
    TEST_BITWISE_SERIES(suffix, series_bor, 12, 3, 10, 2, 14, get_val)                 \
    TEST_BITWISE_SERIES(suffix, series_bxor, 12, 3, 10, 2, 6, get_val)                 \
    TEST_BITWISE_SERIES(suffix, series_shl, 3, 1, 4, 2, 48, get_val)                   \
    TEST_BITWISE_SERIES(suffix, series_shr, 64, 8, 3, 2, 8, get_val)                   \
    TEST_BITWISE_NULLS(suffix)

ALL_BITWISE_TESTS(u8, i32)
ALL_BITWISE_TESTS(u16, i32)
ALL_BITWISE_TESTS(u32, i32)
ALL_BITWISE_TESTS(u64, i64)
ALL_BITWISE_TESTS(i8, i32)
ALL_BITWISE_TESTS(i16, i32)
ALL_BITWISE_TESTS(i32, i32)
ALL_BITWISE_TESTS(i64, i64)

TEST_BNOT(i8, 12, -13, i32)
TEST_BNOT(i16, 12, -13, i32)
TEST_BNOT(i32, 12, -13, i32)
TEST_BNOT(i64, 12, -13, i64)
TEST_BNOT(u8, 15, 240, i32)
TEST_BNOT(u16, 15, 65520, i32)
TEST_BNOT(u32, 15, -16, i32)
TEST_BNOT(u64, 15, -16, i64)

#undef ALL_BITWISE_TESTS
#undef TEST_BITWISE_NULLS
#undef TEST_BNOT
#undef TEST_BITWISE_SERIES
#undef SETUP_TWO_BITWISE
#undef TEST_BITWISE_REVERSE
#undef TEST_BITWISE_ELEMENT
#undef SETUP_BITWISE_FIXTURE
