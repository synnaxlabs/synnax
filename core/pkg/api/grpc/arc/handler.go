...
func (t createRequestTranslator) Forward(
	...
	for i, arc := range msg.Arcs {
		translated, err := translateForward(arc, i)
		if err != nil {
			return nil, err
		}
		arcs[i] = translated
...
func (t createRequestTranslator) Backward(
	...
	for i, arc := range msg.Arcs {
		translated, err := translateBackward(arc, i)
		if err != nil {
...
func translateForward(msg apiarc.Arc, _ int) (*gapi.Arc, error) {
...
func translateBackward(msg *gapi.Arc, _ int) (apiarc.Arc, error) {
	var key uuid.UUID
	if msg.Key != "" {
