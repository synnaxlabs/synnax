...
func embeddedGuard(_ gorp.Context, r Rack) error {
	...
	return errors.Wrapf(validate.Err, "cannot delete embedded rack")
}
...
func (svc *RackService) Delete(
	...
	return res, svc.WithTx(ctx, func(tx gorp.Tx) error {
		...
		if exists {
			return errors.Wrapf(validate.Err, "cannot delete rack when devices are still attached")
		}
		...
		if exists {
			return errors.Wrapf(validate.Err, "cannot delete rack when tasks are still attached")
		}
		w := svc.rack.NewWriter(tx)
