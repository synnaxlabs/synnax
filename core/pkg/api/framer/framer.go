...
type WriterConfig struct {
	...
	// AutoIndexing causes the server to generate timestamps for any index channel in
	// the writer's Keys that is not provided in a given Write frame. The first sample
	// in each Write call is stamped with the gateway Core's current time, and the
