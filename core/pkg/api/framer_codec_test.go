...
var _ = Describe("FramerCodec", Ordered, func() {
	...
	Describe("Frame Write Request", func() {
		It("Should encode and decode single channel int32", func() {
			...
			msg := http.WSMessage[api.FrameWriterRequest]{Type: "data", Payload: req}
			encoded := MustSucceed(v.Encode(ctx, msg))
			Expect(encoded[0]).To(Equal(uint8(255)))
			var resMsg http.WSMessage[api.FrameWriterRequest]
		...
		It("Should encode and decode open message", func() {
			...
			msg := http.WSMessage[api.FrameWriterRequest]{Type: http.WSMessageTypeOpen}
			encoded := MustSucceed(v.Encode(ctx, msg))
			var resMsg http.WSMessage[api.FrameWriterRequest]
			Expect(v.Decode(ctx, encoded, &resMsg)).To(Succeed())
		...
		It("Should encode and decode close message", func() {
			...
			msg := http.WSMessage[api.FrameWriterRequest]{Type: http.WSMessageTypeClose}
			encoded := MustSucceed(v.Encode(ctx, msg))
			var resMsg http.WSMessage[api.FrameWriterRequest]
			Expect(v.Decode(ctx, encoded, &resMsg)).To(Succeed())
	...
	Describe("Frame Writer Response", func() {
		It("Should encode and decode response", func() {
			...
			msg := http.WSMessage[api.FrameWriterResponse]{Type: http.WSMessageTypeData, Payload: res}
			encoded := MustSucceed(v.Encode(ctx, msg))
			var resMsg http.WSMessage[api.FrameWriterResponse]
			Expect(v.Decode(ctx, encoded, &resMsg)).To(Succeed())
			Expect(resMsg.Type).To(Equal(http.WSMessageTypeData))
			Expect(resMsg.Payload.Command).To(Equal(writer.CommandWrite))
	...
	Describe("Frame Streamer Request", func() {
		It("Should encode and decode request", func() {
			...
			msg := http.WSMessage[api.FrameStreamerRequest]{Type: "data", Payload: req}
			encoded := MustSucceed(v.Encode(ctx, msg))
			var resMsg http.WSMessage[api.FrameStreamerRequest]
			Expect(v.Decode(ctx, encoded, &resMsg)).To(Succeed())
			Expect(resMsg.Type).To(Equal(http.WSMessageTypeData))
	...
	Describe("Frame Stream Response", func() {
		It("Should encode and decode single channel int32", func() {
			...
			msg := http.WSMessage[api.FrameStreamerResponse]{Type: "data", Payload: res}
			encoded := MustSucceed(v.Encode(ctx, msg))
			Expect(encoded[0]).To(Equal(uint8(255)))
			var resMsg http.WSMessage[api.FrameStreamerResponse]
			Expect(v.Decode(ctx, encoded, &resMsg)).To(Succeed())
			Expect(resMsg.Type).To(Equal(http.WSMessageTypeData))
			Expect(resMsg.Payload.Frame.KeysSlice()).To(Equal([]channel.Key{1}))
			Expect(resMsg.Payload.Frame.Count()).To(Equal(1))
		...
		It("Should encode and decode open message", func() {
			...
			msg := http.WSMessage[api.FrameStreamerResponse]{Type: http.WSMessageTypeOpen}
			encoded := MustSucceed(v.Encode(ctx, msg))
			var resMsg http.WSMessage[api.FrameStreamerResponse]
			Expect(v.Decode(ctx, encoded, &resMsg)).To(Succeed())
			Expect(resMsg.Type).To(Equal(http.WSMessageTypeOpen))
		...
		It("Should encode and decode close message", func() {
			...
			msg := http.WSMessage[api.FrameStreamerResponse]{Type: http.WSMessageTypeClose}
			encoded := MustSucceed(v.Encode(ctx, msg))
			var resMsg http.WSMessage[api.FrameStreamerResponse]
			Expect(v.Decode(ctx, encoded, &resMsg)).To(Succeed())
			Expect(resMsg.Type).To(Equal(http.WSMessageTypeClose))
