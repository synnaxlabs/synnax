...
func (s *Service) AliasResolve(
	...
	if err := s.
		...
		WhereKeys(req.Range).
		Exec(ctx, nil); err != nil {
		return AliasResolveResponse{}, err
	}
	...
	return AliasResolveResponse{Aliases: aliases}, nil
}
...
func (s *Service) AliasList(
	ctx context.Context,
	req AliasListRequest,
	...
	return AliasListResponse{Aliases: aliases}, nil
}

type (
	AliasRetrieveRequest struct {
		Range    uuid.UUID     `json:"range" msgpack:"range"`
		Channels []channel.Key `json:"channels" msgpack:"channels"`
	}
	AliasRetrieveResponse struct {
		Aliases map[channel.Key]string `json:"aliases" msgpack:"aliases"`
	}
)

func (s *Service) AliasRetrieve(
	ctx context.Context,
	req AliasRetrieveRequest,
	...
	if err := s.access.Enforce(ctx, access.Request{
		...
		return AliasRetrieveResponse{}, err
	}
	var r ranger.Range
	if err := s.
		internal.
	...
	return AliasRetrieveResponse{Aliases: aliases}, nil
}
