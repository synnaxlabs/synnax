...
var _ = Describe("Writer", func() {
	...
	Describe("Register", func() {
		...
		It(
			"Should return a RepeatedUsername error when the username is already registered",
			func(ctx SpecContext) {
				Expect(
					errors.Is(writer.Register(ctx, creds), auth.ErrRepeatedUsername),
				).To(BeTrue())
			},
		)
		It(
		...
		It(
			"Should reject an empty username before touching the store",
			func(ctx SpecContext) {
	...
	Describe("UpdateUsername", func() {
		It("Should rename the credential entry", func(ctx SpecContext) {
			...
			Expect(
				svc.Authenticate(ctx, nil, creds),
			).To(MatchError(auth.ErrInvalidCredentials))
		})
		It(
			"Should do nothing when the new username equals the old",
			func(ctx SpecContext) {
				Expect(
					writer.UpdateUsername(ctx, creds.Username, creds.Username),
				).To(Succeed())
				Expect(svc.Authenticate(ctx, nil, creds)).To(Succeed())
			},
		)
	...
	Describe("Deactivate", func() {
		...
		It("Should delete multiple credentials", func(ctx SpecContext) {
			...
			Expect(
				svc.Authenticate(ctx, nil, creds2),
			).To(MatchError(auth.ErrInvalidCredentials))
		})
	})
})
