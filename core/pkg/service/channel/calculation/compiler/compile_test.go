...
var _ = Describe("Compile", func() {
	It("Should compile simple expression", func(ctx SpecContext) {
		...
		Expect(mod.State.Reads.Slice()).To(ContainElement(base.Key()))
		Expect(mod.State.Writes.Slice()).To(ContainElement(calc.Key()))
	})
	...
	It("Should compile with multiple dependencies", func(ctx SpecContext) {
		...
		Expect(mod.State.Reads.Slice()).To(ContainElements(channel.KeysFromChannels(channels)))
		Expect(mod.State.Writes.Slice()).To(ContainElement(calc.Key()))
	})
