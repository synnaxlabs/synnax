...
var _ = Describe("Graph", func() {
	Describe("Open / Hydration", func() {
		It(
			"Should open successfully with no calculated channels",
		...
		It(
			"Should open with a valid calculated channel and set no status",
			func(ctx SpecContext) {
		...
		It(
			"Should set error status for an unresolvable reference",
			func(ctx SpecContext) {
				base := channel.Channel{
		...
		It(
			"Should handle a mix of valid and invalid calculated channels",
			func(ctx SpecContext) {
				base := channel.Channel{
					Name:     "hy_mix_base",
					DataType: telem.Int64T,
		...
		It(
			"Should handle an orphan calculated channel with no dependencies",
			func(ctx SpecContext) {
				calc := channel.Channel{
		...
		Context("DataType Repair", func() {
			It(
				"Should not repair when DataType already matches",
				func(ctx SpecContext) {
					base := channel.Channel{
						Name:     "hy_norep_base",
						DataType: telem.Int64T,
					...
					openGraph(ctx)
					Expect(
						retrieveChannelDataType(ctx, calc.Key()),
					).To(Equal(telem.Int64T))
				},
			)

			It(
				"Should repair a stale DataType during hydration",
				func(ctx SpecContext) {
					base := channel.Channel{
						Name:     "hy_rep_base",
						DataType: telem.Int64T,
			...
			It(
				"Should repair cascaded DataType when a dependent has a lower key than its dependency",
				func(ctx SpecContext) {
					base := channel.Channel{
						Name:     "hy_ooo_base",
						DataType: telem.Int64T,
					...
					Expect(
						retrieveChannelDataType(ctx, calc1.Key()),
					).To(Equal(telem.Float32T))
					Expect(
						retrieveChannelDataType(ctx, calc2.Key()),
					).To(Equal(telem.Float32T))
	...
	Describe("Reactive Change Handling", func() {
		Context("Creating Channels", func() {
			...
			It(
				"Should set error status for a newly-invalidated calculated channel",
				func(ctx SpecContext) {
					openGraph(ctx)
			...
			It(
				"Should handle incrementally building a chain after graph open",
				func(ctx SpecContext) {
					openGraph(ctx)
					base := channel.Channel{
						Name:     "rc_chain_base",
			...
			It(
				"Should process a batch CreateMany in a single handleChanges call",
				func(ctx SpecContext) {
					openGraph(ctx)
					base := channel.Channel{
						Name:     "rc_batch_base",
					...
					calcs := []channel.Channel{
						{
							...
							DataType:   telem.Int64T,
							Virtual:    true,
							Expression: "return rc_batch_base + 1",
						},
						{
							Name:       "rc_batch_c2",
							DataType:   telem.Int64T,
							Virtual:    true,
							Expression: "return rc_batch_base * 2",
						},
						{
							Name:       "rc_batch_c3",
							DataType:   telem.Int64T,
		...
		Context("Deleting Channels", func() {
			It(
				"Should set error status when a base dependency is deleted",
				func(ctx SpecContext) {
					openGraph(ctx)
					base := channel.Channel{
						Name:     "rc_del_base",
			...
			It(
				"Should set error on downstream calc when intermediate calc is deleted",
				func(ctx SpecContext) {
					openGraph(ctx)
					base := channel.Channel{
						Name:     "rc_del_mid_base",
						DataType: telem.Int64T,
						Virtual:  true,
					}
					Expect(channelWriter.Create(ctx, &base)).To(Succeed())
			...
			It(
				"Should leave upstream unaffected when a leaf calc is deleted",
				func(ctx SpecContext) {
					openGraph(ctx)
					base := channel.Channel{
						Name:     "rc_del_leaf_base",
						DataType: telem.Int64T,
						Virtual:  true,
					}
					Expect(channelWriter.Create(ctx, &base)).To(Succeed())
			...
			It(
				"Should not cascade invalidity through reconcileQueued in a diamond",
				func(ctx SpecContext) {
					openGraph(ctx)
					base := channel.Channel{
						Name:     "rc_del_dia_base",
						DataType: telem.Int64T,
						Virtual:  true,
					}
					Expect(channelWriter.Create(ctx, &base)).To(Succeed())
		...
		Context("Updating Channels", func() {
			It(
				"Should update deps when expression changes to use a different base",
				func(ctx SpecContext) {
					openGraph(ctx)
					base1 := channel.Channel{
						Name:     "rc_upd_b1",
						DataType: telem.Int64T,
						Virtual:  true,
					}
					base2 := channel.Channel{
						Name:     "rc_upd_b2",
						DataType: telem.Int64T,
						Virtual:  true,
					}
			...
			It(
				"Should set error status when a dependency is removed",
				func(ctx SpecContext) {
					openGraph(ctx)
			...
			It(
				"Should clear error status when a broken dependency is restored",
				func(ctx SpecContext) {
					openGraph(ctx)
		...
		Context("Cascading Reconciliation", func() {
			It(
				"Should not cascade invalidity from reconcileQueued to further dependents",
				func(ctx SpecContext) {
					openGraph(ctx)
					base := channel.Channel{
						Name:     "rc_cas_base",
			...
			It(
				"Should re-inspect dependents when a calculated channel is updated",
				func(ctx SpecContext) {
					openGraph(ctx)
					base := channel.Channel{
						Name:     "rc_reins_base",
					...
					By(
						"Updating calc1 expression - calc2 should be re-inspected via BFS",
					)
		...
		Context("DataType Persistence", func() {
			It(
				"Should persist DataType changes to the DB when a dependency type changes",
				func(ctx SpecContext) {
					openGraph(ctx)
			...
			It(
				"Should persist cascaded DataType changes through a chain",
				func(ctx SpecContext) {
					openGraph(ctx)
					...
					By(
						"Updating calc1 to return a different type, which should cascade to calc2",
					)
		...
		Context("Unresolved Name Auto-Heal", func() {
			It(
				"Should auto-fix a broken calc when its deleted dependency is recreated",
				func(ctx SpecContext) {
					openGraph(ctx)
			...
			It(
				"Should auto-fix multiple calcs waiting on the same restored name",
				func(ctx SpecContext) {
					openGraph(ctx)
					createDep(ctx, "rc_unres_shared_dep")
					calc1 := channel.Channel{
						Name:       "rc_unres_multi_c1",
						DataType:   telem.Int64T,
						Virtual:    true,
						Expression: "return rc_unres_shared_dep + 1",
					}
					calc2 := channel.Channel{
						Name:       "rc_unres_multi_c2",
						DataType:   telem.Int64T,
						Virtual:    true,
			...
			It(
				"Should auto-fix a chain when a deleted base is recreated",
				func(ctx SpecContext) {
					openGraph(ctx)
					calc1 := createBrokenCalc(
						ctx,
						"rc_unres_chain_c1",
						"rc_unres_chain_base",
					)
	...
	Describe("Status Communication", func() {
		It(
			"Should set status with correct structure and details",
			func(ctx SpecContext) {
		...
		It(
			"Should clear status when a broken dependency is restored",
			func(ctx SpecContext) {
				openGraph(ctx)
		...
		It("Should overwrite status when the error changes", func(ctx SpecContext) {
			...
			By(
				"Restoring st_ow_a and deleting st_ow_b so the error becomes a different one",
			)
			createDep(ctx, "st_ow_a")
	...
	Describe("Lifecycle", func() {
		...
		It("Should fail to open with nil Channel", func(ctx SpecContext) {
			Expect(
				graph.Open(ctx, graph.Config{Status: statusSvc}),
			).Error().
				To(HaveOccurred())
		})
		...
		It("Should fail to open with nil Status", func(ctx SpecContext) {
			Expect(
				graph.Open(ctx, graph.Config{Channel: channelSvc}),
			).Error().
				To(HaveOccurred())
		})
	...
	Describe("Concurrency", func() {
		It("Should handle concurrent channel creation", func(ctx SpecContext) {
			...
			for i := range n {
				go func() {
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, &calcs[i]),
					).To(Succeed())
				}()
			}
			wg.Wait()
		...
		It(
			"Should produce a consistent state under concurrent create and delete",
			func(ctx SpecContext) {
				openGraph(ctx)
				base := channel.Channel{
					Name:     "cc_race_base",
				...
				go func() {
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, &newCalc),
					).To(Succeed())
				}()
				wg.Wait()
			},
	...
	Describe("Complex Dependency Topologies", Ordered, func() {
		...
		Context("Long Chain With Mid-Chain Deletion", func() {
			...
			It(
				"Should only error the immediate dependent of a deleted node",
				func(ctx SpecContext) {
					base := channel.Channel{
						Name:     "topo_lc_base",
						DataType: telem.Int64T,
					...
					By(
						"c4 is not re-inspected because invalidity does not cascade from c3",
					)
					eventuallyExpectNoStatus(ctx, c4.Key())
				},
			)
