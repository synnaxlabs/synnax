...
var _ = Describe("Graph", func() {
	...
	Describe("Open / Hydration", func() {
		...
		It("Should open successfully with no calculated channels", func(ctx SpecContext) {
			...
			Expect(channelSvc.NewWriterNoAnalysis(nil).CreateMany(ctx, &bases)).To(Succeed())
			openGraph(ctx)
		})
		...
		It("Should open with a valid calculated channel and set no status", func(ctx SpecContext) {
			...
			Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &base)).To(Succeed())
		...
		It("Should handle a mix of valid and invalid calculated channels", func(ctx SpecContext) {
			...
			Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &calcOk)).To(Succeed())
			calcBad := channel.Channel{
				Name: "hy_mix_bad", DataType: telem.Int64T, Virtual: true,
				Expression: "return hy_no_such_channel",
			}
			Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &calcBad)).To(Succeed())
			openGraph(ctx)
		...
		Context("Dependency Topologies", func() {
			It("Should hydrate a diamond dependency graph", func(ctx SpecContext) {
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &calcC)).To(Succeed())
			...
			It("Should hydrate a deep chain (4 levels)", func(ctx SpecContext) {
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &c1)).To(Succeed())
				c2 := channel.Channel{
					Name: "hy_deep_c2", DataType: telem.Int64T, Virtual: true,
					Expression: "return hy_deep_c1 + 1",
				}
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &c2)).To(Succeed())
				c3 := channel.Channel{
					Name: "hy_deep_c3", DataType: telem.Int64T, Virtual: true,
					Expression: "return hy_deep_c2 + 1",
				}
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &c3)).To(Succeed())
				c4 := channel.Channel{
					Name: "hy_deep_c4", DataType: telem.Int64T, Virtual: true,
					Expression: "return hy_deep_c3 + 1",
				}
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &c4)).To(Succeed())
				openGraph(ctx)
	...
	Describe("Reactive Change Handling", func() {
		...
		Context("Updating Channels", func() {
			It("Should update deps when expression changes to use a different base", func(ctx SpecContext) {
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &base1)).To(Succeed())
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &base2)).To(Succeed())
				calc := channel.Channel{
			...
			It("Should clear error status when a broken expression is fixed", func(ctx SpecContext) {
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &dep)).To(Succeed())
		...
		Context("Multiple Independent Subgraphs", func() {
			It("Should isolate failures to their own subgraph", func(ctx SpecContext) {
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &baseA)).To(Succeed())
				calcA := channel.Channel{
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &baseB)).To(Succeed())
				calcB := channel.Channel{
	...
	Describe("Concurrency", func() {
		It("Should handle concurrent channel creation", func(ctx SpecContext) {
			...
			for i := range n {
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &bases[i])).To(Succeed())
			}
			wg.Add(n)
			for i := range n {
				go func() {
					...
					Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &calcs[i])).To(Succeed())
				}()
			}
			wg.Wait()
			for i := range n {
		...
		It("Should produce a consistent state under concurrent create and delete", func(ctx SpecContext) {
			...
			go func() {
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &newCalc)).To(Succeed())
			}()
			wg.Wait()
		})
	...
	Describe("Complex Dependency Topologies", Ordered, func() {
		Context("Build and Tear Down a 3-Level Diamond", func() {
			...
			BeforeAll(func(ctx SpecContext) {
				...
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &mid1)).To(Succeed())
				mid2 = channel.Channel{
					Name: "topo_dia_m2", DataType: telem.Int64T, Virtual: true,
					Expression: "return topo_dia_b1 * 2",
				}
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &mid2)).To(Succeed())
				top = channel.Channel{
					Name: "topo_dia_top", DataType: telem.Int64T, Virtual: true,
					Expression: "return topo_dia_m1 + topo_dia_m2",
				}
				Expect(channelSvc.NewWriterNoAnalysis(nil).Create(ctx, &top)).To(Succeed())
			})
