...
var _ = Describe("Multi Node", Ordered, func() {
	...
	It(
		"Should make a channel created on node 1 retrievable from node 2",
	...
	It(
		"Should allocate a remote-leased channel on its leaseholder",
		func(ctx SpecContext) {
			ch := channel.Channel{
				Name:        "mn_remote",
				DataType:    telem.TimeStampT,
			...
			Expect(
				node1.Storage.TS.RetrieveChannel(ctx, ch.Key().StorageKey()),
			).Error().
				To(
	...
	It(
		"Should propagate a rename performed on the leaseholder to the other node",
		func(ctx SpecContext) {
			ch := channel.Channel{
				Name:        "mn_rename_old",
				DataType:    telem.TimeStampT,
	...
	It(
		"Should propagate a delete performed on the leaseholder to the other node",
		func(ctx SpecContext) {
			ch := channel.Channel{
				Name:        "mn_deleted",
				DataType:    telem.TimeStampT,
