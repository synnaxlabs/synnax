...
func OpenService(ctx context.Context, cfgs ...ServiceConfig) (s *Service, err error) {
	...
	if s.Read, err = common.OpenConfigService[ReadConfig, *ReadConfig](
		ctx, base, common.ConfigServiceConfig{Type: ontology.ResourceTypeLabjackRead},
	); !ok(err, s.Read) {
		return nil, err
	}
	if s.Write, err = common.OpenConfigService[WriteConfig, *WriteConfig](
		ctx, base, common.ConfigServiceConfig{Type: ontology.ResourceTypeLabjackWrite},
	); !ok(err, s.Write) {
		return nil, err
	}
	if s.Scan, err = common.OpenConfigService[ScanConfig, *ScanConfig](
		ctx, base, common.ConfigServiceConfig{Type: ontology.ResourceTypeLabjackScan},
	); !ok(err, s.Scan) {
		return nil, err
	}
	return s, nil
