...
var _ = Describe("Task", Ordered, func() {
	...
	newTextFactory := func(ctx context.Context, prof arc.Text) *runtime.Factory {
	...
	newTask := func(ctx context.Context, factory *runtime.Factory) driver.Task {
		svcTask := task.Task{
		...
		t := MustBeOk(MustSucceed2(factory.ConfigureTask(newContext(ctx), svcTask)))
		return t
	}
	...
	Describe("Factory.ConfigureTask", func() {
		It("Should return false for non-arc task types", func(ctx SpecContext) {
			...
			t, handled := MustSucceed2(factory.ConfigureTask(newContext(ctx), svcTask))
		...
		It("Should return error for invalid config", func(ctx SpecContext) {
			...
			task, ok, err := factory.ConfigureTask(newContext(ctx), svcTask)
		...
		It("Should return error when CompileProgram fails", func(ctx SpecContext) {
			...
			t, handled, err := factory.ConfigureTask(newContext(ctx), svcTask)
		...
		It("Should set error status when config is invalid", func(ctx SpecContext) {
			...
			_, _, err := factory.ConfigureTask(newContext(ctx), svcTask)
			Expect(err).To(HaveOccurred())
	...
	Describe("Task Lifecycle", func() {
		...
		It("Should return correct task key", func(ctx SpecContext) {
	...
	Describe("ConfigureTask Error Paths", func() {
		It("Should return error when graph has unknown node type", func(ctx SpecContext) {
			...
			_, ok, err := newGraphFactory(badNodeGraph).ConfigureTask(newContext(ctx), svcTask)
			Expect(ok).To(BeTrue())
	...
	Describe("Runtime Error Handling", func() {
		It("Should report WASM division by zero via status service", func(ctx SpecContext) {
			...
			t := MustBeOk(MustSucceed2(newTextFactory(ctx, prog).ConfigureTask(newContext(ctx), svcTask)))
			Expect(t.Exec(ctx, task.Command{Type: "start"})).To(Succeed())
			defer func() {
				Expect(t.Stop(false)).To(Succeed())
			}()
