...
var _ = Describe("Reducer", func() {
	...
	Describe("SetNode", func() {
		It("Should append the node when no node has the same key", func() {
			...
			out := MustSucceed(
				arc.Reduce(state, arc.NewSetNodeAction(arc.SetNodePayload{
					Node: gnode("n2", 1, 2),
				})),
			)
			Expect(
				out.Graph.Nodes,
			).To(Equal(graph.Nodes{gnode("n1", 0, 0), gnode("n2", 1, 2)}))
		})
	...
	Describe("SetNodeInputs", func() {
		It("Should merge the configuration into the existing entry", func() {
			...
			out := MustSucceed(
				arc.Reduce(state, arc.NewSetNodeInputsAction(arc.SetNodeInputsPayload{
					Key:    "n1",
			...
			Expect(
				out.Graph.Inputs["n1"],
			).To(Equal(msgpack.EncodedJSON{"gain": 1, "offset": 2}))
		})
		It(
			"Should overwrite fields present in both the existing and payload configs",
			func() {
				state := withGraph(graph.Nodes{gnode("n1", 0, 0)}, nil)
				state.Graph.Inputs = map[string]msgpack.EncodedJSON{"n1": {"gain": 1}}
				out := MustSucceed(
					arc.Reduce(
						state,
						arc.NewSetNodeInputsAction(arc.SetNodeInputsPayload{
							Key:    "n1",
		...
		It("Should write the configuration even when no node has the key", func() {
			...
			Expect(
				out.Graph.Inputs["ghost"],
			).To(Equal(msgpack.EncodedJSON{"offset": 2}))
		})
	})

	Describe("RemoveNode", func() {
		It(
			"Should remove the matching node, its config, and every edge connected to it",
			func() {
				state := withGraph(
					graph.Nodes{
						gnode("n1", 0, 0),
					...
					graph.Edges{
						gedge("e1", "n1", "out", "n2", "in"),
						gedge("e2", "n2", "out", "n3", "in"),
					},
				)
				...
				out := MustSucceed(
					arc.Reduce(state, arc.NewRemoveNodeAction(arc.RemoveNodePayload{
						Key: "n2",
					})),
				)
				Expect(
					out.Graph.Nodes,
				).To(Equal(graph.Nodes{gnode("n1", 0, 0), gnode("n3", 2, 2)}))
		...
		It("Should keep unrelated edges intact", func() {
			...
			Expect(
				out.Graph.Edges,
			).To(Equal(graph.Edges{gedge("e1", "n1", "out", "n3", "in")}))
		})
		It("Should be a no-op when the key does not match any node", func() {
			state := withGraph(
				graph.Nodes{gnode("n1", 0, 0)},
				graph.Edges{gedge("e1", "n1", "out", "n1", "in")},
			)
			out := MustSucceed(
				arc.Reduce(state, arc.NewRemoveNodeAction(arc.RemoveNodePayload{
					Key: "ghost",
	...
	Describe("AddEdge", func() {
		It("Should append an edge whose source and target are not yet present", func() {
			...
			out := MustSucceed(
				arc.Reduce(state, arc.NewAddEdgeAction(arc.AddEdgePayload{
		...
		It(
			"Should be a no-op when an edge with the same source and target exists",
			func() {
				state := withGraph(nil, graph.Edges{gedge("e1", "a", "o", "b", "i")})
				out := MustSucceed(
					arc.Reduce(state, arc.NewAddEdgeAction(arc.AddEdgePayload{
	...
	Describe("RemoveEdge", func() {
		It("Should remove the edge matching the key", func() {
			...
			out := MustSucceed(
				arc.Reduce(state, arc.NewRemoveEdgeAction(arc.RemoveEdgePayload{
					Key: "e1",
				})),
			)
			Expect(
				out.Graph.Edges,
			).To(Equal(graph.Edges{gedge("e2", "b", "o", "c", "i")}))
		})
	...
	Describe("ReconnectEdge", func() {
		It(
			"Should rewrite the endpoints of the edge with the key, preserving key and kind",
			func() {
				state := withGraph(nil, graph.Edges{gedge("e1", "a", "o", "b", "i")})
				out := MustSucceed(
					arc.Reduce(
						state,
						arc.NewReconnectEdgeAction(arc.ReconnectEdgePayload{
							Key:    "e1",
				...
				Expect(
					out.Graph.Edges,
				).To(Equal(graph.Edges{gedge("e1", "a", "o", "c", "i")}))
			},
		)
		It("Should be a no-op when no edge matches the key", func() {
			state := withGraph(nil, graph.Edges{gedge("e1", "a", "o", "b", "i")})
			out := MustSucceed(
				arc.Reduce(state, arc.NewReconnectEdgeAction(arc.ReconnectEdgePayload{
					Key:    "missing",
					Source: ir.Handle{Node: "a", Param: "o"},
					Target: ir.Handle{Node: "c", Param: "i"},
				})),
		...
		It("Should build a complete graph from an empty module", func() {
			actions := []arc.Action{
				...
				arc.NewAddEdgeAction(
					arc.AddEdgePayload{Edge: gedge("e1", "src", "out", "op", "in")},
				),
				arc.NewAddEdgeAction(
					arc.AddEdgePayload{Edge: gedge("e2", "op", "out", "sink", "in")},
				),
			}
	...
	Describe("Action envelope", func() {
		It("Should return state unchanged when Type is unrecognized", func() {
			state := withGraph(graph.Nodes{gnode("n1", 0, 0)}, nil)
			Expect(
				MustSucceed(arc.Reduce(state, arc.Action{Type: "totally_made_up"})),
			).To(Equal(state))
		})
		...
		It("Should dispatch on Type and ignore extra payload pointers", func() {
			state := withGraph(graph.Nodes{gnode("n1", 0, 0)}, nil)
			pos := arc.SetNodePositionPayload{
				Key:      "n1",
