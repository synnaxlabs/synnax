...
import (
	...
	v0 "github.com/synnaxlabs/synnax/pkg/service/group/versions/v0"
...
var _ = Describe("Group", func() {
	Describe("GorpKey", func() {
		It("Should return the group's key", func() {
			k := uuid.New()
			Expect(v0.Group{Key: k}.GorpKey()).To(Equal(k))
		})
	})
...
var _ = Describe("NormalizeKeys", func() {
	It(
		"Should lift a Group row stored under the pre-v0.54 key format",
		func(ctx SpecContext) {
			kvDB := memkv.New()
			db := DeferClose(gorp.Wrap(kvDB, gorp.WithCodec(msgpack.Codec)))
			e := v0.Group{Key: uuid.New(), Name: "Devices"}
			...
			table := MustOpen(gorp.OpenTable(ctx, gorp.TableConfig[v0.Key, v0.Group]{
				DB:         db,
				Migrations: []migrate.Migration{v0.NormalizeKeys},
			}))
			var res v0.Group
			Expect(table.NewRetrieve().
				Where(gorp.MatchKeys[v0.Key, v0.Group](e.GorpKey())).
