...
var _ = Describe("Driver", func() {
	...
	Describe("Open", func() {
		...
		It(
			"should set integrations on the rack from factory names",
		...
		It(
			"should update integrations on existing rack when reopened with different factories",
			func(ctx SpecContext) {
	...
	Describe("Task Management", func() {
		It("should configure on start command, not on create", func(ctx SpecContext) {
			...
			Consistently(
				func() bool { return configuredTask.Load() != nil },
			).Should(BeFalse())
		...
		It(
			...
			func(ctx SpecContext) {
				...
				writeCommand(
					ctx,
					task.Command{Task: t.Key, Type: "start", Key: "cmd-2"},
				)
				Consistently(
					func() int32 { return configCount.Load() },
				).Should(Equal(int32(1)))
				...
				writeCommand(
					ctx,
					task.Command{Task: t.Key, Type: "start", Key: "cmd-3"},
				)
				Eventually(
					func() int32 { return configCount.Load() },
		...
		It(
			"should not rebuild when only metadata changes between starts",
			func(ctx SpecContext) {
				var (
					configCount atomic.Int32
				...
				Eventually(
					func() int32 { return execCount.Load() },
				).Should(Equal(int32(1)))
		...
		It(
			"should ignore start commands for tasks on other racks",
			func(ctx SpecContext) {
				...
				Consistently(
					func() int32 { return configuredCount.Load() },
				).Should(Equal(countAfterOpen))
			},
		)

		It(
			"should stop the live instance when a started task moves to another rack",
			func(ctx SpecContext) {
				var (
					stopped         atomic.Bool
					...
					configureFunc: func(
						...
						return &mockTask{
							...
							stopFunc: func(sendStatus bool) error {
								...
								return statusSvc.NewWriter[task.StatusDetails](
									nil,
								).Set(ctx, &stat)
							},
						}, nil
		...
		It(
			"should not run the task when factory returns not handled",
			func(ctx SpecContext) {
				var (
				...
				Eventually(
					func() bool { return configureCalled.Load() },
				).Should(BeTrue())
		...
		It(
			"should handle factory configuration error gracefully",
			func(ctx SpecContext) {
				var (
		...
		It(
			"should acknowledge a start command whose deploy fails",
			func(ctx SpecContext) {
				...
				writeCommand(
					ctx,
					task.Command{Task: t.Key, Type: "start", Key: "cmd-ack"},
				)
		...
		It(
			"should continue processing new tasks after a configuration error",
			func(ctx SpecContext) {
				var (
					knownKeys   sync.Map
					configCount atomic.Int32
				...
				writeCommand(
					ctx,
					task.Command{Task: t1.Key, Type: "start", Key: "cmd-1"},
				)
				Eventually(
					func() int32 { return configCount.Load() },
				).Should(Equal(int32(1)))
				...
				writeCommand(
					ctx,
					task.Command{Task: t2.Key, Type: "start", Key: "cmd-2"},
				)
				Eventually(
					func() int32 { return configCount.Load() },
				).Should(Equal(int32(2)))
		...
		It(
			"should continue configuring new tasks after a configuration panic",
			func(ctx SpecContext) {
				var (
					knownKeys         sync.Map
				...
				Eventually(
					func() int32 { return configAttempts.Load() },
				).Should(Equal(int32(1)))
				...
				Eventually(
					func() bool { return healthyConfigured.Load() },
				).Should(BeTrue())
			},
		)

		It(
			"should handle task stop error gracefully during reconfiguration",
			func(ctx SpecContext) {
				var (
					stopCalled  atomic.Bool
					configCount atomic.Int32
	...
	Describe("Heartbeat", func() {
		It("should send periodic status updates", func(ctx SpecContext) {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(statusSvc.NewRetrieve[rack.StatusDetails]().
		...
		It("should stop heartbeat when driver is closed", func(ctx SpecContext) {
			...
			var statuses []rack.Status
			Expect(statusSvc.NewRetrieve[rack.StatusDetails]().
				Where(status.MatchKeys[rack.StatusDetails](statusKey)).
				Entries(&statuses).
				Exec(ctx, node.DB)).To(Succeed())
	...
	Describe("Command Processing", func() {
		It(
			"should handle malformed command JSON without crashing",
			func(ctx SpecContext) {
				...
				Consistently(
					func() bool { return execCalled.Load() },
		...
		It(
			"should execute the start command on the freshly deployed task",
			func(ctx SpecContext) {
				var (
					execCalled  atomic.Bool
		...
		It(
			"should route non-start commands to the live instance",
			func(ctx SpecContext) {
				var (
					receivedCmd atomic.Value
					taskKey     atomic.Value
				...
				Eventually(
					func() bool { return receivedCmd.Load() != nil },
					time.Second*2,
				).Should(BeTrue())
		...
		It(
			"should ignore non-start commands with no live instance",
			func(ctx SpecContext) {
				var execCalled atomic.Bool
				factory := &mockFactory{
					name: "test",
		...
		It(
			"should not crash the process when a task's Exec panics",
			func(ctx SpecContext) {
				var (
				...
				Eventually(
					func() bool { return healthyExecCalled.Load() },
					time.Second*2,
				).Should(BeTrue())

				writeCommand(
					ctx,
					task.Command{Task: t.Key, Type: "panic", Key: "cmd-panic"},
				)
				Eventually(
					func() bool { return panicExecCalled.Load() },
					time.Second*2,
				).Should(BeTrue())
				...
				writeCommand(
					ctx,
					task.Command{Task: t.Key, Type: "start", Key: "cmd-healthy"},
				)
				Eventually(
					func() bool { return healthyExecCalled.Load() },
					time.Second*2,
		...
		It(
			"should log warning for unsupported command without crashing",
			func(ctx SpecContext) {
				var (
					execCalled atomic.Bool
					taskKey    atomic.Value
				...
				writeCommand(
					ctx,
					task.Command{Task: t.Key, Type: "start", Key: "cmd-unsupported"},
				)
	...
	Describe("Parallelism", func() {
		It(
			"should configure existing tasks in parallel on startup",
			func(ctx SpecContext) {
				var (
					configCount  atomic.Int32
