...
var _ = Describe("Calculator", Ordered, func() {
	...
	open := func(
		...
		if indexes != nil {
			Expect(channelSvc.NewWriter(nil).CreateMany(ctx, indexes)).To(Succeed())
		}
		if bases != nil {
	...
	Describe("Group", func() {
		...
		It("Should aggregate ReadFrom keys from all calculators", func(ctx SpecContext) {
			idx := []channel.Channel{{Name: RandomName(), DataType: telem.TimeStampT, IsIndex: true}}
			b1 := []channel.Channel{{Name: RandomName(), DataType: telem.Int64T}}
			b2 := []channel.Channel{{Name: RandomName(), DataType: telem.Int64T, Virtual: true}}
			c1 := channel.Channel{
				Name:       RandomName(),
				DataType:   telem.Int64T,
				Virtual:    true,
	...
	Describe("Leading Literal Type Coercion", func() {
		...
		Specify("Stale f64 output type from old inference with f32 channel and leading literal", func(ctx SpecContext) {
			...
			Expect(channelSvc.NewWriter(nil).CreateMany(ctx, &base)).To(Succeed())
			Expect(channelSvc.NewWriter(nil).Create(ctx, &calc)).To(Succeed())
			mod := MustSucceed(compiler.Compile(ctx, compiler.Config{
				ChannelService: channelSvc,
				Channel:        calc,
