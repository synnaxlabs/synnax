		...
		Describe("Calculations", func() {
			...
			Describe("Nested Calculations", func() {
				...
				It("Should handle mixed calculated and concrete channels", func(ctx SpecContext) {
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, calcMixed),
					).To(Succeed())
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, calcMixedNested),
					).To(Succeed())
			...
			Describe("Three Domain Calculations", func() {
				...
				BeforeAll(func(ctx SpecContext) {
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, threeDomainIndexCh),
					).To(Succeed())
					threeDomainDataCh = &channel.Channel{
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, threeDomainDataCh),
					).To(Succeed())
				...
				It("Should correctly handle large gap between domains", func(ctx SpecContext) {
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, gapIndexCh),
					).To(Succeed())
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, gapDataCh),
					).To(Succeed())
				...
				It("Should correctly handle multiple calculations on same source across three domains", func(ctx SpecContext) {
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, calcDouble),
					).To(Succeed())
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, calcSquare),
					).To(Succeed())
					...
					Expect(
						channelSvc.NewWriter(nil).Create(ctx, calcPlusTen),
					).To(Succeed())
