...
var _ = Describe("MigrateLog", func() {
	...
	Describe("from testdata fixtures", func() {
		...
		It(
			"Should lift a legacy v0 body forward through the chain",
			func(ctx SpecContext) {
				out := MustSucceed(v3.MigrateLog(
	...
	Describe("storage integration", func() {
		It(
			...
			func(ctx SpecContext) {
				...
				MustSucceed(
					gorp.OpenTable(ctx, gorp.TableConfig[v2.Key, v2.Log]{DB: db}),
				)
				Expect(
					gorp.NewCreate[v2.Key, v2.Log]().Entry(&seed).Exec(ctx, db),
				).To(Succeed())
