...
var _ = Describe("Log", func() {
	Describe("GorpKey", func() {
		It("Should return the log's key", func() {
			k := uuid.New()
			Expect(v0.Log{Key: k}.GorpKey()).To(Equal(k))
		})
	})

	Describe("SetOptions", func() {
		It("Should return no options", func() {
			Expect(v0.Log{}.SetOptions()).To(BeNil())
		})
	})
})
...
var _ = Describe("NormalizeKeys", func() {
	It(
		"Should lift a Log row stored under the pre-v0.54 key format",
		func(ctx SpecContext) {
			kvDB := memkv.New()
			db := DeferClose(gorp.Wrap(kvDB, gorp.WithCodec(msgpack.Codec)))
			e := v0.Log{Key: uuid.New(), Name: "Events"}
			...
			table := MustOpen(gorp.OpenTable(ctx, gorp.TableConfig[v0.Key, v0.Log]{
				DB:         db,
				Migrations: []migrate.Migration{v0.NormalizeKeys},
			}))
			var res v0.Log
			Expect(table.NewRetrieve().
				Where(gorp.MatchKeys[v0.Key, v0.Log](e.GorpKey())).
