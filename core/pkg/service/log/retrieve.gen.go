...
type Filter func(r Retrieve) gorp.Filter[Key, Log]
...
func Match(
	f func(ctx gorp.Context, r Retrieve, e *Log) (bool, error),
) Filter {
	return func(r Retrieve) gorp.Filter[Key, Log] {
		return gorp.Match(func(ctx gorp.Context, e *Log) (bool, error) {
			return f(ctx, r, e)
		})
	}
}
...
func And(fs ...Filter) Filter {
	return func(r Retrieve) gorp.Filter[Key, Log] {
		inner := make([]gorp.Filter[Key, Log], len(fs))
		for i, f := range fs {
			inner[i] = f(r)
		}
