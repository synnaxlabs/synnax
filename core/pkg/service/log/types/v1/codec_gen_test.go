// Copyright 2026 Synnax Labs, Inc.
//
// Use of this software is governed by the Business Source License included in the file
// licenses/BSL.txt.
//
// As of the Change Date specified in that file, in accordance with the Business Source
// License, use of this software will be governed by the Apache License, Version 2.0,
// included in the file licenses/APL.txt.

// Code generated by oracle. DO NOT EDIT.

package v1_test

import (
	"github.com/google/uuid"
	"reflect"
	"testing"

	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"
	channelv0 "github.com/synnaxlabs/synnax/pkg/service/channel/types/v0"
	"github.com/synnaxlabs/synnax/pkg/service/log/types/v1"
	colorv0 "github.com/synnaxlabs/x/color/types/v0"
	"github.com/synnaxlabs/x/encoding/orc"
	notationv0 "github.com/synnaxlabs/x/notation/types/v0"
	telemv1 "github.com/synnaxlabs/x/telem/types/v1"
)

var _ = Describe("Codec", func() {
	Describe("ChannelEntry", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v1.ChannelEntry) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v1.ChannelEntry
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v1.ChannelEntry{
				Channel: channelv0.Key(2),
				Color: colorv0.Color{
					R: 4,
					G: 5,
					B: 6,
					A: 6.5,
				},
				Notation:  notationv0.Notation("standard"),
				Precision: 9,
				Alias:     "test_9",
				Timestamp: v1.TimestampConfig{
					Format: telemv1.TimestampFormat("ISO"),
					Tz:     telemv1.TimeZone("local"),
				},
			}),
			Entry("zero values", v1.ChannelEntry{
				Channel: channelv0.Key(0),
				Color: colorv0.Color{
					R: 0,
					G: 0,
					B: 0,
					A: 0,
				},
				Notation:  notationv0.Notation(""),
				Precision: 0,
				Alias:     "",
				Timestamp: v1.TimestampConfig{
					Format: telemv1.TimestampFormat(""),
					Tz:     telemv1.TimeZone(""),
				},
			}),
		)
	})
	Describe("Log", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v1.Log) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v1.Log
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v1.Log{
				Key:  uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
				Name: "test_2",
				Channels: []v1.ChannelEntry{
					{
						Channel: channelv0.Key(5),
						Color: colorv0.Color{
							R: 7,
							G: 8,
							B: 9,
							A: 9.5,
						},
						Notation:  notationv0.Notation("standard"),
						Precision: 12,
						Alias:     "test_12",
						Timestamp: v1.TimestampConfig{
							Format: telemv1.TimestampFormat("ISO"),
							Tz:     telemv1.TimeZone("local"),
						},
					},
				},
				TimestampPrecision:   17,
				HideChannelNames:     true,
				HideReceiptTimestamp: false,
			}),
			Entry("zero values", v1.Log{
				Key:                  uuid.Nil,
				Name:                 "",
				Channels:             nil,
				TimestampPrecision:   0,
				HideChannelNames:     false,
				HideReceiptTimestamp: false,
			}),
			Entry("empty collections", v1.Log{
				Key:                  uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
				Name:                 "test_2",
				Channels:             []v1.ChannelEntry{},
				TimestampPrecision:   5,
				HideChannelNames:     true,
				HideReceiptTimestamp: false,
			}),
		)
	})
	Describe("TimestampConfig", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v1.TimestampConfig) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v1.TimestampConfig
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v1.TimestampConfig{
				Format: telemv1.TimestampFormat("ISO"),
				Tz:     telemv1.TimeZone("local"),
			}),
			Entry("zero values", v1.TimestampConfig{
				Format: telemv1.TimestampFormat(""),
				Tz:     telemv1.TimeZone(""),
			}),
		)
	})
})

func BenchmarkEncodeDecodeChannelEntry(b *testing.B) {
	ce := v1.ChannelEntry{
		Channel: channelv0.Key(2),
		Color: colorv0.Color{
			R: 4,
			G: 5,
			B: 6,
			A: 6.5,
		},
		Notation:  notationv0.Notation("standard"),
		Precision: 9,
		Alias:     "test_9",
		Timestamp: v1.TimestampConfig{
			Format: telemv1.TimestampFormat("ISO"),
			Tz:     telemv1.TimeZone("local"),
		},
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for i := 0; i < b.N; i++ {
		w.Reset()
		if err := ce.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v1.ChannelEntry
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeLog(b *testing.B) {
	lv := v1.Log{
		Key:  uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
		Name: "test_2",
		Channels: []v1.ChannelEntry{
			{
				Channel: channelv0.Key(5),
				Color: colorv0.Color{
					R: 7,
					G: 8,
					B: 9,
					A: 9.5,
				},
				Notation:  notationv0.Notation("standard"),
				Precision: 12,
				Alias:     "test_12",
				Timestamp: v1.TimestampConfig{
					Format: telemv1.TimestampFormat("ISO"),
					Tz:     telemv1.TimeZone("local"),
				},
			},
		},
		TimestampPrecision:   17,
		HideChannelNames:     true,
		HideReceiptTimestamp: false,
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for i := 0; i < b.N; i++ {
		w.Reset()
		if err := lv.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v1.Log
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeTimestampConfig(b *testing.B) {
	tc := v1.TimestampConfig{
		Format: telemv1.TimestampFormat("ISO"),
		Tz:     telemv1.TimeZone("local"),
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for i := 0; i < b.N; i++ {
		w.Reset()
		if err := tc.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v1.TimestampConfig
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func FuzzDecodeChannelEntry(f *testing.F) {
	{
		seed := v1.ChannelEntry{
			Channel: channelv0.Key(2),
			Color: colorv0.Color{
				R: 4,
				G: 5,
				B: 6,
				A: 6.5,
			},
			Notation:  notationv0.Notation("standard"),
			Precision: 9,
			Alias:     "test_9",
			Timestamp: v1.TimestampConfig{
				Format: telemv1.TimestampFormat("ISO"),
				Tz:     telemv1.TimeZone("local"),
			},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v1.ChannelEntry{
			Channel: channelv0.Key(0),
			Color: colorv0.Color{
				R: 0,
				G: 0,
				B: 0,
				A: 0,
			},
			Notation:  notationv0.Notation(""),
			Precision: 0,
			Alias:     "",
			Timestamp: v1.TimestampConfig{
				Format: telemv1.TimestampFormat(""),
				Tz:     telemv1.TimeZone(""),
			},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v1.ChannelEntry
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v1.ChannelEntry
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		w2 := orc.NewWriter(w1.Len())
		if err := redecoded.EncodeOrc(w2); err != nil {
			t.Fatalf("re-encode failed: %v", err)
		}
		if w1.Len() != w2.Len() {
			t.Fatalf("encoded length differs between cycles: w1=%d w2=%d", w1.Len(), w2.Len())
		}
		if !reflect.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded values differ after re-encode/re-decode cycle")
		}
	})
}

func FuzzDecodeLog(f *testing.F) {
	{
		seed := v1.Log{
			Key:  uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
			Name: "test_2",
			Channels: []v1.ChannelEntry{
				{
					Channel: channelv0.Key(5),
					Color: colorv0.Color{
						R: 7,
						G: 8,
						B: 9,
						A: 9.5,
					},
					Notation:  notationv0.Notation("standard"),
					Precision: 12,
					Alias:     "test_12",
					Timestamp: v1.TimestampConfig{
						Format: telemv1.TimestampFormat("ISO"),
						Tz:     telemv1.TimeZone("local"),
					},
				},
			},
			TimestampPrecision:   17,
			HideChannelNames:     true,
			HideReceiptTimestamp: false,
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v1.Log{
			Key:                  uuid.Nil,
			Name:                 "",
			Channels:             nil,
			TimestampPrecision:   0,
			HideChannelNames:     false,
			HideReceiptTimestamp: false,
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v1.Log{
			Key:                  uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
			Name:                 "test_2",
			Channels:             []v1.ChannelEntry{},
			TimestampPrecision:   5,
			HideChannelNames:     true,
			HideReceiptTimestamp: false,
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v1.Log
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v1.Log
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		w2 := orc.NewWriter(w1.Len())
		if err := redecoded.EncodeOrc(w2); err != nil {
			t.Fatalf("re-encode failed: %v", err)
		}
		if w1.Len() != w2.Len() {
			t.Fatalf("encoded length differs between cycles: w1=%d w2=%d", w1.Len(), w2.Len())
		}
		if !reflect.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded values differ after re-encode/re-decode cycle")
		}
	})
}

func FuzzDecodeTimestampConfig(f *testing.F) {
	{
		seed := v1.TimestampConfig{
			Format: telemv1.TimestampFormat("ISO"),
			Tz:     telemv1.TimeZone("local"),
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v1.TimestampConfig{
			Format: telemv1.TimestampFormat(""),
			Tz:     telemv1.TimeZone(""),
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v1.TimestampConfig
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v1.TimestampConfig
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		w2 := orc.NewWriter(w1.Len())
		if err := redecoded.EncodeOrc(w2); err != nil {
			t.Fatalf("re-encode failed: %v", err)
		}
		if w1.Len() != w2.Len() {
			t.Fatalf("encoded length differs between cycles: w1=%d w2=%d", w1.Len(), w2.Len())
		}
		if !reflect.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded values differ after re-encode/re-decode cycle")
		}
	})
}
