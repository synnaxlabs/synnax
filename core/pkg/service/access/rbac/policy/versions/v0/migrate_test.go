...
var _ = Describe("NormalizeKeys", func() {
	It(
		"Should lift a Policy row stored under the pre-v0.54 key format",
		func(ctx SpecContext) {
			kvDB := memkv.New()
			db := DeferClose(gorp.Wrap(kvDB, gorp.WithCodec(msgpack.Codec)))
			e := v0.Policy{Key: uuid.New()}
			...
			table := MustOpen(gorp.OpenTable(ctx, gorp.TableConfig[v0.Key, v0.Policy]{
				DB:         db,
				Migrations: []migrate.Migration{v0.NormalizeKeys},
			}))
			var res v0.Policy
			Expect(table.NewRetrieve().
				Where(gorp.MatchKeys[v0.Key, v0.Policy](e.GorpKey())).
