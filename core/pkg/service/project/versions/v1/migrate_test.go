...
var _ = Describe("Workspace to project migration", func() {
	...
	It(
		"Should lift workspaces, repoint the ontology, and rename the group",
		func(ctx SpecContext) {
			...
			Expect(
				groupTable.NewCreate().
					Entry(&group.Group{Key: groupKey, Name: "Workspaces"}).
					Exec(ctx, db),
			).To(Succeed())
			groupID := group.OntologyID(groupKey)
			wsAID := ontology.ID{
				Type: ontology.ResourceType("workspace"),
				Key:  wsA.String(),
			}
			wsBID := ontology.ID{
				Type: ontology.ResourceType("workspace"),
			...
			var pA v1.Project
			Expect(
				projectTable.NewRetrieve().
					Where(gorp.MatchKeys[v1.Key, v1.Project](wsA)).
					Entry(&pA).
					Exec(ctx, db),
			).To(Succeed())
			...
			Expect(
				projectTable.NewRetrieve().
					Where(gorp.MatchKeys[v1.Key, v1.Project](wsB)).
					Exists(ctx, db),
			).To(BeTrue())
			...
			Expect(
				wsTable.NewRetrieve().
					Where(gorp.MatchKeys[v0.Key, v0.Workspace](wsA, wsB)).
					Entries(&[]v0.Workspace{}).
					Exec(ctx, db),
			).To(MatchError(query.ErrNotFound))

			By("Re-keying the workspace resource nodes to project")
			projectAID := ontology.ID{
				Type: ontology.ResourceTypeProject,
				Key:  wsA.String(),
			}
			Expect(
				resTable.NewRetrieve().
					Where(gorp.MatchKeys[string, ontology.Resource](projectAID.String())).
					Exists(ctx, db),
			).To(BeTrue())
			Expect(
				resTable.NewRetrieve().
					Where(gorp.MatchKeys[string, ontology.Resource](wsAID.String())).
					Exists(ctx, db),
			).To(BeFalse())

			By("Repointing workspace relationships and leaving unrelated ones intact")
			projectBID := ontology.ID{
				Type: ontology.ResourceTypeProject,
				Key:  wsB.String(),
			}
			...
			var g group.Group
			Expect(
				groupTable.NewRetrieve().
					Where(gorp.MatchKeys[group.Key, group.Group](groupKey)).
					Entry(&g).
					Exec(ctx, db),
			).To(Succeed())
	...
	It(
		"Should be a no-op on a cluster that never created a workspace",
		func(ctx SpecContext) {
			db := DeferClose(gorp.Wrap(memkv.New()))
			runMigrations(ctx, db)
...
var _ = Describe("Remove author relationships migration", func() {
	...
	It(
		"Should delete user-to-project parent relationships and leave others intact",
		func(ctx SpecContext) {
			db := DeferClose(gorp.Wrap(memkv.New()))
			relTable := MustOpen(gorp.OpenTable(
				ctx, gorp.TableConfig[string, ontology.Relationship]{DB: db},
...
var _ = Describe("Project layout staging migration", func() {
	It(
		"Should stage each non-empty layout under its key and skip empty ones",
		func(ctx SpecContext) {
			db := DeferClose(gorp.Wrap(memkv.New()))
			...
			blob, closer := MustSucceed2(
				db.Get(ctx, []byte(v1.LegacyLayoutKVPrefix+withLayout.String())),
			)
