...
// Retrieve is used to retrieve Project records from the database using a
// builder pattern for constructing queries.
type Retrieve struct {
	baseTX     gorp.Tx
	gorp       gorp.Retrieve[Key, Project]
	search     *search.Index
	searchTerm string
}
...
type Filter = gorp.BoundFilter[Retrieve, Key, Project]
...
func Match(f func(ctx gorp.Context, r Retrieve, e *Project) (bool, error)) Filter {
	return gorp.MatchBound[Retrieve, Key, Project](f)
}

// And returns a filter that matches when all provided filters match.
func And(fs ...Filter) Filter {
	return gorp.AndBound[Retrieve, Key, Project](fs...)
}

// Or returns a filter that matches when any provided filter matches.
func Or(fs ...Filter) Filter {
	return gorp.OrBound[Retrieve, Key, Project](fs...)
}

// Not returns a filter that inverts the provided filter.
func Not(f Filter) Filter {
	return gorp.NotBound[Retrieve, Key, Project](f)
}
...
// Count returns the number of projects matching the query.
...
// Exists checks whether any projects match the query.
