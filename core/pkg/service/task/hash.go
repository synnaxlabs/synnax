...
// configContent returns config without the record key, so hashes track config content
// rather than record identity: equal configs on different tasks hash equally.
