...
type Filter func(r Retrieve) gorp.Filter[Key, Task]
...
func Match(
	f func(ctx gorp.Context, r Retrieve, e *Task) (bool, error),
) Filter {
	return func(r Retrieve) gorp.Filter[Key, Task] {
		return gorp.Match(func(ctx gorp.Context, e *Task) (bool, error) {
			return f(ctx, r, e)
		})
	}
}
...
func And(fs ...Filter) Filter {
	return func(r Retrieve) gorp.Filter[Key, Task] {
		inner := make([]gorp.Filter[Key, Task], len(fs))
		for i, f := range fs {
			inner[i] = f(r)
		}
...
func (r Retrieve) Where(filters ...Filter) Retrieve {
	bound := make([]gorp.Filter[Key, Task], len(filters))
