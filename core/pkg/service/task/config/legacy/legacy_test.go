...
var _ = Describe("Rewrite", func() {
	DescribeTable("era normalization",
		...
		Entry("splits an uppercase run entered from lowercase like the wire codec",
			legacy.Rewrite{},
			msgpack.EncodedJSON{"fooXY": float64(1)},
			msgpack.EncodedJSON{"foo_x_y": float64(1)},
		),
		Entry("keeps a leading uppercase run intact like the wire codec",
			legacy.Rewrite{},
			msgpack.EncodedJSON{"NS=1;ID=5": float64(1)},
			msgpack.EncodedJSON{"NS=1;ID=5": float64(1)},
		),
		Entry("prefers the snake_case spelling when both are present",
			legacy.Rewrite{},
			msgpack.EncodedJSON{"sampleRate": float64(1), "sample_rate": float64(2)},
			msgpack.EncodedJSON{"sample_rate": float64(2)},
		),
	)
