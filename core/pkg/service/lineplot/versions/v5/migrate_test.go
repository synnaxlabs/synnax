...
func migrateSeed(ctx SpecContext, seed v0.LinePlot) v5.LinePlot {
	...
	Expect(gorp.Migrate(ctx, gorp.MigrateConfig{
		DB:         db,
		Namespace:  "LinePlot",
		Migrations: []migrate.Migration{v0.Migration, v5.Migration},
	})).To(Succeed())
	var got v5.LinePlot
	Expect(gorp.NewRetrieve[v5.Key, v5.LinePlot]().
		Where(gorp.MatchKeys[v5.Key, v5.LinePlot](seed.Key)).
...
var _ = Describe("MigrateLinePlot", func() {
	...
	Describe("storage integration", func() {
		It(
			...
			func(ctx SpecContext) {
				key := uuid.New()
				got := migrateSeed(ctx, v0.LinePlot{
				...
				Expect(*got.Axes.X1.Type).To(Equal(v5.TickTypeTime))
		...
		It(
			...
			func(ctx SpecContext) {
				...
				Expect(*got.Axes.X2.Type).To(Equal(v5.TickTypeTime))
	...
	Describe("lift semantics", func() {
		...
		It(
			...
			func(ctx SpecContext) {
				...
				Expect(out.Lines[0].DownsampleMode).To(Equal(v5.DownsampleModeAverage))
			},
		)
		...
		It(
			...
			func(ctx SpecContext) {
				...
				Expect(out.Rules[0].Axis).To(Equal(v5.AxisKeyY1))
