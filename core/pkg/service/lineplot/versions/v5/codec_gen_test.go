// Copyright 2026 Synnax Labs, Inc.
//
// Use of this software is governed by the Business Source License included in the file
// licenses/BSL.txt.
//
// As of the Change Date specified in that file, in accordance with the Business Source
// License, use of this software will be governed by the Apache License, Version 2.0,
// included in the file licenses/APL.txt.

// Code generated by oracle. DO NOT EDIT.

package v5_test

import (
	"testing"
	"uuid"

	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"
	"github.com/synnaxlabs/synnax/pkg/service/channel"
	"github.com/synnaxlabs/synnax/pkg/service/lineplot/versions/v5"
	color "github.com/synnaxlabs/x/color/versions/v0"
	"github.com/synnaxlabs/x/encoding/orc"
	spatial "github.com/synnaxlabs/x/spatial/versions/v0"
	"github.com/synnaxlabs/x/testutil"
	text "github.com/synnaxlabs/x/text/versions/v0"
)

var _ = Describe("Codec", func() {
	Describe("Axes", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.Axes) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.Axes
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.Axes{
				X1: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_3",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 7.5, Upper: 8.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    12.5,
					Type:           new(v5.TickType("linear")),
				},
				X2: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_16",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 20.5, Upper: 21.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    25.5,
					Type:           new(v5.TickType("linear")),
				},
				Y1: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_29",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 33.5, Upper: 34.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    38.5,
					Type:           new(v5.TickType("linear")),
				},
				Y2: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_42",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 46.5, Upper: 47.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    51.5,
					Type:           new(v5.TickType("linear")),
				},
				Y3: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_55",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 59.5, Upper: 60.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    64.5,
					Type:           new(v5.TickType("linear")),
				},
				Y4: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_68",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 72.5, Upper: 73.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    77.5,
					Type:           new(v5.TickType("linear")),
				},
			}),
			Entry("zero values", v5.Axes{
				X1: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				X2: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				Y1: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				Y2: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				Y3: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				Y4: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
			}),
		)
	})
	Describe("Axis", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.Axis) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.Axis
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_2",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 6.5, Upper: 7.5},
				ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
				TickSpacing:    11.5,
				Type:           new(v5.TickType("linear")),
			}),
			Entry("zero values", v5.Axis{
				Key:            v5.AxisKey(""),
				Label:          "",
				LabelDirection: spatial.Direction(""),
				LabelLevel:     text.Level(""),
				Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
				TickSpacing:    0,
				Type:           nil,
			}),
		)
	})
	Describe("Channels", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.Channels) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.Channels
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.Channels{
				X1: channel.Key(2),
				X2: channel.Key(3),
				Y1: []channel.Key{channel.Key(4)},
				Y2: []channel.Key{channel.Key(5)},
				Y3: []channel.Key{channel.Key(6)},
				Y4: []channel.Key{channel.Key(7)},
			}),
			Entry("zero values", v5.Channels{
				X1: channel.Key(0),
				X2: channel.Key(0),
				Y1: []channel.Key{},
				Y2: []channel.Key{},
				Y3: []channel.Key{},
				Y4: []channel.Key{},
			}),
			Entry("empty collections", v5.Channels{
				X1: channel.Key(2),
				X2: channel.Key(3),
				Y1: []channel.Key{},
				Y2: []channel.Key{},
				Y3: []channel.Key{},
				Y4: []channel.Key{},
			}),
		)
	})
	Describe("Legend", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.Legend) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.Legend
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.Legend{
				Hidden: true,
				Position: spatial.StickyXY{
					X: 3.5,
					Y: 4.5,
					Root: spatial.CornerLocation{
						X: spatial.XLocation("left"),
						Y: spatial.YLocation("top"),
					},
					Units: spatial.StickyUnits{X: spatial.StickyUnit("px"), Y: spatial.StickyUnit("px")},
				},
			}),
			Entry("zero values", v5.Legend{
				Hidden: false,
				Position: spatial.StickyXY{
					X:     0,
					Y:     0,
					Root:  spatial.CornerLocation{X: spatial.XLocation(""), Y: spatial.YLocation("")},
					Units: spatial.StickyUnits{X: spatial.StickyUnit(""), Y: spatial.StickyUnit("")},
				},
			}),
		)
	})
	Describe("Line", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.Line) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.Line
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.Line{
				Key:   "test_1",
				Label: new(string("test_2")),
				Color: new(color.Color{
					R: 5,
					G: 6,
					B: 7,
					A: 7.5,
				}),
				StrokeWidth:    8.5,
				Downsample:     10,
				DownsampleMode: v5.DownsampleMode("average"),
			}),
			Entry("zero values", v5.Line{
				Key:            "",
				Label:          nil,
				Color:          nil,
				StrokeWidth:    0,
				Downsample:     0,
				DownsampleMode: v5.DownsampleMode(""),
			}),
		)
	})
	Describe("LinePlot", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.LinePlot) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.LinePlot
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.LinePlot{
				Key:   uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
				Name:  "test_2",
				Title: v5.Title{Level: text.Level("h1"), Visible: true},
				Legend: v5.Legend{
					Hidden: true,
					Position: spatial.StickyXY{
						X: 9.5,
						Y: 10.5,
						Root: spatial.CornerLocation{
							X: spatial.XLocation("left"),
							Y: spatial.YLocation("top"),
						},
						Units: spatial.StickyUnits{X: spatial.StickyUnit("px"), Y: spatial.StickyUnit("px")},
					},
				},
				Channels: v5.Channels{
					X1: channel.Key(19),
					X2: channel.Key(20),
					Y1: []channel.Key{channel.Key(21)},
					Y2: []channel.Key{channel.Key(22)},
					Y3: []channel.Key{channel.Key(23)},
					Y4: []channel.Key{channel.Key(24)},
				},
				Ranges: v5.Ranges{X1: []string{"test_25"}, X2: []string{"test_26"}},
				Axes: v5.Axes{
					X1: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_30",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 34.5, Upper: 35.5},
						ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
						TickSpacing:    39.5,
						Type:           new(v5.TickType("linear")),
					},
					X2: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_43",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 47.5, Upper: 48.5},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
						TickSpacing:    52.5,
						Type:           new(v5.TickType("linear")),
					},
					Y1: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_56",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 60.5, Upper: 61.5},
						ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
						TickSpacing:    65.5,
						Type:           new(v5.TickType("linear")),
					},
					Y2: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_69",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 73.5, Upper: 74.5},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
						TickSpacing:    78.5,
						Type:           new(v5.TickType("linear")),
					},
					Y3: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_82",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 86.5, Upper: 87.5},
						ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
						TickSpacing:    91.5,
						Type:           new(v5.TickType("linear")),
					},
					Y4: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_95",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 99.5, Upper: 100.5},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
						TickSpacing:    104.5,
						Type:           new(v5.TickType("linear")),
					},
				},
				Lines: []v5.Line{
					{
						Key:   "test_107",
						Label: new(string("test_108")),
						Color: new(color.Color{
							R: 111,
							G: 112,
							B: 113,
							A: 113.5,
						}),
						StrokeWidth:    114.5,
						Downsample:     116,
						DownsampleMode: v5.DownsampleMode("average"),
					},
				},
				Rules: []v5.Rule{
					{
						Key:   "test_118",
						Label: "test_119",
						Color: new(color.Color{
							R: 122,
							G: 123,
							B: 124,
							A: 124.5,
						}),
						Axis:      v5.AxisKey("x1"),
						LineWidth: 126.5,
						LineDash:  127.5,
						Units:     "test_128",
						Position:  129.5,
					},
				},
			}),
			Entry("zero values", v5.LinePlot{
				Key:   uuid.Nil(),
				Name:  "",
				Title: v5.Title{Level: text.Level(""), Visible: false},
				Legend: v5.Legend{
					Hidden: false,
					Position: spatial.StickyXY{
						X:     0,
						Y:     0,
						Root:  spatial.CornerLocation{X: spatial.XLocation(""), Y: spatial.YLocation("")},
						Units: spatial.StickyUnits{X: spatial.StickyUnit(""), Y: spatial.StickyUnit("")},
					},
				},
				Channels: v5.Channels{
					X1: channel.Key(0),
					X2: channel.Key(0),
					Y1: []channel.Key{},
					Y2: []channel.Key{},
					Y3: []channel.Key{},
					Y4: []channel.Key{},
				},
				Ranges: v5.Ranges{X1: []string{}, X2: []string{}},
				Axes: v5.Axes{
					X1: v5.Axis{
						Key:            v5.AxisKey(""),
						Label:          "",
						LabelDirection: spatial.Direction(""),
						LabelLevel:     text.Level(""),
						Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
						TickSpacing:    0,
						Type:           nil,
					},
					X2: v5.Axis{
						Key:            v5.AxisKey(""),
						Label:          "",
						LabelDirection: spatial.Direction(""),
						LabelLevel:     text.Level(""),
						Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
						TickSpacing:    0,
						Type:           nil,
					},
					Y1: v5.Axis{
						Key:            v5.AxisKey(""),
						Label:          "",
						LabelDirection: spatial.Direction(""),
						LabelLevel:     text.Level(""),
						Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
						TickSpacing:    0,
						Type:           nil,
					},
					Y2: v5.Axis{
						Key:            v5.AxisKey(""),
						Label:          "",
						LabelDirection: spatial.Direction(""),
						LabelLevel:     text.Level(""),
						Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
						TickSpacing:    0,
						Type:           nil,
					},
					Y3: v5.Axis{
						Key:            v5.AxisKey(""),
						Label:          "",
						LabelDirection: spatial.Direction(""),
						LabelLevel:     text.Level(""),
						Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
						TickSpacing:    0,
						Type:           nil,
					},
					Y4: v5.Axis{
						Key:            v5.AxisKey(""),
						Label:          "",
						LabelDirection: spatial.Direction(""),
						LabelLevel:     text.Level(""),
						Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
						TickSpacing:    0,
						Type:           nil,
					},
				},
				Lines: []v5.Line{},
				Rules: []v5.Rule{},
			}),
			Entry("empty collections", v5.LinePlot{
				Key:   uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
				Name:  "test_2",
				Title: v5.Title{Level: text.Level("h1"), Visible: true},
				Legend: v5.Legend{
					Hidden: true,
					Position: spatial.StickyXY{
						X: 9.5,
						Y: 10.5,
						Root: spatial.CornerLocation{
							X: spatial.XLocation("left"),
							Y: spatial.YLocation("top"),
						},
						Units: spatial.StickyUnits{X: spatial.StickyUnit("px"), Y: spatial.StickyUnit("px")},
					},
				},
				Channels: v5.Channels{
					X1: channel.Key(19),
					X2: channel.Key(20),
					Y1: []channel.Key{},
					Y2: []channel.Key{},
					Y3: []channel.Key{},
					Y4: []channel.Key{},
				},
				Ranges: v5.Ranges{X1: []string{}, X2: []string{}},
				Axes: v5.Axes{
					X1: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_30",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 34.5, Upper: 35.5},
						ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
						TickSpacing:    39.5,
						Type:           new(v5.TickType("linear")),
					},
					X2: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_43",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 47.5, Upper: 48.5},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
						TickSpacing:    52.5,
						Type:           new(v5.TickType("linear")),
					},
					Y1: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_56",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 60.5, Upper: 61.5},
						ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
						TickSpacing:    65.5,
						Type:           new(v5.TickType("linear")),
					},
					Y2: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_69",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 73.5, Upper: 74.5},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
						TickSpacing:    78.5,
						Type:           new(v5.TickType("linear")),
					},
					Y3: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_82",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 86.5, Upper: 87.5},
						ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
						TickSpacing:    91.5,
						Type:           new(v5.TickType("linear")),
					},
					Y4: v5.Axis{
						Key:            v5.AxisKey("x1"),
						Label:          "test_95",
						LabelDirection: spatial.Direction("x"),
						LabelLevel:     text.Level("h1"),
						Bounds:         spatial.Bounds{Lower: 99.5, Upper: 100.5},
						ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
						TickSpacing:    104.5,
						Type:           new(v5.TickType("linear")),
					},
				},
				Lines: []v5.Line{},
				Rules: []v5.Rule{},
			}),
		)
	})
	Describe("ManualBounds", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.ManualBounds) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.ManualBounds
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.ManualBounds{Lower: true, Upper: false}),
			Entry("zero values", v5.ManualBounds{Lower: false, Upper: false}),
		)
	})
	Describe("Ranges", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.Ranges) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.Ranges
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.Ranges{X1: []string{"test_1"}, X2: []string{"test_2"}}),
			Entry("zero values", v5.Ranges{X1: []string{}, X2: []string{}}),
			Entry("empty collections", v5.Ranges{X1: []string{}, X2: []string{}}),
		)
	})
	Describe("Rule", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.Rule) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.Rule
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.Rule{
				Key:   "test_1",
				Label: "test_2",
				Color: new(color.Color{
					R: 5,
					G: 6,
					B: 7,
					A: 7.5,
				}),
				Axis:      v5.AxisKey("x1"),
				LineWidth: 9.5,
				LineDash:  10.5,
				Units:     "test_11",
				Position:  12.5,
			}),
			Entry("zero values", v5.Rule{
				Key:       "",
				Label:     "",
				Color:     nil,
				Axis:      v5.AxisKey(""),
				LineWidth: 0,
				LineDash:  0,
				Units:     "",
				Position:  0,
			}),
		)
	})
	Describe("Title", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v5.Title) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v5.Title
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v5.Title{Level: text.Level("h1"), Visible: false}),
			Entry("zero values", v5.Title{Level: text.Level(""), Visible: false}),
		)
	})
})

func BenchmarkEncodeDecodeAxes(b *testing.B) {
	seed := v5.Axes{
		X1: v5.Axis{
			Key:            v5.AxisKey("x1"),
			Label:          "test_3",
			LabelDirection: spatial.Direction("x"),
			LabelLevel:     text.Level("h1"),
			Bounds:         spatial.Bounds{Lower: 7.5, Upper: 8.5},
			ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
			TickSpacing:    12.5,
			Type:           new(v5.TickType("linear")),
		},
		X2: v5.Axis{
			Key:            v5.AxisKey("x1"),
			Label:          "test_16",
			LabelDirection: spatial.Direction("x"),
			LabelLevel:     text.Level("h1"),
			Bounds:         spatial.Bounds{Lower: 20.5, Upper: 21.5},
			ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
			TickSpacing:    25.5,
			Type:           new(v5.TickType("linear")),
		},
		Y1: v5.Axis{
			Key:            v5.AxisKey("x1"),
			Label:          "test_29",
			LabelDirection: spatial.Direction("x"),
			LabelLevel:     text.Level("h1"),
			Bounds:         spatial.Bounds{Lower: 33.5, Upper: 34.5},
			ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
			TickSpacing:    38.5,
			Type:           new(v5.TickType("linear")),
		},
		Y2: v5.Axis{
			Key:            v5.AxisKey("x1"),
			Label:          "test_42",
			LabelDirection: spatial.Direction("x"),
			LabelLevel:     text.Level("h1"),
			Bounds:         spatial.Bounds{Lower: 46.5, Upper: 47.5},
			ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
			TickSpacing:    51.5,
			Type:           new(v5.TickType("linear")),
		},
		Y3: v5.Axis{
			Key:            v5.AxisKey("x1"),
			Label:          "test_55",
			LabelDirection: spatial.Direction("x"),
			LabelLevel:     text.Level("h1"),
			Bounds:         spatial.Bounds{Lower: 59.5, Upper: 60.5},
			ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
			TickSpacing:    64.5,
			Type:           new(v5.TickType("linear")),
		},
		Y4: v5.Axis{
			Key:            v5.AxisKey("x1"),
			Label:          "test_68",
			LabelDirection: spatial.Direction("x"),
			LabelLevel:     text.Level("h1"),
			Bounds:         spatial.Bounds{Lower: 72.5, Upper: 73.5},
			ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
			TickSpacing:    77.5,
			Type:           new(v5.TickType("linear")),
		},
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.Axes
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeAxis(b *testing.B) {
	seed := v5.Axis{
		Key:            v5.AxisKey("x1"),
		Label:          "test_2",
		LabelDirection: spatial.Direction("x"),
		LabelLevel:     text.Level("h1"),
		Bounds:         spatial.Bounds{Lower: 6.5, Upper: 7.5},
		ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
		TickSpacing:    11.5,
		Type:           new(v5.TickType("linear")),
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.Axis
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeChannels(b *testing.B) {
	seed := v5.Channels{
		X1: channel.Key(2),
		X2: channel.Key(3),
		Y1: []channel.Key{channel.Key(4)},
		Y2: []channel.Key{channel.Key(5)},
		Y3: []channel.Key{channel.Key(6)},
		Y4: []channel.Key{channel.Key(7)},
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.Channels
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeLegend(b *testing.B) {
	seed := v5.Legend{
		Hidden: true,
		Position: spatial.StickyXY{
			X: 3.5,
			Y: 4.5,
			Root: spatial.CornerLocation{
				X: spatial.XLocation("left"),
				Y: spatial.YLocation("top"),
			},
			Units: spatial.StickyUnits{X: spatial.StickyUnit("px"), Y: spatial.StickyUnit("px")},
		},
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.Legend
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeLine(b *testing.B) {
	seed := v5.Line{
		Key:   "test_1",
		Label: new(string("test_2")),
		Color: new(color.Color{
			R: 5,
			G: 6,
			B: 7,
			A: 7.5,
		}),
		StrokeWidth:    8.5,
		Downsample:     10,
		DownsampleMode: v5.DownsampleMode("average"),
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.Line
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeLinePlot(b *testing.B) {
	seed := v5.LinePlot{
		Key:   uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
		Name:  "test_2",
		Title: v5.Title{Level: text.Level("h1"), Visible: true},
		Legend: v5.Legend{
			Hidden: true,
			Position: spatial.StickyXY{
				X: 9.5,
				Y: 10.5,
				Root: spatial.CornerLocation{
					X: spatial.XLocation("left"),
					Y: spatial.YLocation("top"),
				},
				Units: spatial.StickyUnits{X: spatial.StickyUnit("px"), Y: spatial.StickyUnit("px")},
			},
		},
		Channels: v5.Channels{
			X1: channel.Key(19),
			X2: channel.Key(20),
			Y1: []channel.Key{channel.Key(21)},
			Y2: []channel.Key{channel.Key(22)},
			Y3: []channel.Key{channel.Key(23)},
			Y4: []channel.Key{channel.Key(24)},
		},
		Ranges: v5.Ranges{X1: []string{"test_25"}, X2: []string{"test_26"}},
		Axes: v5.Axes{
			X1: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_30",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 34.5, Upper: 35.5},
				ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
				TickSpacing:    39.5,
				Type:           new(v5.TickType("linear")),
			},
			X2: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_43",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 47.5, Upper: 48.5},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
				TickSpacing:    52.5,
				Type:           new(v5.TickType("linear")),
			},
			Y1: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_56",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 60.5, Upper: 61.5},
				ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
				TickSpacing:    65.5,
				Type:           new(v5.TickType("linear")),
			},
			Y2: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_69",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 73.5, Upper: 74.5},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
				TickSpacing:    78.5,
				Type:           new(v5.TickType("linear")),
			},
			Y3: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_82",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 86.5, Upper: 87.5},
				ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
				TickSpacing:    91.5,
				Type:           new(v5.TickType("linear")),
			},
			Y4: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_95",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 99.5, Upper: 100.5},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
				TickSpacing:    104.5,
				Type:           new(v5.TickType("linear")),
			},
		},
		Lines: []v5.Line{
			{
				Key:   "test_107",
				Label: new(string("test_108")),
				Color: new(color.Color{
					R: 111,
					G: 112,
					B: 113,
					A: 113.5,
				}),
				StrokeWidth:    114.5,
				Downsample:     116,
				DownsampleMode: v5.DownsampleMode("average"),
			},
		},
		Rules: []v5.Rule{
			{
				Key:   "test_118",
				Label: "test_119",
				Color: new(color.Color{
					R: 122,
					G: 123,
					B: 124,
					A: 124.5,
				}),
				Axis:      v5.AxisKey("x1"),
				LineWidth: 126.5,
				LineDash:  127.5,
				Units:     "test_128",
				Position:  129.5,
			},
		},
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.LinePlot
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeManualBounds(b *testing.B) {
	seed := v5.ManualBounds{Lower: true, Upper: false}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.ManualBounds
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeRanges(b *testing.B) {
	seed := v5.Ranges{X1: []string{"test_1"}, X2: []string{"test_2"}}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.Ranges
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeRule(b *testing.B) {
	seed := v5.Rule{
		Key:   "test_1",
		Label: "test_2",
		Color: new(color.Color{
			R: 5,
			G: 6,
			B: 7,
			A: 7.5,
		}),
		Axis:      v5.AxisKey("x1"),
		LineWidth: 9.5,
		LineDash:  10.5,
		Units:     "test_11",
		Position:  12.5,
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.Rule
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkEncodeDecodeTitle(b *testing.B) {
	seed := v5.Title{Level: text.Level("h1"), Visible: false}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
		if err := seed.EncodeOrc(w); err != nil {
			b.Fatal(err)
		}
		var decoded v5.Title
		r.ResetBytes(w.Bytes())
		if err := decoded.DecodeOrc(r); err != nil {
			b.Fatal(err)
		}
	}
}

func FuzzDecodeAxes(f *testing.F) {
	{
		seed := v5.Axes{
			X1: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_3",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 7.5, Upper: 8.5},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
				TickSpacing:    12.5,
				Type:           new(v5.TickType("linear")),
			},
			X2: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_16",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 20.5, Upper: 21.5},
				ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
				TickSpacing:    25.5,
				Type:           new(v5.TickType("linear")),
			},
			Y1: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_29",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 33.5, Upper: 34.5},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
				TickSpacing:    38.5,
				Type:           new(v5.TickType("linear")),
			},
			Y2: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_42",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 46.5, Upper: 47.5},
				ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
				TickSpacing:    51.5,
				Type:           new(v5.TickType("linear")),
			},
			Y3: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_55",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 59.5, Upper: 60.5},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
				TickSpacing:    64.5,
				Type:           new(v5.TickType("linear")),
			},
			Y4: v5.Axis{
				Key:            v5.AxisKey("x1"),
				Label:          "test_68",
				LabelDirection: spatial.Direction("x"),
				LabelLevel:     text.Level("h1"),
				Bounds:         spatial.Bounds{Lower: 72.5, Upper: 73.5},
				ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
				TickSpacing:    77.5,
				Type:           new(v5.TickType("linear")),
			},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Axes{
			X1: v5.Axis{
				Key:            v5.AxisKey(""),
				Label:          "",
				LabelDirection: spatial.Direction(""),
				LabelLevel:     text.Level(""),
				Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
				TickSpacing:    0,
				Type:           nil,
			},
			X2: v5.Axis{
				Key:            v5.AxisKey(""),
				Label:          "",
				LabelDirection: spatial.Direction(""),
				LabelLevel:     text.Level(""),
				Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
				TickSpacing:    0,
				Type:           nil,
			},
			Y1: v5.Axis{
				Key:            v5.AxisKey(""),
				Label:          "",
				LabelDirection: spatial.Direction(""),
				LabelLevel:     text.Level(""),
				Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
				TickSpacing:    0,
				Type:           nil,
			},
			Y2: v5.Axis{
				Key:            v5.AxisKey(""),
				Label:          "",
				LabelDirection: spatial.Direction(""),
				LabelLevel:     text.Level(""),
				Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
				TickSpacing:    0,
				Type:           nil,
			},
			Y3: v5.Axis{
				Key:            v5.AxisKey(""),
				Label:          "",
				LabelDirection: spatial.Direction(""),
				LabelLevel:     text.Level(""),
				Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
				TickSpacing:    0,
				Type:           nil,
			},
			Y4: v5.Axis{
				Key:            v5.AxisKey(""),
				Label:          "",
				LabelDirection: spatial.Direction(""),
				LabelLevel:     text.Level(""),
				Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
				ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
				TickSpacing:    0,
				Type:           nil,
			},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.Axes
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.Axes
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeAxis(f *testing.F) {
	{
		seed := v5.Axis{
			Key:            v5.AxisKey("x1"),
			Label:          "test_2",
			LabelDirection: spatial.Direction("x"),
			LabelLevel:     text.Level("h1"),
			Bounds:         spatial.Bounds{Lower: 6.5, Upper: 7.5},
			ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
			TickSpacing:    11.5,
			Type:           new(v5.TickType("linear")),
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Axis{
			Key:            v5.AxisKey(""),
			Label:          "",
			LabelDirection: spatial.Direction(""),
			LabelLevel:     text.Level(""),
			Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
			ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
			TickSpacing:    0,
			Type:           nil,
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.Axis
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.Axis
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeChannels(f *testing.F) {
	{
		seed := v5.Channels{
			X1: channel.Key(2),
			X2: channel.Key(3),
			Y1: []channel.Key{channel.Key(4)},
			Y2: []channel.Key{channel.Key(5)},
			Y3: []channel.Key{channel.Key(6)},
			Y4: []channel.Key{channel.Key(7)},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Channels{
			X1: channel.Key(0),
			X2: channel.Key(0),
			Y1: []channel.Key{},
			Y2: []channel.Key{},
			Y3: []channel.Key{},
			Y4: []channel.Key{},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Channels{
			X1: channel.Key(2),
			X2: channel.Key(3),
			Y1: []channel.Key{},
			Y2: []channel.Key{},
			Y3: []channel.Key{},
			Y4: []channel.Key{},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.Channels
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.Channels
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeLegend(f *testing.F) {
	{
		seed := v5.Legend{
			Hidden: true,
			Position: spatial.StickyXY{
				X: 3.5,
				Y: 4.5,
				Root: spatial.CornerLocation{
					X: spatial.XLocation("left"),
					Y: spatial.YLocation("top"),
				},
				Units: spatial.StickyUnits{X: spatial.StickyUnit("px"), Y: spatial.StickyUnit("px")},
			},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Legend{
			Hidden: false,
			Position: spatial.StickyXY{
				X:     0,
				Y:     0,
				Root:  spatial.CornerLocation{X: spatial.XLocation(""), Y: spatial.YLocation("")},
				Units: spatial.StickyUnits{X: spatial.StickyUnit(""), Y: spatial.StickyUnit("")},
			},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.Legend
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.Legend
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeLine(f *testing.F) {
	{
		seed := v5.Line{
			Key:   "test_1",
			Label: new(string("test_2")),
			Color: new(color.Color{
				R: 5,
				G: 6,
				B: 7,
				A: 7.5,
			}),
			StrokeWidth:    8.5,
			Downsample:     10,
			DownsampleMode: v5.DownsampleMode("average"),
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Line{
			Key:            "",
			Label:          nil,
			Color:          nil,
			StrokeWidth:    0,
			Downsample:     0,
			DownsampleMode: v5.DownsampleMode(""),
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.Line
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.Line
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeLinePlot(f *testing.F) {
	{
		seed := v5.LinePlot{
			Key:   uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
			Name:  "test_2",
			Title: v5.Title{Level: text.Level("h1"), Visible: true},
			Legend: v5.Legend{
				Hidden: true,
				Position: spatial.StickyXY{
					X: 9.5,
					Y: 10.5,
					Root: spatial.CornerLocation{
						X: spatial.XLocation("left"),
						Y: spatial.YLocation("top"),
					},
					Units: spatial.StickyUnits{X: spatial.StickyUnit("px"), Y: spatial.StickyUnit("px")},
				},
			},
			Channels: v5.Channels{
				X1: channel.Key(19),
				X2: channel.Key(20),
				Y1: []channel.Key{channel.Key(21)},
				Y2: []channel.Key{channel.Key(22)},
				Y3: []channel.Key{channel.Key(23)},
				Y4: []channel.Key{channel.Key(24)},
			},
			Ranges: v5.Ranges{X1: []string{"test_25"}, X2: []string{"test_26"}},
			Axes: v5.Axes{
				X1: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_30",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 34.5, Upper: 35.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    39.5,
					Type:           new(v5.TickType("linear")),
				},
				X2: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_43",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 47.5, Upper: 48.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    52.5,
					Type:           new(v5.TickType("linear")),
				},
				Y1: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_56",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 60.5, Upper: 61.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    65.5,
					Type:           new(v5.TickType("linear")),
				},
				Y2: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_69",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 73.5, Upper: 74.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    78.5,
					Type:           new(v5.TickType("linear")),
				},
				Y3: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_82",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 86.5, Upper: 87.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    91.5,
					Type:           new(v5.TickType("linear")),
				},
				Y4: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_95",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 99.5, Upper: 100.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    104.5,
					Type:           new(v5.TickType("linear")),
				},
			},
			Lines: []v5.Line{
				{
					Key:   "test_107",
					Label: new(string("test_108")),
					Color: new(color.Color{
						R: 111,
						G: 112,
						B: 113,
						A: 113.5,
					}),
					StrokeWidth:    114.5,
					Downsample:     116,
					DownsampleMode: v5.DownsampleMode("average"),
				},
			},
			Rules: []v5.Rule{
				{
					Key:   "test_118",
					Label: "test_119",
					Color: new(color.Color{
						R: 122,
						G: 123,
						B: 124,
						A: 124.5,
					}),
					Axis:      v5.AxisKey("x1"),
					LineWidth: 126.5,
					LineDash:  127.5,
					Units:     "test_128",
					Position:  129.5,
				},
			},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.LinePlot{
			Key:   uuid.Nil(),
			Name:  "",
			Title: v5.Title{Level: text.Level(""), Visible: false},
			Legend: v5.Legend{
				Hidden: false,
				Position: spatial.StickyXY{
					X:     0,
					Y:     0,
					Root:  spatial.CornerLocation{X: spatial.XLocation(""), Y: spatial.YLocation("")},
					Units: spatial.StickyUnits{X: spatial.StickyUnit(""), Y: spatial.StickyUnit("")},
				},
			},
			Channels: v5.Channels{
				X1: channel.Key(0),
				X2: channel.Key(0),
				Y1: []channel.Key{},
				Y2: []channel.Key{},
				Y3: []channel.Key{},
				Y4: []channel.Key{},
			},
			Ranges: v5.Ranges{X1: []string{}, X2: []string{}},
			Axes: v5.Axes{
				X1: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				X2: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				Y1: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				Y2: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				Y3: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
				Y4: v5.Axis{
					Key:            v5.AxisKey(""),
					Label:          "",
					LabelDirection: spatial.Direction(""),
					LabelLevel:     text.Level(""),
					Bounds:         spatial.Bounds{Lower: 0, Upper: 0},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: false},
					TickSpacing:    0,
					Type:           nil,
				},
			},
			Lines: []v5.Line{},
			Rules: []v5.Rule{},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.LinePlot{
			Key:   uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
			Name:  "test_2",
			Title: v5.Title{Level: text.Level("h1"), Visible: true},
			Legend: v5.Legend{
				Hidden: true,
				Position: spatial.StickyXY{
					X: 9.5,
					Y: 10.5,
					Root: spatial.CornerLocation{
						X: spatial.XLocation("left"),
						Y: spatial.YLocation("top"),
					},
					Units: spatial.StickyUnits{X: spatial.StickyUnit("px"), Y: spatial.StickyUnit("px")},
				},
			},
			Channels: v5.Channels{
				X1: channel.Key(19),
				X2: channel.Key(20),
				Y1: []channel.Key{},
				Y2: []channel.Key{},
				Y3: []channel.Key{},
				Y4: []channel.Key{},
			},
			Ranges: v5.Ranges{X1: []string{}, X2: []string{}},
			Axes: v5.Axes{
				X1: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_30",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 34.5, Upper: 35.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    39.5,
					Type:           new(v5.TickType("linear")),
				},
				X2: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_43",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 47.5, Upper: 48.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    52.5,
					Type:           new(v5.TickType("linear")),
				},
				Y1: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_56",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 60.5, Upper: 61.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    65.5,
					Type:           new(v5.TickType("linear")),
				},
				Y2: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_69",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 73.5, Upper: 74.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    78.5,
					Type:           new(v5.TickType("linear")),
				},
				Y3: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_82",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 86.5, Upper: 87.5},
					ManualBounds:   v5.ManualBounds{Lower: true, Upper: false},
					TickSpacing:    91.5,
					Type:           new(v5.TickType("linear")),
				},
				Y4: v5.Axis{
					Key:            v5.AxisKey("x1"),
					Label:          "test_95",
					LabelDirection: spatial.Direction("x"),
					LabelLevel:     text.Level("h1"),
					Bounds:         spatial.Bounds{Lower: 99.5, Upper: 100.5},
					ManualBounds:   v5.ManualBounds{Lower: false, Upper: true},
					TickSpacing:    104.5,
					Type:           new(v5.TickType("linear")),
				},
			},
			Lines: []v5.Line{},
			Rules: []v5.Rule{},
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.LinePlot
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.LinePlot
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeManualBounds(f *testing.F) {
	{
		seed := v5.ManualBounds{Lower: true, Upper: false}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.ManualBounds{Lower: false, Upper: false}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.ManualBounds
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.ManualBounds
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeRanges(f *testing.F) {
	{
		seed := v5.Ranges{X1: []string{"test_1"}, X2: []string{"test_2"}}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Ranges{X1: []string{}, X2: []string{}}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Ranges{X1: []string{}, X2: []string{}}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.Ranges
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.Ranges
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeRule(f *testing.F) {
	{
		seed := v5.Rule{
			Key:   "test_1",
			Label: "test_2",
			Color: new(color.Color{
				R: 5,
				G: 6,
				B: 7,
				A: 7.5,
			}),
			Axis:      v5.AxisKey("x1"),
			LineWidth: 9.5,
			LineDash:  10.5,
			Units:     "test_11",
			Position:  12.5,
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Rule{
			Key:       "",
			Label:     "",
			Color:     nil,
			Axis:      v5.AxisKey(""),
			LineWidth: 0,
			LineDash:  0,
			Units:     "",
			Position:  0,
		}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.Rule
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.Rule
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}

func FuzzDecodeTitle(f *testing.F) {
	{
		seed := v5.Title{Level: text.Level("h1"), Visible: false}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	{
		seed := v5.Title{Level: text.Level(""), Visible: false}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
		f.Add(w.Bytes())
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		var decoded v5.Title
		r := orc.NewReader(nil)
		r.ResetBytes(data)
		if err := decoded.DecodeOrc(r); err != nil {
			return
		}
		w1 := orc.NewWriter(len(data))
		if err := decoded.EncodeOrc(w1); err != nil {
			t.Fatalf("encode after successful decode failed: %v", err)
		}
		var redecoded v5.Title
		r.ResetBytes(w1.Bytes())
		if err := redecoded.DecodeOrc(r); err != nil {
			t.Fatalf("re-decode failed: %v", err)
		}
		if !testutil.DeepEqual(decoded, redecoded) {
			t.Fatal("round-trip mismatch: decoded value changed after an encode/decode cycle")
		}
	})
}
