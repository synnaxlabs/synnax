...
var _ = Describe("LinePlot", func() {
	Describe("GorpKey", func() {
		It("Should return the lineplot's key", func() {
			k := uuid.New()
			Expect(v0.LinePlot{Key: k}.GorpKey()).To(Equal(k))
		})
	})

	Describe("SetOptions", func() {
		It("Should return no options", func() {
			Expect(v0.LinePlot{}.SetOptions()).To(BeNil())
		})
	})
})
...
var _ = Describe("NormalizeKeys", func() {
	It(
		"Should lift a LinePlot row stored under the pre-v0.54 key format",
		func(ctx SpecContext) {
			kvDB := memkv.New()
			db := DeferClose(gorp.Wrap(kvDB, gorp.WithCodec(msgpack.Codec)))
			e := v0.LinePlot{Key: uuid.New(), Name: "Pressures"}
			...
			table := MustOpen(gorp.OpenTable(ctx, gorp.TableConfig[v0.Key, v0.LinePlot]{
				DB:         db,
				Migrations: []migrate.Migration{v0.NormalizeKeys},
			}))
			var res v0.LinePlot
			Expect(table.NewRetrieve().
				Where(gorp.MatchKeys[v0.Key, v0.LinePlot](e.GorpKey())).
