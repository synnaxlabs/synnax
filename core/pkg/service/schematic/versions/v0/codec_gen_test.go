...
import (
	...
	"github.com/synnaxlabs/synnax/pkg/service/schematic/versions/v6"
...
var _ = Describe("Codec", func() {
	Describe("Schematic", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v6.Schematic) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v6.Schematic
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v6.Schematic{
				Key:      uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801"),
				Name:     "test_2",
				Data:     msgpack.EncodedJSON{"key_3": "value_3"},
				Snapshot: false,
			}),
			Entry("zero values", v6.Schematic{
				Key:      uuid.Nil,
				Name:     "",
				Data:     nil,
				Snapshot: false,
