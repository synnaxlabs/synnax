...
// measured dimensions and decoding its opaque configs into the element config union. A
// config the union rejects is dropped: the lift runs unattended over every stored
// schematic, so one bad entry must not fail it. Prefer ImportSchematic wherever the
...
// ImportSchematic lifts a v7 schematic into the v8 shape for an import. Unlike
...
func ImportSchematic(ctx context.Context, old v7.Schematic) (Schematic, error) {
	out, err := autoMigrateSchematic(ctx, old)
	if err != nil {
		return Schematic{}, err
	}
