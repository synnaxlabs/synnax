...
// falls back to the service's permanent symbol group. Symbols have exactly one wire
// version — the resource never shipped before server-side export — so any other
// version is rejected with a path-scoped validation error.
func (s *Service) Import(
	...
	if env.Version < Version {
		return ontology.ID{}, validate.PathedError(
			errors.Wrapf(
				...
				"unsupported schematic_symbol version %d (supported: %d)",
				env.Version, Version,
			),
			"version",
		)
	...
	sym, err := imex.Decode[Symbol](ctx, env)
	if err != nil {
