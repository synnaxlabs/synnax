...
import (
	...
	"github.com/synnaxlabs/synnax/pkg/service/schematic/symbol/versions/v0"
...
var _ = Describe("Codec", func() {
	...
	Describe("Region", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v0.Region) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v0.Region
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v0.Region{
				Key:         "test_1",
				Name:        "test_2",
			...
			Entry("empty collections", v0.Region{
				Key:         "test_1",
				Name:        "test_2",
				Selectors:   []string{},
				StrokeColor: new(string("test_4")),
	...
	Describe("Spec", func() {
		DescribeTable("should round-trip encode and decode",
			...
			Entry("empty collections", v0.Spec{
				Svg:             "test_1",
				States:          []v0.State{},
				Variant:         "test_3",
				Handles:         []v0.Handle{},
	...
	Describe("State", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v0.State) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v0.State
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v0.State{
				Key:  "test_1",
				Name: "test_2",
				Regions: []v0.Region{
					{
			...
			Entry("zero values", v0.State{
				Key:     "",
				Name:    "",
				Regions: nil,
			}),
			Entry("empty collections", v0.State{
				Key:     "test_1",
				Name:    "test_2",
				Regions: []v0.Region{},
			}),
	...
	Describe("Symbol", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v0.Symbol) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v0.Symbol
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v0.Symbol{
