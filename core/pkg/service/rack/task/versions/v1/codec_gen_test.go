...
import (
	...
	"github.com/synnaxlabs/synnax/pkg/service/rack/task/versions/v1"
...
var _ = Describe("Codec", func() {
	Describe("StatusConfig", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v1.StatusConfig) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v1.StatusConfig
				r := orc.NewReader(nil)
			...
			Entry("zero values", v1.StatusConfig{Keyed: config.Keyed{Key: uuid.Nil}}),
		)
	})
})
...
func FuzzDecodeStatusConfig(f *testing.F) {
	...
	{
		seed := v1.StatusConfig{Keyed: config.Keyed{Key: uuid.Nil}}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
