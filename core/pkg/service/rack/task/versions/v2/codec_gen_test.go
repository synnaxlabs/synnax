...
import (
	...
	"github.com/synnaxlabs/synnax/pkg/service/rack/task/versions/v2"
...
var _ = Describe("Codec", func() {
	Describe("StatusConfig", func() {
		DescribeTable("should round-trip encode and decode",
			func(original v2.StatusConfig) {
				w := orc.NewWriter(0)
				Expect(original.EncodeOrc(w)).To(Succeed())
				var decoded v2.StatusConfig
				r := orc.NewReader(nil)
				r.ResetBytes(w.Bytes())
				Expect(decoded.DecodeOrc(r)).To(Succeed())
				Expect(decoded).To(Equal(original))
			},
			Entry("fully populated", v2.StatusConfig{
				Keyed: config.Keyed{Key: uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801")},
			}),
			Entry("zero values", v2.StatusConfig{Keyed: config.Keyed{Key: uuid.Nil}}),
		)
	})
})

func BenchmarkEncodeDecodeStatusConfig(b *testing.B) {
	seed := v2.StatusConfig{
		Keyed: config.Keyed{Key: uuid.MustParse("a1b2c3d4-e5f6-7890-abcd-ef1234567801")},
	}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
...
func FuzzDecodeStatusConfig(f *testing.F) {
	...
	{
		seed := v2.StatusConfig{Keyed: config.Keyed{Key: uuid.Nil}}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
