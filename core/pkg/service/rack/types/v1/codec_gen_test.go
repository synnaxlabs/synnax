...
var _ = Describe("Codec", func() {
	...
	Describe("StatusDetails", func() {
		DescribeTable("should round-trip encode and decode",
			...
			Entry("fully populated", v1.StatusDetails{Rack: v1.Key(2)}),
			Entry("zero values", v1.StatusDetails{Rack: v1.Key(0)}),
		)
	})
})
...
func BenchmarkEncodeDecodeStatusDetails(b *testing.B) {
	seed := v1.StatusDetails{Rack: v1.Key(2)}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
...
func FuzzDecodeStatusDetails(f *testing.F) {
	...
	{
		seed := v1.StatusDetails{Rack: v1.Key(0)}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
