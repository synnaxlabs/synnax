...
var _ = Describe("Writer", func() {
	Describe("Create", func() {
		...
		It(
			"should return an error if the time range is invalid",
			func(ctx SpecContext) {
				r := &ranger.Range{
					Name: "Range",
					TimeRange: telem.TimeRange{
		...
		Context("Parent Management", func() {
			...
			It(
				"Should NOT re-set the custom parent when the range exists but no parent is provided",
				func(ctx SpecContext) {
					parent := ranger.Range{
						Name:      "Parent",
						TimeRange: telem.SecondTS.SpanRange(telem.Second),
			...
			It(
				"Should change the custom parent when the range exists and a new parent is provided",
				func(ctx SpecContext) {
			...
			It(
				"Should create multiple ranges with the same parent",
				func(ctx SpecContext) {
					parent := ranger.Range{
						Name:      "Parent",
						TimeRange: telem.SecondTS.SpanRange(telem.Second),
			...
			It(
				"Should resolve the parent's name and time range on create",
				func(ctx SpecContext) {
					parent := ranger.Range{
						Name:      "Parent",
						TimeRange: telem.SecondTS.SpanRange(telem.Second),
			...
			It(
				"Should NOT resolve the parent's labels on create",
				func(ctx SpecContext) {
			...
			It(
				"Should not resolve the parent's own parent on create",
				func(ctx SpecContext) {
		...
		Context("Label Management", func() {
			It(
				"Should define LabeledBy relationships for labels on the range",
				func(ctx SpecContext) {
					...
					resolved := MustSucceed(
						labelSvc.RetrieveFor(ctx, r.OntologyID(), tx),
					)
					resolvedKeys := lo.Map(
						resolved,
			...
			It(
				"Should leave label relationships untouched when r.Labels is empty",
				func(ctx SpecContext) {
					...
					Expect(
						labelSvc.NewWriter(tx).
							Label(ctx, r.OntologyID(), []label.Key{l.Key}),
					).To(Succeed())
					Expect(w.Create(ctx, r)).To(Succeed())
					resolved := MustSucceed(
						labelSvc.RetrieveFor(ctx, r.OntologyID(), tx),
			...
			It(
				"Should add to existing label relationships when a range is re-created with new labels",
				func(ctx SpecContext) {
	...
	Describe("Delete", func() {
		It("Should delete a range by its key", func(ctx SpecContext) {
			...
			Expect(
				svc.NewRetrieve().
					Where(ranger.MatchKeys(r.Key)).
					Entry(&retrieveR).
					Exec(ctx, tx),
			).ToNot(Succeed())
		})
		It(
			"Should delete all child ranges when a range is deleted",
			func(ctx SpecContext) {
				parent := ranger.Range{
					Name: "Parent",
					TimeRange: telem.TimeRange{
		...
		It(
			"Should delete multiple ranges in a single Delete call",
			func(ctx SpecContext) {
				r1 := ranger.Range{
					Name:      "Range1",
					TimeRange: telem.SecondTS.SpanRange(telem.Second),
				...
				Expect(svc.NewWriter(tx).CreateMany(ctx, &[]ranger.Range{r1, r2, r3})).
					To(Succeed())
				Expect(
					svc.NewWriter(tx).Delete(ctx, r1.Key, r2.Key, r3.Key),
				).To(Succeed())
				Expect(
					svc.NewRetrieve().Where(ranger.MatchKeys(r1.Key)).Exists(ctx, tx),
				).
					To(BeFalse())
				Expect(
					svc.NewRetrieve().Where(ranger.MatchKeys(r2.Key)).Exists(ctx, tx),
				).
					To(BeFalse())
				Expect(
					svc.NewRetrieve().Where(ranger.MatchKeys(r3.Key)).Exists(ctx, tx),
				).
					To(BeFalse())
			},
		)
		...
		It(
			"Should recursively delete grandchild ranges when a range is deleted",
			func(ctx SpecContext) {
				grandparent := ranger.Range{
					Name:      "Grandparent",
					TimeRange: telem.SecondTS.SpanRange(telem.Second),
				...
				Expect(
					svc.NewRetrieve().
						Where(ranger.MatchKeys(grandparent.Key)).
						Exists(ctx, tx),
				).
					To(BeFalse())
				Expect(
					svc.NewRetrieve().
						Where(ranger.MatchKeys(parent.Key)).
						Exists(ctx, tx),
				).
					To(BeFalse())
				Expect(
					svc.NewRetrieve().
						Where(ranger.MatchKeys(child.Key)).
						Exists(ctx, tx),
				).
					To(BeFalse())
			},
	...
	Describe("SetEnd", func() {
		It(
			"Should set the end bound while preserving the start",
			func(ctx SpecContext) {
				r := ranger.Range{
					Name: "set_end",
					TimeRange: telem.TimeRange{
		...
		It("Should overwrite a previously set end bound", func(ctx SpecContext) {
			r := ranger.Range{
				...
				TimeRange: telem.TimeRange{
					Start: telem.SecondTS,
					End:   telem.SecondTS * 5,
				},
			}
			Expect(svc.NewWriter(tx).Create(ctx, &r)).To(Succeed())
			Expect(
				svc.NewWriter(tx).SetEnd(ctx, r.Key, telem.SecondTS*20),
			).To(Succeed())
			var updated ranger.Range
			Expect(svc.NewRetrieve().Where(ranger.MatchKeys(r.Key)).Entry(&updated).
				Exec(ctx, tx)).To(Succeed())
		...
		It(
			"Should return query.ErrNotFound when the range does not exist",
			func(ctx SpecContext) {
		...
		It(
			"Should return a validation error when end precedes start",
			func(ctx SpecContext) {
				r := ranger.Range{
