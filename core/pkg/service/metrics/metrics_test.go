...
var _ = Describe("Metrics", func() {
	...
	Describe("Metric Collection", func() {
		...
		It("Should write metrics at configured interval", func(ctx SpecContext) {
			...
			// The index timestamp is generated by the writer's AutoIndex rather than
			// emitted by the collector, so it no longer occupies a fixed position in
			// the streamed frame. Look each series up by channel key instead.
			...
			timeSeries := res.Frame.Get(keyByName[names[0]]).Series[0]
			Expect(timeSeries.DataType).To(Equal(telem.TimeStampT))
			Expect(timeSeries.Len()).To(Equal(int64(1)))

			cpuSeries := res.Frame.Get(keyByName[names[1]]).Series[0]
			...
			memSeries := res.Frame.Get(keyByName[names[2]]).Series[0]
			...
			tsSizeSeries := res.Frame.Get(keyByName[names[4]]).Series[0]
			...
			kvSizeSeries := res.Frame.Get(keyByName[names[5]]).Series[0]
			...
			totalSizeSeries := res.Frame.Get(keyByName[names[3]]).Series[0]
