...
var _ = Describe("Writer", func() {
	...
	Describe("Update", func() {
		It("Should rename a workspace", func() {
			...
			Expect(gorp.NewRetrieve[uuid.UUID, workspace.Workspace](nil).WhereKeys(ws.Key).Entry(&res).Exec(ctx, tx)).To(Succeed())
	...
	Describe("DeleteChannel", func() {
		It("Should delete a workspace", func() {
			...
			var res workspace.Workspace
			Expect(gorp.NewRetrieve[uuid.UUID, workspace.Workspace](nil).WhereKeys(ws.Key).Entry(&res).Exec(ctx, tx)).ToNot(Succeed())
		})
	})
})
