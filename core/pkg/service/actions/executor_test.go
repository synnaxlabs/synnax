...
var _ = Describe("Executor", func() {
	var (
		...
		exec *actions.Executor[string, testAction]
		rec  *Recorder[string, testAction]
	)

	BeforeEach(func(ctx SpecContext) {
		db = DeferClose(gorptestutil.OpenGorpMsgpackDB())
		st = actions.NewState[string, testAction]()
		exec = actions.NewExecutor(db, st.Dispatcher())
		rec = &Recorder[string, testAction]{}
	...
	Describe("Dispatch", func() {
		It("Should apply the staged change and notify it", func(ctx SpecContext) {
			...
			Expect(exec.Dispatch(ctx, "c", "dk", acts, increment(ctx))).To(Succeed())
