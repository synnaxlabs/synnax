...
var _ = Describe("NormalizeKeys", func() {
	It(
		"Should lift a Status row stored under the pre-v0.54 key format",
		func(ctx SpecContext) {
			kvDB := memkv.New()
			db := DeferClose(gorp.Wrap(kvDB, gorp.WithCodec(msgpack.Codec)))
			e := v0.Status[any]{Key: "rack-1", Name: "Rack 1"}
			...
			table := MustOpen(
				gorp.OpenTable(ctx, gorp.TableConfig[v0.Key, v0.Status[any]]{
					DB:         db,
					Migrations: []migrate.Migration{v0.NormalizeKeys},
				}),
			)
			var res v0.Status[any]
			Expect(table.NewRetrieve().
				Where(gorp.MatchKeys[v0.Key, v0.Status[any]](e.GorpKey())).
