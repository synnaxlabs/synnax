...
// ServiceConfig is the configuration for opening the OPC UA task config service.
...
// OpenService opens the OPC UA task config service with the provided configuration.
...
func OpenService(ctx context.Context, cfgs ...ServiceConfig) (s *Service, err error) {
	...
	if s.Read, err = common.OpenConfigService[ReadConfig](
		ctx, base, common.ConfigServiceConfig{Type: ontology.ResourceTypeOpcRead},
	); !ok(err, s.Read) {
		return nil, err
	}
	if s.Write, err = common.OpenConfigService[WriteConfig](
		ctx, base, common.ConfigServiceConfig{Type: ontology.ResourceTypeOpcWrite},
	); !ok(err, s.Write) {
		return nil, err
	}
	if s.Scan, err = common.OpenConfigService[ScanConfig](
		ctx, base, common.ConfigServiceConfig{Type: ontology.ResourceTypeOpcScan},
	); !ok(err, s.Scan) {
		return nil, err
	}
	return s, nil
