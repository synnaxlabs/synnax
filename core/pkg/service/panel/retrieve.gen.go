...
// Retrieve is used to retrieve Panel records from the database using a
// builder pattern for constructing queries.
type Retrieve struct {
	baseTX     gorp.Tx
	gorp       gorp.Retrieve[Key, Panel]
	search     *search.Index
	searchTerm string
}
...
type Filter = gorp.BoundFilter[Retrieve, Key, Panel]
...
func Match(f func(ctx gorp.Context, r Retrieve, e *Panel) (bool, error)) Filter {
	return gorp.MatchBound[Retrieve, Key, Panel](f)
}

// And returns a filter that matches when all provided filters match.
func And(fs ...Filter) Filter {
	return gorp.AndBound[Retrieve, Key, Panel](fs...)
}

// Or returns a filter that matches when any provided filter matches.
func Or(fs ...Filter) Filter {
	return gorp.OrBound[Retrieve, Key, Panel](fs...)
}

// Not returns a filter that inverts the provided filter.
func Not(f Filter) Filter {
	return gorp.NotBound[Retrieve, Key, Panel](f)
}
...
// Count returns the number of panels matching the query.
...
// Exists checks whether any panels match the query.
