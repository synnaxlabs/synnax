...
var _ = Describe("Retrieve", func() {
	...
	It("Should retrieve multiple panels via Entries", func(ctx SpecContext) {
		keys := createN(ctx, 3)
		var res []panel.Panel
		Expect(svc.NewRetrieve().Where(panel.MatchKeys(keys...)).Entries(&res).Exec(ctx, tx)).
			To(Succeed())
		Expect(res).To(HaveLen(3))
	})

	It("Should return ErrNotFound when the key does not exist", func(ctx SpecContext) {
		Expect(svc.NewRetrieve().Where(panel.MatchKeys(uuid.New())).Entry(&panel.Panel{}).
	...
	It("Should skip results with Offset", func(ctx SpecContext) {
		keys := createN(ctx, 3)
		var res []panel.Panel
		Expect(svc.NewRetrieve().Where(panel.MatchKeys(keys...)).Offset(1).Entries(&res).
			Exec(ctx, tx)).To(Succeed())
		Expect(res).To(HaveLen(2))
	})
	...
	It("Should count the matching panels", func(ctx SpecContext) {
		keys := createN(ctx, 3)
		Expect(svc.NewRetrieve().Where(panel.MatchKeys(keys...)).Count(ctx, tx)).To(Equal(3))
	})

	It("Should report existence", func(ctx SpecContext) {
		keys := createN(ctx, 1)
		Expect(svc.NewRetrieve().Where(panel.MatchKeys(keys[0])).Exists(ctx, tx)).To(BeTrue())
