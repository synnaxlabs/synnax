...
type Client struct {
	...
	ingestionSvc ingestionconfigsv1.IngestionConfigServiceClient
...
func NewClient(ctx context.Context, props DeviceProperties) (*Client, error) {
	...
	return &Client{
		conn:         conn,
		props:        props,
		ingestionSvc: ingestionconfigsv1.NewIngestionConfigServiceClient(conn),
...
func (c *Client) GetOrCreateIngestionConfig(
	...
	protoFlows := make([]*ingestionconfigsv1.FlowConfig, len(flows))
	for i, flow := range flows {
		channels := make([]*ingestionconfigsv1.ChannelConfig, len(flow.Channels))
		for j, ch := range flow.Channels {
			channels[j] = &ingestionconfigsv1.ChannelConfig{
				Name:        ch.Name,
		...
		protoFlows[i] = &ingestionconfigsv1.FlowConfig{
			Name:     flow.Name,
			Channels: channels,
		}
	}
