...
type Service struct {
	...
	// table state on reconnect and reset their per-key high-water marks
...
func OpenService(ctx context.Context, cfgs ...ServiceConfig) (s *Service, err error) {
	...
	if cfg.Signals != nil {
		...
		if sig, err = cfg.Signals.PublishFromObservable(ctx, signals.ObservablePublisherConfig{
			Name:       "table_actions",
			Observable: actions,
			SetChannel: channel.Channel{Name: "sy_table_set", DataType: telem.JSONT, Internal: true},
		}); !ok(err, sig) {
			return nil, err
		}
		deleteCfg := signals.GorpPublisherConfigUUID[Table](s.table.Observe())
