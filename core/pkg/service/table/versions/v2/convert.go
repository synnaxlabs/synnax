...
// opaqueConfigFields names cell config fields whose contents carry semantic keys (telem
// pipeline segment names) and must not be case-converted.
var opaqueConfigFields = set.New("props")

// normalizeConfigKeys converts a cell config payload's field keys from the camelCase
// the
// Console wrote verbatim to the snake_case wire form of the cell config union. Already
// snake_case input passes through unchanged, so the conversion is idempotent. Values
...
// declared onto the flex alignments the alignment form control has always written.
var legacyAligns = map[string]string{"left": "start", "right": "end"}

// legacyWeights maps named CSS font weights onto the numeric weights the weight form
// control writes.
...
// extractLegacyArgs rewrites a stored cell config's legacy fields into the semantic
// arguments the schema now declares, in place on the normalized wire map. Value cells
// extract their channel, rolling average, precision, and notation from the stored telem
// pipeline spec; a zero channel (the legacy unset default) produces no argument. Spec
// fields are removed regardless so the entry decodes under the args-based schema. Text
// cells map legacy x-location alignments and named font weights onto their typed
// counterparts.
