...
var _ = Describe("Table", func() {
	Describe("GorpKey", func() {
		It("Should return the table's key", func() {
			k := uuid.New()
			Expect(v0.Table{Key: k}.GorpKey()).To(Equal(k))
		})
	})

	Describe("SetOptions", func() {
		It("Should return no options", func() {
			Expect(v0.Table{}.SetOptions()).To(BeNil())
		})
	})
})
...
var _ = Describe("NormalizeKeys", func() {
	It(
		"Should lift a Table row stored under the pre-v0.54 key format",
		func(ctx SpecContext) {
			kvDB := memkv.New()
			db := DeferClose(gorp.Wrap(kvDB, gorp.WithCodec(msgpack.Codec)))
			e := v0.Table{Key: uuid.New(), Name: "Valves"}
			...
			table := MustOpen(gorp.OpenTable(ctx, gorp.TableConfig[v0.Key, v0.Table]{
				DB:         db,
				Migrations: []migrate.Migration{v0.NormalizeKeys},
			}))
			var res v0.Table
			Expect(table.NewRetrieve().
				Where(gorp.MatchKeys[v0.Key, v0.Table](e.GorpKey())).
