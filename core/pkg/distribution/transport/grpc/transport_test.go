...
var _ = Describe("Transport", func() {
	Describe("Construction", func() {
		...
		It("Should report the gRPC protocol and protobuf encoding", func() {
			Expect(
				transportgrpc.New(openPool()).Report(),
			).To(Equal(fgrpc.Reporter.Report()))
		})
	})

	Describe("Wire round-trip", func() {
		It(
			"Should round-trip a channel create through the bundled transport",
		...
		It(
			"Should round-trip a framer delete through the bundled transport",
			func(ctx SpecContext) {
