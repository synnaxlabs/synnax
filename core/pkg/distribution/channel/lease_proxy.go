		...
		if ch.IsCalculated() {
			...
			if ch.LocalIndex != 0 && ch.LocalKey == 0 {
				return validate.PathedError(
					errors.Wrap(validate.Err, "calculated channels cannot specify an index manually"),
					"local_index",
				)
			}
...
func validateChannelNames(
	...
	for i, name := range names {
		if namesSeen.Contains(name) {
			return validate.PathedError(
				errors.Wrapf(validate.Err, "duplicate channel name '%s' in request", name),
	...
	for i, name := range names {
		...
		return validate.PathedError(
			errors.Wrapf(validate.Err, "channel with name '%s' already exists", name),
			fmt.Sprintf("[%d].name", i),
		)
	}
	return nil
...
func channelNameUpdater(allowInternal bool, keys Keys, names []string) gorp.ChangeFunc[Key, Channel] {
	return func(_ gorp.Context, c Channel) (Channel, error) {
		if c.Internal && !allowInternal {
			return c, errors.Wrapf(validate.Err, "cannot rename internal channel %v", c)
		}
