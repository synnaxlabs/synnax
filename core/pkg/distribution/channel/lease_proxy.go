...
func newLeaseProxy(
	...
	if cfg.HostResolver.HostKey() == core.KeyBootstrapper {
...
func (lp *leaseProxy) create(ctx context.Context, tx gorp.Tx, _channels *[]Channel, opts CreateOptions) error {
	...
	for i, ch := range channels {
		...
		if ch.IsCalculated() {
			...
			channels[i].Leaseholder = core.KeyFree
	...
	if len(batch.Free) > 0 {
		if !lp.cfg.HostResolver.HostKey().IsBootstrapper() {
			remoteChannels, err := lp.createRemote(ctx, core.KeyBootstrapper, batch.Free, opts)
			if err != nil {
				return err
			}
			oChannels = append(oChannels, remoteChannels...)
...
func (lp *leaseProxy) deleteRemote(ctx context.Context, target core.Key, keys Keys) error {
	addr, err := lp.cfg.HostResolver.Resolve(target)
	if err != nil {
		return err
	}
...
func (r renameBatchEntry) Lease() core.Key { return r.key.Lease() }
...
func (lp *leaseProxy) renameRemote(ctx context.Context, target core.Key, keys Keys, names []string) error {
	addr, err := lp.cfg.HostResolver.Resolve(target)
	if err != nil {
		return err
	}
