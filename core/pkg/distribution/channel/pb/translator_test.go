...
var _ = Describe("Translator", func() {
	Describe("CreateMessage", func() {
		...
		It(
			"Should round-trip the minimal storage and routing fields",
		...
		It(
			"Should return an error when forwarding an unrecognized concurrency value",
			func(ctx SpecContext) {
				msg := channel.CreateMessage{Channels: []channel.Channel{
				...
				Expect(t.Forward(ctx, msg)).Error().To(
					MatchError(
						ContainSubstring("unrecognized control.Concurrency value"),
					),
				)
			},
		)
		It(
			"Should return an error when reversing an unrecognized concurrency value",
			func(ctx SpecContext) {
