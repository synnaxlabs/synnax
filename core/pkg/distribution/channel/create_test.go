...
var _ = Describe("Create", Ordered, func() {
	...
	Context("Single channel", func() {
		...
		Context("Node is local", func() {
			...
			It("Should not create the channel if it already exists by name", func(ctx SpecContext) {
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &ch, channel.RetrieveIfNameExists())).To(Succeed())
			...
			It("Should create the channel without error", func(ctx SpecContext) {
				ch.Leaseholder = 1
				ch.Name = channel.NewRandomName()
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &ch, channel.CreateWithoutGroupRelationship())).To(Succeed())
				Expect(ch.Key().Leaseholder()).To(Equal(aspen.NodeKey(1)))
		...
		Context("Node is remote", func() {
			...
			It("Should assign a sequential key to the channels on each node",
				func(ctx SpecContext) {
					...
					Expect(mockCluster.Nodes[1].ChannelService().NewWriter(nil).Create(ctx, ch2)).To(Succeed())
			...
			It("Should correctly create a virtual channel", func(ctx SpecContext) {
				...
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, ch3)).To(Succeed())
			...
			It("Should create an index channel", func(ctx SpecContext) {
				...
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, ch4)).To(Succeed())
		...
		Context("error cases", func() {
			It("Should return an error if the name is invalid", func(ctx SpecContext) {
				...
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &ch)).
			...
			It("Should return an error if the name is a duplicate", func(ctx SpecContext) {
				...
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &ch2)).
	...
	Context("Multiple channels", func() {
		It("Should create multiple channels without error", func(ctx SpecContext) {
			...
			Expect(mockCluster.Nodes[1].ChannelService().CreateMany(ctx, &chs)).To(Succeed())
		...
		It("Should return an error if the names are duplicates", func(ctx SpecContext) {
			...
			Expect(mockCluster.Nodes[1].ChannelService().CreateMany(ctx, &[]channel.Channel{ch1, ch2})).
	...
	Context("Creating if name doesn't exist", func() {
		...
		Describe("OverwriteIfNameExists", func() {
			...
			It("Should overwrite the channel if it already exists by name and the new channel has different properties than the old one", func(ctx SpecContext) {
				...
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &newCh, channel.OverwriteIfNameExistsAndDifferentProperties())).To(Succeed())

				var resChannels []channel.Channel
				Expect(mockCluster.Nodes[1].ChannelService().NewRetrieve().Where(channel.MatchKeys(newCh.Key())).
				...
				err := mockCluster.Nodes[1].ChannelService().NewRetrieve().Where(channel.MatchKeys(originalKey)).Entries(&resChannels).Exec(ctx, nil)
			...
			It("Should not overwrite the channel if it already exists by name and the new channel has the same properties as the old one", func(ctx SpecContext) {
				...
				var resChannels []channel.Channel
				Expect(mockCluster.Nodes[1].ChannelService().NewRetrieve().Where(channel.MatchKeys(originalKey)).Entries(&resChannels).Exec(ctx, nil)).To(Succeed())
				Expect(resChannels).To(HaveLen(1))
	...
	Context("Calculated Channel with Auto-Created Index", func() {
		It("Should automatically create an index channel for calculated channels", func(ctx SpecContext) {
			...
			Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &calcCh)).To(Succeed())
			...
			Expect(mockCluster.Nodes[1].ChannelService().NewRetrieve().
		...
		It("Should reject calculated channel with manually-specified index", func(ctx SpecContext) {
			...
			err := mockCluster.Nodes[1].ChannelService().Create(ctx, &calcCh)
		...
		It("Should retrieve existing calculated channel with its index", func(ctx SpecContext) {
			...
			Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &calcCh2, channel.RetrieveIfNameExists())).To(Succeed())
		...
		It("Should handle batch create with calculated and regular channels", func(ctx SpecContext) {
			...
			Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &indexCh1)).To(Succeed())
			Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &indexCh2)).To(Succeed())
			...
			for i := range channels {
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &channels[i])).To(Succeed())
			}
	...
	Context("Updating Calculated Channel Expression", func() {
		It("Should update expression when Create() called with existing key", func(ctx SpecContext) {
			...
			Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &sensor1)).To(Succeed())
	...
	Context("Updating a channel", func() {
		...
		BeforeEach(func(ctx SpecContext) {
			...
			Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &ch)).To(Succeed())
			Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &ch2)).To(Succeed())
		})
		It("Should update the channel name without error", func(ctx SpecContext) {
			...
			var resChannels []channel.Channel
			Expect(mockCluster.Nodes[1].ChannelService().NewRetrieve().Where(channel.MatchKeys(ch.Key())).Entries(&resChannels).Exec(ctx, nil)).To(Succeed())
			Expect(resChannels).To(HaveLen(1))
		...
		It("Should assign a new key when attempting to update a non-virtual channel",
			func(ctx SpecContext) {
				...
				Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &nonVirtualCh)).To(Succeed())
				...
				var resChannels []channel.Channel
				Expect(mockCluster.Nodes[1].ChannelService().NewRetrieve().Where(channel.MatchKeys(originalKey, nonVirtualCh.Key())).Entries(&resChannels).Exec(ctx, nil)).To(Succeed())
...
var _ = Context("Name Validation Disabled", func() {
	Describe("Channel Creation", Ordered, func() {
		...
		It("Should allow renaming to a name with special characters", func(ctx SpecContext) {
			...
			Expect(mockCluster.Nodes[1].ChannelService().Create(ctx, &ch)).To(Succeed())
			Expect(mockCluster.Nodes[1].ChannelService().Rename(ctx, ch.Key(), "new name with spaces!", false)).To(Succeed())
			var retrieved channel.Channel
			Expect(mockCluster.Nodes[1].ChannelService().NewRetrieve().
				Where(channel.MatchKeys(ch.Key())).
				Entry(&retrieved).
