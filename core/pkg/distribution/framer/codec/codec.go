...
func (c *Codec) encodeInternal(ctx context.Context, src framer.Frame) (err error) {
	...
	for rawI, s := range src.RawSeries() {
		...
		if dt != s.DataType {
			return errors.Wrapf(
				validate.Err, "data type %s for channel %s does not match series data type %s",
...
func (c *Codec) DecodeStream(reader io.Reader) (frame framer.Frame, err error) {
	...
	if !ok {
		...
		err = errors.Wrapf(validate.Err, "[framer.codec] - remote sent invalid sequence number %d. Valid rawIndices are %v", seqNum, states)
		return frame, err
	}
