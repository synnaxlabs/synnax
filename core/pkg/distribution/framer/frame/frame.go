...
// Frame is a record that maps channel keys to telemetry series. It wraps the base
// telem.Frame type with channel.Key as the key type, providing distribution-specific
// functionality for handling telemetry data across nodes.
type Frame struct{ telem.Frame[channel.Key] }

// Append adds a new key-series pair to the frame and returns the updated frame. The key
// must be a valid channel.Key and the series must be a valid telem.Series.
...
// Alloc allocates a new frame with a capacity that can hold up to the specified number
// of series before a re-allocation, This is useful for pre-allocating frames when the
// expected number of series is known.
func Alloc(cap int) Frame { return Frame{telem.AllocFrame[channel.Key](cap)} }
...
func (f Frame) SplitByLeaseholder() map[core.Key]Frame {
	frames := make(map[core.Key]Frame)
...
func (f Frame) SplitByHost(host core.Key) (local Frame, remote Frame, free Frame) {
...
func (f Frame) Extend(frame Frame) Frame { return Frame{f.Frame.Extend(frame.Frame)} }
...
// ShallowCopy creates a shallow copy of the frame. The keys and series slices are
// copied, but the series data itself is not duplicated.
