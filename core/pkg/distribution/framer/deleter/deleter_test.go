...
var _ = Describe("Deleter", Ordered, func() {
	...
	for _, createScenario := range scenarios {
		var s scenario
		BeforeAll(func() { ShouldNotLeakGoroutines(); s = createScenario(context.Background()) })
	...
	Describe("Mixed Gateway and Peer", Ordered, func() {
		var s scenario
		BeforeAll(func(ctx SpecContext) { ShouldNotLeakGoroutines(); s = mixedScenario(context.Background()) })
		AfterAll(func() { Expect(s.closer.Close()).To(Succeed()) })
