...
type tapper struct {
	...
	taps map[core.Key]tapController
	Config
}
...
func newTapper(config Config) confluence.Segment[demand, Response] {
	t := &tapper{
		...
		taps:       make(map[core.Key]tapController),
...
// channel data from.
func (t *tapper) updateDemands(d demand) map[core.Key]channel.Keys {
	if d.Variant == change.VariantDelete {
		delete(t.demands, d.Key)
	} else {
		t.demands[d.Key] = d.Value.Keys
	}
	nodeDemands := make(map[core.Key]channel.Keys, len(t.taps))
...
func (t *tapper) Flow(sCtx signal.Context, opts ...confluence.Option) {
	t.taps[core.KeyFree], _ = t.tapInto(sCtx, core.KeyFree, channel.Keys{})
...
func (t *tapper) close() {
	...
	if err := t.taps[core.KeyFree].closer.Close(); err != nil {
...
func (t *tapper) updateTaps(
	ctx context.Context,
	nodeDemands map[core.Key]channel.Keys,
) {
...
func (t *tapper) tapIntoPeer(ctx context.Context, nodeKey core.Key) (tap, error) {
