...
// Retrieve is a query builder for retrieving actions. It should not be instantiated
// directly, and should instead be instantiated via the NewRetrieve method on
// action.Service.
type Retrieve struct {
	baseTX gorp.Tx
	gorp   gorp.Retrieve[uuid.UUID, Action]
}

// WhereKeys filters the actions by the given keys.
func (r Retrieve) WhereKeys(keys ...uuid.UUID) Retrieve {
	r.gorp = r.gorp.WhereKeys(keys...)
	return r
}

// Entry binds the given action to the query. This pointer is where the results of the
// query will be stored after Exec is called.
func (r Retrieve) Entry(action *Action) Retrieve {
	r.gorp = r.gorp.Entry(action)
	return r
}

// Entries binds the given slice of actions to the query. This pointer is where the results
// of the query will be stored after Exec is called.
func (r Retrieve) Entries(actions *[]Action) Retrieve {
	r.gorp = r.gorp.Entries(actions)
	return r
}
