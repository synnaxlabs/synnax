...
func Publish(
	...
	resourceObserver := observe.Translator[iter.Nexter[core.Change], []change.Change[[]byte, struct{}]]{
		Observable: otg.ResourceObserver,
		Translate: func(nexter iter.Nexter[core.Change]) []change.Change[[]byte, struct{}] {
			return iter.MapToSlice(ctx, nexter, func(ch core.Change) change.Change[[]byte, struct{}] {
...
func DecodeIDs(ser []byte) ([]ontology.ID, error) {
	...
	for _, b := range ser {
		if b == '\n' {
			id, err := core.ParseID(buf.String())
			if err != nil {
				return nil, err
			}
			ids = append(ids, id)
