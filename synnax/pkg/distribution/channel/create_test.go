...
var _ = Describe("Create", Ordered, func() {
	...
	Context("Calculated Channels", func() {
		...
		It("Should return an error if the requires field is empty", func() {
			...
			Expect(mockCluster.Nodes[1].Channels.Create(ctx, &ch)).To(MatchError(ContainSubstring("requires: calculated channels must require at least one channel")))
		})
		It("Should return an error if the calculated channel depends on both a virtual and index channel", func() {
			...
			Expect(mockCluster.Nodes[1].Channels.Create(ctx, &ch)).To(MatchError(ContainSubstring("requires: cannot use a mix of virtual and non-virtual channels in calculations")))
		})
		...
		It("Should return an error if all required channels do not share the same index", func() {
			...
			Expect(mockCluster.Nodes[1].Channels.Create(ctx, &ch)).To(MatchError(ContainSubstring("requires: all required channels must share the same index")))
		})
