...
var _ = Describe("Logging", func() {
	DescribeTable("New", func(cfg ...distribution.Config) {
		...
		Expect(coreOne.Cluster.HostID()).To(Equal(core.NodeKey(1)))
		Expect(coreTwo.Cluster.HostID()).To(Equal(core.NodeKey(2)))
		Expect(coreThree.Cluster.HostID()).To(Equal(core.NodeKey(3)))
