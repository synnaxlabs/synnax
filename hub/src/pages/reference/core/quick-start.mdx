...
A standalone Core needs a license from your organization. A Core without one starts and
answers login and connectivity checks, but refuses everything else until a token is
activated. The Console shows an activation screen for it. The
[production guide](/reference/core/production#licensing) covers the activation flow and
the start flags that take a token.
