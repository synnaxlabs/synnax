...
<Table.Data
  ...
  data={[
    ...
    },
    {
      ...
      default: '""',
      description:
        'License token to activate at start. See <a href="/reference/core/production#licensing">Licensing</a>.',
    },
    {
      option: "--license-file",
      default: '""',
      description: "Path to a file that holds the license token to activate at start.",
    },
    {
      option: "--debug",
      default: "false",
    ...
    {
      option: "--stop-on-stdin-close",
      default: "false",
      description:
        "Stops the Core when its standard input closes. Use it when a parent process starts the Core and holds its standard input, so that the Core stops if the parent exits. Do not set it for a Core that runs as a service, because a service has no standard input.",
    },
    {
...
<Table.Data
  ...
  data={[
    ...
    { option: "--license-file", env: "SYNNAX_LICENSE_FILE" },
    ...
    { option: "--stop-on-stdin-close", env: "SYNNAX_STOP_ON_STDIN_CLOSE" },
...
```json
{
  ...
  "username": "synnax",
  "password": "seldon",
  "license-file": "/usr/local/synnax/synnax.license",
  "debug": false,
...
<Table.Data
  ...
  data={[
    ...
    },
    {
      field: "loopback",
      description:
        "Binds the listener to the loopback interface only, so no other machine can reach it. The address host must be <code>localhost</code> or <code>127.0.0.1</code>. By default, a listener binds its port on every network interface.",
    },
  ]}
/>
