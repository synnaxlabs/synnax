...
<Client.Tabs client:load exclude={["console", "cpp"]}>
  ...
  <Fragment slot="python">
    ...
    The `synnax` library requires Python 3.12 or higher, and is available on
...
To
[authenticate with a Synnax Core](/reference/client/authentication#authentication-methods),
simply instantiate a new client with your connection parameters and credentials:
...
[Create a channel](/reference/client/channels#creating-channels) using the
...
[Write data](/reference/client/write-data) to the time and temperature channels. The
index channel must be written to before writing to the data channel.
...
<Note.Note variant="info" className="compact">
  Data can also be read in chunks with [iterators](/reference/client/advanced/iterators)
  or live-streamed with [streamers](/reference/client/read-data#streaming-data). Data
  can also be read from a specific time range with [ranges](/reference/client/ranges).
</Note.Note>
