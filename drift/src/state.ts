...
/**
 * Sets the status of the window with the given key.
 *
...
export const setWindowStatus = (
  state: WindowState,
  key?: string
): DriftAction => actions.setWindowState({ state, key });
 ...
 * @returns an action that can be dispatched.
 */
export const closeWindow = (key?: string): DriftAction =>
  actions.closeWindow({ key });
 ...
 * @returns  an action that can be dispatched.
 */
export const setWindowKey = (key: string): DriftAction =>
...
>(
  ...
  switch (type) {
    case actions.createWindow.type: {
      ...
      if (!runtime.exists(key)) {
        runtime.createWindow(payload as KeyedWindowProps);
      } else {
        runtime.focus(key);
      }
      break;
    ...
    case actions.completeProcess.type: {
      ...
      // closure.
      if (readyToClose(1 /* threshold */, windows[key])) runtime.close(key);
      break;
    }
  }
};
