// Copyright 2026 Synnax Labs, Inc.
//
// Use of this software is governed by the Business Source License included in the file
// licenses/BSL.txt.
//
// As of the Change Date specified in that file, in accordance with the Business Source
// License, use of this software will be governed by the Apache License, Version 2.0,
// included in the file licenses/APL.txt.

import { afterEach, beforeEach, describe, expect, it, test } from "vitest";

import { binary } from "@/binary";
import { primitive } from "@/primitive";
import {
  telem } from "@/telem";

describe("TimeStamp", () => {
  test("construct", () => {
    const ts = new telem.TimeStamp(1000);
    expect(ts.equals(telem.TimeSpan.MICROSECOND)).toBe(true);
  });

  test("construct from NaN", () => {
    const ts = new telem.TimeStamp(NaN);
    expect(ts.isZero).toBe(true);
  });

  test("construct from infinity", () => {
    const ts = new telem.TimeStamp(Infinity);
    expect(ts.equals(telem.TimeStamp.MAX)).toBe(true);
  });

  test("construct from negative infinity", () => {
    const ts = new telem.TimeStamp(-Infinity);
    expect(ts.equals(telem.TimeStamp.MIN)).toBe(true);
  });

  test("construct from CrudeValueExtension", () => {
    const ts = new telem.TimeStamp({ value: 1000n });
    expect(ts.equals(1000)).toBe(true);
  });

  test("toString", () => {
    const ts = new telem.TimeStamp(telem.TimeSpan.days(90))
      .add(telem.TimeSpan.minutes(20))
      .add(telem.TimeSpan.milliseconds(283))
      .add(telem.TimeSpan.microseconds(900));
    const tsString = ts.toString();
    expect(tsString).toEqual("1970-04-01T00:20:00.283Z");
  });

  test("encode", () => {
    const ts = telem.TimeStamp.now();
    new binary.JSONCodec().encode(ts);
  });

  test("construct from TimeStamp", () => {
    const ts = new telem.TimeStamp(telem.TimeSpan.microseconds(10));
    expect(ts.equals(telem.TimeSpan.microseconds(10))).toBe(true);
  });

  test("construct from local TimeZone", () => {
    const ts = new telem.TimeStamp(telem.TimeSpan.microseconds(10), "local");
    expect(ts.equals(telem.TimeSpan.microseconds(10).add(telem.TimeStamp.utcOffset))).toBe(true);
  });

  test("constructing from MIN and MAX as numbers", () => {
    expect(new telem.TimeStamp(telem.TimeStamp.MIN.nanoseconds).equals(telem.TimeStamp.MIN)).toBe(true);
    expect(new telem.TimeStamp(telem.TimeStamp.MAX.nanoseconds).equals(telem.TimeStamp.MAX)).toBe(true);
  });

  test("construct from time string", () => {
    const ts = new telem.TimeStamp("12:30", "UTC");
    expect(ts.date().getUTCHours()).toEqual(12);
    expect(ts.equals(telem.TimeSpan.hours(12).add(telem.TimeSpan.minutes(30)))).toBe(true);
    const ts2 = new telem.TimeStamp("12:30:00.22");
    expect(
      ts2.equals(
        telem.TimeSpan.hours(12).add(telem.TimeSpan.minutes(30).add(telem.TimeSpan.milliseconds(22))),
      ),
    ).toBe(true);
    const ts3 = new telem.TimeStamp("12:30:00.22", "local");
    expect(
      ts3.equals(
        telem.TimeSpan.hours(12)
          .add(telem.TimeSpan.minutes(30).add(telem.TimeSpan.milliseconds(22)))
          .add(telem.TimeStamp.utcOffset),
      ),
    ).toBe(true);
  });

  describe("datetime-local format parsing", () => {
    test("should parse as UTC by default", () => {
      const ts = new telem.TimeStamp("2025-11-03T17:44:45.500");
      const utcTS = new telem.TimeStamp("2025-11-03T17:44:45.500Z");

      expect(ts.valueOf()).toEqual(utcTS.valueOf());
    });

    test("should handle 1-digit milliseconds with default UTC", () => {
      const ts = new telem.TimeStamp("2025-11-03T17:44:45.5");
      expect(ts.millisecond).toBe(500);
    });

    test("should handle 1-digit milliseconds with local", () => {
      const ts = new telem.TimeStamp("2025-11-03T17:44:45.5", "local");
      expect(ts.millisecond).toBe(500);
    });

    test("should handle 2-digit milliseconds", () => {
      const ts = new telem.TimeStamp("2025-11-03T17:44:45.50");
      expect(ts.millisecond).toBe(500);
    });

    test("should handle 3-digit milliseconds", () => {
      const ts = new telem.TimeStamp("2025-11-03T17:44:45.809");
      expect(ts.millisecond).toBe(809);
    });

    test("should handle 810 milliseconds", () => {
      const ts = new telem.TimeStamp("2025-11-03T17:44:45.810");
      expect(ts.millisecond).toBeGreaterThanOrEqual(809);
      expect(ts.millisecond).toBeLessThanOrEqual(810);
    });

    test("should handle datetime without milliseconds", () => {
      const ts = new telem.TimeStamp("2025-11-03T17:44:45");
      expect(ts.millisecond).toBe(0);
    });

    test("should round-trip when using local tzInfo", () => {
      const input = "2025-11-03T17:44:45.809";
      const ts1 = new telem.TimeStamp(input, "local");
      const output = ts1.toString("ISO", "local").slice(0, -1);
      const ts2 = new telem.TimeStamp(output, "local");

      expect(ts1.valueOf()).toEqual(ts2.valueOf());
    });

    test("should round-trip across DST boundaries", () => {
      // 2025-03-09 is US DST spring-forward, 2025-11-02 is US DST fall-back. These
      // dates may have a different UTC offset than the current date, so we verify the
      // round-trip works regardless of DST transitions.
      const dstDates = [
        "2025-03-09T12:00:00.000",
        "2025-03-10T12:00:00.000",
        "2025-11-02T12:00:00.000",
        "2025-11-03T12:00:00.000",
        "2025-06-15T12:00:00.000",
        "2025-01-15T12:00:00.000",
      ];
      for (const input of dstDates) {
        const ts1 = new telem.TimeStamp(input, "local");
        const output = ts1.toString("ISO", "local").slice(0, -1);
        const ts2 = new telem.TimeStamp(output, "local");
        expect(ts1.valueOf()).toEqual(ts2.valueOf());
      }
    });
  });

  test("construct from date", () => {
    const ts = new telem.TimeStamp([2021, 1, 1], "UTC");
    expect(ts.date().getUTCFullYear()).toEqual(2021);
    expect(ts.date().getUTCMonth()).toEqual(0);
    expect(ts.date().getUTCDate()).toEqual(1);
    expect([0, 1]).toContain(ts.date().getUTCHours());
    expect(ts.date().getUTCMinutes()).toEqual(0);
  });

  test("construct from date time string", () => {
    const ts = new telem.TimeStamp("2021-01-01T00:00:00.000Z", "UTC");
    expect(ts.date().getUTCFullYear()).toEqual(2021);
    expect(ts.date().getUTCHours()).toEqual(0);
    const ts2 = new telem.TimeStamp("2021-01-01", "local");
    expect(ts2.date().getUTCFullYear()).toEqual(2021);
    expect(ts2.date().getUTCHours()).toEqual(
      Number(telem.TimeStamp.utcOffset.valueOf() / telem.TimeStamp.HOUR.valueOf()),
    );
    expect(ts2.date().getUTCMinutes()).toEqual(0);
  });

  describe("equals", () => {
    it("should return true when comparing two equal TimeStamps", () => {
      const ts1 = new telem.TimeStamp(1000);
      const ts2 = new telem.TimeStamp(1000);
      expect(ts1.equals(ts2)).toBe(true);
    });

    it("should handle an object with a 'value' property", () => {
      const ts1 = new telem.TimeStamp(1000);
      const ts2 = { value: 1000n };
      expect(ts1.equals(ts2)).toBe(true);
    });

    it("should return false when comparing two different TimeStamps", () => {
      const ts1 = new telem.TimeStamp(1000);
      const ts2 = new telem.TimeStamp(2000);
      expect(ts1.equals(ts2)).toBe(false);
    });

    it("should handle comparison with bigint values", () => {
      const ts = new telem.TimeStamp(1000n);
      expect(ts.equals(1000n)).toBe(true);
      expect(ts.equals(2000n)).toBe(false);
    });

    it("should handle comparison with number values", () => {
      const ts = new telem.TimeStamp(1000);
      expect(ts.equals(1000)).toBe(true);
      expect(ts.equals(2000)).toBe(false);
    });

    it("should handle comparison with TimeSpan values", () => {
      const ts = new telem.TimeStamp(telem.TimeSpan.microseconds(500));
      expect(ts.equals(telem.TimeSpan.microseconds(500))).toBe(true);
      expect(ts.equals(telem.TimeSpan.microseconds(600))).toBe(false);
    });

    it("should handle comparison with another TimeStamp instance", () => {
      const ts1 = new telem.TimeStamp(telem.TimeSpan.seconds(10));
      const ts2 = new telem.TimeStamp(telem.TimeSpan.seconds(10));
      const ts3 = new telem.TimeStamp(telem.TimeSpan.seconds(20));
      expect(ts1.equals(ts2)).toBe(true);
      expect(ts1.equals(ts3)).toBe(false);
    });

    it("should handle comparison with Date objects", () => {
      const date = new Date("2024-01-15T10:30:00.000Z");
      const ts = new telem.TimeStamp(date);
      expect(ts.equals(date)).toBe(true);

      const differentDate = new Date("2024-01-16T10:30:00.000Z");
      expect(ts.equals(differentDate)).toBe(false);
    });

    it("should handle comparison with DateComponents arrays", () => {
      const ts = new telem.TimeStamp([2024, 3, 15], "UTC");
      const sameComponents: [number, number, number] = [2024, 3, 15];
      const differentComponents: [number, number, number] = [2024, 3, 16];

      expect(ts.equals(new telem.TimeStamp(sameComponents, "UTC"))).toBe(true);
      expect(ts.equals(new telem.TimeStamp(differentComponents, "UTC"))).toBe(false);
    });

    it("should handle comparison with string representations", () => {
      const ts = new telem.TimeStamp("2021-01-01T00:00:00.000Z", "UTC");
      const sameString = "2021-01-01T00:00:00.000Z";
      const differentString = "2021-01-02T00:00:00.000Z";

      expect(ts.equals(new telem.TimeStamp(sameString, "UTC"))).toBe(true);
      expect(ts.equals(new telem.TimeStamp(differentString, "UTC"))).toBe(false);
    });

    it("should handle edge case comparisons", () => {
      // Zero values
      const zeroTs = new telem.TimeStamp(0);
      expect(zeroTs.equals(0)).toBe(true);
      expect(zeroTs.equals(telem.TimeStamp.ZERO)).toBe(true);
      expect(zeroTs.equals(0n)).toBe(true);

      // MAX values
      const maxTs = telem.TimeStamp.MAX;
      expect(maxTs.equals(telem.TimeStamp.MAX)).toBe(true);
      expect(maxTs.equals(new telem.TimeStamp(Infinity))).toBe(true);

      // MIN values
      const minTs = telem.TimeStamp.MIN;
      expect(minTs.equals(telem.TimeStamp.MIN)).toBe(true);
      expect(minTs.equals(new telem.TimeStamp(-Infinity))).toBe(true);
    });

    it("should handle NaN values", () => {
      const nanTs = new telem.TimeStamp(NaN);
      expect(nanTs.equals(0)).toBe(true); // NaN is converted to zero
      expect(nanTs.equals(telem.TimeStamp.ZERO)).toBe(true);
    });

    it("should handle negative values", () => {
      const negativeTs = new telem.TimeStamp(-1000);
      expect(negativeTs.equals(-1000)).toBe(true);
      expect(negativeTs.equals(-1000n)).toBe(true);
      expect(negativeTs.equals(new telem.TimeStamp(-1000))).toBe(true);
      expect(negativeTs.equals(1000)).toBe(false);
    });

    it("should handle large bigint values", () => {
      const largeValue = 9007199254740992n; // Larger than MAX_SAFE_INTEGER
      const ts = new telem.TimeStamp(largeValue);
      expect(ts.equals(largeValue)).toBe(true);
      expect(ts.equals(largeValue + 1n)).toBe(false);
    });

    it("should handle mixed precision comparisons", () => {
      // TimeStamp created from milliseconds vs microseconds
      const msTs = new telem.TimeStamp(telem.TimeSpan.milliseconds(1));
      const usTs = new telem.TimeStamp(telem.TimeSpan.microseconds(1000));
      expect(msTs.equals(usTs)).toBe(true);

      const differentUsTs = new telem.TimeStamp(telem.TimeSpan.microseconds(999));
      expect(msTs.equals(differentUsTs)).toBe(false);
    });

    it("should maintain transitivity", () => {
      // If a.equals(b) and b.equals(c), then a.equals(c)
      const a = new telem.TimeStamp(1000);
      const b = new telem.TimeStamp(telem.TimeSpan.nanoseconds(1000));
      const c = new telem.TimeStamp(1000n);

      expect(a.equals(b)).toBe(true);
      expect(b.equals(c)).toBe(true);
      expect(a.equals(c)).toBe(true);
    });

    it("should maintain reflexivity", () => {
      // a.equals(a) should always be true
      const ts = new telem.TimeStamp(telem.TimeSpan.seconds(42));
      expect(ts.equals(ts)).toBe(true);
    });

    it("should maintain symmetry", () => {
      // If a.equals(b), then b.equals(a)
      const ts1 = new telem.TimeStamp(1000);
      const ts2 = new telem.TimeStamp(1000);

      expect(ts1.equals(ts2)).toBe(true);
      expect(ts2.equals(ts1)).toBe(true);

      const ts3 = new telem.TimeStamp(2000);
      expect(ts1.equals(ts3)).toBe(false);
      expect(ts3.equals(ts1)).toBe(false);
    });

    it("should handle comparison with complex TimeSpan arithmetic", () => {
      const ts = new telem.TimeStamp(
        telem.TimeSpan.hours(1).add(telem.TimeSpan.minutes(30)).add(telem.TimeSpan.seconds(45)),
      );
      const equivalentSpan = telem.TimeSpan.seconds(5445); // 1h 30m 45s = 5445s
      expect(ts.equals(equivalentSpan)).toBe(true);
    });
  });

  describe("schema", () => {
    it("should parse bigint", () => {
      const ts = telem.TimeStamp.z.parse(1000000000n);
      expect(ts).toBeInstanceOf(telem.TimeStamp);
      expect(ts.valueOf()).toBe(1000000000n);
    });

    it("should parse Date object", () => {
      const date = new Date("2024-01-15T10:30:00.000Z");
      const ts = telem.TimeStamp.z.parse(date);
      expect(ts).toBeInstanceOf(telem.TimeStamp);
      expect(ts.valueOf()).toBe(BigInt(date.getTime()) * 1000000n);
    });

    it("should parse TimeSpan", () => {
      const span = new telem.TimeSpan(5000000000n);
      const ts = telem.TimeStamp.z.parse(span);
      expect(ts).toBeInstanceOf(telem.TimeStamp);
      expect(ts.valueOf()).toBe(5000000000n);
    });

    it("should parse DateComponents array", () => {
      const ts = telem.TimeStamp.z.parse([2024, 3, 15]);
      expect(ts).toBeInstanceOf(telem.TimeStamp);
      const expected = new telem.TimeStamp([2024, 3, 15]);
      expect(ts.valueOf()).toBe(expected.valueOf());
    });

    it("should parse DateComponents with missing elements", () => {
      const ts1 = telem.TimeStamp.z.parse([2024]);
      expect(ts1).toBeInstanceOf(telem.TimeStamp);
      expect(ts1.valueOf()).toBe(new telem.TimeStamp([2024, 1, 1]).valueOf());

      const ts2 = telem.TimeStamp.z.parse([2024, 6]);
      expect(ts2).toBeInstanceOf(telem.TimeStamp);
      expect(ts2.valueOf()).toBe(new telem.TimeStamp([2024, 6, 1]).valueOf());
    });

    it("should parse string representation of bigint", () => {
      const ts = telem.TimeStamp.z.parse("123456789000");
      expect(ts).toBeInstanceOf(telem.TimeStamp);
      expect(ts.valueOf()).toBe(123456789000n);
    });

    it("should parse number", () => {
      const ts = telem.TimeStamp.z.parse(987654321);
      expect(ts).toBeInstanceOf(telem.TimeStamp);
      expect(ts.valueOf()).toBe(987654321n);
    });

    it("should parse object with value property", () => {
      const ts = telem.TimeStamp.z.parse({ value: 555555555n });
      expect(ts).toBeInstanceOf(telem.TimeStamp);
      expect(ts.valueOf()).toBe(555555555n);
    });

    it("should pass through existing TimeStamp instance", () => {
      const original = new telem.TimeStamp(777777777n);
      const ts = telem.TimeStamp.z.parse(original);
      expect(ts).toBe(original);
      expect(ts.valueOf()).toBe(777777777n);
    });

    it("should handle edge cases", () => {
      const ts1 = telem.TimeStamp.z.parse(0);
      expect(ts1.valueOf()).toBe(0n);

      const ts2 = telem.TimeStamp.z.parse(Number.MAX_SAFE_INTEGER);
      expect(ts2.valueOf()).toBe(BigInt(Number.MAX_SAFE_INTEGER));
    });
  });

  test("span", () => {
    const ts = new telem.TimeStamp(0);
    expect(ts.span(new telem.TimeStamp(1000)).equals(telem.TimeSpan.microseconds())).toBe(true);
  });

  test("range", () => {
    const ts = new telem.TimeStamp(0);
    expect(
      ts.range(new telem.TimeStamp(1000)).equals(new telem.TimeRange(ts, telem.TimeSpan.microseconds())),
    ).toBe(true);
  });

  test("spanRange", () => {
    const ts = new telem.TimeStamp(0);
    expect(
      ts
        .spanRange(telem.TimeSpan.microseconds())
        .equals(new telem.TimeRange(ts, ts.add(telem.TimeSpan.microseconds()))),
    ).toBe(true);
  });

  test("isZero", () => {
    const ts = new telem.TimeStamp(0);
    expect(ts.isZero).toBe(true);
  });

  test("after", () => {
    const ts = new telem.TimeStamp(0);
    expect(ts.after(new telem.TimeStamp(-1))).toBe(true);
    const ts2 = new telem.TimeStamp(1);
    expect(ts2.after(new telem.TimeStamp(1))).toBe(false);
  });

  test("before", () => {
    const ts = new telem.TimeStamp(0);
    expect(ts.before(new telem.TimeStamp(1))).toBe(true);
    const ts2 = new telem.TimeStamp(1);
    expect(ts2.before(new telem.TimeStamp(1))).toBe(false);
  });

  test("beforeEq", () => {
    const ts = new telem.TimeStamp(0);
    expect(ts.beforeEq(new telem.TimeStamp(1))).toBe(true);
    const ts2 = new telem.TimeStamp(1);
    expect(ts2.beforeEq(new telem.TimeStamp(1))).toBe(true);
    const ts3 = new telem.TimeStamp(2);
    expect(ts3.beforeEq(new telem.TimeStamp(1))).toBe(false);
  });

  test("afterEq", () => {
    const ts = new telem.TimeStamp(0);
    expect(ts.afterEq(new telem.TimeStamp(-1))).toBe(true);
    const ts2 = new telem.TimeStamp(1);
    expect(ts2.afterEq(new telem.TimeStamp(1))).toBe(true);
    const ts3 = new telem.TimeStamp(0);
    expect(ts3.afterEq(new telem.TimeStamp(1))).toBe(false);
  });

  test("add", () => {
    const ts = new telem.TimeStamp(0);
    expect(
      ts.add(telem.TimeSpan.microseconds()).equals(new telem.TimeStamp(telem.TimeSpan.microseconds(1))),
    ).toBe(true);
  });

  test("sub", () => {
    const ts = new telem.TimeStamp(telem.TimeSpan.microseconds());
    expect(ts.sub(telem.TimeSpan.microseconds()).equals(new telem.TimeStamp(0))).toBe(true);
  });

  describe("arithmetic operations", () => {
    test("add with TimeSpan", () => {
      const ts = new telem.TimeStamp(1000);
      const result = ts.add(telem.TimeSpan.microseconds(500));
      expect(result).toBeInstanceOf(telem.TimeStamp);
      expect(result.valueOf()).toBe(501000n);
    });

    test("add with number", () => {
      const ts = new telem.TimeStamp(1000);
      const result = ts.add(500);
      expect(result).toBeInstanceOf(telem.TimeStamp);
      expect(result.valueOf()).toBe(1500n);
    });

    test("add with bigint", () => {
      const ts = new telem.TimeStamp(1000n);
      const result = ts.add(500n);
      expect(result).toBeInstanceOf(telem.TimeStamp);
      expect(result.valueOf()).toBe(1500n);
    });

    test("sub with TimeSpan", () => {
      const ts = new telem.TimeStamp(1000);
      const result = ts.sub(telem.TimeSpan.nanoseconds(300));
      expect(result).toBeInstanceOf(telem.TimeStamp);
      expect(result.valueOf()).toBe(700n);
    });

    test("sub with number", () => {
      const ts = new telem.TimeStamp(1000);
      const result = ts.sub(300);
      expect(result).toBeInstanceOf(telem.TimeStamp);
      expect(result.valueOf()).toBe(700n);
    });

    test("sub with bigint", () => {
      const ts = new telem.TimeStamp(1000n);
      const result = ts.sub(300n);
      expect(result).toBeInstanceOf(telem.TimeStamp);
      expect(result.valueOf()).toBe(700n);
    });
  });

  describe("toString with formats", () => {
    const ts = new telem.TimeStamp([2022, 12, 15], "UTC")
      .add(telem.TimeSpan.hours(12))
      .add(telem.TimeSpan.minutes(20))
      .add(telem.TimeSpan.milliseconds(12));

    const FORMAT_TESTS: [telem.TimeStampStringFormat, string][] = [
      ["ISO", "2022-12-15T12:20:00.012Z"],
      ["ISODate", "2022-12-15"],
      ["ISOTime", "12:20:00.012"],
      ["time", "12:20:00"],
      ["preciseTime", "12:20:00.012"],
      ["date", "Dec 15"],
      ["preciseDate", "Dec 15 12:20:00.012"],
      ["dateTime", "Dec 15 12:20:00"],
    ];

    FORMAT_TESTS.forEach(([format, expected]) => {
      test(`should format timestamp as ${format}`, () => {
        expect(ts.toString(format, "UTC")).toEqual(expected);
      });
    });
  });

  describe("unit getters", () => {
    test("hour", () => {
      expect(telem.TimeStamp.hours(1).add(telem.TimeSpan.minutes(30)).hour).toEqual(1);
    });
    test("hours", () => {
      expect(telem.TimeStamp.hours(1).add(telem.TimeSpan.minutes(30)).hours).toEqual(1.5);
    });
    test("minute", () => {
      expect(telem.TimeStamp.minutes(1).add(telem.TimeStamp.seconds(20)).minute).toEqual(1);
    });
    test("minutes", () => {
      expect(telem.TimeStamp.minutes(1).add(telem.TimeStamp.seconds(30)).minutes).toEqual(1.5);
    });
    test("second", () => {
      expect(telem.TimeStamp.seconds(1).add(telem.TimeStamp.milliseconds(20)).second).toEqual(1);
    });
    test("seconds", () => {
      expect(telem.TimeStamp.seconds(1).add(telem.TimeStamp.milliseconds(500)).seconds).toEqual(
        1.5,
      );
    });
    test("millisecond", () => {
      expect(
        telem.TimeStamp.milliseconds(1).add(telem.TimeStamp.microseconds(20)).millisecond,
      ).toEqual(1);
    });
    test("milliseconds", () => {
      expect(
        telem.TimeStamp.milliseconds(1).add(telem.TimeStamp.microseconds(500)).milliseconds,
      ).toEqual(1.5);
    });
    test("microseconds", () => {
      expect(
        telem.TimeStamp.microseconds(500).add(telem.TimeSpan.nanoseconds(20)).microseconds,
      ).toEqual(500.02);
    });
    test("nanoseconds", () => {
      expect(
        telem.TimeStamp.microseconds(1).add(telem.TimeSpan.nanoseconds(30)).nanoseconds,
      ).toEqual(1030);
    });
    test("year", () => {
      expect(new telem.TimeStamp([2022, 12, 15]).year).toEqual(2022);
    });
    test("month", () => {
      expect(new telem.TimeStamp([2022, 12, 15]).month).toEqual(11);
    });
    test("day", () => {
      expect(new telem.TimeStamp([2022, 12, 15], "UTC").day).toEqual(15);
    });
  });

  describe("unit setters", () => {
    test("setYear", () => {
      const ts = new telem.TimeStamp([2022, 12, 15]);
      const updated = ts.setYear(2023);
      expect(updated.year).toEqual(2023);
      expect(updated.month).toEqual(ts.month); // Other components should remain unchanged
      expect(updated.day).toEqual(ts.day);
    });

    test("setMonth", () => {
      const ts = new telem.TimeStamp([2022, 12, 15]);
      const updated = ts.setMonth(5); // June (0-indexed)
      expect(updated.month).toEqual(5);
      expect(updated.year).toEqual(ts.year); // Other components should remain unchanged
      expect(updated.day).toEqual(ts.day);
    });

    test("setDay", () => {
      const ts = new telem.TimeStamp([2022, 12, 15]);
      const updated = ts.setDay(20);
      expect(updated.day).toEqual(20);
      expect(updated.year).toEqual(ts.year);
      expect(updated.month).toEqual(ts.month);
    });

    test("setHour", () => {
      const ts = new telem.TimeStamp([2022, 12, 15]).add(telem.TimeSpan.hours(10));
      const updated = ts.setHour(15);
      expect(updated.hour).toEqual(15);
      expect(updated.year).toEqual(ts.year);
      expect(updated.month).toEqual(ts.month);
      expect(updated.day).toEqual(ts.day);
    });

    test("setMinute", () => {
      const ts = new telem.TimeStamp([2022, 12, 15])
        .add(telem.TimeSpan.hours(10))
        .add(telem.TimeSpan.minutes(30));
      const updated = ts.setMinute(45);
      expect(updated.minute).toEqual(45);
      expect(updated.hour).toEqual(ts.hour);
      expect(updated.year).toEqual(ts.year);
      expect(updated.month).toEqual(ts.month);
      expect(updated.day).toEqual(ts.day);
    });

    test("setSecond", () => {
      const ts = new telem.TimeStamp([2022, 12, 15])
        .add(telem.TimeSpan.hours(10))
        .add(telem.TimeSpan.minutes(30))
        .add(telem.TimeSpan.seconds(20));
      const updated = ts.setSecond(45);
      expect(updated.second).toEqual(45);
      expect(updated.minute).toEqual(ts.minute); // Other components should remain unchanged
      expect(updated.hour).toEqual(ts.hour);
      expect(updated.year).toEqual(ts.year);
      expect(updated.month).toEqual(ts.month);
      expect(updated.day).toEqual(ts.day);
    });

    test("setMillisecond", () => {
      const ts = new telem.TimeStamp([2022, 12, 15])
        .add(telem.TimeSpan.hours(10))
        .add(telem.TimeSpan.minutes(30))
        .add(telem.TimeSpan.seconds(20))
        .add(telem.TimeSpan.milliseconds(100));
      const updated = ts.setMillisecond(500);
      expect(updated.millisecond).toEqual(500);
      expect(updated.second).toEqual(ts.second); // Other components should remain unchanged
      expect(updated.minute).toEqual(ts.minute);
      expect(updated.hour).toEqual(ts.hour);
      expect(updated.year).toEqual(ts.year);
      expect(updated.month).toEqual(ts.month);
      expect(updated.day).toEqual(ts.day);
    });

    test("localHour", () => {
      const ts = new telem.TimeStamp([2022, 12, 15]).add(telem.TimeSpan.hours(10));
      const localHour = ts.localHour;
      const expectedLocalHour = ts.date().getHours();
      expect(localHour).toEqual(expectedLocalHour);
      const utcHour = ts.hour;
      const tzOffsetHours = new Date().getTimezoneOffset() / 60;
      if (tzOffsetHours !== 0) expect(localHour).not.toEqual(utcHour);
    });

    test("setLocalHour", () => {
      const ts = new telem.TimeStamp([2022, 12, 15])
        .add(telem.TimeSpan.hours(10))
        .add(telem.TimeSpan.minutes(30))
        .add(telem.TimeSpan.seconds(20));
      const targetLocalHour = 15;
      const updated = ts.setLocalHour(targetLocalHour);
      expect(updated.localHour).toEqual(targetLocalHour);
      expect(updated.date().getMinutes()).toEqual(ts.date().getMinutes());
      expect(updated.date().getSeconds()).toEqual(ts.date().getSeconds());
      expect(updated.year).toEqual(ts.year);
      expect(updated.month).toEqual(ts.month);
      expect(updated.day).toEqual(ts.day);
    });

    test("setLocalHour edge cases", () => {
      // Test setting hour to 0 (midnight)
      const ts1 = new telem.TimeStamp([2022, 12, 15]).add(telem.TimeSpan.hours(12));
      const midnight = ts1.setLocalHour(0);
      expect(midnight.localHour).toEqual(0);

      // Test setting hour to 23 (11 PM)
      const ts2 = new telem.TimeStamp([2022, 12, 15]).add(telem.TimeSpan.hours(12));
      const elevenPM = ts2.setLocalHour(23);
      expect(elevenPM.localHour).toEqual(23);

      // Test that setting local hour might change the day in UTC
      const ts3 = new telem.TimeStamp([2022, 12, 15]);
      const updated = ts3.setLocalHour(23);
      // Depending on timezone, this might be a different day in UTC
      expect(updated.localHour).toEqual(23);
    });

    test("localHour and setLocalHour round trip", () => {
      // Test that getting and setting local hour is consistent
      const ts = new telem.TimeStamp([2022, 12, 15])
        .add(telem.TimeSpan.hours(10))
        .add(telem.TimeSpan.minutes(30));

      const originalLocalHour = ts.localHour;
      const roundTrip = ts.setLocalHour(originalLocalHour);

      // The timestamp should remain effectively the same
      expect(roundTrip.localHour).toEqual(originalLocalHour);
      expect(roundTrip.date().getMinutes()).toEqual(ts.date().getMinutes());
      expect(roundTrip.date().getSeconds()).toEqual(ts.date().getSeconds());

      // The valueOf() might differ slightly due to milliseconds precision
      // but should be within 1 second
      const diff = Math.abs(Number(roundTrip.valueOf() - ts.valueOf()));
      expect(diff).toBeLessThan(1000000000); // Less than 1 second in nanoseconds
    });

    describe("timezone sensitive", () => {
      let originalTZ: string | undefined;
      beforeEach(() => {
        originalTZ = process.env.TZ;
        process.env.TZ = "Pacific/Auckland";
      });
      afterEach(() => {
        process.env.TZ = originalTZ;
      });

      test("localYear", () => {
        // 2022-12-31T23:00:00Z -> Auckland (UTC+13): 2023-01-01T12:00:00
        const ts = new telem.TimeStamp(Date.UTC(2022, 11, 31, 23, 0, 0, 0) * 1e6);
        expect(ts.year).toEqual(2022);
        expect(ts.localYear).toEqual(2023);
      });

      test("setLocalYear", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 30, 20, 0) * 1e6);
        const updated = ts.setLocalYear(2025);
        expect(updated.localYear).toEqual(2025);
        expect(updated.localMonth).toEqual(ts.localMonth);
        expect(updated.localDay).toEqual(ts.localDay);
        expect(updated.localHour).toEqual(ts.localHour);
      });

      test("localMonth", () => {
        // 2022-06-30T23:00:00Z -> Auckland (UTC+12): 2022-07-01T11:00:00
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 30, 23, 0, 0, 0) * 1e6);
        expect(ts.month).toEqual(5); // June (0-indexed)
        expect(ts.localMonth).toEqual(6); // July (0-indexed)
      });

      test("setLocalMonth", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 30, 0, 0) * 1e6);
        const updated = ts.setLocalMonth(11);
        expect(updated.localMonth).toEqual(11);
        expect(updated.localDay).toEqual(ts.localDay);
        expect(updated.localHour).toEqual(ts.localHour);
      });

      test("localDay", () => {
        // 2022-06-15T23:00:00Z -> Auckland (UTC+12): 2022-06-16T11:00:00
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 23, 0, 0, 0) * 1e6);
        expect(ts.day).toEqual(15);
        expect(ts.localDay).toEqual(16);
      });

      test("setLocalDay", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 30, 0, 0) * 1e6);
        const updated = ts.setLocalDay(25);
        expect(updated.localDay).toEqual(25);
        expect(updated.localMonth).toEqual(ts.localMonth);
        expect(updated.localHour).toEqual(ts.localHour);
      });

      test("localMinute", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 30, 0, 0) * 1e6);
        expect(ts.localMinute).toEqual(ts.date().getMinutes());
      });

      test("setLocalMinute", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 0, 0, 0) * 1e6);
        const updated = ts.setLocalMinute(45);
        expect(updated.localMinute).toEqual(45);
        expect(updated.localHour).toEqual(ts.localHour);
        expect(updated.localSecond).toEqual(ts.localSecond);
      });

      test("localSecond", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 0, 42, 0) * 1e6);
        expect(ts.localSecond).toEqual(ts.date().getSeconds());
      });

      test("setLocalSecond", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 0, 0, 0) * 1e6);
        const updated = ts.setLocalSecond(30);
        expect(updated.localSecond).toEqual(30);
        expect(updated.localMinute).toEqual(ts.localMinute);
        expect(updated.localHour).toEqual(ts.localHour);
      });

      test("localMillisecond", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 0, 0, 500) * 1e6);
        expect(ts.localMillisecond).toEqual(ts.date().getMilliseconds());
      });

      test("setLocalMillisecond", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 0, 0, 0) * 1e6);
        const updated = ts.setLocalMillisecond(750);
        expect(updated.localMillisecond).toEqual(750);
        expect(updated.localSecond).toEqual(ts.localSecond);
        expect(updated.localHour).toEqual(ts.localHour);
      });

      test("local getters and setters round trip", () => {
        const ts = new telem.TimeStamp(Date.UTC(2022, 5, 15, 10, 30, 20, 500) * 1e6);
        const roundTrip = ts
          .setLocalYear(ts.localYear)
          .setLocalMonth(ts.localMonth)
          .setLocalDay(ts.localDay)
          .setLocalHour(ts.localHour)
          .setLocalMinute(ts.localMinute)
          .setLocalSecond(ts.localSecond)
          .setLocalMillisecond(ts.localMillisecond);

        expect(roundTrip.localYear).toEqual(ts.localYear);
        expect(roundTrip.localMonth).toEqual(ts.localMonth);
        expect(roundTrip.localDay).toEqual(ts.localDay);
        expect(roundTrip.localHour).toEqual(ts.localHour);
        expect(roundTrip.localMinute).toEqual(ts.localMinute);
        expect(roundTrip.localSecond).toEqual(ts.localSecond);
        expect(roundTrip.localMillisecond).toEqual(ts.localMillisecond);
      });
    });
  });

  describe("remainder", () => {
    test("day", () => {
      const expectedRemainder = telem.TimeStamp.hours(12)
        .add(telem.TimeSpan.minutes(20))
        .add(telem.TimeSpan.milliseconds(12));
      const ts = new telem.TimeStamp([2022, 12, 15])
        .add(telem.TimeStamp.hours(12))
        .add(telem.TimeSpan.minutes(20))
        .add(telem.TimeSpan.milliseconds(12));
      const remainder = ts.remainder(telem.TimeStamp.DAY);
      expect(
        remainder.equals(expectedRemainder),
        `expected ${new telem.TimeSpan(expectedRemainder).toString()} got ${new telem.TimeSpan(
          remainder,
        ).toString()}`,
      ).toBe(true);
    });
    test("second", () => {
      const expectedRemainder = telem.TimeSpan.milliseconds(12);
      const ts = new telem.TimeStamp([2022, 12, 15])
        .add(telem.TimeStamp.hours(12))
        .add(telem.TimeSpan.minutes(20))
        .add(telem.TimeSpan.milliseconds(12));
      const remainder = ts.remainder(telem.TimeSpan.seconds());
      expect(remainder.equals(expectedRemainder)).toBe(true);
    });
  });

  describe("sort", () => {
    interface Spec {
      a: telem.TimeStamp;
      b: telem.TimeStamp;
      expected: number;
    }
    const TESTS: Spec[] = [
      {
        a: telem.TimeStamp.seconds(3),
        b: telem.TimeStamp.seconds(2),
        expected: telem.TimeSpan.seconds(1).nanoseconds,
      },
      {
        a: telem.TimeStamp.seconds(2),
        b: telem.TimeStamp.seconds(3),
        expected: telem.TimeSpan.seconds(-1).nanoseconds,
      },
      { a: telem.TimeStamp.seconds(2), b: telem.TimeStamp.seconds(2), expected: 0 },
    ];
    TESTS.forEach(({ a, b, expected }) => {
      test(`TimeStamp.sort(${a.toString()}, ${b.toString()}) = ${expected}`, () => {
        expect(telem.TimeStamp.sort(a, b)).toEqual(expected);
      });
    });
  });

  describe("formatBySpan", () => {
    test("should return 'shortDate' for spans >= 30 days", () => {
      const ts = new telem.TimeStamp([2022, 12, 15], "UTC");
      const span = telem.TimeSpan.days(30);
      expect(ts.formatBySpan(span)).toBe("shortDate");
    });

    test("should return 'dateTime' for spans >= 1 day", () => {
      const ts = new telem.TimeStamp([2022, 12, 15], "UTC");
      const span = telem.TimeSpan.days(1);
      expect(ts.formatBySpan(span)).toBe("dateTime");
    });

    test("should return 'time' for spans >= 1 hour", () => {
      const ts = new telem.TimeStamp([2022, 12, 15], "UTC");
      const span = telem.TimeSpan.hours(1);
      expect(ts.formatBySpan(span)).toBe("time");
    });

    test("should return 'preciseTime' for spans >= 1 second", () => {
      const ts = new telem.TimeStamp([2022, 12, 15], "UTC");
      const span = telem.TimeSpan.seconds(1);
      expect(ts.formatBySpan(span)).toBe("preciseTime");
    });

    test("should return 'ISOTime' for spans < 1 second", () => {
      const ts = new telem.TimeStamp([2022, 12, 15], "UTC");
      const span = telem.TimeSpan.milliseconds(500);
      expect(ts.formatBySpan(span)).toBe("ISOTime");
    });

    test("should work with very small spans", () => {
      const ts = new telem.TimeStamp([2022, 12, 15], "UTC");
      const span = telem.TimeSpan.microseconds(100);
      expect(ts.formatBySpan(span)).toBe("ISOTime");
    });
  });

  describe("hash", () => {
    it("returns the bigint nanosecond value as a string", () => {
      expect(new telem.TimeStamp(1234567890n).hash()).toBe("1234567890");
    });

    it("is stable across instances representing the same value", () => {
      expect(new telem.TimeStamp(42n).hash()).toBe(new telem.TimeStamp(42n).hash());
    });

    it("differs across distinct values", () => {
      expect(new telem.TimeStamp(1n).hash()).not.toBe(new telem.TimeStamp(2n).hash());
    });

    it("satisfies primitive.isHashable", () => {
      expect(primitive.isHashable(new telem.TimeStamp(0n))).toBe(true);
    });
  });
});

describe("TimeSpan", () => {
  test("construct from static", () => {
    expect(telem.TimeSpan.nanoseconds(1).equals(1)).toBe(true);
    expect(telem.TimeSpan.microseconds(1).equals(1000)).toBe(true);
    expect(telem.TimeSpan.milliseconds(1).equals(1000000)).toBe(true);
    expect(telem.TimeSpan.seconds(1).equals(1e9)).toBe(true);
    expect(telem.TimeSpan.minutes(1).equals(6e10)).toBe(true);
    expect(telem.TimeSpan.hours(1).equals(36e11)).toBe(true);
  });

  test("construct from CrudeValueExtension", () => {
    const ts = new telem.TimeSpan({ value: 1000n });
    expect(ts.equals(1000)).toBe(true);
  });

  describe("fromMilliseconds", () => {
    it("should interpret a pure number or bigint as milliseconds", () => {
      const ts = telem.TimeSpan.fromMilliseconds(1000);
      expect(ts.equals(telem.TimeSpan.seconds())).toBe(true);
    });
    it("should interpret a TimeSpan as a normal TimeSpan", () => {
      const ts = telem.TimeSpan.fromMilliseconds(telem.TimeSpan.milliseconds(30));
      expect(ts.equals(telem.TimeSpan.milliseconds(30))).toBe(true);
    });
  });

  describe("fromSeconds", () => {
    it("should interpret a pure number or bigint as seconds", () => {
      const ts = telem.TimeSpan.fromSeconds(1);
      expect(ts.equals(telem.TimeSpan.SECOND)).toBe(true);
    });
    it("should interpret a TimeSpan as a normal TimeSpan", () => {
      const ts = telem.TimeSpan.fromSeconds(telem.TimeSpan.milliseconds(30));
      expect(ts.equals(telem.TimeSpan.milliseconds(30))).toBe(true);
    });
  });

  test("seconds", () => {
    expect(telem.TimeSpan.seconds(1).seconds).toEqual(1);
  });

  test("isZero", () => {
    expect(telem.TimeSpan.ZERO.isZero).toBe(true);
    expect(telem.TimeSpan.seconds(1).isZero).toBe(false);
  });

  test("add", () => {
    expect(telem.TimeSpan.seconds(1).add(telem.TimeSpan.SECOND).equals(2e9)).toBe(true);
  });

  test("sub", () => {
    expect(telem.TimeSpan.seconds(1).sub(telem.TimeSpan.SECOND).isZero).toBe(true);
  });

  describe("arithmetic operations", () => {
    test("add with TimeSpan", () => {
      const ts1 = new telem.TimeSpan(1000);
      const ts2 = new telem.TimeSpan(500);
      const result = ts1.add(ts2);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(1500n);
    });

    test("add with number", () => {
      const ts = new telem.TimeSpan(1000);
      const result = ts.add(500);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(1500n);
    });

    test("add with bigint", () => {
      const ts = new telem.TimeSpan(1000n);
      const result = ts.add(500n);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(1500n);
    });

    test("sub with TimeSpan", () => {
      const ts1 = new telem.TimeSpan(1000);
      const ts2 = new telem.TimeSpan(300);
      const result = ts1.sub(ts2);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(700n);
    });

    test("sub with number", () => {
      const ts = new telem.TimeSpan(1000);
      const result = ts.sub(300);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(700n);
    });

    test("sub with bigint", () => {
      const ts = new telem.TimeSpan(1000n);
      const result = ts.sub(300n);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(700n);
    });

    test("abs of positive value", () => {
      const ts = new telem.TimeSpan(1000);
      expect(ts.abs().valueOf()).toBe(1000n);
    });

    test("abs of negative value", () => {
      const ts = new telem.TimeSpan(-500);
      expect(ts.abs().valueOf()).toBe(500n);
    });

    test("abs of zero", () => {
      expect(telem.TimeSpan.ZERO.abs().valueOf()).toBe(0n);
    });

    test("abs returns TimeSpan instance", () => {
      const ts = new telem.TimeSpan(-1000);
      expect(ts.abs()).toBeInstanceOf(telem.TimeSpan);
    });

    test("mult", () => {
      const ts = new telem.TimeSpan(1000);
      const result = ts.mult(2);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(2000n);
    });

    test("mult with decimal", () => {
      const ts = new telem.TimeSpan(1000);
      const result = ts.mult(0.5);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(500n);
    });

    test("mult with negative", () => {
      const ts = new telem.TimeSpan(1000);
      const result = ts.mult(-2);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(-2000n);
    });

    test("div", () => {
      const ts = new telem.TimeSpan(1000);
      const result = ts.div(2);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(500n);
    });

    test("div with decimal", () => {
      const ts = new telem.TimeSpan(1000);
      const result = ts.div(0.5);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(2000n);
    });

    test("div resulting in fractional nanoseconds truncates", () => {
      const ts = new telem.TimeSpan(1001);
      const result = ts.div(2);
      expect(result).toBeInstanceOf(telem.TimeSpan);
      expect(result.valueOf()).toBe(500n); // 1001/2 = 500.5, truncated to 500
    });
  });

  const TRUNCATE_TESTS = [
    [telem.TimeSpan.days(1).add(telem.TimeSpan.nanoseconds(50)), telem.TimeSpan.DAY, telem.TimeSpan.days(1)],
    [telem.TimeSpan.hours(1).add(telem.TimeSpan.minutes(30)), telem.TimeSpan.HOUR, telem.TimeSpan.hours(1)],
  ];

  test("truncate", () => {
    TRUNCATE_TESTS.forEach(([ts, unit, expected]) => {
      expect(
        ts.truncate(unit).equals(expected),
        `expected ${expected.toString()} got ${ts.truncate(unit).toString()}`,
      ).toBe(true);
    });
  });

  const REMAINDER_TESTS = [
    [
      telem.TimeSpan.days(1).add(telem.TimeSpan.nanoseconds(50)),
      telem.TimeSpan.DAY,
      telem.TimeSpan.nanoseconds(50),
    ],
    [telem.TimeSpan.hours(1).add(telem.TimeSpan.minutes(30)), telem.TimeSpan.HOUR, telem.TimeSpan.minutes(30)],
  ];

  test("remainder", () => {
    REMAINDER_TESTS.forEach(([ts, unit, expected]) => {
      expect(ts.remainder(unit).equals(expected)).toBe(true);
    });
  });

  const TO_STRING_TESTS = [
    [telem.TimeSpan.nanoseconds(1), "1ns"],
    [telem.TimeSpan.microseconds(1), "1µs"],
    [telem.TimeSpan.milliseconds(1), "1ms"],
    [telem.TimeSpan.seconds(1), "1s"],
    [telem.TimeSpan.minutes(1), "1m"],
    [telem.TimeSpan.hours(1), "1h"],
    [telem.TimeSpan.days(1), "1d"],
    [
      telem.TimeSpan.milliseconds(1)
        .add(telem.TimeSpan.microseconds(500))
        .add(telem.TimeSpan.nanoseconds(50)),
      "1ms 500µs 50ns",
    ],
    [telem.TimeSpan.seconds(1).add(telem.TimeSpan.microseconds(500)), "1s 500µs"],
  ];

  test("toString", () => {
    TO_STRING_TESTS.forEach(([ts, expected]) => {
      expect(ts.toString()).toEqual(expected);
    });
  });

  describe("toString with semantic format", () => {
    const TESTS: [telem.TimeSpan, string][] = [
      // Sub-second durations
      [telem.TimeSpan.ZERO, "0s"],
      [telem.TimeSpan.nanoseconds(50), "< 1s"],
      [telem.TimeSpan.microseconds(50), "< 1s"],
      [telem.TimeSpan.milliseconds(50), "< 1s"],
      [telem.TimeSpan.milliseconds(999), "< 1s"],

      // Seconds
      [telem.TimeSpan.seconds(1), "1s"],
      [telem.TimeSpan.seconds(30), "30s"],
      [telem.TimeSpan.seconds(59), "59s"],

      // Minutes with seconds (< 5 minutes)
      [telem.TimeSpan.seconds(60), "1m"],
      [telem.TimeSpan.seconds(90), "1m 30s"],
      [telem.TimeSpan.seconds(119), "1m 59s"],
      [telem.TimeSpan.seconds(120), "2m"],
      [telem.TimeSpan.seconds(150), "2m 30s"],
      [telem.TimeSpan.seconds(240), "4m"],
      [telem.TimeSpan.seconds(270), "4m 30s"],

      // Minutes without seconds (>= 5 minutes)
      [telem.TimeSpan.seconds(300), "5m"],
      [telem.TimeSpan.seconds(330), "5m"], // seconds dropped
      [telem.TimeSpan.minutes(30), "30m"],
      [telem.TimeSpan.minutes(59), "59m"],

      // Hours with minutes (< 3 hours)
      [telem.TimeSpan.minutes(60), "1h"],
      [telem.TimeSpan.minutes(90), "1h 30m"],
      [telem.TimeSpan.minutes(119), "1h 59m"],
      [telem.TimeSpan.minutes(120), "2h"],
      [telem.TimeSpan.minutes(150), "2h 30m"],
      [telem.TimeSpan.minutes(179), "2h 59m"],

      // Hours without minutes (>= 3 hours)
      [telem.TimeSpan.minutes(180), "3h"],
      [telem.TimeSpan.minutes(195), "3h"], // minutes dropped
      [telem.TimeSpan.hours(12), "12h"],
      [telem.TimeSpan.hours(23), "23h"],

      // Days with hours (< 2 days)
      [telem.TimeSpan.hours(24), "1d"],
      [telem.TimeSpan.hours(25), "1d 1h"],
      [telem.TimeSpan.hours(36), "1d 12h"],
      [telem.TimeSpan.hours(47), "1d 23h"],

      // Days without hours (>= 2 days)
      [telem.TimeSpan.hours(48), "2d"],
      [telem.TimeSpan.hours(50), "2d"], // hours dropped
      [telem.TimeSpan.days(3), "3d"],
      [telem.TimeSpan.days(6), "6d"],

      // Weeks with days (< 2 weeks)
      [telem.TimeSpan.days(7), "1w"],
      [telem.TimeSpan.days(8), "1w 1d"],
      [telem.TimeSpan.days(10), "1w 3d"],
      [telem.TimeSpan.days(13), "1w 6d"],

      // Weeks without days (>= 2 weeks)
      [telem.TimeSpan.days(14), "2w"],
      [telem.TimeSpan.days(15), "2w"], // days dropped
      [telem.TimeSpan.days(21), "3w"],
      [telem.TimeSpan.days(28), "4w"],

      // Months with days (< 3 months)
      [telem.TimeSpan.days(30), "1mo"],
      [telem.TimeSpan.days(35), "1mo 5d"],
      [telem.TimeSpan.days(45), "1mo 15d"],
      [telem.TimeSpan.days(60), "2mo"],
      [telem.TimeSpan.days(75), "2mo 15d"],

      // Months without days (>= 3 months)
      [telem.TimeSpan.days(90), "3mo"],
      [telem.TimeSpan.days(95), "3mo"], // days dropped
      [telem.TimeSpan.days(180), "6mo"],
      [telem.TimeSpan.days(330), "11mo"],

      // Years with months (< 2 years)
      [telem.TimeSpan.days(364), "12mo"],
      [telem.TimeSpan.days(365), "1y"],
      [telem.TimeSpan.days(395), "1y 1mo"],
      [telem.TimeSpan.days(500), "1y 4mo"],
      [telem.TimeSpan.days(700), "1y 11mo"],

      // Years without months (>= 2 years)
      [telem.TimeSpan.days(730), "2y"],
      [telem.TimeSpan.days(750), "2y"], // months dropped
      [telem.TimeSpan.days(1095), "3y"],
      [telem.TimeSpan.days(3650), "10y"],

      // Complex durations
      [telem.TimeSpan.seconds(3661), "1h 1m"],
      [telem.TimeSpan.minutes(1441), "1d"], // 24h 1m, but minutes are dropped at day level
      [telem.TimeSpan.minutes(1500), "1d 1h"], // 25h exactly
      [telem.TimeSpan.hours(169), "1w 1h"],

      // Negative durations
      [telem.TimeSpan.seconds(-30), "-30s"],
      [telem.TimeSpan.minutes(-90), "-1h 30m"],
      [telem.TimeSpan.hours(-25), "-1d 1h"],
      [telem.TimeSpan.days(-8), "-1w 1d"],
      [telem.TimeSpan.days(-400), "-1y 1mo"],
    ];

    TESTS.forEach(([ts, expected]) => {
      test(`${ts.valueOf()} => ${expected}`, () => {
        expect(ts.toString("semantic")).toEqual(expected);
      });
    });
  });

  describe("toString with format", () => {
    test("toString with semantic format", () => {
      const ts = telem.TimeSpan.hours(25);
      expect(ts.toString("semantic")).toEqual("1d 1h");
    });

    test("toString with default format", () => {
      const ts = telem.TimeSpan.hours(25);
      expect(ts.toString()).toEqual("1d 1h");
    });

    test("toString with full format", () => {
      const ts = telem.TimeSpan.hours(25).add(telem.TimeSpan.minutes(30)).add(telem.TimeSpan.seconds(15));
      expect(ts.toString("full")).toEqual("1d 1h 30m 15s");
    });
  });

  describe("schema", () => {
    it("should parse bigint", () => {
      const ts = telem.TimeSpan.z.parse(1000000000n);
      expect(ts).toBeInstanceOf(telem.TimeSpan);
      expect(ts.valueOf()).toBe(1000000000n);
    });

    it("should parse number", () => {
      const ts = telem.TimeSpan.z.parse(987654321);
      expect(ts).toBeInstanceOf(telem.TimeSpan);
      expect(ts.valueOf()).toBe(987654321n);
    });

    it("should parse string representation of bigint", () => {
      const ts = telem.TimeSpan.z.parse("123456789000");
      expect(ts).toBeInstanceOf(telem.TimeSpan);
      expect(ts.valueOf()).toBe(123456789000n);
    });

    it("should parse object with value property", () => {
      const ts = telem.TimeSpan.z.parse({ value: 555555555n });
      expect(ts).toBeInstanceOf(telem.TimeSpan);
      expect(ts.valueOf()).toBe(555555555n);
    });

    it("should pass through existing TimeSpan instance", () => {
      const original = new telem.TimeSpan(777777777n);
      const ts = telem.TimeSpan.z.parse(original);
      expect(ts).toStrictEqual(original);
      expect(ts.valueOf()).toBe(777777777n);
    });

    it("should parse TimeStamp", () => {
      const timestamp = new telem.TimeStamp(999999999n);
      const ts = telem.TimeSpan.z.parse(timestamp);
      expect(ts).toBeInstanceOf(telem.TimeSpan);
      expect(ts.valueOf()).toBe(999999999n);
    });

    it("should parse Rate", () => {
      const rate = new telem.Rate(100);
      const ts = telem.TimeSpan.z.parse(rate);
      expect(ts).toBeInstanceOf(telem.TimeSpan);
      // The schema transforms Rate to TimeSpan using new telem.TimeSpan(rate)
      // which uses rate.valueOf() directly (100) as nanoseconds
      expect(ts.valueOf()).toBe(100n);
    });

    it("should handle edge cases", () => {
      const ts1 = telem.TimeSpan.z.parse(0);
      expect(ts1.valueOf()).toBe(0n);

      const ts2 = telem.TimeSpan.z.parse(Number.MAX_SAFE_INTEGER);
      expect(ts2.valueOf()).toBe(BigInt(Number.MAX_SAFE_INTEGER));
    });
  });

  describe("hash", () => {
    it("returns the bigint nanosecond value as a string", () => {
      expect(telem.TimeSpan.milliseconds(500).hash()).toBe("500000000");
    });

    it("is stable across instances representing the same value", () => {
      expect(telem.TimeSpan.seconds(1).hash()).toBe(telem.TimeSpan.seconds(1).hash());
    });

    it("satisfies primitive.isHashable", () => {
      expect(primitive.isHashable(telem.TimeSpan.ZERO)).toBe(true);
    });
  });
});

describe("Rate", () => {
  test("construct", () => expect(new telem.Rate(1).equals(1)).toBe(true));

  test("period", () => expect(new telem.Rate(1).period.equals(telem.TimeSpan.SECOND)).toBe(true));

  test("period", () =>
    expect(new telem.Rate(2).period.equals(telem.TimeSpan.milliseconds(500))).toBe(true));

  test("sampleCount", () =>
    expect(new telem.Rate(1).sampleCount(telem.TimeSpan.SECOND)).toEqual(1));

  test("byteCount", () =>
    expect(new telem.Rate(1).byteCount(telem.TimeSpan.SECOND, telem.Density.BIT64)).toEqual(8));

  test("span", () =>
    expect(new telem.Rate(1).span(4).equals(telem.TimeSpan.seconds(4))).toBe(true));

  test("byteSpan", () =>
    expect(
      new telem.Rate(1).byteSpan(new telem.Size(32), telem.Density.BIT64).equals(telem.TimeSpan.seconds(4)),
    ).toBe(true));

  test("Hz", () => expect(telem.Rate.hz(1).equals(1)).toBe(true));
  test("KHz", () => expect(telem.Rate.khz(1).equals(1e3)).toBe(true));

  describe("schema", () => {
    it("should parse number", () => {
      const rate = telem.Rate.z.parse(50);
      expect(rate).toBeInstanceOf(telem.Rate);
      expect(rate.valueOf()).toBe(50);
    });

    it("should pass through existing Rate instance", () => {
      const original = new telem.Rate(100);
      const rate = telem.Rate.z.parse(original);
      expect(rate).toBe(original);
      expect(rate.valueOf()).toBe(100);
    });

    const testCases = [
      { input: 1, expected: 1 },
      { input: 0.5, expected: 0.5 },
      { input: 1000, expected: 1000 },
      { input: 0, expected: 0 },
    ];

    testCases.forEach(({ input, expected }) => {
      it(`should parse ${input} to Rate with value ${expected}`, () => {
        const rate = telem.Rate.z.parse(input);
        expect(rate).toBeInstanceOf(telem.Rate);
        expect(rate.valueOf()).toBe(expected);
      });
    });
  });

  describe("arithmetic operations", () => {
    test("add", () => {
      const r1 = new telem.Rate(100);
      const r2 = new telem.Rate(50);
      const result = r1.add(r2);
      expect(result).toBeInstanceOf(telem.Rate);
      expect(result.valueOf()).toBe(150);
    });

    test("add with number", () => {
      const r1 = new telem.Rate(100);
      const result = r1.add(50);
      expect(result).toBeInstanceOf(telem.Rate);
      expect(result.valueOf()).toBe(150);
    });

    test("sub", () => {
      const r1 = new telem.Rate(100);
      const r2 = new telem.Rate(30);
      const result = r1.sub(r2);
      expect(result).toBeInstanceOf(telem.Rate);
      expect(result.valueOf()).toBe(70);
    });

    test("sub with number", () => {
      const r1 = new telem.Rate(100);
      const result = r1.sub(30);
      expect(result).toBeInstanceOf(telem.Rate);
      expect(result.valueOf()).toBe(70);
    });

    test("mult", () => {
      const r1 = new telem.Rate(100);
      const result = r1.mult(2);
      expect(result).toBeInstanceOf(telem.Rate);
      expect(result.valueOf()).toBe(200);
    });

    test("mult with decimal", () => {
      const r1 = new telem.Rate(100);
      const result = r1.mult(0.5);
      expect(result).toBeInstanceOf(telem.Rate);
      expect(result.valueOf()).toBe(50);
    });

    test("div", () => {
      const r1 = new telem.Rate(100);
      const result = r1.div(2);
      expect(result).toBeInstanceOf(telem.Rate);
      expect(result.valueOf()).toBe(50);
    });

    test("div with decimal", () => {
      const r1 = new telem.Rate(100);
      const result = r1.div(0.5);
      expect(result).toBeInstanceOf(telem.Rate);
      expect(result.valueOf()).toBe(200);
    });
  });

  describe("hash", () => {
    it("returns the Hz value as a string", () => {
      expect(new telem.Rate(100).hash()).toBe("100");
    });

    it("is stable across instances representing the same value", () => {
      expect(new telem.Rate(60).hash()).toBe(new telem.Rate(60).hash());
    });

    it("satisfies primitive.isHashable", () => {
      expect(primitive.isHashable(new telem.Rate(1))).toBe(true);
    });
  });
});

describe("TimeRange", () => {
  test("construct", () => {
    const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
    expect(tr.start.equals(new telem.TimeStamp(0))).toBe(true);
    expect(tr.end.equals(new telem.TimeStamp(1000))).toBe(true);
  });

  test("construct from object", () => {
    const tr = new telem.TimeRange({
      start: new telem.TimeStamp(1000),
      end: new telem.TimeStamp(100000),
    });
    expect(tr.start.equals(new telem.TimeStamp(1000))).toBe(true);
    expect(tr.end.equals(new telem.TimeStamp(100000))).toBe(true);
  });

  test("span", () => {
    const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
    expect(tr.span.equals(telem.TimeSpan.MICROSECOND)).toBe(true);
  });

  test("isValid", () => {
    const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
    expect(tr.isValid).toBe(true);
    const tr2 = new telem.TimeRange(new telem.TimeStamp(1000), new telem.TimeStamp(0));
    expect(tr2.isValid).toBe(false);
  });

  test("isZero", () => {
    const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(0));
    expect(tr.isZero).toBe(true);
    const tr2 = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
    expect(tr2.isZero).toBe(false);
  });

  test("swap", () => {
    const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
    expect(tr.swap().equals(new telem.TimeRange(new telem.TimeStamp(1000), new telem.TimeStamp(0)))).toBe(
      true,
    );
  });
  describe("contains", () => {
    test("TimeStamp", () => {
      const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
      expect(tr.contains(new telem.TimeStamp(500))).toBe(true);
      expect(tr.contains(new telem.TimeStamp(1001))).toBe(false);
    });
    test("TimeRange", () => {
      const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
      expect(tr.contains(new telem.TimeRange(new telem.TimeStamp(500), new telem.TimeStamp(600)))).toBe(
        true,
      );
      expect(tr.contains(new telem.TimeRange(new telem.TimeStamp(500), new telem.TimeStamp(1001)))).toBe(
        false,
      );
    });
  });
  describe("overlapsWith", () => {
    it("should return true if the end of one time range is after the start of the next time range", () => {
      const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
      const one = new telem.TimeRange(new telem.TimeStamp(500), new telem.TimeStamp(600));
      expect(tr.overlapsWith(one)).toBe(true);
      expect(one.overlapsWith(tr)).toBe(true);
    });
    it("should return false if two time ranges are clearly separate", () => {
      const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
      const one = new telem.TimeRange(new telem.TimeStamp(1001), new telem.TimeStamp(2000));
      expect(tr.overlapsWith(one)).toBe(false);
      expect(one.overlapsWith(tr)).toBe(false);
    });
    it("should return false if the end of the first time range is the start of the next time range", () => {
      const tr = new telem.TimeRange(new telem.TimeStamp(0), new telem.TimeStamp(1000));
      const one = new telem.TimeRange(new telem.TimeStamp(1000), new telem.TimeStamp(2000));
      expect(tr.overlapsWith(one)).toBe(false);
      expect(one.overlapsWith(tr)).toBe(false);
    });
    it("should return true only if the overlap is within a threshold", () => {
      const tr = new telem.TimeRange(telem.TimeStamp.milliseconds(0), telem.TimeStamp.milliseconds(1000));
      const one = new telem.TimeRange(
        telem.TimeStamp.milliseconds(998),
        telem.TimeStamp.milliseconds(2000),
      );
      expect(tr.overlapsWith(one, telem.TimeSpan.milliseconds(2))).toBe(true);
      expect(one.overlapsWith(tr, telem.TimeSpan.milliseconds(3))).toBe(false);
    });
    it("should return two for two ZERO time ranges", () => {
      const tr = new telem.TimeRange(telem.TimeStamp.ZERO, telem.TimeStamp.ZERO);
      const one = new telem.TimeRange(telem.TimeStamp.ZERO, telem.TimeStamp.ZERO);
      expect(tr.overlapsWith(one)).toBe(true);
    });
  });

  describe("boundBy", () => {
    it("should bound the time range to the provided constraints", () => {
      const tr = new telem.TimeRange(telem.TimeSpan.seconds(1), telem.TimeSpan.seconds(4));
      const bound = new telem.TimeRange(telem.TimeSpan.seconds(2), telem.TimeSpan.seconds(3));
      const bounded = tr.boundBy(bound);
      const expected = new telem.TimeRange(telem.TimeSpan.seconds(2), telem.TimeSpan.seconds(3));
      expect(bounded.equals(expected)).toBe(true);
    });
    it("should bound the time range even if the start is after the end", () => {
      const tr = new telem.TimeRange(telem.TimeSpan.seconds(4), telem.TimeSpan.seconds(1));
      const bound = new telem.TimeRange(telem.TimeSpan.seconds(2), telem.TimeSpan.seconds(3));
      const bounded = tr.boundBy(bound);
      const expected = new telem.TimeRange(telem.TimeSpan.seconds(3), telem.TimeSpan.seconds(2));
      expect(bounded.equals(expected)).toBe(true);
    });
  });

  describe("roughlyEquals", () => {
    it("should return true if the two time ranges are within the provided threshold", () => {
      const tr = new telem.TimeRange(telem.TimeSpan.seconds(1), telem.TimeSpan.seconds(4));
      const one = new telem.TimeRange(
        telem.TimeSpan.seconds(1),
        telem.TimeSpan.seconds(4).add(telem.TimeSpan.milliseconds(500)),
      );
      expect(tr.equals(one, telem.TimeSpan.seconds(1))).toBe(true);
      expect(tr.equals(one, telem.TimeSpan.seconds(0))).toBe(false);
    });
  });

  test("toString", () => {
    const ts = new telem.TimeStamp(telem.TimeSpan.days(2))
      .add(telem.TimeSpan.minutes(20))
      .add(telem.TimeSpan.milliseconds(283))
      .add(telem.TimeSpan.microseconds(900));
    const ts2 = new telem.TimeStamp(telem.TimeSpan.days(4))
      .add(telem.TimeSpan.minutes(20))
      .add(telem.TimeSpan.milliseconds(283))
      .add(telem.TimeSpan.microseconds(900));
    const tr = ts.range(ts2);
    const trString = tr.toString();
    expect(trString).toEqual("1970-01-03T00:20:00.283Z - 1970-01-05T00:20:00.283Z");
  });

  describe("sort", () => {
    interface Spec {
      a: telem.TimeRange;
      b: telem.TimeRange;
      expected: number;
    }
    const TESTS: Spec[] = [
      { a: new telem.TimeRange(1, 2), b: new telem.TimeRange(2, 3), expected: -1 },
      { a: new telem.TimeRange(2, 3), b: new telem.TimeRange(1, 2), expected: 1 },
      { a: new telem.TimeRange(1, 2), b: new telem.TimeRange(1, 2), expected: 0 },
      { a: new telem.TimeRange(2, 0), b: new telem.TimeRange(1, 1), expected: 1 },
      { a: new telem.TimeRange(2, 2), b: new telem.TimeRange(3, 0), expected: -1 },
      { a: new telem.TimeRange(2, 8), b: new telem.TimeRange(2, 9), expected: -1 },
      { a: new telem.TimeRange(2, 9), b: new telem.TimeRange(2, 8), expected: 1 },
    ];
    TESTS.forEach(({ a, b, expected }) => {
      test(`TimeRange.sort(${a.toString()}, ${b.toString()}) = ${expected}`, () => {
        expect(telem.TimeRange.sort(a, b)).toEqual(expected);
      });
    });
  });

  describe("merge", () => {
    it("should merge overlapping time ranges", () => {
      const trs = [
        new telem.TimeRange(telem.TimeSpan.seconds(1), telem.TimeSpan.seconds(4)),
        new telem.TimeRange(telem.TimeSpan.seconds(2), telem.TimeSpan.seconds(3)),
        new telem.TimeRange(telem.TimeSpan.seconds(3), telem.TimeSpan.seconds(5)),
        new telem.TimeRange(telem.TimeSpan.seconds(6), telem.TimeSpan.seconds(7)),
      ];
      const expected = new telem.TimeRange(telem.TimeSpan.seconds(1), telem.TimeSpan.seconds(7));
      const merged = telem.TimeRange.merge(...trs);
      expect(merged).toEqual(expected);
    });
    it("should merge time ranges that are adjacent", () => {
      const trs = [
        new telem.TimeRange(telem.TimeSpan.seconds(1), telem.TimeSpan.seconds(4)),
        new telem.TimeRange(telem.TimeSpan.seconds(4), telem.TimeSpan.seconds(5)),
      ];
      const expected = new telem.TimeRange(telem.TimeSpan.seconds(1), telem.TimeSpan.seconds(5));
      const merged = telem.TimeRange.merge(...trs);
      expect(merged).toEqual(expected);
    });
    it("should make ranges valid by swapping start and end", () => {
      const trs = [
        new telem.TimeRange(telem.TimeSpan.seconds(4), telem.TimeSpan.seconds(1)),
        new telem.TimeRange(telem.TimeSpan.seconds(2), telem.TimeSpan.seconds(3)),
      ];
      const expected = new telem.TimeRange(telem.TimeSpan.seconds(1), telem.TimeSpan.seconds(4));
      const merged = telem.TimeRange.merge(...trs);
      expect(merged).toEqual(expected);
    });
    it("should work with zero ranges", () => {
      const trs: telem.TimeRange[] = [];
      const expected = telem.TimeRange.MAX.swap();
      const merged = telem.TimeRange.merge(...trs);
      expect(merged).toEqual(expected);
    });
  });
  describe("numericBounds", () => {
    it("should return correct numeric bounds for a valid time range", () => {
      const tr = new telem.TimeRange(telem.TimeSpan.seconds(1), telem.TimeSpan.seconds(4));
      const bounds = tr.numericBounds;
      expect(bounds.lower).toBe(tr.start.nanoseconds);
      expect(bounds.upper).toBe(tr.end.nanoseconds);
    });

    it("should return correct numeric bounds for an invalid time range", () => {
      const tr = new telem.TimeRange(telem.TimeSpan.seconds(4), telem.TimeSpan.seconds(1));
      const bounds = tr.numericBounds;
      expect(bounds.lower).toBe(tr.start.nanoseconds);
      expect(bounds.upper).toBe(tr.end.nanoseconds);
    });

    it("should handle zero time range", () => {
      const tr = new telem.TimeRange(telem.TimeStamp.ZERO, telem.TimeStamp.ZERO);
      const bounds = tr.numericBounds;
      expect(bounds.lower).toBe(0);
      expect(bounds.upper).toBe(0);
    });

    it("should handle large time values", () => {
      const tr = new telem.TimeRange(telem.TimeSpan.days(365), telem.TimeSpan.days(730));
      const bounds = tr.numericBounds;
      expect(bounds.lower).toBe(tr.start.nanoseconds);
      expect(bounds.upper).toBe(tr.end.nanoseconds);
    });
  });

  describe("schema", () => {
    it("should parse object with start and end TimeStamps", () => {
      const tr = telem.TimeRange.z.parse({
        start: new telem.TimeStamp(1000),
        end: new telem.TimeStamp(2000),
      });
      expect(tr).toBeInstanceOf(telem.TimeRange);
      expect(tr.start.valueOf()).toBe(1000n);
      expect(tr.end.valueOf()).toBe(2000n);
    });

    it("should pass through existing TimeRange instance", () => {
      const original = new telem.TimeRange(new telem.TimeStamp(1000), new telem.TimeStamp(2000));
      const tr = telem.TimeRange.z.parse(original);
      expect(tr).toStrictEqual(original);
      expect(tr.start.valueOf()).toBe(1000n);
      expect(tr.end.valueOf()).toBe(2000n);
    });

    it("should parse object with various timestamp formats", () => {
      const tr = telem.TimeRange.z.parse({
        start: 1000,
        end: "2000",
      });
      expect(tr).toBeInstanceOf(telem.TimeRange);
      expect(tr.start.valueOf()).toBe(1000n);
      expect(tr.end.valueOf()).toBe(2000n);
    });

    const testCases = [
      {
        input: { start: 0, end: 1000 },
        expectedStart: 0n,
        expectedEnd: 1000n,
      },
      {
        input: { start: new Date("2024-01-01"), end: new Date("2024-01-02") },
        expectedStart: BigInt(new Date("2024-01-01").getTime()) * 1000000n,
        expectedEnd: BigInt(new Date("2024-01-02").getTime()) * 1000000n,
      },
    ];

    testCases.forEach(({ input, expectedStart, expectedEnd }, i) => {
      it(`should parse test case ${i + 1}`, () => {
        const tr = telem.TimeRange.z.parse(input);
        expect(tr).toBeInstanceOf(telem.TimeRange);
        expect(tr.start.valueOf()).toBe(expectedStart);
        expect(tr.end.valueOf()).toBe(expectedEnd);
      });
    });
  });

  describe("hash", () => {
    it("composes the start and end hashes with a dash", () => {
      const tr = new telem.TimeRange(new telem.TimeStamp(100n), new telem.TimeStamp(200n));
      expect(tr.hash()).toBe("100-200");
    });

    it("is stable across instances representing the same range", () => {
      const a = new telem.TimeRange(new telem.TimeStamp(1n), new telem.TimeStamp(2n));
      const b = new telem.TimeRange(new telem.TimeStamp(1n), new telem.TimeStamp(2n));
      expect(a.hash()).toBe(b.hash());
    });

    it("differs when start or end differs", () => {
      const base = new telem.TimeRange(new telem.TimeStamp(1n), new telem.TimeStamp(2n));
      expect(base.hash()).not.toBe(
        new telem.TimeRange(new telem.TimeStamp(1n), new telem.TimeStamp(3n)).hash(),
      );
      expect(base.hash()).not.toBe(
        new telem.TimeRange(new telem.TimeStamp(0n), new telem.TimeStamp(2n)).hash(),
      );
    });

    it("satisfies primitive.isHashable", () => {
      expect(primitive.isHashable(telem.TimeRange.ZERO)).toBe(true);
    });
  });
});

describe("Density", () => {
  describe("construct", () => {
    test("construct from CrudeValueExtension", () => {
      const density = new telem.Density({ value: 8 });
      expect(density.valueOf()).toBe(8);
    });

    test("construct from number", () => {
      const density = new telem.Density(8);
      expect(density.valueOf()).toBe(8);
    });

    test("construct from Density", () => {
      const density = new telem.Density(8);
      expect(density.valueOf()).toBe(8);
    });
  });

  describe("schema", () => {
    it("should parse number", () => {
      const density = telem.Density.z.parse(8);
      expect(density).toBeInstanceOf(telem.Density);
      expect(density.valueOf()).toBe(8);
    });

    it("should pass through existing Density instance", () => {
      const original = new telem.Density(4);
      const density = telem.Density.z.parse(original);
      expect(density).toBe(original);
      expect(density.valueOf()).toBe(4);
    });

    const testCases = [
      { input: 1, expected: 1 },
      { input: 2, expected: 2 },
      { input: 4, expected: 4 },
      { input: 8, expected: 8 },
      { input: 0, expected: 0 },
    ];

    testCases.forEach(({ input, expected }) => {
      it(`should parse ${input} to Density with value ${expected}`, () => {
        const density = telem.Density.z.parse(input);
        expect(density).toBeInstanceOf(telem.Density);
        expect(density.valueOf()).toBe(expected);
      });
    });
  });

  describe("arithmetic operations", () => {
    test("add", () => {
      const d1 = new telem.Density(8);
      const d2 = new telem.Density(4);
      const result = d1.add(d2);
      expect(result).toBeInstanceOf(telem.Density);
      expect(result.valueOf()).toBe(12);
    });

    test("add with number", () => {
      const d1 = new telem.Density(8);
      const result = d1.add(4);
      expect(result).toBeInstanceOf(telem.Density);
      expect(result.valueOf()).toBe(12);
    });

    test("sub", () => {
      const d1 = new telem.Density(8);
      const d2 = new telem.Density(3);
      const result = d1.sub(d2);
      expect(result).toBeInstanceOf(telem.Density);
      expect(result.valueOf()).toBe(5);
    });

    test("sub with number", () => {
      const d1 = new telem.Density(8);
      const result = d1.sub(3);
      expect(result).toBeInstanceOf(telem.Density);
      expect(result.valueOf()).toBe(5);
    });

    test("mult", () => {
      const d1 = new telem.Density(4);
      const result = d1.mult(2);
      expect(result).toBeInstanceOf(telem.Density);
      expect(result.valueOf()).toBe(8);
    });

    test("mult with decimal", () => {
      const d1 = new telem.Density(8);
      const result = d1.mult(0.5);
      expect(result).toBeInstanceOf(telem.Density);
      expect(result.valueOf()).toBe(4);
    });

    test("div", () => {
      const d1 = new telem.Density(8);
      const result = d1.div(2);
      expect(result).toBeInstanceOf(telem.Density);
      expect(result.valueOf()).toBe(4);
    });

    test("div with decimal", () => {
      const d1 = new telem.Density(4);
      const result = d1.div(0.5);
      expect(result).toBeInstanceOf(telem.Density);
      expect(result.valueOf()).toBe(8);
    });
  });
});

describe("DataType", () => {
  describe("isVariable", () => {
    it("should return true if the data type has a variable length", () => {
      expect(telem.DataType.INT32.isVariable).toBe(false);
    });
    it("should return false if the data type does not have a variable length", () => {
      expect(telem.DataType.STRING.isVariable).toBe(true);
    });
    it("should return true for BYTES", () => {
      expect(telem.DataType.BYTES.isVariable).toBe(true);
    });
  });

  describe("construct", () => {
    test("construct from CrudeValueExtension", () => {
      const dt = new telem.DataType({ value: "int32" });
      expect(dt.toString()).toBe("int32");
    });

    [
      [new Int32Array(), telem.DataType.INT32],
      [new Int8Array(), telem.DataType.INT8],
      [new Uint8Array(), telem.DataType.UINT8],
      [new Uint16Array(), telem.DataType.UINT16],
      [new Uint32Array(), telem.DataType.UINT32],
      [new BigInt64Array(), telem.DataType.INT64],
      [new BigUint64Array(), telem.DataType.UINT64],
      [new Float32Array(), telem.DataType.FLOAT32],
      [new Float64Array(), telem.DataType.FLOAT64],
      [new BigInt64Array(), telem.DataType.INT64],
      [new BigUint64Array(), telem.DataType.UINT64],
    ].forEach(([array, expected]) => {
      test(`construct from ${array.constructor.name} to ${expected.toString()}`, () => {
        const dt = new telem.DataType(array);
        expect(dt.toString()).toBe(expected.toString());
      });
    });

    test("from data type", () => {
      const dt = new telem.DataType(telem.DataType.INT32);
      expect(dt.toString()).toBe(telem.DataType.INT32.toString());
    });

    test("from string", () => {
      const dt = new telem.DataType("int32");
      expect(dt.toString()).toBe("int32");
    });
  });

  describe("canSafelyCastTo", () => {
    const TESTS: [telem.DataType, telem.DataType, boolean][] = [
      [telem.DataType.INT32, telem.DataType.INT32, true],
      [telem.DataType.INT32, telem.DataType.INT64, true],
      [telem.DataType.INT32, telem.DataType.FLOAT32, false],
      [telem.DataType.INT32, telem.DataType.FLOAT64, true],
      [telem.DataType.INT32, telem.DataType.STRING, false],
      [telem.DataType.INT32, telem.DataType.INT8, false],
      [telem.DataType.INT64, telem.DataType.INT32, false],
      [telem.DataType.INT64, telem.DataType.INT64, true],
      [telem.DataType.INT64, telem.DataType.FLOAT32, false],
      [telem.DataType.INT64, telem.DataType.FLOAT64, false],
      [telem.DataType.INT64, telem.DataType.STRING, false],
      [telem.DataType.FLOAT64, telem.DataType.FLOAT32, false],
      [telem.DataType.FLOAT64, telem.DataType.FLOAT64, true],
      [telem.DataType.FLOAT64, telem.DataType.STRING, false],
      [telem.DataType.FLOAT32, telem.DataType.FLOAT64, true],
      [telem.DataType.FLOAT32, telem.DataType.FLOAT32, true],
      [telem.DataType.FLOAT32, telem.DataType.STRING, false],
      [telem.DataType.STRING, telem.DataType.STRING, true],
      [telem.DataType.STRING, telem.DataType.INT32, false],
      [telem.DataType.STRING, telem.DataType.INT64, false],
      [telem.DataType.STRING, telem.DataType.FLOAT32, false],
      [telem.DataType.STRING, telem.DataType.FLOAT64, false],
      [telem.DataType.STRING, telem.DataType.INT8, false],
      [telem.DataType.INT8, telem.DataType.FLOAT32, true],
      [telem.DataType.BYTES, telem.DataType.BYTES, true],
      [telem.DataType.BYTES, telem.DataType.INT32, false],
      [telem.DataType.BYTES, telem.DataType.STRING, false],
    ];
    TESTS.forEach(([from, to, expected]) =>
      it(`should return ${expected} when casting from ${from.toString()} to ${to.toString()}`, () => {
        expect(from.canSafelyCastTo(to)).toBe(expected);
      }),
    );
  });

  describe("canCastTo", () => {
    it("should return true for any two numeric data types", () => {
      const numericTypes = [
        telem.DataType.INT32,
        telem.DataType.INT64,
        telem.DataType.FLOAT32,
        telem.DataType.FLOAT64,
      ];
      for (const from of numericTypes)
        for (const to of numericTypes) expect(from.canCastTo(to)).toBe(true);
    });
    it("should return true for non-numeric data types ONLY if they are equal", () => {
      const nonNumericTypes = [telem.DataType.STRING, telem.DataType.BYTES];
      for (const from of nonNumericTypes)
        for (const to of nonNumericTypes) expect(from.canCastTo(to)).toBe(from === to);
    });
  });

  describe("equals", () => {
    const TESTS: [telem.DataType, telem.CrudeDataType, boolean][] = [
      [telem.DataType.INT32, telem.DataType.INT32, true],
      [telem.DataType.INT32, telem.DataType.INT64, false],
      [telem.DataType.INT32, "int32", true],
      [telem.DataType.INT32, "int64", false],
    ];
    TESTS.forEach(([dt, other, expected]) =>
      it(`should return ${expected} when comparing ${dt.toString()} to ${JSON.stringify(other)}`, () => {
        expect(dt.equals(other)).toBe(expected);
      }),
    );
  });

  describe("schema", () => {
    it("should parse string", () => {
      const dt = telem.DataType.z.parse("int32");
      expect(dt).toBeInstanceOf(telem.DataType);
      expect(dt.toString()).toBe("int32");
    });

    it("should pass through existing DataType instance", () => {
      const original = telem.DataType.INT32;
      const dt = telem.DataType.z.parse(original);
      expect(dt).toBe(original);
      expect(dt.toString()).toBe("int32");
    });

    const testCases = [
      { input: "int8", expected: "int8", short: "i8" },
      { input: "int16", expected: "int16", short: "i16" },
      { input: "int32", expected: "int32", short: "i32" },
      { input: "int64", expected: "int64", short: "i64" },
      { input: "uint8", expected: "uint8", short: "u8" },
      { input: "uint16", expected: "uint16", short: "u16" },
      { input: "uint32", expected: "uint32", short: "u32" },
      { input: "uint64", expected: "uint64", short: "u64" },
      { input: "float32", expected: "float32", short: "f32" },
      { input: "float64", expected: "float64", short: "f64" },
      { input: "string", expected: "string", short: "str" },
      { input: "timestamp", expected: "timestamp", short: "ts" },
      { input: "uuid", expected: "uuid", short: "uuid" },
      { input: "json", expected: "json", short: "json" },
      { input: "bytes", expected: "bytes", short: "bytes" },
    ];

    testCases.forEach(({ input, expected, short }) => {
      it(`should parse "${input}" to DataType with value "${expected}"`, () => {
        const dt = telem.DataType.z.parse(input);
        expect(dt).toBeInstanceOf(telem.DataType);
        expect(dt.toString()).toBe(expected);
        expect(dt.toString(true)).toBe(short);
      });
    });
  });

  describe("hash", () => {
    it("returns the data type identifier as a string", () => {
      expect(telem.DataType.FLOAT32.hash()).toBe("float32");
      expect(telem.DataType.STRING.hash()).toBe("string");
    });

    it("is stable across instances representing the same data type", () => {
      expect(new telem.DataType("uint8").hash()).toBe(new telem.DataType("uint8").hash());
    });

    it("differs across distinct data types", () => {
      expect(telem.DataType.FLOAT32.hash()).not.toBe(telem.DataType.FLOAT64.hash());
    });

    it("satisfies primitive.isHashable", () => {
      expect(primitive.isHashable(telem.DataType.UNKNOWN)).toBe(true);
    });
  });
});

describe("Size", () => {
  describe("construct", () => {
    test("construct from CrudeValueExtension", () => {
      const size = new telem.Size({ value: 1024 });
      expect(size.valueOf()).toBe(1024);
    });

    test("construct from number", () => {
      const size = new telem.Size(1024);
      expect(size.valueOf()).toBe(1024);
    });

    test("construct from Size", () => {
      const size = new telem.Size(1024);
      expect(size.valueOf()).toBe(1024);
    });
  });
  const TO_STRING_TESTS = [
    [telem.Size.bytes(1), "1B"],
    [telem.Size.kilobytes(1), "1KB"],
    [telem.Size.megabytes(1), "1MB"],
    [telem.Size.gigabytes(1), "1GB"],
    [telem.Size.terabytes(1), "1TB"],
    [telem.Size.megabytes(4).add(telem.Size.kilobytes(500)), "4MB 500KB"],
  ];

  test("toString", () => {
    TO_STRING_TESTS.forEach(([size, expected]) => {
      expect(size.toString()).toEqual(expected);
    });
  });

  const TRUNCATE_TESTS = [
    [telem.Size.bytes(1).add(telem.Size.kilobytes(1)), telem.Size.KILOBYTE, telem.Size.kilobytes(1)],
    [telem.Size.megabytes(100).add(telem.Size.kilobytes(500)), telem.Size.MEGABYTE, telem.Size.megabytes(100)],
    [
      telem.Size.gigabytes(1).add(telem.Size.megabytes(500)).add(telem.Size.kilobytes(500)),
      telem.Size.MEGABYTE,
      telem.Size.gigabytes(1).add(telem.Size.megabytes(500)),
    ],
  ];

  test("truncate", () => {
    TRUNCATE_TESTS.forEach(([size, unit, expected]) => {
      expect(size.truncate(unit).valueOf()).toEqual(expected.valueOf());
    });
  });

  describe("schema", () => {
    it("should parse number", () => {
      const size = telem.Size.z.parse(1024);
      expect(size).toBeInstanceOf(telem.Size);
      expect(size.valueOf()).toBe(1024);
    });

    it("should pass through existing Size instance", () => {
      const original = new telem.Size(2048);
      const size = telem.Size.z.parse(original);
      expect(size).toBe(original);
      expect(size.valueOf()).toBe(2048);
    });

    const testCases = [
      { input: 0, expected: 0 },
      { input: 1, expected: 1 },
      { input: 1024, expected: 1024 },
      { input: 1048576, expected: 1048576 },
      { input: 1073741824, expected: 1073741824 },
    ];

    testCases.forEach(({ input, expected }) => {
      it(`should parse ${input} to Size with value ${expected}`, () => {
        const size = telem.Size.z.parse(input);
        expect(size).toBeInstanceOf(telem.Size);
        expect(size.valueOf()).toBe(expected);
      });
    });
  });

  describe("arithmetic operations", () => {
    test("add", () => {
      const s1 = telem.Size.kilobytes(10);
      const s2 = telem.Size.kilobytes(5);
      const result = s1.add(s2);
      expect(result).toBeInstanceOf(telem.Size);
      expect(result.valueOf()).toBe(15000);
    });

    test("add with number", () => {
      const s1 = telem.Size.bytes(100);
      const result = s1.add(50);
      expect(result).toBeInstanceOf(telem.Size);
      expect(result.valueOf()).toBe(150);
    });

    test("sub", () => {
      const s1 = telem.Size.kilobytes(10);
      const s2 = telem.Size.kilobytes(3);
      const result = s1.sub(s2);
      expect(result).toBeInstanceOf(telem.Size);
      expect(result.valueOf()).toBe(7000);
    });

    test("sub with number", () => {
      const s1 = telem.Size.bytes(100);
      const result = s1.sub(30);
      expect(result).toBeInstanceOf(telem.Size);
      expect(result.valueOf()).toBe(70);
    });

    test("mult", () => {
      const s1 = telem.Size.kilobytes(5);
      const result = s1.mult(2);
      expect(result).toBeInstanceOf(telem.Size);
      expect(result.valueOf()).toBe(10000);
    });

    test("mult with decimal", () => {
      const s1 = telem.Size.kilobytes(10);
      const result = s1.mult(0.5);
      expect(result).toBeInstanceOf(telem.Size);
      expect(result.valueOf()).toBe(5000);
    });

    test("div", () => {
      const s1 = telem.Size.kilobytes(10);
      const result = s1.div(2);
      expect(result).toBeInstanceOf(telem.Size);
      expect(result.valueOf()).toBe(5000);
    });

    test("div with decimal", () => {
      const s1 = telem.Size.kilobytes(5);
      const result = s1.div(0.5);
      expect(result).toBeInstanceOf(telem.Size);
      expect(result.valueOf()).toBe(10000);
    });
  });
});

describe("convertDataType", () => {
  it("preserves precision when subtracting a bigint offset above 2^53 (INT64 -> FLOAT32)", () => {
    const offset = 1778020940471336960n;
    const value = 1778020940471336960n + 137438953472n;
    const result = telem.convertDataType(telem.DataType.INT64, telem.DataType.FLOAT32, value, offset);
    expect(result).toBe(137438953472);
  });

  it("preserves precision when subtracting a bigint offset above 2^53 (TIMESTAMP -> FLOAT32)", () => {
    const offset = 1778020940471336960n;
    const value = 1778020940471336960n + 1n;
    const result = telem.convertDataType(telem.DataType.TIMESTAMP, telem.DataType.FLOAT32, value, offset);
    expect(result).toBe(1);
  });

  it("returns a bigint when target uses bigint and source does not", () => {
    const result = telem.convertDataType(telem.DataType.FLOAT32, telem.DataType.INT64, 100, 10);
    expect(result).toBe(90n);
  });

  it("falls back to math.sub when neither source nor target use bigint", () => {
    const result = telem.convertDataType(telem.DataType.FLOAT32, telem.DataType.FLOAT64, 5.5, 1.5);
    expect(result).toBe(4);
  });
});
