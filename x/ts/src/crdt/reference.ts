...
// implementation. It is the reference model for differential.spec.ts and must never be
// edited to track the production code. It is test-only and excluded from the package's
// public surface.
...
const sortedInsert = (children: RefElement[], e: RefElement): void => {
  let lo = 0;
...
/** ReferenceText is a replicated text document owned by a single replica, identified at
...
export class ReferenceText {
  ...
  private counter = 0;
  private readonly root: RefElement;
  private readonly elements = new Map<string, RefElement>();
  private readonly tombstones = new Set<string>();
  private pending: Insert[] = [];
  private order: RefElement[] = [];
  private dirty = true;
  // visibleCache caches the live characters in document order; null when stale.
  private visibleCache: RefElement[] | null = null;
  ...
  snapshot(): { inserts: Insert[]; deletes: Delete[] } {
    ...
    const stack: Array<{ node: RefElement; origin: ID; side: spatial.XLocation }> = [
   ...
   * under recursion. */
  private walk(): void {
    const stack: Array<{ node: RefElement; emit: boolean }> = [
  ...
  insert(index: number, text: string): Insert[] {
    ...
    for (const ch of chars) {
      ...
      left = this.place(op) as RefElement;
      ops.push(op);
    }
    return ops;
  }
  ...
  private place(op: Insert): RefElement | null {
