...
var _ = Describe("Sub", func() {
	...
	It("Should give FS of subdirectories", func() {
		...
		sub_FS.Open("yum.txt", os.O_CREATE)
...
var _ = Describe("List", func() {
	It("Should provide a list of all the files and directories", func() {
		...
		Expect(l[0].IsDir()).To(BeTrue())

		Expect(l[1].Name()).To(Equal("sub2"))
		Expect(l[1].IsDir()).To(BeTrue())

		Expect(l[2].Name()).To(Equal("file1.json"))
		Expect(l[2].IsDir()).To(BeFalse())

		Expect(l[3].Name()).To(Equal("file2.json"))
		Expect(l[3].IsDir()).To(BeFalse())
	})
})
