...
func (f *memFile) WriteAt(p []byte, ofs int64) (int, error) {
	...
	if n != len(p) {
		panic(fmt.Sprintf("expected to copy %d bytes into memfile, only copied %d", len(p), n))
	}
...
func (f *memFile) Truncate(size int64) error {
	if !f.write {
		return errors.New("memfs: file was not created for writing (truncate requires write fd)")
	}
	if f.n.isDir {
