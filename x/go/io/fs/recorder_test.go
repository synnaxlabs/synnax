...
var _ = Describe("Recorder", func() {
	for fsName, openFS := range FileSystems {
		Context("FS: "+fsName, Ordered, func() {
			...
			BeforeEach(func() { rec = xfs.NewRecorder(openFS()) })
			...
			Describe("Read", func() {
				It("Should record Read with the requested length", func() {
					// A separate write handle is used so the read handle starts at
					// offset 0; on osFS the read and write offsets are shared, so a
					// Write followed by Read on the same handle hits EOF.
					g := MustSucceed(rec.Open("read.bin", os.O_CREATE|os.O_RDWR))
					MustSucceed(g.Write([]byte("hello")))
					Expect(g.Close()).To(Succeed())
					h := MustSucceed(rec.Open("read.bin", os.O_RDONLY))
					DeferClose(h)
					rec.Reset()
					buf := make([]byte, 5)
					MustSucceed(h.Read(buf))
					Expect(rec.Filter(xfs.MatchName("read.bin"))).To(ConsistOf(
						xfs.Event{Op: xfs.OpRead, Name: "read.bin", Length: 5},
					))
				})
			})
