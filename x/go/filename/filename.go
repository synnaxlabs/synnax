...
const (
	// maxLength is the longest single path element ext4, APFS, and NTFS accept.
...
var (
	// unsafeChars matches the path separators, the characters Windows reserves,
	// and the control characters Windows forbids.
	unsafeChars = regexp.MustCompile(`[/\\<>:"|?*\x00-\x1f]`)
	// reservedNames matches the device names Windows refuses to open a file under,
...
// Sanitize turns a user-supplied name into a file name that writes to disk on
...
// fit shortens name to maxBytes bytes, cutting on a rune boundary, and drops
...
// Fold reduces name to the form two file names must be compared in:
