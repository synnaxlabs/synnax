...
// Files groups diagnostics by the file they originate from. The file key carries the
// origin that Diagnostic does not, mirroring how LSP scopes diagnostics by URI at the
// envelope. Files are kept in first-seen order so downstream output is deterministic.
type Files struct {
	order  []string
	byFile map[string]*Diagnostics
}

var _ error = (*Files)(nil)

// NewFiles constructs an empty Files.
func NewFiles() *Files {
	return &Files{byFile: make(map[string]*Diagnostics)}
}

// at returns the diagnostics for file, allocating and recording order on first use.
func (f *Files) at(file string) *Diagnostics {
	ds, ok := f.byFile[file]
	if !ok {
		ds = &Diagnostics{}
		f.byFile[file] = ds
...
func (f *Files) Errors() []Diagnostic {
	var out []Diagnostic
...
func (f *Files) Each(fn func(file string, d Diagnostic)) {
	for _, file := range f.order {
...
func (f *Files) Error() string { return f.String() }
