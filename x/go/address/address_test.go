...
var _ = Describe("Address", func() {
	...
	Describe("String Operations", func() {
		...
		Context("with malformed address", func() {
			var addr address.Address
			BeforeEach(func() {
				addr = address.Address("invalid")
			})
			It("Should return the regular string for String", func() {
				Expect(addr.String()).To(Equal("invalid"))
			})
			It("Should return empty string for PortString", func() {
				Expect(addr.PortString()).To(BeEmpty())
			})
			It("Should return empty string for Port", func() {
				Expect(addr.Port()).To(Equal(0))
			})
			It("Should return the string for Host", func() {
				Expect(addr.Host()).To(Equal("invalid"))
			})
		})
	})
	Describe("Rand", func() {
		...
		It("Should generate unique addresses", func() {
			...
			for range 1000 {
				addr := address.Rand()
				Expect(seen[addr.String()]).To(BeFalseBecause("addresses are unique"))
