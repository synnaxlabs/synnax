...
var _ = Describe("Analyzer", func() {
	...
	It(
		"Should only emit a single import edit across multiple diagnostics in the same file",
		func() {
			testdata := analysistest.TestData()
			results := analysistest.Run(
				GinkgoT(), testdata, must.Analyzer, "example",
	...
	It(
		"Should emit the import as its own separate diagnostic, not bundled into a code fix",
		func() {
			testdata := analysistest.TestData()
			results := analysistest.Run(
				GinkgoT(), testdata, must.Analyzer, "example",
