...
var _ = Describe("SeriesFactory", func() {
	Describe("NewSeries,NewSeriesV,UnmarshalSeries", func() {
		Describe("Fixed Types", func() {
			Specify(
				"uint8",
				newFixedSeriesRoundtripTest([]uint8{1, 2, 3}, telem.Uint8T),
			)
			Specify(
				"uint16",
				newFixedSeriesRoundtripTest([]uint16{1, 2, 3}, telem.Uint16T),
			)
			Specify(
				"uint32",
				newFixedSeriesRoundtripTest([]uint32{1, 2, 3}, telem.Uint32T),
			)
			Specify(
				"uint64",
			...
			Specify(
				"single value",
				newFixedSeriesRoundtripTest([]int64{1}, telem.Int64T),
			)
		})
		Describe("Variable Types", func() {
			Specify(
				"string",
				newVariableSeriesRoundtripTest(
					[]string{"hello", "world"},
					telem.StringT,
				),
			)
			Specify(
				"single string",
				newVariableSeriesRoundtripTest([]string{"hello"}, telem.StringT),
			)
			Specify(
				"empty strings",
				newVariableSeriesRoundtripTest([]string{"", "", ""}, telem.StringT),
			)
			Specify(
			...
			Specify(
				"single []byte",
				newVariableSeriesRoundtripTest([][]byte{{1}}, telem.BytesT),
			)
			Specify(
				"empty []byte values",
				newVariableSeriesRoundtripTest([][]byte{{}, {}, {}}, telem.BytesT),
			)
			Specify(
				"empty []byte",
				newVariableSeriesRoundtripTest([][]byte{}, telem.BytesT),
	...
	Describe("NewSeriesFromAny", func() {
		...
		DescribeTable(
			"happy path",
			func(input any, dt telem.DataType, expected telem.Series) {
			...
			Entry(
				...
				uint64(200),
				telem.Uint64T,
				telem.NewSeriesV(uint64(200)),
			),
			Entry(
				"uint32 → Uint32T",
				uint32(150),
				telem.Uint32T,
				telem.NewSeriesV(uint32(150)),
			),
			Entry(
				"uint16 → Uint16T",
				uint16(75),
				telem.Uint16T,
				telem.NewSeriesV(uint16(75)),
			),
			Entry(
				"uint8 → Uint8T",
				uint8(37),
				telem.Uint8T,
				telem.NewSeriesV(uint8(37)),
			),
			Entry(
				"float64 → Float64T",
				3.14,
				telem.Float64T,
				telem.NewSeriesV(float64(3.14)),
			),
			Entry(
				"float32 → Float32T",
				float32(2.5),
				telem.Float32T,
				telem.NewSeriesV(float32(2.5)),
			),
			// cross-type numeric conversions
			Entry("int → Float64T", 42, telem.Float64T, telem.NewSeriesV(float64(42))),
			Entry(
				"float64 → Int64T (truncates)",
				42.7,
				telem.Int64T,
			...
			Entry(
				...
				int32(50),
				telem.Uint32T,
				telem.NewSeriesV(uint32(50)),
			),
			Entry(
				"float32 → Float64T",
				float32(1.5),
				telem.Float64T,
				telem.NewSeriesV(float64(float32(1.5))),
			),
			Entry(
				"float64 → Float32T",
				2.5,
			...
			Entry(
				...
				[]byte{1, 2, 3},
				telem.BytesT,
				telem.NewSeriesV([]byte{1, 2, 3}),
			),
			Entry(
				"string → BytesT",
				"hello",
				telem.BytesT,
				telem.NewSeriesV([]byte("hello")),
			),
			...
			Entry(
				...
				uint8(2),
				telem.BytesT,
				telem.NewSeriesV([]byte{2}),
			),
			Entry(
				"uint16 -> BytesT",
				uint16(3),
				telem.BytesT,
				telem.NewSeriesV([]byte{3, 0}),
			),
			Entry(
				"uint32 -> BytesT",
				uint32(4),
				telem.BytesT,
				telem.NewSeriesV([]byte{4, 0, 0, 0}),
			),
			Entry(
				"uint64 -> BytesT",
				uint64(5),
				telem.BytesT,
				telem.NewSeriesV([]byte{5, 0, 0, 0, 0, 0, 0, 0}),
			),
			...
			Entry(
				...
				int16(7),
				telem.BytesT,
				telem.NewSeriesV([]byte{7, 0}),
			),
			Entry(
				"int32 -> BytesT",
				int32(8),
				telem.BytesT,
				telem.NewSeriesV([]byte{8, 0, 0, 0}),
			),
			Entry(
				"int64 -> BytesT",
				int64(9),
				telem.BytesT,
				telem.NewSeriesV([]byte{9, 0, 0, 0, 0, 0, 0, 0}),
			),
			Entry(
				"timestamp -> BytesT",
				telem.TimeStamp(1),
				telem.BytesT,
				telem.NewSeriesV([]byte{1, 0, 0, 0, 0, 0, 0, 0}),
			),
			Entry(
				"timespan -> BytesT",
				telem.TimeSpan(10),
				telem.BytesT,
				telem.NewSeriesV([]byte{10, 0, 0, 0, 0, 0, 0, 0}),
			),
			Entry(
				"float32 -> BytesT",
				float32(1.5),
				telem.BytesT,
				telem.NewSeriesV(
					telem.ByteOrder.AppendUint32(nil, math.Float32bits(1.5)),
				),
			),
			Entry(
				"float64 -> BytesT",
				...
				telem.BytesT,
				telem.NewSeriesV(
					telem.ByteOrder.AppendUint64(nil, math.Float64bits(2.5)),
				),
			),
