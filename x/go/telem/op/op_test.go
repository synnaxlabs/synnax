...
var _ = Describe("Vectorized Operations", func() {
	...
	Describe("Bitwise Operations", func() {
		Context("Series-Series", func() {
			It("should perform BitAndI64", func() {
				a := telem.NewSeriesV[int64](12, 10, 0xFF)
				b := telem.NewSeriesV[int64](10, 6, 0x0F)
				output := telem.Series{DataType: telem.Int64T}

				op.BitAndI64(a, b, &output)

				expected := []int64{8, 2, 0x0F}
				Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
			})

			It("should perform BitOrI32", func() {
				a := telem.NewSeriesV[int32](12, 10, 0xF0)
				b := telem.NewSeriesV[int32](10, 6, 0x0F)
				output := telem.Series{DataType: telem.Int32T}

				op.BitOrI32(a, b, &output)

				expected := []int32{14, 14, 0xFF}
				Expect(telem.UnmarshalSeries[int32](output)).To(Equal(expected))
			})

			It("should perform BitXorU8", func() {
				a := telem.NewSeriesV[uint8](0xFF, 0x0F, 12)
				b := telem.NewSeriesV[uint8](0x0F, 0x0F, 10)
				output := telem.Series{DataType: telem.Uint8T}

				op.BitXorU8(a, b, &output)

				expected := []uint8{0xF0, 0x00, 6}
				Expect(telem.UnmarshalSeries[uint8](output)).To(Equal(expected))
			})

			It("should perform ShiftLeftI64", func() {
				a := telem.NewSeriesV[int64](1, 2, 3)
				b := telem.NewSeriesV[int64](4, 3, 1)
				output := telem.Series{DataType: telem.Int64T}

				op.ShiftLeftI64(a, b, &output)

				expected := []int64{16, 16, 6}
				Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
			})

			It("should perform ShiftRightI64 arithmetically for signed", func() {
				a := telem.NewSeriesV[int64](16, -8)
				b := telem.NewSeriesV[int64](2, 1)
				output := telem.Series{DataType: telem.Int64T}

				op.ShiftRightI64(a, b, &output)

				expected := []int64{4, -4}
				Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
			})

			It("should perform ShiftRightU32 logically for unsigned", func() {
				a := telem.NewSeriesV[uint32](0x80000000, 16)
				b := telem.NewSeriesV[uint32](31, 2)
				output := telem.Series{DataType: telem.Uint32T}

				op.ShiftRightU32(a, b, &output)
			...
			It(
				"should repeat the last value of the shorter series",
				func() {
					a := telem.NewSeriesV[int64](12, 10, 7)
					b := telem.NewSeriesV[int64](8)
					output := telem.Series{DataType: telem.Int64T}

					op.BitAndI64(a, b, &output)

					// b values: [8, 8, 8] (8 repeats)
					expected := []int64{8, 8, 0}
					Expect(output.Len()).To(Equal(int64(3)))
					Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
				},
			)
		...
		Context("Scalar", func() {
			It("should perform BitAndScalarI64", func() {
				a := telem.NewSeriesV[int64](12, 10, 7)
				output := telem.Series{DataType: telem.Int64T}

				op.BitAndScalarI64(a, 8, &output)

				expected := []int64{8, 8, 0}
				Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
			})

			It("should perform BitOrScalarI64", func() {
				a := telem.NewSeriesV[int64](12, 10)
				output := telem.Series{DataType: telem.Int64T}

				op.BitOrScalarI64(a, 1, &output)

				expected := []int64{13, 11}
				Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
			})

			It("should perform BitXorScalarI64", func() {
				a := telem.NewSeriesV[int64](12, 10)
				output := telem.Series{DataType: telem.Int64T}

				op.BitXorScalarI64(a, 0xF, &output)

				expected := []int64{3, 5}
				Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
			})

			It("should perform ShiftLeftScalarI64", func() {
				a := telem.NewSeriesV[int64](1, 2)
				output := telem.Series{DataType: telem.Int64T}

				op.ShiftLeftScalarI64(a, 3, &output)

				expected := []int64{8, 16}
				Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
			})

			It("should perform ShiftRightScalarU64 logically", func() {
				a := telem.NewSeriesV[uint64](0x8000000000000000, 16)
				output := telem.Series{DataType: telem.Uint64T}

				op.ShiftRightScalarU64(a, 63, &output)
		...
		Context("Reverse Scalar Shifts", func() {
			It(
				"should compute scalar << element via ReverseShiftLeftScalarI64",
				func() {
					a := telem.NewSeriesV[int64](1, 3)
					output := telem.Series{DataType: telem.Int64T}

					op.ReverseShiftLeftScalarI64(a, 2, &output)
			...
			It(
				"should compute scalar >> element via ReverseShiftRightScalarI64",
				func() {
					a := telem.NewSeriesV[int64](1, 3)
					output := telem.Series{DataType: telem.Int64T}

					op.ReverseShiftRightScalarI64(a, 16, &output)
		...
		Context("Bitwise NOT", func() {
			It("should perform BitNotI64", func() {
				a := telem.NewSeriesV[int64](12, 10, 0, -1)
				output := telem.Series{DataType: telem.Int64T}

				op.BitNotI64(a, &output)

				expected := []int64{-13, -11, -1, 0}
				Expect(telem.UnmarshalSeries[int64](output)).To(Equal(expected))
			})

			It("should perform BitNotU8", func() {
				a := telem.NewSeriesV[uint8](0x0F, 0xF0, 0xAA)
				output := telem.Series{DataType: telem.Uint8T}

				op.BitNotU8(a, &output)
