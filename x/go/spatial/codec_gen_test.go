...
var _ = Describe("Codec", func() {
	Describe("Viewport", func() {
		DescribeTable("should round-trip encode and decode",
			func(original spatial.Viewport) {
				w := orc.NewWriter(0)
			...
			Entry("fully populated", spatial.Viewport{Zoom: 1.5, Position: spatialv0.XY{X: 3.5, Y: 4.5}}),
			Entry("zero values", spatial.Viewport{Zoom: 0, Position: spatialv0.XY{X: 0, Y: 0}}),
		)
	})
})

func BenchmarkEncodeDecodeViewport(b *testing.B) {
	seed := spatial.Viewport{Zoom: 1.5, Position: spatialv0.XY{X: 3.5, Y: 4.5}}
	w := orc.NewWriter(0)
	r := orc.NewReader(nil)
	for b.Loop() {
		w.Reset()
...
func FuzzDecodeViewport(f *testing.F) {
	...
	{
		seed := spatial.Viewport{Zoom: 0, Position: spatialv0.XY{X: 0, Y: 0}}
		w := orc.NewWriter(0)
		if err := seed.EncodeOrc(w); err != nil {
			f.Fatal(err)
		}
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		var redecoded spatial.Viewport
