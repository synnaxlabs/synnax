	...
	Describe("DataType Enums", func() {
		Describe("Integer DataTypes", func() {
			...
			It("Should fail on a floating point type", func() {
				var dest zyn.DataType
				Expect(zyn.IntegerTypeSchema.Parse(zyn.Float32T, &dest)).To(MatchError(ContainSubstring("invalid enum value")))
			})
		})
		...
		Describe("Floating Point DataTypes", func() {
			for _, t := range zyn.FloatingPointTypes {
				...
				It("Should fail on an integer type", func() {
					var dest zyn.DataType
					Expect(zyn.FloatingPointTypeSchema.Parse(zyn.Int32T, &dest)).To(MatchError(ContainSubstring("invalid enum value")))
				})
			}
		})
		...
		Describe("Numeric DataTypes", func() {
			...
			It("Should fail on a string type", func() {
				var dest zyn.DataType
				Expect(zyn.NumericTypeSchema.Parse(zyn.StringT, &dest)).To(MatchError(ContainSubstring("invalid enum value")))
			})
		})
		...
		Describe("Primitive DataTypes", func() {
			...
			It("Should fail on an object type", func() {
				var dest zyn.DataType
				Expect(zyn.PrimitiveTypeSchema.Parse(zyn.ObjectT, &dest)).To(MatchError(ContainSubstring("invalid enum value")))
			})
		})
		...
		Describe("DataTypes", func() {
			for _, t := range zyn.DataTypes {
				It(fmt.Sprintf("Should parse %s successfully", t), func() {
					var dest zyn.DataType
					Expect(zyn.DataTypeSchema.Parse(t, &dest)).To(Succeed())
				})
			}
			It("Should fail on a random string", func() {
				var dest zyn.DataType
				Expect(zyn.DataTypeSchema.Parse("dog", &dest)).To(MatchError(ContainSubstring("invalid enum value")))
			})
		})
	})
