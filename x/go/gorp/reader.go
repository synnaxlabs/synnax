...
func wrapReader[K Key, E Entry[K]](base BaseReader, codec binary.Codec) *Reader[K, E] {
...
func (r Reader[K, E]) Get(ctx context.Context, key K) (E, error) {
	...
	if r.codec != nil {
		...
		err = r.BaseReader.Decode(ctx, b, &e)
	}
...
func wrapIterator[E any](wrapped kv.Iterator, decoder binary.Decoder, codec binary.Codec) *Iterator[E] {
	return &Iterator[E]{Iterator: wrapped, decoder: decoder, codec: codec}
}
...
func (k *Iterator[E]) Value(ctx context.Context) (entry *E) {
	...
	var err error
	if k.codec != nil {
		err = k.codec.Decode(ctx, k.Iterator.Value(), k.value)
	} else {
		err = k.decoder.Decode(ctx, k.Iterator.Value(), k.value)
	}
	if err != nil {
		k.err = err
		return nil
