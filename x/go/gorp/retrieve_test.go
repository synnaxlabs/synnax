...
var _ = Describe("Retrieve", Ordered, func() {
	...
	Describe("WhereKeys", func() {
		...
		Context("Single Entry", func() {
			...
			It("Should return a query.NotFound error if the where clause matches no entry", func() {
				err := gorp.NewRetrieve[int, entry]().
					Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == 241241, nil }).
					Entry(&entry{}).
					Exec(ctx, tx)
				Expect(err).To(HaveOccurred())
				Expect(errors.Is(err, query.NotFound)).To(BeTrue())
	...
	Describe("Where", func() {
		It("Should retrieve the entry by a filter parameter", func() {
			...
			Expect(gorp.NewRetrieve[int, entry]().
				Entries(&res).
				Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == entries[1].ID, nil }).
				Exec(ctx, tx),
			).To(Succeed())
		...
		It("Should support isMultiple filters", func() {
			...
			Expect(gorp.NewRetrieve[int, entry]().
				Entries(&res).
				Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == entries[1].ID, nil }).
				Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == entries[2].ID, nil }).
				Exec(ctx, tx),
			).To(Succeed())
		...
		It("Should require a filter to match when gorp.Required()", func() {
			...
			Expect(gorp.NewRetrieve[int, entry]().
				Entries(&res).
				Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == entries[1].ID, nil }, gorp.Required()).
				Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == entries[2].ID, nil }).
				Exec(ctx, tx),
			).To(Succeed())
			Expect(res).To(Equal([]entry{entries[1]}))
		...
		It("Should NOT return a query.NamesNotFound error if no entries are found", func() {
			...
			Expect(gorp.NewRetrieve[int, entry]().
				Entries(&res).
				Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == 444444, nil }).
				Exec(ctx, tx),
			).To(Succeed())
		...
		Describe("exists", func() {
			It("Should return true if ANY entries exist", func() {
				exists, err := gorp.NewRetrieve[int, entry]().
					Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == entries[1].ID, nil }).
					Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID == 44444, nil }).
					Exists(ctx, tx)
				Expect(err).To(Not(HaveOccurred()))
				Expect(exists).To(BeTrue())
			})
	...
	Describe("Count", func() {
		...
		Context("Where", func() {
			...
			It("Should return zero for non-matching filters", func() {
				count, err := gorp.NewRetrieve[int, entry]().
					Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID > 100, nil }).
					Count(ctx, tx)
				Expect(err).To(Not(HaveOccurred()))
			...
			It("Should handle multiple filters", func() {
				count, err := gorp.NewRetrieve[int, entry]().
					Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID < 5, nil }, gorp.Required()).
					Where(func(ctx gorp.FilterContext, e *entry) (bool, error) { return e.ID > 2, nil }, gorp.Required()).
					Count(ctx, tx)
				Expect(err).To(Not(HaveOccurred()))
