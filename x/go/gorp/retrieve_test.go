...
var _ = Describe("Retrieve", func() {
	...
	Describe("WhereKeys", func() {
		Context("Multiple Entries", func() {
			...
			It(
				"Should return a query.ErrNotFound error if ANY key is not found",
			...
			It(
				"Should still retrieve all possible entries even if some are not found",
				func(ctx SpecContext) {
					var res []entry
					Expect(gorp.NewRetrieve[int32, entry]().
			...
			Describe("Exists", func() {
				It(
					"Should return true if ALL keys have matching entries",
					func(ctx SpecContext) {
						Expect(gorp.NewRetrieve[int32, entry]().
				...
				It(
					"Should return false if ANY key has no matching entry",
					func(ctx SpecContext) {
						Expect(gorp.NewRetrieve[int32, entry]().
							Where(gorp.MatchKeys[int32, entry](entries[0].GorpKey(), 444444)).
		...
		Context("Composed with another filter", func() {
			...
			DescribeTable(
				"Should NOT return ErrNotFound when MatchKeys is combined with a filter and some keys are missing",
				...
				Entry(
					"Match filter passes every key",
					func() gorp.Retrieve[int32, entry] {
				...
				Entry(
					"Match filter further narrows the bound",
					func() gorp.Retrieve[int32, entry] {
						return gorp.NewRetrieve[int32, entry]().
							Where(gorp.MatchKeys[int32, entry](entries[0].GorpKey(), entries[1].GorpKey(), 444444)).
				...
				Entry(
					"MatchRaw filter passes every key",
					func() gorp.Retrieve[int32, entry] {
						return gorp.NewRetrieve[int32, entry]().
							Where(gorp.MatchKeys[int32, entry](entries[0].GorpKey(), 444444)).
			...
			It(
				"Should still return ErrNotFound when a single-entry bound query has no matches",
				func(ctx SpecContext) {
					Expect(gorp.NewRetrieve[int32, entry]().
						Where(gorp.MatchKeys[int32, entry](entries[0].GorpKey(), 444444)).
		...
		Context("Single Entry", func() {
			...
			It(
				"Should return a query.ErrNotFound error if the key is not found",
				func(ctx SpecContext) {
					Expect(gorp.NewRetrieve[int32, entry]().
			...
			It(
				"Should return a query.ErrNotFound error if the where clause matches no entry",
				func(ctx SpecContext) {
					Expect(gorp.NewRetrieve[int32, entry]().
			...
			Describe("exists", func() {
				It(
					"Should return true if the key has a matching entry",
					func(ctx SpecContext) {
						Expect(gorp.NewRetrieve[int32, entry]().
							Where(gorp.MatchKeys[int32, entry](entries[0].GorpKey())).
							Exists(ctx, tx)).To(BeTrue())
				...
				It(
					"Should return false if the key has no matching entry",
					func(ctx SpecContext) {
						Expect(gorp.NewRetrieve[int32, entry]().
							Where(gorp.MatchKeys[int32, entry](444444)).
							Exists(ctx, tx)).To(BeFalse())
	...
	Describe("WherePrefix", func() {
		Context("With byte-slice keys", func() {
			It("Should retrieve a single entry by exact prefix", func(ctx SpecContext) {
				...
				Expect(
					gorp.NewCreate[string, prefixEntry]().Entry(&r).Exec(ctx, tx),
				).To(Succeed())
				Expect(
					gorp.NewCreate[string, prefixEntry]().Entry(&r2).Exec(ctx, tx),
				).To(Succeed())
				var res []prefixEntry
			...
			It(
				"Should retrieve multiple entries matching a common prefix",
				func(ctx SpecContext) {
					r1 := prefixEntry{ID: 100, Data: "first"}
					r2 := prefixEntry{ID: 101, Data: "second"}
					r3 := prefixEntry{ID: 200, Data: "third"}
					Expect(
						gorp.NewCreate[string, prefixEntry]().Entry(&r1).Exec(ctx, tx),
					).To(Succeed())
					Expect(
						gorp.NewCreate[string, prefixEntry]().Entry(&r2).Exec(ctx, tx),
					).To(Succeed())
					Expect(
						gorp.NewCreate[string, prefixEntry]().Entry(&r3).Exec(ctx, tx),
					).To(Succeed())
					var res []prefixEntry
					Expect(gorp.NewRetrieve[string, prefixEntry]().
			...
			It(
				"Should return empty results when prefix doesn't match any entries",
				func(ctx SpecContext) {
					r := prefixEntry{ID: 123, Data: "data"}
					Expect(
						gorp.NewCreate[string, prefixEntry]().Entry(&r).Exec(ctx, tx),
			...
			It(
				"Should retrieve all entries with common base prefix",
				func(ctx SpecContext) {
	...
	Describe("Where", func() {
		...
		It("Should support Or combinator", func(ctx SpecContext) {
			...
			Expect(gorp.NewRetrieve[int32, entry]().
				...
				Where(gorp.Or(
					gorp.Match(
						func(_ gorp.Context, e *entry) (bool, error) { return e.ID == entries[1].ID, nil },
					),
					gorp.Match(
						func(_ gorp.Context, e *entry) (bool, error) { return e.ID == entries[2].ID, nil },
					),
				)).
				Exec(ctx, tx),
			).To(Succeed())
		...
		It("Should support Not combinator", func(ctx SpecContext) {
			...
			Expect(gorp.NewRetrieve[int32, entry]().
				...
				Where(gorp.Not(
					gorp.Match(
						func(_ gorp.Context, e *entry) (bool, error) { return e.ID >= 5, nil },
					),
				)).
				Exec(ctx, tx),
			).To(Succeed())
		...
		It("Should support nested And(Or(...), Or(...))", func(ctx SpecContext) {
			...
			Expect(gorp.NewRetrieve[int32, entry]().
				...
				Where(gorp.And(
					gorp.Or(
						gorp.Match(
							func(_ gorp.Context, e *entry) (bool, error) { return e.ID == 1, nil },
						),
						gorp.Match(
							func(_ gorp.Context, e *entry) (bool, error) { return e.ID == 2, nil },
						),
						gorp.Match(
							func(_ gorp.Context, e *entry) (bool, error) { return e.ID == 3, nil },
						),
					),
					gorp.Or(
						gorp.Match(
						...
						gorp.Match(
							func(_ gorp.Context, e *entry) (bool, error) { return e.ID == 4, nil },
						),
					),
				)).
				Exec(ctx, tx),
		...
		It(
			"Should NOT return a query.NamesNotFound error if no entries are found",
			func(ctx SpecContext) {
				var res []entry
				Expect(gorp.NewRetrieve[int32, entry]().
					Entries(&res).
			...
			It("Should return true if ANY entries exist", func(ctx SpecContext) {
				Expect(gorp.NewRetrieve[int32, entry]().
					Where(gorp.Or(
						...
						gorp.Match(
							func(_ gorp.Context, e *entry) (bool, error) { return e.ID == 44444, nil },
						),
					)).
					Exists(ctx, tx)).To(BeTrue())
			})
			It(
				"Should return false if ALL entries do not exist",
				func(ctx SpecContext) {
					Expect(gorp.NewRetrieve[int32, entry]().
						Where(gorp.Or(
							gorp.Match(
								func(_ gorp.Context, e *entry) (bool, error) { return e.ID == 444444, nil },
							),
							gorp.Match(
								func(_ gorp.Context, e *entry) (bool, error) { return e.ID == 44444, nil },
							),
	...
	Describe("WhereRaw", func() {
		It(
			"Should filter entries by their raw bytes before decoding",
			func(ctx SpecContext) {
				var res []entry
				Expect(gorp.NewRetrieve[int32, entry]().
					Entries(&res).
		...
		It(
			"Should skip entries that do not match the raw filter",
			func(ctx SpecContext) {
				var res []entry
				Expect(gorp.NewRetrieve[int32, entry]().
					Entries(&res).
		...
		It(
			"Should return an error if the filter encounters an error",
			func(ctx SpecContext) {
				var res []entry
				Expect(gorp.NewRetrieve[int32, entry]().
					Entries(&res).
	...
	Describe("Count", func() {
		...
		Context("Where", func() {
			...
			It(
				"Should pass the correct transaction in the Context of the Where clause",
				func(ctx SpecContext) {
					callCount := 0
					Expect(gorp.NewRetrieve[int32, entry]().
	...
	Describe("Validate", func() {
		...
		It(
			"Should receive the final bound result set via execFilter",
			func(ctx SpecContext) {
				var seen []entry
				var res []entry
				Expect(gorp.NewRetrieve[int32, entry]().
		...
		It(
			"Should receive the filtered result set when Where is also set",
			func(ctx SpecContext) {
				var seen []entry
				var res []entry
				Expect(gorp.NewRetrieve[int32, entry]().
		...
		It("Should short-circuit Exec with a validator error", func(ctx SpecContext) {
			...
			Expect(gorp.NewRetrieve[int32, entry]().
				...
				Exec(ctx, tx),
			).To(MatchError(ContainSubstring("validator rejected query")))
		})

		It(
			"Should short-circuit on the first validator error when multiple are attached",
			func(ctx SpecContext) {
				var (
				...
				Expect(gorp.NewRetrieve[int32, entry]().
					...
					Exec(ctx, tx),
				).To(MatchError(ContainSubstring("second validator failed")))
		...
		It(
			"Should observe an empty slice when no entries match",
			func(ctx SpecContext) {
				var seen []entry
				var res []entry
				Expect(gorp.NewRetrieve[int32, entry]().
		...
		It(
			"Should receive the filtered result set on Count with Where",
			func(ctx SpecContext) {
				var seen []entry
				Expect(gorp.NewRetrieve[int32, entry]().
					Where(gorp.Match(func(_ gorp.Context, e *entry) (bool, error) {
		...
		It(
			"Should short-circuit Count with Where on a validator error",
			func(ctx SpecContext) {
				Expect(gorp.NewRetrieve[int32, entry]().
		...
		It(
			"Should preserve ErrNotFound when WhereKeys partial-match also triggers a validator error",
			func(ctx SpecContext) {
				var res []entry
				err := gorp.NewRetrieve[int32, entry]().
					Entries(&res).
