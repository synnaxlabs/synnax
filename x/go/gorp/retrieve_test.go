...
var _ = Describe("Retrieve", func() {
	...
	Describe("WhereKeys", func() {
		Context("Multiple Entries", func() {
			...
			It("Should return a query.NotFound error if ANY key is not found", func(ctx SpecContext) {
				var res []entry
		...
		Context("Single Entry", func() {
			...
			It("Should return a query.NotFound error if the key is not found", func(ctx SpecContext) {
				Expect(gorp.NewRetrieve[int, entry]().
					WhereKeys(444444).
					Entry(&entry{}).
					Exec(ctx, tx)).Error().To(HaveOccurredAs(query.ErrNotFound))
			})
			It("Should return a query.NotFound error if the where clause matches no entry", func(ctx SpecContext) {
				Expect(gorp.NewRetrieve[int, entry]().
		...
		Context("With byte-slice keys", func() {
			...
			It("Should retrieve multiple entries matching a common prefix", func(ctx SpecContext) {
				...
				Expect(gorp.NewCreate[[]byte, prefixEntry]().
					Entry(&r1).Exec(ctx, tx)).To(Succeed())
				Expect(gorp.NewCreate[[]byte, prefixEntry]().
					Entry(&r2).Exec(ctx, tx)).To(Succeed())
				Expect(gorp.NewCreate[[]byte, prefixEntry]().
					Entry(&r3).Exec(ctx, tx)).To(Succeed())
				var res []prefixEntry
				Expect(gorp.NewRetrieve[[]byte, prefixEntry]().
			...
			It("Should return empty results when prefix doesn't match any entries", func(ctx SpecContext) {
				r := prefixEntry{ID: 123, Data: "data"}
				Expect(gorp.NewCreate[[]byte, prefixEntry]().
					Entry(&r).Exec(ctx, tx)).To(Succeed())
				var res []prefixEntry
				Expect(gorp.NewRetrieve[[]byte, prefixEntry]().
