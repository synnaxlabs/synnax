...
// Reader reads primitive data types using a given byte order. It supports two
...
// NewReader creates a new Reader in io.Reader mode with the given byte order.
...
func (r *Reader) Uint16() (uint16, error) {
	if r.data != nil {
		if r.pos+2 > len(r.data) {
			return 0, r.shortDataErr(2)
		}
		v := r.byteOrder.Uint16(r.data[r.pos:])
		r.pos += 2
		return v, nil
	}
...
func (r *Reader) Uint64() (uint64, error) {
	if r.data != nil {
		if r.pos+8 > len(r.data) {
			return 0, r.shortDataErr(8)
		}
		v := r.byteOrder.Uint64(r.data[r.pos:])
		r.pos += 8
		return v, nil
	}
...
func (r *Reader) shortDataErr(_ int) error {
	if r.pos >= len(r.data) {
		return io.EOF
	}
	return io.ErrUnexpectedEOF
