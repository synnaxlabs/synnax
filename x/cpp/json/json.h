...
public:
    ...
    /// @brief converts the parser's accumulated errors into a errors::Error.
    /// @returns errors::NIL if no errors, a simple validation error if there's a
    ...
    [[nodiscard]] errors::Error error() const {
        ...
        return errors::Error{errors::VALIDATION, error_json().dump()};
    }
