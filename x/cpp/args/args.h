...
///     x::args::Parser parser(argc, argv);
///
///     // Required argument
...
public:
    ...
    std::vector<errors::Error> errors; ///< Any errors encountered during parsing
    ...
    /// @return The first error or errors::NIL if no errors
    [[nodiscard]] errors::Error error() const {
        if (errors.empty()) return errors::NIL;
        return errors.at(0);
    }
