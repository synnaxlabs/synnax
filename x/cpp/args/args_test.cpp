...
TEST_F(ArgsTest, TestRequiredStringHappyPath) {
...
TEST_F(ArgsTest, TestRequiredIntegerHappyPath) {
...
TEST_F(ArgsTest, TestRequiredIntegerInvalidFormat) {
...
TEST_F(ArgsTest, TestOptionalWithDefault) {
    auto [argc, argv] = make_args({"program"});
    parser = Parser(argc, argv);
...
TEST_F(ArgsTest, TestOptionalWithValue) {
    auto [argc, argv] = make_args({"program", "--count", "42"});
    parser = Parser(argc, argv);
    const auto count = parser.field<int>("--count", 100);
    ASSERT_NIL(parser.error());
...
TEST_F(ArgsTest, TestMultipleArguments) {
...
TEST_F(ArgsTest, TestEqualsFormatString) {
    auto [argc, argv] = make_args({"program", "--name=test"});
    parser = Parser(argc, argv);
    const auto name = parser.field<std::string>("--name");
    ASSERT_NIL(parser.error());
...
TEST_F(ArgsTest, TestEqualsFormatInteger) {
    auto [argc, argv] = make_args({"program", "--count=42"});
    parser = Parser(argc, argv);
    const auto count = parser.field<int>("--count");
    ASSERT_NIL(parser.error());
...
TEST_F(ArgsTest, TestEqualsFormatOptional) {
...
TEST_F(ArgsTest, TestEqualsFormatInvalid) {
    auto [argc, argv] = make_args({"program", "--count=not_a_number"});
    parser = Parser(argc, argv);
    const auto count = parser.field<int>("--count");
    ASSERT_EQ(count, 0);
...
TEST_F(ArgsTest, TestMixedFormatArguments) {
    auto [argc, argv] = make_args(
...
TEST_F(ArgsTest, TestPrefixHandling) {
    auto [argc, argv] = make_args(
...
TEST_F(ArgsTest, TestSingleLetterFlags) {
...
TEST_F(ArgsTest, TestWeirdArgumentNames) {
    auto [argc, argv] = make_args(
        {"program",
         "---triple-dash",
         "----quad-dash=value",
...
TEST_F(ArgsTest, TestDuplicateArguments) {
    auto [argc, argv] = make_args(
        {"program",
         "--name",
         "first",
...
TEST_F(ArgsTest, TestRegressionDash) {
    auto [argc, argv] = make_args({
        "program",
        "--correct-skew=true",
    });
...
TEST_F(ArgsTest, TestVectorArguments) {
    auto [argc, argv] = make_args(
        {"program",
