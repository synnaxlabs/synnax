// Code generated by oracle. DO NOT EDIT.

#pragma once

#include <utility>
#include "x/cpp/json/json.h"
#include "x/cpp/errors/errors.h"
#include "x/go/spatial/pb/spatial.pb.h"


namespace x::spatial {
struct XY {
    double x;
    double y;

    static XY parse(x::json::Parser parser);
    [[nodiscard]] x::json::json to_json() const;

    using proto_type = ::x::spatial::pb::XY;
    [[nodiscard]] ::x::spatial::pb::XY to_proto() const;
    static std::pair<XY, x::errors::Error> from_proto(const ::x::spatial::pb::XY& pb);
};

struct ClientXY {
    double client_x;
    double client_y;

    static ClientXY parse(x::json::Parser parser);
    [[nodiscard]] x::json::json to_json() const;

    using proto_type = ::x::spatial::pb::ClientXY;
    [[nodiscard]] ::x::spatial::pb::ClientXY to_proto() const;
    static std::pair<ClientXY, x::errors::Error> from_proto(const ::x::spatial::pb::ClientXY& pb);
};

struct Dimensions {
    double width;
    double height;

    static Dimensions parse(x::json::Parser parser);
    [[nodiscard]] x::json::json to_json() const;

    using proto_type = ::x::spatial::pb::Dimensions;
    [[nodiscard]] ::x::spatial::pb::Dimensions to_proto() const;
    static std::pair<Dimensions, x::errors::Error> from_proto(const ::x::spatial::pb::Dimensions& pb);
};

struct SignedDimensions {
    double signed_width;
    double signed_height;

    static SignedDimensions parse(x::json::Parser parser);
    [[nodiscard]] x::json::json to_json() const;

    using proto_type = ::x::spatial::pb::SignedDimensions;
    [[nodiscard]] ::x::spatial::pb::SignedDimensions to_proto() const;
    static std::pair<SignedDimensions, x::errors::Error> from_proto(const ::x::spatial::pb::SignedDimensions& pb);
};

struct Bounds {
    double lower;
    double upper;

    static Bounds parse(x::json::Parser parser);
    [[nodiscard]] x::json::json to_json() const;

    using proto_type = ::x::spatial::pb::Bounds;
    [[nodiscard]] ::x::spatial::pb::Bounds to_proto() const;
    static std::pair<Bounds, x::errors::Error> from_proto(const ::x::spatial::pb::Bounds& pb);
};
}
