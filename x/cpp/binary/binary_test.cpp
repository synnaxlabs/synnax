...
TEST(BinaryReader, testMultipleReads) {
    std::vector<uint8_t> buffer = {
        ...
        0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11  // uint64
    };
...
TEST(BinaryReader, testStartingOffset) {
    std::vector<uint8_t> buffer = {
        0x01, 0x02, 0x03,                   // initial bytes to skip
        0xAA,                               // uint8
        0xEE, 0xDD, 0xCC, 0xBB, 0x00        // uint32
    };
...
TEST(BinaryStressTest, testVariousBytePatterns) {
    ...
    for (const auto& value : test_values) {
        ...
        ASSERT_EQ(decoded, value) << "Failed for value 0x" 
                                 << std::hex << value;
    }
}