...
/// @return errors::Error if there's an unsupported JSON type, NIL on success
inline errors::Error push_json_value(lua_State *L, const json::json &value) {
    if (value.is_null())
        lua_pushnil(L);
        ...
        return errors::Error("unsupported JSON type");
    return errors::NIL;
}
...
/// @return errors::Error if there's an error pushing the JSON value, NIL on
/// success
inline errors::Error
set_global_json_value(lua_State *L, const std::string &name, const json::json &value) {
...
/// @return errors::Error if the input is not a JSON object or if setting any value
/// fails
inline errors::Error
set_globals_from_json_object(lua_State *L, const json::json &object) {
    if (!object.is_object())
        return errors::Error(errors::VALIDATION, "input must be a JSON object");
...
/// @return errors::Error if there's a type mismatch or JSON parsing error
[[nodiscard]] inline errors::Error set_global_sample_value(
    lua_State *L,
    ...
    try {
        if (data_type == telem::FLOAT64_T)
            ...
            try {
                ...
                const auto parsed = json::json::parse(str_val);
...
[[nodiscard]] inline std::pair<telem::Series, errors::Error>
to_series(lua_State *L, const int index, const telem::DataType &data_type) {
    ...
    if (data_type == telem::STRING_T) {
        if (is_boolean) {
            ...
            return {telem::Series(value), errors::NIL};
        }
        if (is_numeric) {
            return {
    ...
    if (data_type == telem::FLOAT64_T)
        return {telem::Series(get_numeric_value(index), data_type), errors::NIL};
