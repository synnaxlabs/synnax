...
TEST_F(XLuaTest, SetGlobalTelemFloat64) {
    const auto err = set_global_sample_value(L, "val", telem::FLOAT64_T, 3.14159);
...
TEST_F(XLuaTest, SetGlobalTelemFloat32) {
    const auto err = set_global_sample_value(L, "val", telem::FLOAT32_T, 3.14f);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalTelemInt64) {
    const auto err = set_global_sample_value(L, "val", telem::INT64_T, int64_t{42});
    ASSERT_FALSE(err) << err;
...
TEST_F(XLuaTest, SetGlobalTelemInt32) {
    const auto
        err = set_global_sample_value(L, "val", telem::INT32_T, int32_t{2147483647});
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalTelemInt16) {
    const auto err = set_global_sample_value(L, "val", telem::INT16_T, int16_t{32767});
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalTelemInt8) {
    const auto err = set_global_sample_value(L, "val", telem::INT8_T, int8_t{127});
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalTelemUInt32) {
    const auto
        err = set_global_sample_value(L, "val", telem::UINT32_T, uint32_t{4294967295});
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalTelemUInt16) {
    const auto
        err = set_global_sample_value(L, "val", telem::UINT16_T, uint16_t{65535});
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalTelemUInt8) {
    const auto err = set_global_sample_value(L, "val", telem::UINT8_T, uint8_t{255});
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalTelemString) {
    const auto
        err = set_global_sample_value(L, "val", telem::STRING_T, std::string("hello"));
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalTelemUInt64Normal) {
    uint64_t val = 1000;
    const auto err = set_global_sample_value(L, "val", telem::UINT64_T, val);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_isinteger(L, -1));
...
TEST_F(XLuaTest, SetGlobalTelemTypeMismatchFloat64) {
    const auto err = set_global_sample_value(
        L,
        "val",
        telem::FLOAT64_T,
...
TEST_F(XLuaTest, SetGlobalTelemTypeMismatchInt64) {
    const auto err = set_global_sample_value(L, "val", telem::INT64_T, 3.14159);
    EXPECT_TRUE(err);
    EXPECT_EQ(err, errors::VALIDATION);
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_isnil(L, -1));
...
TEST_F(XLuaTest, SetGlobalJsonNull) {
    ...
    const auto err = set_global_json_value(L, "val", j_null);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_isnil(L, -1));
    lua_pop(L, 1);
...
TEST_F(XLuaTest, SetGlobalJsonBoolean) {
    ...
    const auto err = set_global_json_value(L, "val", j_bool);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalJsonInteger) {
    ...
    const auto err = set_global_json_value(L, "val", j_int);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_isinteger(L, -1));
    EXPECT_EQ(lua_tointeger(L, -1), 42);
...
TEST_F(XLuaTest, SetGlobalJsonFloat) {
    ...
    const auto err = set_global_json_value(L, "val", j_float);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_isnumber(L, -1));
    EXPECT_DOUBLE_EQ(lua_tonumber(L, -1), 3.14159);
...
TEST_F(XLuaTest, SetGlobalJsonString) {
    ...
    const auto err = set_global_json_value(L, "val", j_string);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_isstring(L, -1));
...
TEST_F(XLuaTest, SetGlobalJsonArray) {
    ...
    const auto err = set_global_json_value(L, "val", j_array);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
...
TEST_F(XLuaTest, SetGlobalJsonObject) {
    ...
    const auto err = set_global_json_value(L, "val", j_object);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_istable(L, -1));
...
TEST_F(XLuaTest, SetGlobalJsonNestedStructure) {
    ...
    const auto err = set_global_json_value(L, "val", j_nested);
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_istable(L, -1));
...
TEST_F(XLuaTest, SetGlobalsFromJsonObjectSimple) {
    ...
    EXPECT_TRUE(set_globals_from_json_object(L, globals).ok());
...
TEST_F(XLuaTest, SetGlobalsFromJsonObjectInvalid) {
    ...
    EXPECT_FALSE(set_globals_from_json_object(L, invalid_json).ok());
}
...
TEST_F(XLuaTest, SetGlobalTelemJsonSimple) {
    ...
    const auto
        err = set_global_sample_value(L, "val", telem::JSON_T, nlohmann::to_string(j));
    ASSERT_FALSE(err) << err;
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_istable(L, -1));
    lua_getfield(L, -1, "key");
...
TEST_F(XLuaTest, SetGlobalTelemJsonComplex) {
    ...
    ASSERT_NIL(set_global_sample_value(L, "val", telem::JSON_T, j.dump()));
...
TEST_F(XLuaTest, SetGlobalTelemJsonInvalid) {
    ASSERT_OCCURRED_AS(
        set_global_sample_value(L, "val", telem::JSON_T, "invalid json::json"),
        errors::VALIDATION
    );
    lua_getglobal(L, "val");
    EXPECT_TRUE(lua_isnil(L, -1));
...
TEST_F(XLuaTest, ToSeriesBooleanCoercion) {
    ...
    {
        auto [series, err] = to_series(L, -1, telem::FLOAT64_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::FLOAT64_T);
        EXPECT_DOUBLE_EQ(series.at<double>(0), 1.0);
    }

    {
        auto [series, err] = to_series(L, -1, telem::FLOAT32_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::FLOAT32_T);
        EXPECT_FLOAT_EQ(series.at<float>(0), 1.0f);
    }

    {
        auto [series, err] = to_series(L, -1, telem::INT64_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::INT64_T);
        EXPECT_EQ(series.at<int64_t>(0), 1);
    }

    {
        auto [series, err] = to_series(L, -1, telem::INT32_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::INT32_T);
        EXPECT_EQ(series.at<int32_t>(0), 1);
    }

    {
        auto [series, err] = to_series(L, -1, telem::INT16_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::INT16_T);
        EXPECT_EQ(series.at<int16_t>(0), 1);
    }

    {
        auto [series, err] = to_series(L, -1, telem::INT8_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::INT8_T);
        EXPECT_EQ(series.at<int8_t>(0), 1);
    }

    {
        auto [series, err] = to_series(L, -1, telem::UINT64_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::UINT64_T);
        EXPECT_EQ(series.at<uint64_t>(0), 1);
    }

    {
        auto [series, err] = to_series(L, -1, telem::UINT32_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::UINT32_T);
        EXPECT_EQ(series.at<uint32_t>(0), 1);
    }

    {
        auto [series, err] = to_series(L, -1, telem::UINT16_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::UINT16_T);
        EXPECT_EQ(series.at<uint16_t>(0), 1);
    }

    {
        auto [series, err] = to_series(L, -1, telem::UINT8_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::UINT8_T);
...
TEST_F(XLuaTest, ToSeriesStringHandling) {
    ...
    {
        auto [series, err] = to_series(L, -1, telem::STRING_T);
        EXPECT_FALSE(err) << err;
        EXPECT_EQ(series.data_type(), x::telem::STRING_T);
...
TEST_F(XLuaTest, booleanToString) {
    // Test true
    lua_pushboolean(L, true);
    auto [series1, err1] = to_series(L, -1, telem::STRING_T);
    EXPECT_FALSE(err1) << err1;
    EXPECT_EQ(series1.data_type(), x::telem::STRING_T);
    EXPECT_EQ(series1.at<std::string>(0), "true");
    lua_pop(L, 1);

    // Test false
    lua_pushboolean(L, false);
    auto [series2, err2] = to_series(L, -1, telem::STRING_T);
    EXPECT_FALSE(err2) << err2;
    EXPECT_EQ(series2.data_type(), x::telem::STRING_T);
...
TEST_F(XLuaTest, ToSeriesTypeMismatch) {
    ...
    lua_pushstring(L, "not a number");
    auto [series2, err2] = to_series(L, -1, telem::INT32_T);
...
TEST_F(XLuaTest, ToSeriesNilHandling) {
    ...
    auto [series1, err1] = to_series(L, -1, telem::FLOAT64_T);
    EXPECT_TRUE(err1);
    EXPECT_EQ(err1, errors::VALIDATION);
    auto [series2, err2] = to_series(L, -1, telem::INT32_T);
    EXPECT_TRUE(err2);
    EXPECT_EQ(err2, errors::VALIDATION);
    auto [series3, err3] = to_series(L, -1, telem::STRING_T);
    EXPECT_TRUE(err3);
...
TEST_F(XLuaTest, ToSeriesInvalidIndex) {
    auto [series1, err1] = to_series(L, 999, telem::FLOAT64_T);
    EXPECT_TRUE(err1);
    EXPECT_EQ(err1, errors::VALIDATION);
}
...
TEST_F(XLuaTest, ToSeriesUnsupportedTypes) {
    ...
    auto [series2, err2] = to_series(L, -1, telem::FLOAT64_T);
    EXPECT_TRUE(err2);
    EXPECT_EQ(err2, errors::VALIDATION);
    lua_pop(L, 1);
    ...
    auto [series3, err3] = to_series(L, -1, telem::FLOAT64_T);
    EXPECT_TRUE(err3);
    EXPECT_EQ(err3, errors::VALIDATION);
    lua_pop(L, 1);
}
...
TEST_F(XLuaTest, Int64Max) {
    ...
    auto [series1, err1] = to_series(L, -1, telem::INT64_T);
    EXPECT_FALSE(err1) << err1;
