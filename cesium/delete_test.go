	...
	for fsName, makeFS := range fileSystems {
		Context("FS: "+fsName, Ordered, func() {
			...
			Describe("Delete chunks", Ordered, func() {
				...
				//			Expect(db.CreateChannel(
				//				ctx,
				//				cesium.Channel{Key: basic4, DataType: telem.Int64T, Rate: 1 * telem.Hz},
				//			)).To(Succeed())
				//
				//			By("Writing data to the channel")
				//			for i := 1; i <= 9; i++ {
				...
				//				}
				//				Expect(db.WriteArray(ctx, basic4, telem.TimeStamp(10*i)*telem.SecondTS, telem.NewSeriesV[int64](data...))).To(Succeed())
				//			}
				//
				//			// should have been written to 10 - 99
				//			By("Deleting channel data")
				//			Expect(db.DeleteTimeRange(ctx, []cesium.ChannelKey{basic4}, telem.TimeRange{
				//				Start: 33 * telem.SecondTS,
				//				End:   75 * telem.SecondTS,
				//			})).To(Succeed())
				//
				//			frame, err := db.Read(ctx, telem.TimeRange{Start: 10 * telem.SecondTS, End: 100 * telem.SecondTS}, basic4)
				//			Expect(err).To(BeNil())
				...
				//			// should have been written to 10 - 99
				//			By("Deleting channel data")
				//			Expect(db.DeleteTimeRange(ctx, []cesium.ChannelKey{basic6}, telem.TimeRangeMax)).To(Succeed())
				//
				//			frame := MustSucceed(db.Read(ctx, telem.TimeRangeMax, basic6))
