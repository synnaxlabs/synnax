	...
	for fsName, fs := range fileSystems {
		...
		Context("FS: "+fsName, func() {
			Describe("Index", func() {
				...
				Context("Persist stranded data on close", func() {
			...
			Describe("Channel Indexed", func() {
				...
				Describe("Auto-Commit", func() {
					It("Should automatically commit each write with an approximate timestamp when there is no end on the writer", func() {
						By("Opening a writer")
						w1, _ := MustSucceed2(indexDB.OpenWriter(ctx, unary.WriterConfig{
							Start:      telem.TimeStamp(0),
							Subject:    control.Subject{Key: "index_foo"},
							AutoCommit: config.True(),
						}))
						w2, _ := MustSucceed2(db.OpenWriter(ctx, unary.WriterConfig{
							Start:      telem.TimeStamp(0),
							Subject:    control.Subject{Key: "foo"},
							AutoCommit: config.True(),
						}))
						...
						Expect(MustSucceed(w2.Write(ctx, telem.NewSeriesV[int64](100, 101, 102, 103, 104, 105)))).To(Equal(telem.Alignment(0)))
						...
						Expect(i.Next(ctx, 10*telem.Second)).To(BeTrue())
						Expect(i.Value().Series[0].TimeRange).To(Equal((0 * telem.SecondTS).Range(5*telem.SecondTS + 1)))
						Expect(i.Value().Series[0].Data).To(Equal(telem.NewSeriesV[int64](100, 101, 102, 103, 104, 105).Data))

						By("Writing more data to the writer")
						Expect(MustSucceed(w1.Write(ctx, telem.NewSecondsTSV(10, 11, 12, 13, 14))))
						Expect(MustSucceed(w2.Write(ctx, telem.NewSeriesV[int64](110, 111, 112, 113, 114)))).To(Equal(telem.Alignment(6)))
						Expect(i.SeekFirst(ctx)).To(BeTrue())
						Expect(i.Next(ctx, 20*telem.Second)).To(BeTrue())
						Expect(i.Value().Series[0].TimeRange).To(Equal((0 * telem.SecondTS).Range(14*telem.SecondTS + 1)))
						Expect(i.Value().Series[0].Data).To(Equal(telem.NewSeriesV[int64](100, 101, 102, 103, 104, 105, 110, 111, 112, 113, 114).Data))
				...
				It("Should automatically commit each write with an exact timestamp when there is an end on the writer", func() {
					...
					By("Writing more data to the writer")
					Expect(MustSucceed(w1.Write(ctx, telem.NewSecondsTSV(10, 11, 12, 13, 14, 15)))).To(Equal(telem.Alignment(6)))
					Expect(MustSucceed(w2.Write(ctx, telem.NewSeriesV[int64](200, 201, 202, 203, 204, 205)))).To(Equal(telem.Alignment(6)))
					Expect(i.SeekFirst(ctx)).To(BeTrue())
					Expect(i.Next(ctx, 50*telem.Second)).To(BeTrue())
					Expect(i.Value().Series[0].TimeRange).To(Equal((0 * telem.SecondTS).Range(20 * telem.SecondTS)))
					Expect(i.Value().Series[0].Data).To(Equal(telem.NewSeriesV[int64](100, 101, 102, 103, 104, 105, 200, 201, 202, 203, 204, 205).Data))

					By("Writing data beyond the end")
					_, err := w2.Write(ctx, telem.NewSecondsTSV(18, 19, 20, 21, 22))
					Expect(err).To(MatchError(ContainSubstring("discontinuous")))
				...
				It("Should automatically commit and persist the index each time when the threshold is not set", func() {
					...
					By("Writing more data to the writer")
					Expect(MustSucceed(w1.Write(ctx, telem.NewSecondsTSV(10, 11, 12, 13)))).To(Equal(telem.Alignment(6)))
					Expect(MustSucceed(w2.Write(ctx, telem.NewSeriesV[int64](200, 201, 202, 203)))).To(Equal(telem.Alignment(6)))
					f = MustSucceed(db.FS.Open("index.domain", os.O_RDONLY))
					p = extractPointer(f)
					Expect(f.Close()).To(Succeed())
					Expect(p.Start).To(Equal(0 * telem.SecondTS))
					...
					// TODO: check the underlying data is correct
				...
				It("Should automatically commit and persist the index each time the threshold is reached", func() {
					...
					By("Writing more telemetry to the writer")
					Expect(MustSucceed(w1.Write(ctx, telem.NewSecondsTSV(8)))).To(Equal(telem.Alignment(6)))
					Expect(MustSucceed(w2.Write(ctx, telem.NewSeriesV[int64](108)))).To(Equal(telem.Alignment(6)))
					...
					By("Writing more telemetry to the writer")
					Expect(MustSucceed(w1.Write(ctx, telem.NewSecondsTSV(10, 13, 15, 16)))).To(Equal(telem.Alignment(7)))
					Expect(MustSucceed(w2.Write(ctx, telem.NewSeriesV[int64](200, 203, 205, 206)))).To(Equal(telem.Alignment(7)))
					...
					By("Writing more data to the writer")
					Expect(MustSucceed(w1.Write(ctx, telem.NewSecondsTSV(20, 21, 22, 23)))).To(Equal(telem.Alignment(11)))
					Expect(MustSucceed(w2.Write(ctx, telem.NewSeriesV[int64](300, 301, 302, 303)))).To(Equal(telem.Alignment(11)))
