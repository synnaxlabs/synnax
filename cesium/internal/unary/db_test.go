...
var _ = Describe("DB Metadata Operations", func() {
	...
	Describe("Close", func() {
		...
		It("Should return an error when methods are called on a closed DB", func() {
			...
			Expect(db.RenameChannelInMeta("new_name")).To(MatchError(unary.ErrDBClosed))
			Expect(db.SetChannelKeyInMeta(testutil.GenerateChannelKey())).To(MatchError(unary.ErrDBClosed))
			Expect(db.SetIndexKeyInMeta(testutil.GenerateChannelKey())).To(MatchError(unary.ErrDBClosed))
			Expect(db.SetChannelKeyInMeta(testutil.GenerateChannelKey())).To(MatchError(unary.ErrDBClosed))
