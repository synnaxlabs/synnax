		...
		Context("FS: "+fsName, Ordered, func() {
			...
			Describe("Write + Read", func() {
				...
				It("Should write and read JSON data", func(ctx SpecContext) {
					...
					Expect(
						unary.Write(ctx, jsonDB, 20*telem.SecondTS, jsonSeries),
					).To(Succeed())
					frame := MustSucceed(
						jsonDB.Read(
							ctx,
							(20 * telem.SecondTS).Range(22*telem.SecondTS),
						),
					)
					Expect(frame.Count()).To(Equal(1))
				...
				It("Should write and read bytes data", func(ctx SpecContext) {
					...
					frame := MustSucceed(
						bytesDB.Read(
							ctx,
							(30 * telem.SecondTS).Range(33*telem.SecondTS),
						),
					)
					Expect(frame.Count()).To(Equal(1))
				...
				It("Should write and read empty strings", func(ctx SpecContext) {
					...
					frame := MustSucceed(
						dataDB.Read(
							ctx,
							(40 * telem.SecondTS).Range(43*telem.SecondTS),
						),
					)
					Expect(frame.Count()).To(Equal(1))
					Expect(
				...
				It(
					"Should write and read strings containing newlines",
					func(ctx SpecContext) {
						...
						frame := MustSucceed(
							dataDB.Read(
								ctx,
								(50 * telem.SecondTS).Range(52*telem.SecondTS),
							),
						)
						Expect(frame.Count()).To(Equal(1))
						Expect(
			...
			Describe("Offset cache", func() {
				It(
					...
					func(ctx SpecContext) {
						...
						Expect(
							rec.Count(xfs.MatchOp(xfs.OpReadAt), xfs.MatchLength(4)),
						).To(BeZero())
					},
				)

				It(
					"Should rebuild a cold cache without per-sample length-prefix reads",
					func(ctx SpecContext) {
			...
			Describe("Writer", func() {
				It(
					"Should track alignment correctly across writes",
					func(ctx SpecContext) {
				...
				It("Should reject writes with wrong data type", func(ctx SpecContext) {
					...
					Expect(
						w.Write(telem.NewSeriesV[int64](1, 2, 3)),
					).Error().
			...
			Describe("Delete", func() {
				It("Should delete data in a time range", func(ctx SpecContext) {
					...
					Expect(
						dataDB.Delete(
							ctx,
							(302 * telem.SecondTS).Range(304*telem.SecondTS),
						),
					).To(Succeed())
					...
					frame := MustSucceed(
						dataDB.Read(
							ctx,
							(300 * telem.SecondTS).Range(305*telem.SecondTS),
						),
					)
			...
			Describe("GarbageCollect", func() {
				It("Should garbage collect after deletion", func(ctx SpecContext) {
					...
					Expect(
						gcDB.Delete(
							ctx,
							(502 * telem.SecondTS).Range(504*telem.SecondTS),
						),
					).To(Succeed())
					...
					frame := MustSucceed(
						gcDB.Read(
							ctx,
							(500 * telem.SecondTS).Range(505*telem.SecondTS),
						),
					)
					Expect(frame.Count()).To(Equal(2))
