...
import (
	...
	"github.com/synnaxlabs/cesium/s/segment"
	...
	Context("Single Segment", func() {
		Context("Within Bounds", func() {
			It("Should accumulate the segment correctly", func() {
				a := &segment.Accumulator{Density: 1, Bounds: position.Range{End: 100}}
				Expect(a.Accumulate(segment.MD{
					Start:  0,
					Size:   100,
					Offset: 0,
				})).To(BeTrue())
				Expect(a.Segments).To(HaveLen(1))
				Expect(a.Segments[0].Start).To(Equal(position.Position(0)))
		...
		Context("After Bounds", func() {
			It("Should not accumulate the segment", func() {
				a := &segment.Accumulator{Density: 1, Bounds: position.Range{Start: 100, End: 200}}
				Expect(a.Accumulate(segment.MD{
					Start:  0,
					Size:   100,
					Offset: 0,
		...
		Context("Overlaps with Beginning of Bounds", func() {
			It("Should accumulate the segment correctly", func() {
				a := &segment.Accumulator{Density: 1, Bounds: position.Range{Start: 50, End: 200}}
				Expect(a.Accumulate(segment.MD{
					Start:  0,
					Size:   100,
					Offset: 0,
				...
				Expect(a.Segments).To(HaveLen(1))
				Expect(a.Segments[0].Start).To(Equal(position.Position(50)))
		...
		Context("Overlaps with End of Bounds", func() {
			It("Should accumulate the segment correctly", func() {
				a := &segment.Accumulator{Density: 1, Bounds: position.Range{Start: 0, End: 50}}
				Expect(a.Accumulate(segment.MD{
					Start:  0,
					Size:   100,
					Offset: 0,
