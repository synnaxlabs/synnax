...
func newRollingBinarySearchIndex(cap int) *legindex.BinarySearch {
	return &legindex.BinarySearch{Array: array.Searchable[legindex.Alignment]{
		Array: array.NewRolling[legindex.Alignment](cap),
	}}
}
	...
	Context("Exact Match in first index", func() {
		Describe("SeekP", func() {
			It("Should return a certain approximation", func() {
				...
				c := legindex.CompoundSearcher{i1, i2}
				Expect(i1.Write([]legindex.Alignment{{0, 0}, {2, 4}, {6, 8}})).To(Succeed())
				Expect(i2.Write([]legindex.Alignment{{0, 0}, {2, 4}, {6, 8}})).To(Succeed())
	...
	Context("Exact Match in second index", func() {
		Describe("SeekP", func() {
			It("Should return a certain approximation", func() {
				...
				i2 := newRollingBinarySearchIndex(10)
				c := legindex.CompoundSearcher{i1, i2}
				Expect(i1.Write([]legindex.Alignment{{6, 8}, {10, 12}, {7, 14}})).To(Succeed())
				Expect(i2.Write([]legindex.Alignment{{0, 0}, {2, 4}, {6, 8}})).To(Succeed())
	...
	Context("Inexact Match in both indexes", func() {
		Describe("SeekP", func() {
			It("Should return the least uncertain approximation", func() {
				...
				i2 := newRollingBinarySearchIndex(10)
				c := legindex.CompoundSearcher{i1, i2}
				Expect(i1.Write([]legindex.Alignment{{2, 4}, {6, 8}, {7, 14}})).To(Succeed())
				Expect(i2.Write([]legindex.Alignment{{0, 0}, {3, 5}, {6, 8}})).To(Succeed())
