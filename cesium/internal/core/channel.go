...
// processing result.
type Channel struct {
	// Key is a unique identifier to the channel within the DB.
	// [REQUIRED]
	Key string `json:"key"`
	...
	// regularly spaced.
	Rate telem.Rate `json:"rate"`
	// Index is the key of the channel used to index the channel's values. The Index is used to
