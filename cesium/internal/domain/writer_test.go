		...
		Context("FS: "+fsName, func() {
			...
			Describe("Closed database", func() {
				It("Should not allow opening a writer on a closed database", func() {
					...
					Expect(err).To(HaveOccurredAs(channel.NewErrResourceClosed("domain.db")))
				})
			})
			...
			Describe("Close", func() {
				It("Should not allow operations on a closed writer", func() {
					var (
						...
						e = channel.NewErrResourceClosed("domain.writer")
					)
					Expect(w.Close()).To(Succeed())
				...
				It("Should not write on a closed database", func() {
					Expect(db.Close()).To(Succeed())
					Expect(domain.Write(ctx, db, telem.TimeStamp(0).Range(telem.TimeStamp(1)), []byte{1, 2, 3})).To(HaveOccurredAs(channel.NewErrResourceClosed("domain.db")))
				})
			})
		})
	}
