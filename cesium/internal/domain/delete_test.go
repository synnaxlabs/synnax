...
var _ = Describe("Delete", Ordered, func() {
	...
	Context("Multiple Pointers", func() {
		DescribeTable("Deleting time range", func(
			...
			Expect(db.Delete(ctx, startPosition, endPosition, int64(startOffset), int64(endOffset), tr, true)).To(Succeed())
		...
		It("Should delete with the end being end of db", func() {
			Expect(db.Delete(ctx, 0, 2, int64(2), int64(10), (12 * telem.SecondTS).Range(40*telem.SecondTS), true)).To(Succeed())
			iter := db.NewIterator(domain.IteratorConfig{Bounds: telem.TimeRangeMax})
			Expect(iter.SeekFirst(ctx)).To(BeTrue())
		...
		It("Should delete nothing", func() {
			Expect(db.Delete(ctx, 1, 1, int64(4), int64(4), (24 * telem.SecondTS).Range(24*telem.SecondTS), true)).To(Succeed())
			iter := db.NewIterator(domain.IteratorConfig{Bounds: telem.TimeRangeMax})
			Expect(iter.SeekFirst(ctx)).To(BeTrue())
		...
		It("Should delete the entire db", func() {
			Expect(db.Delete(ctx, 0, 2, int64(0), int64(10), (10 * telem.SecondTS).Range(40*telem.SecondTS), true)).To(Succeed())
			iter := db.NewIterator(domain.IteratorConfig{Bounds: telem.TimeRangeMax})
		...
		It("Should delete multiple pointers", func() {
			Expect(db.Delete(ctx, 0, 0, int64(2), int64(3), (12 * telem.SecondTS).Range(13*telem.SecondTS), true)).To(Succeed())
			// at this point, pointer 0 splits into two: 10, 11 / 13, 14, 15, 16, ..., 19
			Expect(db.Delete(ctx, 0, 1, int64(1), int64(2), (11 * telem.SecondTS).Range(15*telem.SecondTS), true)).To(Succeed())
			// 10 / 15, 16, ..., 19
			Expect(db.Delete(ctx, 1, 1, 2, 4, (17 * telem.SecondTS).Range(19*telem.SecondTS), true)).To(Succeed())
			// 10 / 15, 16 / 19
			iter := db.NewIterator(domain.IteratorConfig{Bounds: telem.TimeRangeMax})
			Expect(iter.SeekFirst(ctx)).To(BeTrue())
