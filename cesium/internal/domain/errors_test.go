...
var _ = Describe("Errors", func() {
	Describe("NewRangeWriteConflictError", func() {
		It("Should correctly format a range overlap error", func() {
			...
			Expect(domain.NewRangeWriteConflictError(writerTr, existingTr)).
				To(MatchError(Equal("write for range 1970-01-01T00:00:05Z - :08 (3s) overlaps with existing data occupying time range 1970-01-01T00:00:02Z - :06 (4s) for a time span of 1s: write overlaps with existing data in database: validation error")))
		})
	})
