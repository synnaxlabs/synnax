	...
	for fsName, openFS := range FileSystems {
		Context("FS: "+fsName, func() {
			...
			Context("Happy path - one file", func() {
				It("Should garbage collect one tombstone", func(ctx SpecContext) {
					...
					Expect(
						domain.Write(
							ctx,
							db,
							(20 * telem.SecondTS).Range(28*telem.SecondTS+1),
							[]byte{20, 21, 22, 23, 24, 25, 26, 27, 28},
						),
					).To(Succeed())
					...
					Expect(
						i.TimeRange(),
					).To(Equal((16*telem.SecondTS + 1).Range(19*telem.SecondTS + 1)))
					r = MustSucceed(i.OpenReader(ctx))
					buf = make([]byte, 3)
					MustSucceed(r.ReadAt(buf, 0))
					...
					Expect(
						i.TimeRange(),
					).To(Equal((20 * telem.SecondTS).Range(28*telem.SecondTS + 1)))
					r = MustSucceed(i.OpenReader(ctx))
					buf = make([]byte, 9)
					MustSucceed(r.ReadAt(buf, 0))
				...
				It("Should garbage collect multiple tombstones", func(ctx SpecContext) {
					...
					Expect(
						domain.Write(
							ctx,
							db,
							(30 * telem.SecondTS).Range(36*telem.SecondTS+1),
					...
					w1 := MustSucceed(
						db.OpenWriter(
							ctx,
							domain.WriterConfig{Start: 50 * telem.SecondTS},
						),
					)
					w2 := MustSucceed(
						db.OpenWriter(
					...
					By("Asserting that the data did not change", func() {
						...
						Expect(
							i.TimeRange(),
						).To(Equal((35 * telem.SecondTS).Range(36*telem.SecondTS + 1)))
						r = MustSucceed(i.OpenReader(ctx))
						buf = make([]byte, 2)
						MustSucceed(r.ReadAt(buf, 0))
						...
						Expect(
							i.TimeRange(),
						).To(Equal((50 * telem.SecondTS).Range(52*telem.SecondTS + 1)))
						r = MustSucceed(i.OpenReader(ctx))
						buf = make([]byte, 3)
						MustSucceed(r.ReadAt(buf, 0))
				...
				It(
					"Should garbage collect multiple tombstones based on the threshold",
					func(ctx SpecContext) {
						...
						By(
							"Garbage collecting and asserting the file did not get smaller as the threshold is not reached.",
						)
						Expect(
						...
						By("Asserting that the data did not change", func() {
							...
							Expect(
								i.TimeRange(),
							).To(Equal((33*telem.SecondTS + 1).Range(36*telem.SecondTS + 1)))
							r = MustSucceed(i.OpenReader(ctx))
							buf = make([]byte, 3)
							MustSucceed(r.ReadAt(buf, 0))
				...
				It(
					"Should not garbage collect a file that is oversize but not still being written to",
					func(ctx SpecContext) {
						db = MustSucceed(domain.Open(domain.Config{
							FS:              fs,
						...
						Expect(
							domain.Write(
								ctx,
								db,
								(1 * telem.SecondTS).Range(7*telem.SecondTS+1),
								[]byte{1, 2, 3, 4, 5, 6, 7},
							),
						).To(Succeed())
						Expect(db.Delete(
						...
						By(
							"Closing the writer and expecting garbage collection to occur",
						)
						...
						By("Asserting that the data did not change", func() {
							...
							Expect(
								i.TimeRange(),
							).To(Equal((3 * telem.SecondTS).Range(7*telem.SecondTS + 1)))
							r := MustSucceed(i.OpenReader(ctx))
							buf := make([]byte, 5)
							MustSucceed(r.ReadAt(buf, 0))
				...
				It(
					"Should not garbage collect a file that has an open reader on it",
					func(ctx SpecContext) {
						db = MustSucceed(domain.Open(domain.Config{
							FS:              fs,
							FileSize:        9 * telem.Byte,
						...
						data := MustSucceed(
							domain.Read(
								ctx,
								db,
								(20 * telem.SecondTS).Range(29*telem.SecondTS+1),
							),
						)
						Expect(data).To(HaveLen(9))
						Expect(
							data,
						).To(Equal([]byte{20, 21, 22, 23, 24, 25, 26, 27, 28}))
			...
			Context("Happy path - multiple files", func() {
				It("Should garbage collect multiple tombstones", func(ctx SpecContext) {
					...
					Expect(
						domain.Write(
							ctx,
							db,
							(100 * telem.SecondTS).Range(105*telem.SecondTS+1),
					...
					Expect(
						domain.Write(
							ctx,
							db,
							(110 * telem.SecondTS).Range(115*telem.SecondTS+1),
					...
					By("Asserting that the data did not change", func() {
						...
						Expect(
							i.TimeRange(),
						).To(Equal((100 * telem.SecondTS).Range(105*telem.SecondTS + 1)))
						r = MustSucceed(i.OpenReader(ctx))
						buf = make([]byte, 6)
						MustSucceed(r.ReadAt(buf, 0))
						...
						Expect(
							i.TimeRange(),
						).To(Equal((110 * telem.SecondTS).Range(115*telem.SecondTS + 1)))
						r = MustSucceed(i.OpenReader(ctx))
						buf = make([]byte, 6)
						MustSucceed(r.ReadAt(buf, 0))
				...
				It(
					"Should garbage collect multiple tombstones across many files",
					func(ctx SpecContext) {
						db = MustSucceed(domain.Open(domain.Config{
							FS:              fs,
						...
						Expect(
							domain.Write(
								ctx,
								db,
								(20 * telem.SecondTS).Range(25*telem.SecondTS+1),
						...
						Expect(
							MustSucceed(fs.Stat("2.domain")).Size(),
						).To(Equal(int64(7)))
						Expect(
							MustSucceed(fs.Stat("3.domain")).Size(),
						).To(Equal(int64(3)))
						...
						Expect(
							domain.Write(
								ctx,
								db,
								(44 * telem.SecondTS).Range(44*telem.SecondTS+1),
								[]byte{44},
							),
						).To(Succeed())
						Expect(
						...
						By("Asserting that the data did not change", func() {
							...
							Expect(
								i.TimeRange(),
							).To(Equal((20 * telem.SecondTS).Range(23 * telem.SecondTS)))
							r := MustSucceed(i.OpenReader(ctx))
							buf := make([]byte, 3)
							MustSucceed(r.ReadAt(buf, 0))
							...
							Expect(
								i.TimeRange(),
							).To(Equal((41 * telem.SecondTS).Range(43*telem.SecondTS + 1)))
							r = MustSucceed(i.OpenReader(ctx))
							buf = make([]byte, 2)
							MustSucceed(r.ReadAt(buf, 0))
							...
							Expect(
								i.TimeRange(),
							).To(Equal((44 * telem.SecondTS).Range(44*telem.SecondTS + 1)))
							r = MustSucceed(i.OpenReader(ctx))
							buf = make([]byte, 1)
							MustSucceed(r.ReadAt(buf, 0))
				...
				It(
					"Should garbage collect tombstones based on the threshold",
					func(ctx SpecContext) {
						db = MustSucceed(domain.Open(domain.Config{
							FS:              fs,
							FileSize:        7 * telem.Byte,
			...
			Context("Tombstone persist", func() {
				It(
					"Should preserve the tombstones after database closure",
					func(ctx SpecContext) {
						db = MustSucceed(domain.Open(domain.Config{
							FS:              fs,
							FileSize:        7 * telem.Byte,
						...
						Expect(
							domain.Write(
								ctx,
								db,
								(40 * telem.SecondTS).Range(46*telem.SecondTS+1),
								[]byte{40, 41, 43, 44, 45, 46},
							),
						).To(Succeed())
						// file 3
						Expect(db.Delete(
						...
						Expect(
							domain.Write(
								ctx,
								db,
								(30 * telem.SecondTS).Range(35*telem.SecondTS+1),
						...
						By("Asserting that the data did not change", func() {
							...
							Expect(
								i.TimeRange(),
							).To(Equal((30 * telem.SecondTS).Range(35*telem.SecondTS + 1)))
							r = MustSucceed(i.OpenReader(ctx))
							buf = make([]byte, 6)
							MustSucceed(r.ReadAt(buf, 0))
							...
							Expect(
								i.TimeRange(),
							).To(Equal((41 * telem.SecondTS).Range(46*telem.SecondTS + 1)))
							r = MustSucceed(i.OpenReader(ctx))
							buf = make([]byte, 5)
							MustSucceed(r.ReadAt(buf, 0))
			...
			Context("Regression", func() {
				...
				Specify("Reader should be recycled", func(ctx SpecContext) {
					...
					// file 3
					i := db.OpenIterator(
						domain.IteratorConfig{Bounds: telem.TimeRangeMax},
					)
