		...
		Context("FS: "+fsName, func() {
			...
			Context("One file", func() {
				It("Should garbage collect one tombstone", func() {
					...
					Expect(db.Delete(ctx, 0, 0, 3, 3, telem.TimeRange{Start: 12*telem.SecondTS + 1, End: 16*telem.SecondTS + 1}, 1)).To(Succeed())
				...
				It("Should garbage collect multiple tombstones", func() {
					...
					Expect(db.Delete(ctx, 0, 2, 3, 3, telem.TimeRange{Start: 12*telem.SecondTS + 1, End: 33*telem.SecondTS + 1}, 1)).To(Succeed())
			...
			Context("Multiple files", func() {
				...
				It("Should garbage collect multiple tombstones across many files", func() {
					...
					Expect(db.Delete(ctx, 1, 3, 3, 2, telem.TimeRange{Start: 23 * telem.SecondTS, End: 41 * telem.SecondTS}, 1)).To(Succeed())
