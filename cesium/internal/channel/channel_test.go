...
var _ = Describe("Channel", func() {
	Describe("String", func() {
		It(
			"Should return a nicely formatting string with the channel's key and name",
			func() {
	...
	Describe("ValidateSeries", func() {
		It(
			"Should return an error if the series data type does not match the channel data type",
			func() {
				...
				Expect(c.ValidateSeries(s)).To(
					And(
						MatchError(
							ContainSubstring(
								"invalid data type for channel [cat]<1>, expected int64, got float64",
							),
						),
	...
	DescribeTable("Validation", func(substring string, ch channel.Channel) {
