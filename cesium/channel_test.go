		...
		Context("FS: "+fsName, Ordered, func() {
			...
			Describe("Rekey", func() {
				...
				It("Should rekey a unary channel into another", func(ctx SpecContext) {
					...
					Expect(
						db.WriteSeries(ctx, unaryKey, 5*telem.SecondTS, series2),
					).To(Succeed())
					...
					Expect(
						MustSucceed(fs.Exists(channelKeyToPath(unaryKey))),
					).To(BeFalse())
					...
					Expect(
						db.WriteSeries(ctx, unaryKeyNew, 10*telem.SecondTS, series3),
					).To(Succeed())
					...
					By("Asserting that the meta file got changed too", func() {
						f := MustSucceed(
							fs.Open(
								channelKeyToPath(unaryKeyNew)+"/meta.json",
								os.O_RDWR,
							),
						)
				...
				It(
					"Should rekey a virtual channel into another",
					func(ctx SpecContext) {
						By("Re-keying the channel")
						Expect(
							db.RekeyChannel(ctx, virtualKey, virtualKeyNew),
						).To(Succeed())
						...
						Expect(err).To(MatchError(channel.ErrNotFound))
						Expect(
							MustSucceed(fs.Exists(channelKeyToPath(virtualKey))),
						).To(BeFalse())
						...
						By("Asserting that the meta file got changed too", func() {
							f := MustSucceed(
								fs.Open(
									channelKeyToPath(virtualKeyNew)+"/meta.json",
									os.O_RDWR,
								),
							)
							s := MustSucceed(f.Stat()).Size()
				...
				It("Should rekey an index channel", func(ctx SpecContext) {
					...
					Expect(err).To(MatchError(channel.ErrNotFound))
					Expect(
						MustSucceed(fs.Exists(channelKeyToPath(indexKey))),
					).To(BeFalse())
					...
					By("Asserting that the meta file got changed too", func() {
						f := MustSucceed(
							fs.Open(
								channelKeyToPath(indexKeyNew)+"/meta.json",
								os.O_RDWR,
							),
						)
						s := MustSucceed(f.Stat()).Size()
				...
				Describe("Rekey of channel with a writer", func() {
					Specify("Unary", func(ctx SpecContext) {
						...
						By("Trying to rekey")
						Expect(
							db.RekeyChannel(ctx, errorKey1, errorKey1New),
						...
						Expect(err).To(MatchError(channel.ErrNotFound))
						Expect(
							MustSucceed(fs.Exists(channelKeyToPath(errorKey1))),
						).To(BeFalse())
						...
						By("Asserting that the meta file got changed too", func() {
							f := MustSucceed(
								fs.Open(
									channelKeyToPath(errorKey1New)+"/meta.json",
									os.O_RDWR,
								),
							)
							s := MustSucceed(f.Stat()).Size()
					...
					Specify("Virtual", func(ctx SpecContext) {
						...
						By("Asserting that rekey is successful now")
						Expect(
							db.RekeyChannel(ctx, errorKey2, errorKey2New),
						).To(Succeed())
						...
						Expect(err).To(MatchError(channel.ErrNotFound))
						Expect(
							MustSucceed(fs.Exists(channelKeyToPath(errorKey2))),
						).To(BeFalse())
						...
						By("Asserting that the meta file got changed too", func() {
							f := MustSucceed(
								fs.Open(
									channelKeyToPath(errorKey2New)+"/meta.json",
									os.O_RDWR,
								),
							)
							s := MustSucceed(f.Stat()).Size()
				...
				It(
					"Should do nothing for a channel that does not exist",
					func(ctx SpecContext) {
						By("Trying to rekey")
						Expect(
							db.RekeyChannel(ctx, errorKey3, errorKey3New),
						).To(Succeed())
					},
				)
			})
