	...
	Describe("Create", func() {
		...
		DescribeTable("Validation", func(expected error, channels ...cesium.Channel) {
			...
			Entry("Channel has no density",
				errors.Wrap(validate.ValidationError, "[cesium] - density must be positive"),
				cesium.Channel{Rate: 10 * telem.Hz},
			),
			Entry("Channel key already exists",
				errors.Wrap(validate.ValidationError, "[cesium] - provided key 1 already assigned"),
				cesium.Channel{Key: 1, Rate: 10 * telem.Hz, Density: telem.Bit64},
			),
			Entry("Channel IsIndex - Non Bit64 Density",
				errors.Wrap(validate.ValidationError, "[cesium] - index channel must use int64 timestamps"),
				cesium.Channel{IsIndex: true, Density: telem.Bit32},
			),
			Entry("Channel IsIndex - Index non-zero",
				errors.Wrap(validate.ValidationError, "[cesium] - index channel can not be indexed"),
			...
			Entry("Channel has index - Index does not exist",
				errors.Wrapf(validate.ValidationError, "[cesium] - provided index %s does not exist", cesium.ChannelKey(40000)),
				cesium.Channel{Index: 40000, Density: telem.Bit32},
			),
			Entry("Channel has no index - fixed rate not provided",
				errors.Wrap(validate.ValidationError, "[cesium] - rate must be positive"),
				cesium.Channel{Density: telem.Bit32},
			),
			Entry("Channel has index - provided index key is not an indexed channel",
				errors.Wrapf(validate.ValidationError, "[cesium] - provided channel %s is not an index", cesium.ChannelKey(60)),
