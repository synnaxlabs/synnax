...
func (db *DB) RenameChannels(ctx context.Context, keys []ChannelKey, names []string) error {
	...
	if len(keys) != len(names) {
		return errors.Wrapf(validate.Err, "keys and names must have the same length")
	}
	for i := range keys {
...
func (db *DB) validateNewChannel(ch Channel) error {
	...
	if unaryExists || virtualExists {
		return errors.Wrapf(validate.Err, "cannot create channel %v because it already exists", ch)
	}
	if ch.Virtual {
		return nil
	}
	if ch.Index != 0 && !ch.IsIndex {
		...
		if !indexDB.Channel().IsIndex {
			return validate.PathedError(
				errors.Wrapf(validate.Err, "channel %v is not an index", indexDB.Channel()),
				"index",
			)
		}
	}
