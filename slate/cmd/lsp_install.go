...
//go:embed slate-language.vsix
var vsixFile []byte

var lspInstallCmd = &cobra.Command{
	Use:   "install",
	Short: "Install Slate language support for editors",
	Long:  `Install the Slate Language Server extension for various editors`,
}

var vscodeInstallCmd = &cobra.Command{
	Use:   "vscode",
	Short: "Install Slate extension for VS Code",
	Long:  `Install the Slate Language Server extension for Visual Studio Code`,
...
func runVSCodeInstall(cmd *cobra.Command, args []string) error {
	...
	tempDir, err := os.MkdirTemp("", "slate-lsp-*")
	if err != nil {
		return fmt.Errorf("failed to create temp directory: %w", err)
	}
	defer os.RemoveAll(tempDir)

	vsixPath := filepath.Join(tempDir, "slate-language.vsix")
	if err := os.WriteFile(vsixPath, vsixFile, 0644); err != nil {
		return fmt.Errorf("failed to write VSIX file: %w", err)
	}

	// Install the extension
	fmt.Println("Installing Slate Language extension for VS Code...")
	...
	fmt.Println("\n✓ Slate Language extension installed successfully!")
