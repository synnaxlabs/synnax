...
export const combineDateAndTimeValue = (date: number, time: number): telem.TimeStamp =>
  new telem.TimeStamp(date).add(time).sub(telem.TimeStamp.utcOffset);
...
const DRAG_SCALE = {
  x: Number(telem.TimeSpan.SECOND.valueOf()) * 0.5,
  y: Number(telem.TimeSpan.MINUTE.valueOf()),
};
...
export const useTime = ({ value, onChange, tzInfo }: UseTimeProps): UseTimeReturn => {
  ...
  // All good.
  if (local.after(telem.TimeStamp.DAY)) {
    // Chop off the extra time.
    const tsV = local.remainder(telem.TimeStamp.DAY);
