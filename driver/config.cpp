...
bool Config::integration_enabled(const std::string &integration) const {
    return std::find(integrations.begin(), integrations.end(), integration) 
        != integrations.end();
}

#ifdef _WIN32
const std::vector<std::string> DEFAULT_INTEGRATIONS = {
    opc::INTEGRATION_NAME, ni::INTEGRATION_NAME, sequence::INTEGRATION_NAME, labjack::INTEGRATION_NAME
};
#else
const std::vector<std::string> DEFAULT_INTEGRATIONS = {
