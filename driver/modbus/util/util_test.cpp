...
protected:
    template<typename T>
    void expect_format_parse_eq(const T& value, const telem::DataType& dt, bool swap_bytes = false, bool swap_words = false) {
        ...
        const auto parsed_value  = ASSERT_NIL_P(util::parse_register(registers, 0, dt, swap_bytes, swap_words));
...
TEST_F(ModbusUtilTest, testInvalidDataType) {
    ...
    auto [_, parse_err] = util::parse_register(registers, 0, telem::UNKNOWN_T, false, false);
    ASSERT_OCCURRED_AS(parse_err, xerrors::VALIDATION);
    
    // Test formatting with invalid data type
    auto format_err = util::format_register(0, registers, telem::UNKNOWN_T, false, false);
