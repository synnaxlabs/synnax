...
inline std::pair<x::telem::SampleValue, x::errors::Error> parse_register_value(
    const uint16_t *data,
    ...
    try {
        ...
        if (dt == x::telem::INT32_T) {
            ...
            return {static_cast<int32_t>(raw), x::errors::NIL};
        }
        ...
        if (dt == x::telem::INT64_T) {
            ...
            if (swap_words)
                ...
                raw = static_cast<uint64_t>(words[3]) << 48 |
                      static_cast<uint64_t>(words[2]) << 32 |
                      static_cast<uint64_t>(words[1]) << 16 | words[0];
            return {static_cast<int64_t>(raw), x::errors::NIL};
        }
        if (dt == x::telem::FLOAT64_T) {
            uint64_t raw = 0;
            uint16_t words[4];
            for (int i = 0; i < 4; i++)
                words[i] = swap_bytes_if_needed(data[i]);
        ...
        if (dt == x::telem::UINT8_T) {
            const uint16_t raw = swap_bytes_if_needed(data[0]);
            return {static_cast<uint8_t>(raw & 0xFF), x::errors::NIL};
        }
        if (dt == x::telem::INT8_T) {
            const uint16_t raw = swap_bytes_if_needed(data[0]);
            return {static_cast<int8_t>(raw & 0xFF), x::errors::NIL};
        }
        return {
            x::telem::SampleValue(),
            x::errors::Error(
...
/// @brief formats a x::telem::SampleValue into a destination buffer of uint16_t values
/// representing modbus registers.
/// @param dt the data type fo the sample value.
/// @param dest the destination buffer to write to.
/// @param swap_bytes whether to swap the byte order of the data.
/// @param swap_words whether to swap the word order of the data.
inline x::errors::Error format_register(
    ...
    if (dest == nullptr) return x::errors::Error("modbus destination buffer is null");
    ...
    try {
        if (dt == x::telem::UINT16_T) {
            dest[0] = swap_bytes_if_needed(x::telem::cast<uint16_t>(value));
            return x::errors::NIL;
        }
        if (dt == x::telem::INT16_T) {
            dest[0] = swap_bytes_if_needed(x::telem::cast<int16_t>(value));
            return x::errors::NIL;
        }
        if (dt == x::telem::UINT32_T) {
            const auto raw = x::telem::cast<uint32_t>(value);
            if (swap_words) {
        ...
        if (dt == x::telem::INT32_T) {
            const auto raw = x::telem::cast<int32_t>(value);
            if (swap_words) {
                dest[0] = swap_bytes_if_needed(raw >> 16);
                dest[1] = swap_bytes_if_needed(raw & 0xFFFF);
            } else {
        ...
        if (dt == x::telem::FLOAT32_T) {
            const auto val = x::telem::cast<float>(value);
            uint32_t raw;
        ...
        if (dt == x::telem::UINT64_T) {
            const auto raw = x::telem::cast<uint64_t>(value);
            if (swap_words) {
        ...
        if (dt == x::telem::INT64_T) {
            const uint64_t raw = x::telem::cast<int64_t>(value);
            if (swap_words) {
                dest[0] = swap_bytes_if_needed(raw >> 48);
                dest[1] = swap_bytes_if_needed(raw >> 32 & 0xFFFF);
                dest[2] = swap_bytes_if_needed(raw >> 16 & 0xFFFF);
        ...
        if (dt == x::telem::FLOAT64_T) {
            const auto val = x::telem::cast<double>(value);
            uint64_t raw;
        ...
        if (dt == x::telem::UINT8_T) {
            dest[0] = swap_bytes_if_needed(x::telem::cast<uint8_t>(value));
            return x::errors::NIL;
        }
        if (dt == x::telem::INT8_T) {
            dest[0] = swap_bytes_if_needed(x::telem::cast<int8_t>(value));
            return x::errors::NIL;
        }
