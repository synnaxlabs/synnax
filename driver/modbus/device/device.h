...
#include "vendor/libmodbus/libmodbus/src/modbus.h"
...
inline xerrors::Error parse_error(const int code) {
    const auto err = modbus_strerror(code);
...
struct Device {
    ...
    xerrors::Error read_bits(const int addr, const size_t nb, uint8_t *dest) const {
        return parse_error(modbus_read_bits(ctx.get(), addr, nb, dest));
    }
    xerrors::Error read_input_bits(const int addr, const size_t nb, uint8_t *dest) const {
        return parse_error(modbus_read_input_bits(ctx.get(), addr, nb, dest));
    }
    xerrors::Error read_registers(const int addr, const size_t nb, uint16_t *dest) const {
        return parse_error(modbus_read_registers(ctx.get(), addr, nb, dest));
    }
    xerrors::Error read_input_registers(const int addr, const size_t nb, uint16_t *dest) const {
        return parse_error(modbus_read_input_registers(ctx.get(), addr, nb, dest));
    }
    xerrors::Error write_bits(const int addr, const size_t nb, const uint8_t *src) const {
        return parse_error(modbus_write_bits(ctx.get(), addr, nb, src));
    }
    xerrors::Error write_registers(const int addr, const size_t nb, const uint16_t *src) const {
        return parse_error(modbus_write_registers(ctx.get(), addr, nb, src));
    }
};
...
struct ConnectionConfig {
    ...
    uint16_t port;
    /// @brief The Modbus slave/unit ID to communicate with
    uint8_t slave_id;
    ...
    explicit ConnectionConfig(
        ...
        slave_id(parser.required<uint8_t>("slave_id")),
