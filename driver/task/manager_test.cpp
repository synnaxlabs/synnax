...
TEST_F(TaskManagerTest, ReconfigureStopsOld) {
    ...
    EVENTUALLY([&] { return first->stopped.load(); }, [] { return "not stopped"; });
...
TEST_F(ShutdownTest, WithPendingOps) {
    ...
    for (int i = 0; i < 3; i++) {
        auto task = synnax::task::Task(rack.key, "t" + std::to_string(i), "blocking", "");
        ASSERT_NIL(rack.tasks.create(task));
    }
...
TEST_F(ShutdownTest, ParallelTaskStop) {
    ...
    for (int i = 0; i < 4; i++) {
        auto task = synnax::task::Task(rack.key, "t" + std::to_string(i), "slow_stop", "");
        ASSERT_NIL(rack.tasks.create(task));
    }
    std::this_thread::sleep_for((200 * telem::MILLISECOND).chrono());
