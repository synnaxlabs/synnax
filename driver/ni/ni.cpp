...
int ni::Source::init() {
    ...
    if (!config_parser.ok()) {
        ...
        // Log error
        this->logError("failed to parse configuration for " + this->reader_config.task_name + " Parser Error: " +
...
freighter::Error ni::Source::start() {
    ...
    if (this->checkNIError(ni::NiDAQmxInterface::StartTask(this->task_handle))) {
        this->logError(
                "failed while starting reader for task " + this->reader_config.task_name + " requires reconfigure");
        this->clearTask();
