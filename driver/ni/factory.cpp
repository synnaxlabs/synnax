...
bool driver::ni::Factory::check_health() const {
...
std::unique_ptr<driver::ni::Factory> driver::ni::Factory::create(driver::task::common::TimingConfig timing_cfg) {
    ...
    return std::make_unique<driver::ni::Factory>(
...
std::pair<std::unique_ptr<driver::task::Task>, bool> driver::ni::Factory::configure_task(
    ...
    if (task.type == SCAN_TASK_TYPE)
        ...
        res = configure<
            ...
            driver::ni::ReadTaskConfig,
            driver::ni::ReadTaskSource<double>,
            driver::task::common::ReadTask>(ctx, task);
    else if (task.type == DIGITAL_READ_TASK_TYPE)
        res = configure<
            hardware::daqmx::DigitalReader,
            driver::ni::ReadTaskConfig,
            driver::ni::ReadTaskSource<uint8_t>,
            driver::task::common::ReadTask>(ctx, task);
        ...
        res = configure<
            ...
            driver::ni::WriteTaskConfig,
            driver::ni::WriteTaskSink<double>,
            driver::task::common::WriteTask>(ctx, task);
    else if (task.type == DIGITAL_WRITE_TASK_TYPE)
        res = configure<
            hardware::daqmx::DigitalWriter,
            driver::ni::WriteTaskConfig,
            driver::ni::WriteTaskSink<uint8_t>,
            driver::task::common::WriteTask>(ctx, task);
...
std::pair<driver::task::common::ConfigureResult, x::errors::Error> driver::ni::Factory::configure_scan(
    ...
    res.task = std::make_unique<driver::task::common::ScanTask>(
        std::make_unique<driver::ni::Scanner>(this->syscfg, cfg, task),
        ctx,
        task,
