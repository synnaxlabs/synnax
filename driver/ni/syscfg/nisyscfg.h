...
// Functions that auto-restart a target use a default timeout of 180 seconds (3
// minutes). Call NISysCfgSetSystemProperty with
// NISysCfgSystemPropertyAutoRestartTimeout to change it.
#define NISYSCFG_REBOOT_DEFAULT_TIMEOUT_MSEC 180000

// Functions that operate on a remote target use a default timeout of 300 seconds (5
// minutes). Call NISysCfgSetRemoteTimeout to change it.
...
typedef enum {
    ...
    NISysCfgItemTypeSystem = 2, // Required system component (hidden package or base
                                // system image installed by the host)
    ...
    NISysCfgItemTypeSystemPackage = 7 // Base system image installed using a package
                                      // from feeds on ni.com
} NISysCfgComponentType;

typedef enum {
    NISysCfgIncludeItemsAllVisible = 0x0000, // All visible (standard, startup,
                                             // essential)
...
typedef enum {
    NISysCfgImportMergeItems = 0, // Source data "wins" in the case of overwrite
                                  // conflicts
    NISysCfgImportDeleteConfigFirst = 0x100000, // Delete product data at destination
                                                // prior to copying
    NISysCfgImportPreserveConflictItems = 0x200000 // Destination data "wins" in the
                                                   // case of overwrite conflicts
} NISysCfgImportMode;
...
// NOTE: For string properties, callers pass in a pointer to a buffer or array they have
// allocated.
typedef enum {
    ...
    NISysCfgResourcePropertyFeatureActivationState =
        17534976, // NISysCfgFeatureActivationState
    ...
    NISysCfgResourcePropertyExternalCalibrationLastTime =
        16863232, // NISysCfgTimestampUTC
    NISysCfgResourcePropertyExternalCalibrationLastAdjustTime =
        17502208, // NISysCfgTimestampUTC
    NISysCfgResourcePropertyRecommendedNextCalibrationTime =
        16871424, // NISysCfgTimestampUTC
    ...
    NISysCfgResourcePropertyWlanCurrentConnectionType =
        219381760, // NISysCfgConnectionType
...
#define NISysCfgResourcePropertyNumberOfCpus                                           \
    NISysCfgResourcePropertyNumberOfCpuLogicalProcessors
#define NISysCfgResourcePropertyPxiPciSlotLinkWidth (NISysCfgResourceProperty) 16982016
#define NISysCfgResourcePropertyInternalCalibrationLastTime                            \
    (NISysCfgResourceProperty) NISysCfgIndexedPropertyInternalCalibrationLastTime
#define NISysCfgResourcePropertyInternalCalibrationLastTemp                            \
    (NISysCfgResourceProperty) NISysCfgIndexedPropertyInternalCalibrationLastTemp
...
typedef enum {
    ...
    NISysCfgIndexedPropertyWlanAvailableConnectionType =
        219340800, // NISysCfgConnectionType
    NISysCfgIndexedPropertyWlanAvailableSecurityType =
        219344896, // NISysCfgSecurityType
    ...
    NISysCfgIndexedPropertyInternalCalibrationLastTime =
        16846848, // NISysCfgTimestampUTC
...
#define NISysCfgResourcePropertyWlanAvailableCount                                     \
...
// This function is used to close a session or any other handle type returned by this
// API.
NISYSCFGCFUNC NISysCfgCloseHandle(void *syscfgHandle);
...
NISYSCFGCFUNC
NISysCfgResetHardware(NISysCfgResourceHandle resourceHandle, unsigned int mode);
...
NISYSCFGCFUNC
NISysCfgUninstallAll(NISysCfgSessionHandle sessionHandle, NISysCfgBool autoRestart);
...
NISYSCFGCFUNC
NISysCfgRemoveSoftwareFeed(NISysCfgSessionHandle sessionHandle, const char *feedName);
...
NISYSCFGCFUNC
NISysCfgCreateComponentsEnum(NISysCfgEnumSoftwareComponentHandle *componentEnumHandle);
...
NISYSCFGCFUNC NISysCfgFreeDetailedString(char str[]);
...
NISYSCFGCFUNC
NISysCfgNextSystemInfo(NISysCfgEnumSystemHandle systemEnumHandle, char system[]);
...
// Caller should free dependerDetailedDescription and dependeeDetailedDescription using
...
NISYSCFGCFUNC NISysCfgResetEnumeratorGetCount(void *enumHandle, unsigned int *count);
