    ...
    parser.iter("channels",
                [&](config::Parser &channel_builder) {
                    ...
                    config.name = (this->writer_config.device_name + "/port" +
                                   std::to_string(channel_builder.required<std::uint64_t>("port")) + "/line" +
...
int ni::DigitalWriteSink::init() {
    ...
    for (auto &channel: channels) {
        if (channel.channel_type != "index") {
            err = this->checkNIError(ni::NiDAQmxInterface::CreateDOChan(this->task_handle, channel.name.c_str(), "",
...
freighter::Error ni::DigitalWriteSink::formatData(synnax::Frame frame) {
    ...
    for (auto key: *(frame.channels)) { // the order the keys were pushed into the vector is the order the data is written
        ...
        if (it != this->writer_config.drive_cmd_channel_keys.end()) {
            ...
            cmd_channel_index = std::distance(this->writer_config.drive_cmd_channel_keys.begin(),
                                              it); // this corressponds to where in the order its NI channel was created
...
std::vector<synnax::ChannelKey> ni::DigitalWriteSink::getCmdChannelKeys() {
    ...
    for (auto &channel: this->writer_config.channels) {
        if (channel.channel_type != "index") {
            keys.push_back(
                    channel.channel_key); // could either be the key to a cmd channel or a key to an cmd index channel
        }
    }
    return keys;
}
...
//                                    StateSource                                //
///////////////////////////////////////////////////////////////////////////////////
ni::StateSource::StateSource(std::uint64_t state_rate, synnax::ChannelKey &drive_state_index_key,
...
synnax::Frame ni::StateSource::getDriveState() {
    ...
    drive_state_frame.add(this->drive_state_index_key,
                          synnax::Series(std::vector<uint64_t>{synnax::TimeStamp::now().value}, synnax::TIMESTAMP));
