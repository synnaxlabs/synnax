...
struct Input : virtual Base {
    ...
    explicit Input(x::json::Parser &cfg):
        Base(cfg), synnax_key(cfg.field<synnax::channel::Channel::Key>("channel")) {}

    /// @brief binds remotely fetched information to the channel.
    void bind_remote_info(const synnax::channel::Channel::Channel &ch, const std::string &dev_loc) {
        this->ch = ch;
...
struct Output : virtual Base {
    ...
    const synnax::channel::Channel::Key cmd_ch_key;
    /// @brief the key of the state channel that we'll write the state of the
    /// command channel to.
    const synnax::channel::Channel::Key state_ch_key;
    ...
    synnax::channel::Channel::Channel state_ch;

    explicit Output(x::json::Parser &cfg):
        Base(cfg),
        cmd_ch_key(cfg.field<synnax::channel::Channel::Key>("cmd_channel")),
        state_ch_key(cfg.field<synnax::channel::Channel::Key>("state_channel")) {}

    /// @brief binds remotely fetched information to the channel.
    void bind_remote_info(const synnax::channel::Channel::Channel &state_ch, const std::string &dev_loc) {
