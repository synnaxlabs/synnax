...
"a1", data.key, 0, -10.0, 10.0, "Default", scale_config);
...
!= 0) std::cout << "Failed to initialize reader" <<
std::endl;
reader.
...
"a1", data.key, 0, -10.0, 10.0, "Default", scale_config);
add_AI_channel_JSON(config,
"a2", data1.key, 1, -10.0, 10.0, "Default", scale_config);
add_AI_channel_JSON(config,
"a3", data2.key, 2, -10.0, 10.0, "Default", scale_config);
add_AI_channel_JSON(config,
"a4", data3.key, 3, -10.0, 10.0, "Default", scale_config);
...
for(
int i = 0;
i < 30; i++ ) { // test for 50 read cycles
std::uint64_t initial_timestamp = (synnax::TimeStamp::now()).value;
auto [frame, err] = reader.read();
std::uint64_t final_timestamp = (synnax::TimeStamp::now()).value;
//iterate through each series and print the data
...
std::cout << s[j] << ", ";
ASSERT_NEAR(s[j],
0, 10); // can be any value of a sine wave from -10 to 10
}
}
else if(frame.series->
at(i)
...
add_AI_channel_JSON(config,
"a1", data.key, 0, 0, 10.0, "Default", scale_config);
...
add_AI_channel_JSON(config,
"a1", data.key, 0, 0, 100, "Default", scale_config);
...
add_AI_channel_JSON(config,
"a1", data.key, 0, 0, 500.0, "Default", scale_config);
