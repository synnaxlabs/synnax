...
freighter::Error ni::TaskHandler::start(const std::string &cmd_key) {
    if (this->breaker.running()) return freighter::NIL;
    if (!this->ok()) return freighter::NIL;

    this->breaker.start();

    this->start_ni(); // Error handled in start_ni


    // TODO: for read tasks - make sure to add the sample_thread start
    json details = {
        {"running", true},
...
freighter::Error ni::TaskHandler::start_ni() {
    if (this->check_ni_error(ni::NiDAQmxInterface::StartTask(this->task_handle))) {
        this->log_error(
            "failed while starting ni task for task " + this->task_name +
...
freighter::Error ni::TaskHandler::stop_ni() {
    if (this->check_ni_error(ni::NiDAQmxInterface::StopTask(this->task_handle))) {
        this->log_error(
            "failed while stopping ni task for task " + this->task_name + " requires reconfigure");
        return driver::CRITICAL_HARDWARE_ERROR;
    }
    return freighter::NIL;
}
...
freighter::Error ni::TaskHandler::stop(const std::string &cmd_key) {
    if (!this->breaker.running()) return freighter::NIL;
    if (!this->ok()) return freighter::NIL;

    this->breaker.stop();

    // TODO: join thread for read tasks
    this->stop_ni();
    json details = {
        {"running", false},
...
void ni::TaskHandler::clear_task() {
    if (this->check_ni_error(ni::NiDAQmxInterface::ClearTask(this->task_handle))) {
        this->log_error(
            "failed while clearing ni task for task " + this->task_name);
    }
}

void ni::TaskHandler::stopped_with_err(const freighter::Error &err) {
...
freighter::Error ni::TaskHandler::cycle() {
...
int ni::TaskHandler::check_ni_error(int32 error) {
    if (error == 0) return 0;
...
void ni::TaskHandler::jsonify_error(std::string s) {
...
bool ni::TaskHandler::ok() {
    return this->ok_state;
}

void ni::TaskHandler::log_error(std::string err_msg) {
