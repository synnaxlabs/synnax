 ...
 **/
void PrintStreamConclusion(
    unsigned int timeStart,
    unsigned int timeEnd,
    int numReads,
    int scansPerRead,
    int numChannels,
    int totalSkippedScans
);
...
/**
 * Prints the iteration number, and prints the backlog values if they are greater than
 *their respective thresholds.
 **/
void OutputStreamIterationInfo(
    int iteration,
    int deviceScanBacklog,
    int deviceScanBacklogThreshold,
    int LJMScanBacklog,
    int LJMScanBacklogThreshold
);
...
void PrintScans(
    ...
    for (scanI = 0; scanI < maxScansPerChannel * numChannels; scanI += numChannels) {
        for (chanI = 0; chanI < numChannels; chanI++) {
            if (aData[scanI + chanI] == LJM_DUMMY_VALUE) { ++numSkippedScans; }
            if (channelAddresses[chanI] < 1000) {
...
int CountAndOutputNumSkippedSamples(
    int numInChannels,
    int scansPerRead,
    double *aData
) {
    ...
    for (j = 0; j < numInChannels * scansPerRead; j++) {
        if (aData[j] == LJM_DUMMY_VALUE) { ++numSkippedSamples; }
    }
    if (numSkippedSamples) {
        printf(
            "****** %d data values were placeholders for scans that were skipped "
            "******\n",
            numSkippedSamples
        );
        printf(
            "****** %.01f %% of the scans were skipped ******\n",
...
double CalculateSleepFactor(int scansPerRead, int LJMScanBacklog) {
    ...
    if (portionScansReady > DECREASE_TOTAL) { return 0; }
