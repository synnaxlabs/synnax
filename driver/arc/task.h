    ...
    static std::pair<std::unique_ptr<Task>, x::errors::Error> create(
        ...
        if (!task->runtime->write_channels.empty()) {
            ...
            auto resolve = [&](
                ...
                if (err) return ctx->client->channels.create(
                    name, dt, index, is_index
                );
                return {ch, x::errors::NIL};
            };
            auto [idx_ch, idx_err] = resolve(
                base + "_time", x::telem::TIMESTAMP_T, 0, true
            );
            if (!idx_err) {
                auto [avg_ch, avg_err] = resolve(
                    base + "_us", x::telem::FLOAT32_T, idx_ch.key
                );
                auto [min_ch, min_err] = resolve(
                    base + "_min_us", x::telem::FLOAT32_T, idx_ch.key
                );
                auto [max_ch, max_err] = resolve(
                    base + "_max_us", x::telem::FLOAT32_T, idx_ch.key
                );
                if (!avg_err && !min_err && !max_err) {
                    task->runtime->set_cycle_latency_keys(
                        avg_ch.key, idx_ch.key,
                        min_ch.key, max_ch.key
                    );
