...
void Master::populate_slaves() {
    ...
    for (int i = 1; i <= this->context.slavecount; ++i) {
        ...
        result.properties.vendor_id = soem_slave.eep_man;
        result.properties.product_code = soem_slave.eep_id;
        result.properties.revision = soem_slave.eep_rev;
        result.properties.serial = soem_slave.eep_ser;
        result.properties.name = soem_slave.name;
        result.properties.input_bits = soem_slave.Ibits;
        result.properties.output_bits = soem_slave.Obits;
        result.status.state = convert_state(soem_slave.state);
...
void Master::cache_pdo_offsets() {
    ...
    for (const auto &slave: this->slave_list) {
        ...
        if (props.position == 0 ||
            props.position > static_cast<uint16_t>(this->context.slavecount))
            continue;

        if (this->context.slavelist[props.position].group != 0) continue;

        const auto &soem_slave = this->context.slavelist[props.position];
...
void Master::discover_pdos_sii(slave::DiscoveryResult &slave) {
    ...
    for (uint8_t t = 0; t <= 1; ++t) {
        ...
        uint16_t a = startpos;
        uint16_t length = ecx_siigetbyte(&this->context, props.position, a++);
        length += (ecx_siigetbyte(&this->context, props.position, a++) << 8);
        ...
        uint16_t c = 1;
        while (c < length) {
            uint16_t pdo_index = ecx_siigetbyte(&this->context, props.position, a++);
            pdo_index += (ecx_siigetbyte(&this->context, props.position, a++) << 8);
            c++;
            ...
            for (uint8_t er = 0; er < num_entries; ++er) {
                c += 4;
                uint16_t obj_idx = ecx_siigetbyte(&this->context, props.position, a++);
                obj_idx += (ecx_siigetbyte(&this->context, props.position, a++) << 8);
    ...
    if (props.input_pdos.empty() && props.output_pdos.empty()) {
        ...
        if (soem_slave.Ibits > 0 || soem_slave.Obits > 0) {
            VLOG(2) << "Slave " << props.position << " has Ibits=" << soem_slave.Ibits
