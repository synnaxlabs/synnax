...
// Mock implementation of driver::pipeline::Streamer for testing.
...
inline std::shared_ptr<driver::pipeline::mock::StreamerFactory> simple_streamer_factory(
    ...
    const auto factory = std::make_shared<driver::pipeline::mock::StreamerFactory>(
        std::vector<x::errors::Error>{},
        std::make_shared<std::vector<driver::pipeline::mock::StreamerConfig>>(
            std::vector{driver::pipeline::mock::StreamerConfig{reads, {}, x::errors::NIL}}
        )
    );
    return factory;
}

// Mock implementation of driver::pipeline::Writer for testing.
...
// Mock implementation of driver::pipeline::Sink for testing.
class Sink : public driver::pipeline::Sink {
public:
...
// Mock implementation of driver::pipeline::Source for testing.
class Source : public driver::pipeline::Source {
public:
...
inline std::shared_ptr<driver::pipeline::mock::Source>
simple_source(const std::shared_ptr<std::vector<x::telem::Frame>> &reads) {
    return std::make_shared<driver::pipeline::mock::Source>(reads);
}
}
