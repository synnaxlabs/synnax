...
import { TimeRange } from "@/telem/text/TimeRange";
...
describe("TimeRange", () => {
  it("should render time range with start and end", () => {
    ...
    const c = render(
      <TimeRange suppliedTZ="UTC" displayTZ="UTC">
        {range}
      </TimeRange>,
    );
  ...
  it("should render open time range with Now", () => {
    ...
    const c = render(<TimeRange>{range}</TimeRange>);
  ...
  it("should show span when showSpan is true", () => {
    ...
    const range = new XTimeRange(start, end);
    const c = render(<TimeRange showSpan>{range}</TimeRange>);
  ...
  it("should not show span when showSpan is false", () => {
    ...
    const end = TimeSpan.hours(14).add(TimeSpan.minutes(30));
    const range = new XTimeRange(start, end);
    const c = render(<TimeRange showSpan={false}>{range}</TimeRange>);
