...
export interface PIDContext {
  render: <P>(type: string, props: P) => JSX.Element | null;
}
...
export type contextRenderer = <P>(type: string, props: P) => JSX.Element | null;
export type canvasRenderer<P> = (ctx: CanvasRenderContext, props: P) => void;
export type jsxRenderer<P> = (props: P) => JSX.Element;
...
export const workerRenderer = (worker: Worker | null): contextRenderer => {
  return <P extends unknown>(type: string, props: P): JSX.Element | null => {
