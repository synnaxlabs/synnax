...
describe("Log Text Wrapping - Comprehensive Memory Analysis", () => {
  it("should show complete memory profile for text wrapping implementation", () => {
    ...
    console.log(
      "║          LOG TEXT WRAPPING - MEMORY ANALYSIS REPORT                ║",
    );
    console.log(
      "╚════════════════════════════════════════════════════════════════════╝\n",
    );
    ...
    console.log(
      "This implementation adds soft text wrapping (word-boundary wrapping) to",
    );
    console.log(
      "the log viewer. Long lines are split at word boundaries to fit the viewport.",
    );
    console.log("");
    console.log("ARCHITECTURE:");
    console.log(
      "  • Pre-computed Cache: All lines are wrapped once and cached in a Map",
    );
    ...
    for (const test of tests) {
      ...
      const cacheBuildMemory =
        (cacheBuildAfter.heapUsed - cacheBuildBefore.heapUsed) / 1024 / 1024;
      ...
      console.log(
        `   Time per line:      ${(cacheBuildTime / test.count).toFixed(3)} ms`,
      );
      console.log(`   Memory allocated:   ${cacheBuildMemory.toFixed(2)} MB`);
      console.log(
        `   Memory per line:    ${((cacheBuildMemory / test.count) * 1024).toFixed(2)} KB`,
      );
      console.log(`   Cache entries:      ${cacheSize}`);
      console.log(
        `   Visual lines:       ${visualLines} (${(visualLines / test.count).toFixed(1)}x wrapping)`,
      );
      ...
      console.log(
        `   Per logical line:   ${((totalMemory / test.count) * 1024).toFixed(2)} KB`,
      );
      console.log(
        `   Per visual line:    ${((totalMemory / visualLines) * 1024).toFixed(2)} KB`,
      );

      let totalChars = 0;
      for (const value of series)
        if (value != null) totalChars += value.toString().length;
      ...
      for (const value of series) if (value != null) dataArray.push(value.toString());
      ...
      console.log(
        `   Avg render time:    ${avgRenderTime.toFixed(3)} ms (${iterations} iterations)`,
      );
      console.log(
        `   Render memory:      ${renderMemory.toFixed(2)} MB (temp allocations)`,
      );
      console.log(
        `   Memory per line:    ${((renderMemory / visualLines) * 1024).toFixed(2)} KB`,
      );
      ...
      console.log(
        `   ├─ Cache storage:    ${cacheOnlyMemory.toFixed(2)} MB (${((cacheOnlyMemory / totalMemory) * 100).toFixed(1)}% of total)`,
      );
      console.log(
        `   ├─ Base overhead:    ${(totalMemory - cacheOnlyMemory).toFixed(2)} MB (log instance, etc.)`,
      );
      console.log(
        `   └─ Render temps:     ${renderMemory.toFixed(2)} MB (per frame, GC'd)`,
      );
      console.log("");
      ...
      if (test.length === "short") {
        console.log(
          `   • Short lines: Minimal wrapping (${(visualLines / test.count).toFixed(1)}x)`,
        );
        console.log(
          `   • Cache cost: ${cacheEfficiency.toFixed(2)} KB/line (mostly Map overhead)`,
        );
      } else {
        console.log(
          `   • Long lines: ${(visualLines / test.count).toFixed(1)}x wrapping (avg ${Math.floor(visualLines / test.count)} visual lines per logical line)`,
        );
        console.log(
          `   • Cache cost: ${cacheEfficiency.toFixed(2)} KB/line (stores ${Math.floor(visualLines / test.count)} strings per line)`,
        );
      }
      console.log(
        `   • Trade-off: One-time ${cacheBuildTime.toFixed(2)}ms build for instant ${avgRenderTime.toFixed(3)}ms renders`,
      );
      console.log("");

      console.log("   Performance Characteristics:");
      console.log(
        `   • Cache rebuild: O(n) - ${(cacheBuildTime / test.count).toFixed(3)}ms per line`,
      );
      console.log(`   • Render: O(visible) - Only renders visible lines from cache`);
      console.log(
        `   • Memory: O(n) - ${((totalMemory / test.count) * 1024).toFixed(2)} KB per logical line`,
      );
