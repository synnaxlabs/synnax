...
import "@/schematic/edge/segmented/Segmented.css";
...
import { type Base } from "@/schematic/edge/base";
import {
  createConnector,
  dragSegment,
  extractMiddle,
  ...
  segmentsToPoints,
  stitchEdge,
} from "@/schematic/edge/segmented/connector";
import { Form } from "@/schematic/edge/segmented/Form";
...
const create = (Path: FC<PathProps>): Edge<Config> => {
  const E: Edge<Config> = ({
    source,
    target,
    sourceNode,
...
export const createSpec = <V extends string = string>(
  ...
  Edge: create(path),
  Form,
  defaultConfig: () => ({ color: color.ZERO, segments: [] }),
});
