...
describe("User queries", () => {
  ...
  describe("useList", () => {
    it("should return a list of user keys", async () => {
      const user1 = await client.users.create({
        username: `list-user-1-${Date.now()}`,
        firstName: "List",
        lastName: "User1",
        password: "password123",
      });
      const user2 = await client.users.create({
        username: `list-user-2-${Date.now()}`,
        firstName: "List",
        lastName: "User2",
        password: "password123",
      });

      const { result } = renderHook(() => User.useList(), {
        wrapper,
      });
      act(() => {
      ...
      expect(result.current.data).toContain(user1.key);
      expect(result.current.data).toContain(user2.key);
    });

    it("should get individual users using getItem", async () => {
      const testUser = await client.users.create({
        username: `getitem-user-${Date.now()}`,
        firstName: "GetItem",
        lastName: "Test",
        password: "password123",
      });
      ...
      const retrievedUser = result.current.getItem(testUser.key);
      expect(retrievedUser?.key).toEqual(testUser.key);
      expect(retrievedUser?.username).toEqual(testUser.username);
      expect(retrievedUser?.firstName).toEqual("GetItem");
      expect(retrievedUser?.lastName).toEqual("Test");
    });

    it("should filter users by keys", async () => {
      const user1 = await client.users.create({
        username: `filter-user-1-${Date.now()}`,
        firstName: "Filter",
        lastName: "User1",
        password: "password123",
      });
      const user2 = await client.users.create({
        username: `filter-user-2-${Date.now()}`,
        firstName: "Filter",
        lastName: "User2",
        password: "password123",
      });
      ...
      act(() => {
        result.current.retrieve({ keys: [user1.key, user2.key] });
      });
      await waitFor(() => expect(result.current.variant).toEqual("success"));
    ...
    it("should update the list when a user is created", async () => {
      const { result } = renderHook(() => User.useList(), {
        wrapper,
      });
      act(() => {
      ...
      const newUser = await client.users.create({
        username: `new-list-user-${Date.now()}`,
        firstName: "New",
        lastName: "User",
        password: "password123",
      });
      ...
      await waitFor(() => {
        ...
        expect(result.current.data).toContain(newUser.key);
      });
    });

    it("should update the list when a user is updated", async () => {
      const testUser = await client.users.create({
        username: `update-list-user-${Date.now()}`,
        firstName: "Original",
        lastName: "Name",
        password: "password123",
      });
      ...
      expect(result.current.getItem(testUser.key)?.firstName).toEqual("Original");

      await client.users.rename(testUser.key, "Updated", "Name");

      await waitFor(() => {
        expect(result.current.getItem(testUser.key)?.firstName).toEqual("Updated");
      });
    });

    it("should remove user from list when deleted", async () => {
      const testUser = await client.users.create({
        username: `delete-list-user-${Date.now()}`,
        firstName: "ToDelete",
        lastName: "User",
        password: "password123",
      });
      ...
      expect(result.current.data).toContain(testUser.key);

      await client.users.delete(testUser.key);

      await waitFor(() => {
        expect(result.current.data).not.toContain(testUser.key);
      });
    });

    it("should retrieve multiple specific users by keys", async () => {
      const users = await Promise.all([
        client.users.create({
          username: `multi-user-1-${Date.now()}`,
          firstName: "Multi",
          lastName: "User1",
          password: "password123",
        }),
        client.users.create({
          username: `multi-user-2-${Date.now()}`,
          firstName: "Multi",
          lastName: "User2",
          password: "password123",
        }),
        client.users.create({
          username: `multi-user-3-${Date.now()}`,
          firstName: "Multi",
          lastName: "User3",
          password: "password123",
        }),
      ...
      act(() => {
        result.current.retrieve({
          keys: [users[0].key, users[2].key],
        });
      });
      await waitFor(() => expect(result.current.variant).toEqual("success"));
      expect(result.current.data).toHaveLength(2);
      expect(result.current.data).toContain(users[0].key);
      expect(result.current.data).toContain(users[2].key);
      expect(result.current.data).not.toContain(users[1].key);
    });
  });
