...
export const Tree = ({ services }: TreeProps): ReactElement => {
  ...
  const handleDrop: Core.TreeProps["onDrop"] = (
    key,
    { source, items }
  ): Haul.Item[] => {
  ...
  const handleExpand = ({ action, clicked }: Core.HandleExpandProps): void => {
  ...
  const handleRename: Core.TreeProps["onRename"] = (key, name) => {
  ...
  const handleDoubleClick: Core.TreeProps["onDoubleClick"] = (key) => {
    const id = new ontology.ID(key);
    const svc = services[id.type];
    svc.onSelect({
      client,
  ...
  const props = Core.use({ onExpand: handleExpand });
  ...
  return (
    <Menu.ContextMenu
      ...
      menu={({ keys }) => {
        ...
        const parent = Core.findNodeParent(nodes, keys[0]);
