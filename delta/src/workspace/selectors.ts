 ...
 */
const selectWorkSpaceState = (state: StoreState): SliceState => state[SLICE_NAME];
 ...
 */
const selectActiveRangeKey = (state: StoreState): string | null =>
...
export const useSelectRange = (key?: string): Range | null | undefined =>
  useMemoSelect((state: StoreState) => selectRange(state, key), [key]);
...
export const useSelectRanges = (keys?: string[]): Range[] =>
  useMemoSelect((state: StoreState) => selectRanges(state, keys), [keys]);
