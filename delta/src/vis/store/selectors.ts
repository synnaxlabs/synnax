 ...
 * does not exist.
 */
export const selectVis = <V extends Vis>(
...
export const selectRequiredVis = <V extends Vis>(
  state: VisualizationStoreState & LayoutStoreState,
  variant: V["variant"],
  layoutKey: string
): V => {
  const v = selectVis(state, variant, layoutKey);
  if (v == null)
    throw new Error(`[vis] - ${variant}: ${layoutKey} required but not found`);
  return v;
};
