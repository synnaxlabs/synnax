 ...
 * @property role - The role of the entity issuing the request. @see Role
 ...
 * middleware should be used to execute the request.
 */
 ...
 */
export class MiddlewareCollector implements Transport {
   ...
   * @returns A tuple containing the context and an error if one was encountered.
   */
  async executeMiddleware(
    ...
    let i = 0;
    const next = async (ctx: Context): Promise<[Context, Error | null]> => {
      if (i === this.middleware.length) return await finalizer(ctx);
      const _mw = this.middleware[i];
      i++;
      return await _mw(ctx, next);
    };
