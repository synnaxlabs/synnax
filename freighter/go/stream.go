...
type StreamClient[RQ, RS Payload] interface {
	Transport
	// Stream opens a stream to the target server using the given context. If the stream
	// cannot be opened (ex. the server cannot be reached), Stream will return an error.
	// For the semantics of stream operation, see the ClientStream interface.
	Stream(context.Context, address.Address) (ClientStream[RQ, RS], error)
}
...
type StreamServer[RQ, RS Payload] interface {
	...
	// context is cancelled, the server is expected to discard unprocessed requests,
	// free resources related to the stream, and return an error to the caller (ideally
	// this error should wrap a context error in some form).
	//
	...
	//
	// For details on the semantics of the handler, see the ServerStream interface.
...
type ClientStream[RQ, RS Payload] interface {
	// StreamReceiver - Receive blocks until a message is received from the server or
	...
	//
	// Repeated calls to Receive will immediately return the same error the stream
	// closed with.
	StreamReceiver[RS]
	// StreamSenderCloser -
	//
	...
	//
	// If the client continues to call Send after receiving an error, Send will continue
	// to return the same error.
	//
	// CloseSend lets the server know it's time to shut down. If the client attempts to
	// call Send after CloseSend, Send will return StreamClosed. The client is free to
	// continue receiving messages from the server after calling CloseSend.
	StreamSenderCloser[RQ]
}

// ServerStream is the server-side interface of Stream freighter. ServerStream is
