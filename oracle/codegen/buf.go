...
//
// Verifying the input stamp alone is unsafe: a buf-version bump or a hand edit on a
// generated .pb.go leaves the on-disk file diverged from what current buf would emit,
// but with unchanged inputs the stamp keeps matching and sync silently skips. Tracking
// the output stamp too lets the cache self-heal: any drift forces a re-run.
//
// changedProtos contains repo-relative paths to generated `.proto` files that the
// format step actually wrote. When non-empty the run is scoped via `--path` so protoc
// only runs over the slice that changed.
type RunBufGenerateResult struct {
	// Cached is true when both input and output stamps matched and no `buf generate`
...
// bufOutputStamp returns a content-derived stamp covering every .pb.go and _grpc.pb.go
// file under repoRoot. Same skip-list as bufInputStamp.
...
// bufInputStamp returns a content-derived stamp covering every input `buf generate`
// consults: the buf.yaml / buf.gen.yaml configs and every committed `.proto` file under
// repoRoot. We hash the file contents (not just mtimes) so a `git checkout` of
// identical bytes doesn't trigger an unnecessary regeneration.
...
// findProtoFiles walks repoRoot for .proto files, skipping common vendor / build / .git
// directories.
func findProtoFiles(repoRoot string) ([]string, error) {
	return walkExtensions(repoRoot, []string{".proto"}, nil)
}

// findPbGoFiles walks repoRoot for the .pb.go and _grpc.pb.go outputs `buf generate`
// emits next to their .proto sources, skipping the same vendor / build / .git
// directories findProtoFiles skips.
...
// FormatBufOutputs runs the configured formatter chain over every .pb.go and
// _grpc.pb.go file `buf generate` produced in the repo, writing back any whose
// canonical bytes differ. This restores the license header (and re-applies gofmt) to
// protoc's raw output, which otherwise lands on disk header-less.
//
// Returns the count of files actually rewritten. The walk is bounded by the same
// skip-list as findProtoFiles, so vendor / .oracle / build directories are not touched.
//
// Idempotent: re-running over already-canonical files is a no-op because the license
// formatter detects an existing header (any year) and returns the content unchanged,
// and gofmt is stable.
