...
func generate(
	...
	newTable, err := analyze(ctx, newSource, loader)
	if err != nil {
		return nil, err
	}
	req := &plugin.Request{Resolutions: newTable, RepoRoot: loader.RepoRoot()}
	if oldSource != "" {
		oldTable, err := analyze(ctx, oldSource, loader)
		if err != nil {
			return nil, err
		}
...
var _ = Describe("Go Migrate Plugin", func() {
	...
	Describe("Generate", func() {
		Context("no schema change", func() {
			It("Should generate no files when schemas are identical", func() {
				...
				`
				resp := MustSucceed(generate(ctx, schema, schema, loader, p))
		...
		Context("pre-versioning snapshot", func() {
			It(
				"Should generate no files against a snapshot with no @go version",
				func() {
					oldSchema := `
					@go output "out"
					Key = uuid
				...
				`
					resp := MustSucceed(
						generate(ctx, oldSchema, newSchema, loader, p),
					)
					Expect(resp.Files).To(BeEmpty())
				},
			)
		...
		Context("no old resolutions", func() {
			It("Should generate no files with no old resolutions", func() {
				...
				`
				resp := MustSucceed(generate(ctx, "", schema, loader, p))
				Expect(resp.Files).To(BeEmpty())
			})
		})
		...
		Context("version discipline", func() {
			It("Should error when the shape changed without a version bump", func() {
				...
				`
				Expect(generate(ctx, oldSchema, newSchema, loader, p)).
			...
			It("Should error when @go migrate lacks a @go version", func() {
				...
				`
				Expect(generate(ctx, schema, schema, loader, p)).
					Error().To(MatchError(ContainSubstring(
		...
		Context("marshal directive change", func() {
			It(
				"Should generate a migration when @go marshal omit is added to a field",
				func() {
					oldSchema := `
					@go output "out"
					Key = uuid {
		...
		Context("generic types", func() {
			It("Should emit type-param decoration on a generic autoMigrate", func() {
				...
				Expect(
					autoCopy,
				).To(ContainSubstring("func autoMigrateEntry[Details any]"))
			...
			It(
				"Should emit type-param decoration on the Migrate developer template",
				func() {
					oldSchema := `
					@go output "out"
					Key = string {
				...
				`
					...
					Expect(
						transform,
					).To(ContainSubstring("func migrateEntry[Details any]"))
		...
		Context("slice fields", func() {
			It(
				"Should generate loop preamble for arrays of Oracle element types",
				func() {
					oldSchema := `
					@go output "out"
					Key = uuid {
		...
		Context("cast-only structs", func() {
			It(
				"Should use type casts for unchanged structs with only builtins",
				func() {
					oldSchema := `
					@go output "out"
					Key = uuid {
		...
		Context("helpers stay with the definer", func() {
			It("Should leave helpers.go in the outgoing version package", func() {
				...
				resp := MustSucceed(generate(
					ctx, oldSchema, newSchema, customLoader, p,
				))
				Expect(
					filePaths(resp),
				).NotTo(ContainElement("out/versions/v2/helpers.go"))
		...
		Context("multiple entry types", func() {
			It(
				"Should generate a single migrate.go for multiple @go migrate types",
				func() {
					oldSchema := `
					@go output "out"
					KeyA = uuid {
		...
		Context("wrapper visibility", func() {
			It(
				"Should unexport entry and locally consumed wrappers, keeping externally referenced ones exported",
				func() {
					loader.Add("schemas/dep", `
					@go output "dep"
					Item struct {
		...
		Context(
			"regression: non-optional preamble should not add unused import",
			func() {
				It(
					"Should not import the live package for a changed external field",
					func() {
						oldIR := `
					@go output "ir"
					Leaf struct {
				...
				`
						oldTable := MustSucceed(analyze(ctx, oldSchema, loader))
						loader.Add("schemas/ir", newIR)
						newTable := MustSucceed(analyze(ctx, newSchema, loader))
						req := &plugin.Request{
						...
						content := fileContent(
							resp,
							"out/versions/v2/migrate_auto.gen.go",
						)
						Expect(content).To(ContainSubstring("autoMigrateEntry"))
						Expect(
							content,
						).NotTo(ContainSubstring(`"github.com/synnaxlabs/synnax/ir"`))
					},
				)
			},
		)
	...
	Describe("alias-to-slice field migration", func() {
		It("Should migrate alias-to-slice elements instead of casting", func() {
			...
			`
			resp := MustSucceed(generate(ctx, oldSchema, newSchema, loader, p))
			autoCopy := fileContent(resp, "migrate_auto.gen.go")
	...
	Describe("schemadiff.SchemaDiff", func() {
		It("Should detect schemadiff.TypeChanged for added field", func() {
			oldTable := MustSucceed(analyze(ctx, `@go output "out"
				Entry struct { name string }`, loader))
			newTable := MustSucceed(analyze(ctx, `@go output "out"
				Entry struct { name string  age int32 }`, loader))
		...
		It(
			"Should detect schemadiff.TypeDescendantChanged for nested type change",
			func() {
				oldTable := MustSucceed(analyze(ctx, `@go output "out"
				Inner struct { value int32 }
				Outer struct { inner Inner }`, loader))
				newTable := MustSucceed(analyze(ctx, `@go output "out"
				...
				Expect(
					diff["test.Outer"].Kind,
				).To(Equal(schemadiff.TypeDescendantChanged))
			},
		)
		...
		It("Should return empty diff for identical schemas", func() {
			...
			oldTable := MustSucceed(analyze(ctx, schema, loader))
			newTable := MustSucceed(analyze(ctx, schema, loader))
			oldEntry := MustBeOk(oldTable.Get("test.Entry"))
			newEntry := MustBeOk(newTable.Get("test.Entry"))
			Expect(
				schemadiff.SchemaDiff(oldEntry, newEntry, oldTable, newTable),
			).To(BeEmpty())
		})
		...
		It("Should detect field type change", func() {
			oldTable := MustSucceed(analyze(ctx, `@go output "out"
				Entry struct { value int32 }`, loader))
			newTable := MustSucceed(analyze(ctx, `@go output "out"
				Entry struct { value float64 }`, loader))
			oldEntry := MustBeOk(oldTable.Get("test.Entry"))
			newEntry := MustBeOk(newTable.Get("test.Entry"))
			diff := schemadiff.SchemaDiff(oldEntry, newEntry, oldTable, newTable)
			Expect(diff["test.Entry"].Kind).To(Equal(schemadiff.TypeChanged))
		...
		It("Should handle recursive types without infinite loop", func() {
			oldTable := MustSucceed(analyze(ctx, `@go output "out"
				Node struct { value int32  child Node? }`, loader))
			newTable := MustSucceed(analyze(ctx, `@go output "out"
				Node struct { value int32  child Node?  label string }`, loader))
		...
		It("Should propagate changes through alias types", func() {
			oldTable := MustSucceed(analyze(ctx, `@go output "out"
				...
				Items Item[]
				Container struct { items Items }`, loader))
			newTable := MustSucceed(analyze(ctx, `@go output "out"
