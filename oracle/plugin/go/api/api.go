...
import (
	...
	"github.com/synnaxlabs/oracle/plugin/go/internal/match"
...
type filterFieldInfo struct {
	...
	IsScalar bool
	// IsSlice is true when the underlying field is a slice. Slice filters
	// use a singular-form Match function with contains-one semantics.
	IsSlice bool
	...
	HasIndex bool
	// MatchFuncName is the canonical service-package Match function name
	// for this field, computed via the shared match.Func convention.
...
func extractFilterNodeInfo(
	...
	for _, field := range allFields {
		...
		if hasFilter {
			...
			fi.IsScalar = primitive == "bool" ||
				plugindomain.HasExprFromField(field, "filter", "scalar")
			fi.IsSlice = field.Type.Name == "Array" && len(field.Type.TypeArgs) == 1
			fi.MatchFuncName = match.Func(match.Field{
				GoName:   goFieldName,
				IsKey:    plugindomain.HasDomainFromField(field, "key"),
				IsBool:   primitive == "bool",
				IsSlice:  fi.IsSlice,
				IsScalar: fi.IsScalar,
			})
	...
	fields = append(fields, goVirtualFilters(typ)...)
...
func goVirtualFilters(typ resolution.Type) []filterFieldInfo {
	...
	for _, expr := range d.Expressions {
		...
		goFieldName := naming.ToPascalCase(expr.Name)
		fi := classifyFilterField(primitive, primitive, goFieldName, resolution.Field{}, nil)
		fi.FieldName = expr.Name
		fi.GoName = goFieldName
		fi.GoType = primitive
		fi.IsScalar = primitive == "bool"
		fi.MatchFuncName = match.Func(match.Field{
...
var templateFuncs = template.FuncMap{
	...
	"pluralize": match.PluralizeDistinct,
}
...
{{- if eq $f.FilterKind "bool"}}
		if n.{{$f.GoName}}.Eq != nil {
			filters = append(filters, {{$node.ServicePkg}}.{{$f.MatchFuncName}}(*n.{{$f.GoName}}.Eq))
		}
...
{{- if $f.IsSlice}}
			inFilters := make([]{{$node.ServicePkg}}.Filter, len(n.{{$f.GoName}}.In))
			for i, v := range n.{{$f.GoName}}.In {
				inFilters[i] = {{$node.ServicePkg}}.{{$f.MatchFuncName}}(v)
			}
			filters = append(filters, {{$node.ServicePkg}}.Or(inFilters...))
{{- else if $f.IsScalar}}
			filters = append(filters, {{$node.ServicePkg}}.{{$f.MatchFuncName}}(n.{{$f.GoName}}.In[0]))
{{- else}}
			filters = append(filters, {{$node.ServicePkg}}.{{$f.MatchFuncName}}(n.{{$f.GoName}}.In...))
{{- end}}
		}
...
{{- if $f.IsSlice}}
				for _, v := range e.{{$f.GoName}} {
					if strings.Contains({{if $f.NeedsCast}}{{$f.CastType}}(v){{else}}v{{end}}, c) {
						return true, nil
					}
				}
				return false, nil
...
{{- if $f.IsSlice}}
					for _, v := range e.{{$f.GoName}} {
						if rx.MatchString({{if $f.NeedsCast}}{{$f.CastType}}(v){{else}}v{{end}}) {
							return true, nil
						}
					}
					return false, nil
...
{{- else if eq $f.FilterKind "numeric"}}
		if n.{{$f.GoName}}.Eq != nil {
			filters = append(filters, {{$node.ServicePkg}}.{{$f.MatchFuncName}}({{if $f.NeedsCast}}{{$node.ServicePkg}}.{{$f.GoType}}({{end}}*n.{{$f.GoName}}.Eq{{if $f.NeedsCast}}){{end}}))
		} else if len(n.{{$f.GoName}}.In) > 0 {
{{- if $f.IsSlice}}
			inFilters := make([]{{$node.ServicePkg}}.Filter, len(n.{{$f.GoName}}.In))
			for i, v := range n.{{$f.GoName}}.In {
				inFilters[i] = {{$node.ServicePkg}}.{{$f.MatchFuncName}}({{if $f.NeedsCast}}{{$node.ServicePkg}}.{{$f.GoType}}(v){{else}}v{{end}})
			}
			filters = append(filters, {{$node.ServicePkg}}.Or(inFilters...))
{{- else if $f.NeedsCast}}
			casted := make([]{{$node.ServicePkg}}.{{$f.GoType}}, len(n.{{$f.GoName}}.In))
			for i, v := range n.{{$f.GoName}}.In {
				casted[i] = {{$node.ServicePkg}}.{{$f.GoType}}(v)
			}
{{- if $f.IsScalar}}
			filters = append(filters, {{$node.ServicePkg}}.{{$f.MatchFuncName}}(casted[0]))
{{- else}}
			filters = append(filters, {{$node.ServicePkg}}.{{$f.MatchFuncName}}(casted...))
{{- end}}
{{- else if $f.IsScalar}}
			filters = append(filters, {{$node.ServicePkg}}.{{$f.MatchFuncName}}(n.{{$f.GoName}}.In[0]))
{{- else}}
