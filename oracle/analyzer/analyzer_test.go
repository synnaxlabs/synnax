...
var _ = Describe("Analyzer", func() {
	...
	Describe("AnalyzeSource", func() {
		...
		It("Should collect an object-literal struct default", func(ctx SpecContext) {
			...
			Expect(val.ObjectFields).To(HaveLen(2))
			Expect(val.ObjectFields[0].Name).To(Equal("format"))
			Expect(val.ObjectFields[0].Value.Kind).To(Equal(resolution.ValueKindString))
			Expect(val.ObjectFields[0].Value.StringValue).To(Equal("preciseDate"))
			Expect(val.ObjectFields[1].Name).To(Equal("tz"))
			Expect(val.ObjectFields[1].Value.StringValue).To(Equal("local"))
		})
