...
func (f *fakeFormatter) Format(content []byte, _ string) ([]byte, error) {
	return append([]byte(f.tag+":"), content...), nil
}

var _ = Describe("Registry", func() {
	It("Should pass content through unchanged when no formatter is registered", func() {
		r := format.NewRegistry()
		out := MustSucceed(r.Format([]byte("hello"), "/x/foo.unknown"))
		Expect(string(out)).To(Equal("hello"))
	})

	It("Should route by extension and chain in registration order", func() {
		r := format.NewRegistry()
		r.Register(".go", &fakeFormatter{tag: "first"})
		r.Register(".go", &fakeFormatter{tag: "second"})
		out := MustSucceed(r.Format([]byte("body"), "/x/main.go"))
		Expect(string(out)).To(Equal("second:first:body"))
	})

	It("Should ignore other extensions", func() {
		r := format.NewRegistry()
		r.Register(".go", &fakeFormatter{tag: "go"})
		out := MustSucceed(r.Format([]byte("ts source"), "/x/main.ts"))
	...
	Describe("FormatBatch", func() {
		It("Should preserve order of input files", func() {
			r := format.NewRegistry()
			...
			out := MustSucceed(r.FormatBatch(context.Background(), files, 4))
		...
		It("Should pass through files with unknown extensions", func() {
			r := format.NewRegistry()
			files := []format.File{{Path: "/x/a.unknown", Content: []byte("body")}}
			out := MustSucceed(r.FormatBatch(context.Background(), files, 1))
