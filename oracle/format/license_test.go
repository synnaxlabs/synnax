...
var _ = Describe("License", func() {
	...
	It("Should prepend a slash header to a Go file with no header", func() {
		l := MustSucceed(format.NewLicense(repoRoot))
		out := MustSucceed(l.Format([]byte("package x\n"), "/abs/foo.go"))
	...
	It("Should prepend a hash header to a Python file", func() {
		l := MustSucceed(format.NewLicense(repoRoot))
		out := MustSucceed(l.Format([]byte("def foo(): pass\n"), "/abs/foo.py"))
		Expect(string(out)).To(HavePrefix("#  Copyright"))
		Expect(strings.Contains(string(out), "#  Governed by BSL.")).To(BeTrue())
	})

	It("Should prepend a block header to a CSS file", func() {
		l := MustSucceed(format.NewLicense(repoRoot))
		out := MustSucceed(l.Format([]byte(".cls { color: red; }\n"), "/abs/foo.css"))
		Expect(string(out)).To(HavePrefix("/*\n * Copyright"))
		Expect(string(out)).To(ContainSubstring(" */\n\n.cls"))
	})

	It("Should be idempotent on a file with the correct header already", func() {
		l := MustSucceed(format.NewLicense(repoRoot))
		first := MustSucceed(l.Format([]byte("package x\n"), "/abs/foo.go"))
		second := MustSucceed(l.Format(first, "/abs/foo.go"))
		Expect(string(first)).To(Equal(string(second)))
	})

	It("Should replace a stale-year header", func() {
		l := MustSucceed(format.NewLicense(repoRoot))
		stale := []byte("// Copyright 1999 Synnax Labs, Inc.\n//\n// Governed by BSL.\n\npackage x\n")
		out := MustSucceed(l.Format(stale, "/abs/foo.go"))
		Expect(string(out)).ToNot(ContainSubstring("1999"))
		Expect(string(out)).To(HaveSuffix("package x\n"))
	})

	It("Should leave files with non-Synnax leading comments alone", func() {
		l := MustSucceed(format.NewLicense(repoRoot))
		raw := []byte("// some unrelated comment\npackage x\n")
		out := MustSucceed(l.Format(raw, "/abs/foo.go"))
		Expect(string(out)).To(HavePrefix("// Copyright"))
		Expect(string(out)).To(ContainSubstring("// some unrelated comment"))
	})

	It("Should pass through unsupported extensions unchanged", func() {
		l := MustSucceed(format.NewLicense(repoRoot))
		out := MustSucceed(l.Format([]byte("body"), "/abs/foo.unknown"))
