...
import { useExport } from "@/project/imex";
import { selectOptionalActiveKey } from "@/project/selectors";
import { activeate, maybeRename } from "@/project/session/slice";
...
const handleSelect: Ontology.HandleSelect = ({
  ...
  handleError(async () => {
    const { layout: _layout, ...proj } = await client.projects.retrieve(
      selection[0].id.key,
    );
    store.dispatch(activeate(proj));
