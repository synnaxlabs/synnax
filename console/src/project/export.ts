...
export const export_ = (
  ...
  handleError(async () => {
    ...
    if (active?.key === key || key == null) {
      ...
      // Non-active projects can't be exported yet under the new ownership
      // model: project layout now lives across its Panels rather than as a
      // single blob on the Project row. The export feature will be rewritten
      // to iterate over the project's panels via Flux.
      if (client == null) throw new DisconnectedError();
      const p = await client.projects.retrieve(key);
      name = p.name;
      throw new Error(
        `Exporting an inactive project (${name}) is not supported in this build. ` +
