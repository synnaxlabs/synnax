...
import { stage } from "@synnaxlabs/client";
...
export const HAUL_TYPE = "stage-element";
...
export const Loaded: Layout.Renderer = ({ layoutKey, visible }) => {
  ...
  const stage = useSelect(layoutKey);
  ...
  const viewportRef = useSyncedRef(stage.viewport);
  ...
  if (!canBeEditable && stage.editable)
  ...
  const canEditStage = useSelectHasPermission() && !stage.snapshot;
  ...
  return (
    ...
    >
      <Control.Controller
        ...
        acquireTrigger={stage.controlAcquireTrigger}
      ...
      >
        <Diagram.Diagram
          ...
          // effects on the stage sizing and ensures that we position all the lines
          // in the correct place.
          viewport={{ ...stage.viewport, zoom: 1 }}
          ...
          triggers={triggers}
          onDoubleClick={handleDoubleClick}
          fitViewOnResize={stage.fitViewOnResize}
        ...
        >
          ...
          <Diagram.Controls>
            {canEditStage && (
              <Diagram.ToggleEditControl disabled={stage.control === "acquired"} />
            )}
            <Diagram.FitViewControl />
            {!stage.snapshot && (
              <Button.ToggleIcon
                value={stage.control === "acquired"}
...
export const Stage: Layout.Renderer = ({ layoutKey, ...rest }) => {
  const loaded = useLoadRemote({
    ...
    fetcher: async (client, layoutKey) => {
      const { key, data } = await client.workspaces.stage.retrieve(layoutKey);
...
export const LAYOUT_TYPE = "stage";
...
export const create =
  ...
  ({ dispatch, store }) => {
    const canEditStage = selectHasPermission(store.getState());
    const { name = "Stage", location = "mosaic", window, tab, ...rest } = initial;
    if (!canEditStage && tab?.editable) tab.editable = false;
    const key = stage.keyZ.safeParse(initial.key).data ?? uuid();
