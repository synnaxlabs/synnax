...
import { stage, UnexpectedError } from "@synnaxlabs/client";
...
export const select = (state: StoreState, key: string): State =>
  selectSliceState(state).stages[key];
...
export const selectSelectedElementDigests = (
  ...
  const stage = select(state, layoutKey);
  return stage.nodes
...
export const selectSelectedElementsProps = (
  ...
  const nodes: ElementInfo[] = stage.nodes
    .filter((node) => node.selected)
    .map((node) => ({
      key: node.key,
      type: "node",
      node,
      props: stage.props[node.key],
    }));
  const edges: ElementInfo[] = stage.edges
...
export const selectHasPermission = (state: Permissions.StoreState): boolean =>
  Permissions.selectCanUseType(state, stage.ONTOLOGY_TYPE);
