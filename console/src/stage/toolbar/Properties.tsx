...
import { setElementProps, setNodePositions } from "@/stage/slice";
    ...
    if (digests.length === 0)
      return (
        <Status.Text.Centered variant="disabled" hideIcon>
          Select a Stage element to configure its properties.
...
const IndividualProperties = ({
  ...
  const C = Stage.SYMBOLS[props.key];
      ...
      .map((el) => {
        ...
        try {
          ...
          const handles = Array.from(handleEls).map((el) => {
            ...
            if (match == null)
              throw new Error(`[stage] - cannot find handle orientation`);
