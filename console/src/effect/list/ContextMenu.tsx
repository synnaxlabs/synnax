...
export const ContextMenu = ({ keys, getItem }: ContextMenuProps) => {
  ...
  const { update: del } = Effect.useDelete.useDirect({ params: {} });

  const handleSelect: PMenu.MenuProps["onChange"] = {
    rename: () => {
      rename({ initialValue: effects[0].name }, { icon: "Effect", name: "Effect.Rename" })
        .then((renamed) => {
    ...
    delete: () => {
      confirm(effects)
        .then((confirmed) => {
          if (confirmed) del(effects.map((e) => e.key));
        })
        .catch(console.error);
    },
  };
