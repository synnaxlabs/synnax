...
describe("Schematic Slice", () => {
  ...
  describe("element properties management", () => {
    const schematicKey = "test-schematic";
    ...
    it("should update label properties on a node", () => {
      const nodeKey = "valve-1";
      store.dispatch(
        actions.addElement({
          key: schematicKey,
          ...
          props: {
            key: "valve",
            label: { label: "Test Valve", level: "p", maxInlineSize: 150 },
          },
          node: { position: { x: 100, y: 100 } },
        }),
      );
      ...
      store.dispatch(
        actions.setElementProps({
          ...
          key: nodeKey,
          props: { label: { label: "Test Valve", level: "h4", maxInlineSize: 150 } },
        }),
      );

      const state = store.getState()[SLICE_NAME];
      const schematic = state.schematics[schematicKey];
      expect(schematic.props[nodeKey].label?.level).toBe("h4");
    });

    it("should preserve other label properties when updating one", () => {
      const nodeKey = "valve-1";
      store.dispatch(
        actions.addElement({
          key: schematicKey,
      ...
      // Update only maxInlineSize, preserving other properties
      store.dispatch(
        actions.setElementProps({
          layoutKey: schematicKey,
          key: nodeKey,
      ...
      const state = store.getState()[SLICE_NAME];
      const schematic = state.schematics[schematicKey];
      const label = schematic.props[nodeKey].label;

      expect(label?.maxInlineSize).toBe(200);
      expect(label?.level).toBe("p");
      expect(label?.align).toBe("center");
      expect(label?.direction).toBe("x");
    });

    it("should update multiple nodes with same label property", () => {
      // Add multiple nodes
      store.dispatch(
        actions.addElement({
          key: schematicKey,
          elKey: "valve-1",
          props: { key: "valve", label: { label: "Valve 1", level: "p" } },
          node: { position: { x: 0, y: 0 } },
        }),
      );
      store.dispatch(
        actions.addElement({
          key: schematicKey,
          elKey: "valve-2",
          props: { key: "valve", label: { label: "Valve 2", level: "p" } },
          node: { position: { x: 100, y: 0 } },
        }),
      );

      // Update both to same level (simulating group label update)
      store.dispatch(
        actions.setElementProps({
          layoutKey: schematicKey,
          key: "valve-1",
          props: { label: { label: "Valve 1", level: "h4" } },
        }),
      );
      store.dispatch(
        actions.setElementProps({
          layoutKey: schematicKey,
          key: "valve-2",
          props: { label: { label: "Valve 2", level: "h4" } },
        }),
      );
      ...
      expect(schematic.props["valve-1"].label?.level).toBe("h4");
      expect(schematic.props["valve-2"].label?.level).toBe("h4");
    });

    it("should update node color property", () => {
      const nodeKey = "valve-1";
      store.dispatch(
        actions.addElement({
          key: schematicKey,
      ...
      const state = store.getState()[SLICE_NAME];
      const schematic = state.schematics[schematicKey];
      expect(schematic.props[nodeKey].color).toBe("#00ff00");
    });

    it("should update node orientation property", () => {
      const nodeKey = "valve-1";
      store.dispatch(
        actions.addElement({
          key: schematicKey,
          elKey: nodeKey,
          props: { key: "valve", orientation: "left" },
          node: { position: { x: 100, y: 100 } },
        }),
      );
      ...
      const state = store.getState()[SLICE_NAME];
      const schematic = state.schematics[schematicKey];
      expect(schematic.props[nodeKey].orientation).toBe("right");
    });
  });
