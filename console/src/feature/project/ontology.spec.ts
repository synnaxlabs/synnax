...
describe("project ontology service", () => {
  it("should expose creation, import, export, and link actions", async () => {
    ...
    assertDefined(Project.ONTOLOGY_SERVICE.TreeContextMenu);
    await renderTreeContextMenu(Project.ONTOLOGY_SERVICE.TreeContextMenu, {
      client,
  ...
  it("should create a log inside the project from the context menu", async () => {
    const p = await createProject();
    assertDefined(Project.ONTOLOGY_SERVICE.TreeContextMenu);
    const { store } = await renderTreeContextMenu(
      Project.ONTOLOGY_SERVICE.TreeContextMenu,
      {
        client,
        resources: [projectResource(p.key, p.name)],
        store: await createStoreWithActive(p.key),
  ...
  it("should select the project and install its layout on select", async () => {
    ...
    Project.ONTOLOGY_SERVICE.onSelect({
      ...createBaseProps({
        client,
        store,
        overrides: { handleError: createExecutingHandleError() },
      }),
      selection: [projectResource(p.key, p.name)],
    });
    await waitFor(() =>
      expect(Session.Project.selectOptionalSelected(store.getState())).toBe(p.key),
    );
  ...
  it("should only accept mosaic-compatible children on drop", () => {
    const { canDrop } = Project.ONTOLOGY_SERVICE;
