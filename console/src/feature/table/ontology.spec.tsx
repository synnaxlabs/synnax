...
import { ontology, table as clientTable, type table } from "@synnaxlabs/client";
...
const renderMenu = async ({ tables, overrides, withCluster = false }: SetupArgs) => {
  ...
  const Menu = Table.ONTOLOGY_SERVICE.TreeContextMenu;
...
describe("table/ontology", () => {
  ...
  describe("haulItems", () => {
    it("returns a mosaic tab haul item for the resource", () => {
      ...
      const items = Table.ONTOLOGY_SERVICE.haulItems(createResource(id, "My Table"));
