...
const renderMenu = async (
  ...
  assertDefined(User.ONTOLOGY_SERVICE.TreeContextMenu);
  await renderTreeContextMenu(User.ONTOLOGY_SERVICE.TreeContextMenu, {
    client,
    resources,
  });
};
...
describe("user ontology service", () => {
  ...
  it("should build user haul items from the resource payload", () => {
    ...
    const items = User.ONTOLOGY_SERVICE.haulItems(
      createResource(user.ontologyID("u1"), "u", data),
    );
    expect(items).toHaveLength(1);
    expect(items[0].key).toBe("u1");
    expect(
      User.ONTOLOGY_SERVICE.haulItems(createResource(user.ontologyID("u2"), "u2")),
    ).toHaveLength(0);
  });
});
