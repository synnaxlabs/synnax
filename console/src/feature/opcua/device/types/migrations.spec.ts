...
import * as v0 from "@/feature/opcua/device/types/v0";
import * as v1 from "@/feature/opcua/device/types/v1";

interface TestCase {
  description: string;
  input: OPCUA.Device.AnyProperties;
...
describe("migrations", () =>
  TEST_RECORD.forEach(({ description, input, output }) =>
    it(description, () =>
      expect(OPCUA.Device.migrateProperties(input)).toEqual(output),
    ),
  ));
