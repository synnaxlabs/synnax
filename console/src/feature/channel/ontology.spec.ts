...
const retrieveResource = async (key: channelClient.Key): Promise<ontology.Resource> =>
  await client.ontology.retrieve(channelClient.ontologyID(key));

describe("channel/ontology", () => {
  describe("haulItems", () => {
    it("hauls a regular channel as a channel item", async () => {
      const ch = await createChannel();
      const items = Channel.ONTOLOGY_SERVICE.haulItems(
