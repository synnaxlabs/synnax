...
describe("role ontology service", () => {
  it("should expose rename and delete for a non-internal role", async () => {
    ...
    assertDefined(Access.Role.ONTOLOGY_SERVICE.TreeContextMenu);
    await renderTreeContextMenu(Access.Role.ONTOLOGY_SERVICE.TreeContextMenu, {
      client,
  ...
  it("should accept only non-root user haul items on drop", () => {
    const { canDrop } = Access.Role.ONTOLOGY_SERVICE;
...
describe("policy ontology service", () => {
  it("should stay hidden in the tree and declare no children", () => {
    expect(Access.Policy.ONTOLOGY_SERVICE.type).toBe("policy");
    expect(
      Access.Policy.ONTOLOGY_SERVICE.visible?.(
        createResource(access.policy.ontologyID("p1"), "p1"),
      ),
    ).toBe(false);
    expect(Access.Policy.ONTOLOGY_SERVICE.hasChildren).toBe(false);
  });
});
