...
describe("Modbus device queries", () => {
  it("should retrieve the device with its typed properties", async () => {
    ...
    const dev = await createModbusDevice(client, { properties: { connection } });
    ...
    await waitFor(() =>
      expect(result.current?.properties.connection.host).toBe("10.0.0.7"),
    );
  });
