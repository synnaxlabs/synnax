...
// expect.poll options for server round-trips that outlive the default async window
// when the full suite loads the shared cluster. Kept under the global 15s test
// timeout so a slow poll fails with the assertion, not the test-level timeout.
...
describe("Schematic toolbar Properties", () => {
  ...
  describe("multiple selection", () => {
    ...
    it("applies label wrap width to every selected symbol", async () => {
      ...
      await expect
        .poll(async () => (await retrieveConfig(key, "n1")).label, POLL)
