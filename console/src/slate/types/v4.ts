...
import * as v3 from "@/slate/types/v3";

// This file is mostly pointless, as the state is exactly the same as the previous
// version. But, customers have existing slates and slices with the 'version' key
...
export const sliceStateZ = v3.sliceStateZ
  .omit({ version: true, slates: true })
  .extend({ version: z.literal(VERSION), slates: z.record(z.string(), stateZ) });

export interface SliceState extends Omit<v3.SliceState, "version" | "slates"> {
...
export const sliceMigration = migrate.createMigration<v3.SliceState, SliceState>({
  ...
  migrate: (sliceState) => ({
    ...sliceState,
    slates: Object.fromEntries(
      Object.entries(sliceState.slates).map(([key, state]) => [
        key,
