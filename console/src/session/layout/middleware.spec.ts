...
// The remaining window state is the same as the main window's initial state.
const window = (key: string): Drift.WindowState => ({
  ...Drift.ZERO_SLICE_STATE.windows[MAIN_WINDOW],
  key,
});

const createMainDrift = (
...
const createChildDrift = (): Drift.SliceState => ({
  ...Drift.ZERO_SLICE_STATE,
  label: "child",
...
const createStore = (layout: Layout.SliceState, drift: Drift.SliceState) => {
  const dispatch = vi.fn();
  const store: MiddlewareAPI<Dispatch<PayloadAction<unknown>>, StoreState> = {
    getState: () => ({
...
describe("layout middleware", () => {
  describe("effectMiddleware", () => {
    it("should invoke effects only for declared action types", () => {
      ...
      const { store } = createStore(createLayoutState(), createMainDrift());
      const next = vi.fn((a: unknown) => a);
      const dispatch = mw(store)(next);
  ...
  describe("createWindowOnPlaceEffect", () => {
    it("should create a window when placing a window layout from the main window", () => {
      const { store, dispatch } = createStore(createLayoutState(), createMainDrift());
    ...
    it("should not create a window when dispatched from a non-main window", () => {
      const { store, dispatch } = createStore(createLayoutState(), createChildDrift());
      Layout.createWindowOnPlaceEffect({
        store,
        action: Layout.place(windowLayout("win-1")),
      });
  ...
  describe("closeWindowOnRemoveEffect", () => {
    it("should close windows for removed window-located layouts", () => {
      const layout = createLayoutState({ "win-1": windowLayout("win-1") });
      const { store, dispatch } = createStore(layout, createMainDrift());
    ...
    it("should not close a window for a mosaic-located layout", () => {
      const layout = createLayoutState({ "m-1": mosaicLayout("m-1", MAIN_WINDOW) });
      const { store, dispatch } = createStore(layout, createMainDrift());
      Layout.closeWindowOnRemoveEffect({
        store,
    ...
    it("should do nothing from a non-main window", () => {
      const layout = createLayoutState({ "win-1": windowLayout("win-1") });
      const { store, dispatch } = createStore(layout, createChildDrift());
      Layout.closeWindowOnRemoveEffect({
        store,
        action: Layout.remove({ keys: ["win-1"] }),
      });
  ...
  describe("closeWindowOnEmptyMosaicEffect", () => {
    it("should close a non-main window whose mosaic is empty", () => {
      const layout = createLayoutState({}, { "mosaic-2": emptyMosaic() });
      const { store, dispatch } = createStore(
        layout,
        createMainDrift({ "mosaic-2": window("mosaic-2") }),
      );
    ...
    it("should not close a window whose mosaic still has tabs", () => {
      const layout = createLayoutState({}, { "mosaic-2": filledMosaic() });
      const { store, dispatch } = createStore(
        layout,
        createMainDrift({ "mosaic-2": window("mosaic-2") }),
      );
    ...
    it("should close orphaned mosaic windows that no longer have a mosaic", () => {
      const { store, dispatch } = createStore(
        createLayoutState(),
        createMainDrift({ "mosaicWindow-9": window("mosaicWindow-9") }),
      );
      Layout.closeWindowOnEmptyMosaicEffect({
        store,
        action: { type: "layout/remove", payload: { keys: [] } },
