...
export const Item = ({ id, resource, icon: _, ...rest }: Ontology.TreeItemProps) => {
  ...
  const isLegacy = res?.requires.length > 0;
  ...
  return (
    <Tooltip.Dialog>
      <Text.Text>
        Uses legacy-based lua calculation syntax and couldn't be automatically migrated. It will not
        work in future releases. Please update the calculation manually.
      </Text.Text>
      {content}
    </Tooltip.Dialog>
  );
