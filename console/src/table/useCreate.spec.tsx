...
describe("useCreate", () => {
  ...
  describe("project resolution", () => {
    it("prefers the prop project over the active project", async () => {
      ...
      await act(async () => {
        result.current({ name: "ProvidedProj" });
      });
      const placedKey = await waitForPlacedLayout(store);
      const retrieved = await client.tables.retrieve({ key: placedKey });
      expect(retrieved.name).toEqual("ProvidedProj");
      expect(store.getState()[Project.SLICE_NAME].active?.key).toEqual(projectB.key);
    });
    ...
    it("falls back to the active project when no prop is given", async () => {
      ...
      await act(async () => {
        result.current({ name: "ActiveProj" });
      });
      const placedKey = await waitForPlacedLayout(store);
      const retrieved = await client.tables.retrieve({ key: placedKey });
      expect(retrieved.name).toEqual("ActiveProj");
      expect(store.getState()[Project.SLICE_NAME].active?.key).toEqual(projectA.key);
    });
    ...
    it("creates a project-less table when neither prop nor active project is set", async () => {
      ...
      expect(store.getState()[Project.SLICE_NAME].active).toBeNull();
    });
  });
  ...
  describe("project switching", () => {
    it("does not flip the active project when the table is created in the active one", async () => {
      const { wrapper, store } = await buildHarness({ activeProject: projectA });
      const beforeActive = store.getState()[Project.SLICE_NAME].active;
      const { result } = renderHook(() => useCreate({ project: projectA.key }), {
        wrapper,
      });
      await act(async () => {
        result.current({ name: "SameProj" });
      });
      await waitForPlacedLayout(store);
      expect(store.getState()[Project.SLICE_NAME].active).toBe(beforeActive);
    });
  });
});
