...
const StaticListItem = (props: List.ItemProps<string>): ReactElement | null => {
  ...
  const spec = List.useItem<string, Arc.Spec>(itemKey);
...
export const StaticSymbolList = ({
  groupKey,
  onSelect,
}: SymbolsProps): ReactElement => {
  const symbols = useMemo(() => {
    const g = Arc.GROUPS.find((g) => g.key === groupKey);
    return Object.values(Arc.REGISTRY).filter((s) => g?.symbols.includes(s.key));
  }, [groupKey]);
  const { data, getItem } = List.useStaticData<string, Arc.Spec>({
    data: symbols,
  });
  return (
    <Select.Frame<string, Arc.Spec>
      data={data}
      getItem={getItem}
      value={undefined}
      allowNone
      onChange={onSelect}
    >
      <List.Items x className={CSS.BE("arc", "symbols", "group")} wrap>
        {staticListItem}
      </List.Items>
    </Select.Frame>
  );
...
const groupListItem = Component.renderProp((props: List.ItemProps<string>) => {
  const { itemKey } = props;
  const group = List.useItem<string, Arc.Group>(itemKey);
...
const GroupList = ({ value, onChange }: GroupListProps) => {
  const { data, getItem } = List.useStaticData<string, Arc.Group>({
    data: Arc.GROUPS,
  });
  return (
    <Select.Frame<string, Arc.Group>
      ...
      onChange={onChange}
    >
      <List.Items<string, Arc.Group> x gap="small">
        {groupListItem}
      </List.Items>
    </Select.Frame>
  );
...
export const Symbols = ({ layoutKey }: { layoutKey: string }): ReactElement => {
  ...
  return (
    <Flex.Box y empty full className={CSS.BE("arc", "symbols")}>
      <Flex.Box x sharp className={CSS.BE("arc", "symbols", "group", "list")}>
        <GroupList value={selectedGroup} onChange={setSelectedGroup} />
      </Flex.Box>
      <StaticSymbolList groupKey={selectedGroup} onSelect={addSymbol} />
    </Flex.Box>
  );
};
