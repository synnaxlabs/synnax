...
const sequence = (...snapshots: ClusterServices.Snapshot[]) => {
  let i = 0;
...
describe("connectToCluster", () => {
  it("should throw if the cluster is unknown", async () => {
    ...
    await expect(
      ClusterServices.connectToCluster("missing", {
  ...
  it("should return the managed client when already active and connected", async () => {
    const setActive = vi.fn();
    const active = client();
    const result = await ClusterServices.connectToCluster("a", {
  ...
  it("should switch clusters and resolve once the new client connects", async () => {
    ...
    const prior = client();
    const next = client();
    const result = await ClusterServices.connectToCluster("b", {
  ...
  it("should throw when the target connection fails", async () => {
    const active = client();
    await expect(
      ClusterServices.connectToCluster("a", {
        getState: () => makeState(["a"], "a"),
        getSnapshot: sequence(
