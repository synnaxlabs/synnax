...
import { useMemoSelect } from "@/hooks/selectors";
...
describe("useMemoSelect", () => {
  ...
  it("keeps a constructed object stable until its inputs change", () => {
    const store = makeStore();
    const { result } = renderHook(
      () => useMemoSelect((s: TestState) => ({ doubled: s.tracked.value * 2 }), []),
      { wrapper: wrapperFor(store) },
    );
    const first = result.current;
  ...
  it("tracks only the accessed entry, not its siblings", () => {
    const store = makeStore();
    const { result } = renderHook(
      () => useMemoSelect((s: TestState) => s.items.a, []),
      { wrapper: wrapperFor(store) },
    );
    const first = result.current;
    act(() => {
  ...
  it("re-points the selector when its dependencies change", () => {
    ...
    const { result, rerender } = renderHook(
      ({ key }: { key: string }) =>
        useMemoSelect((s: TestState) => s.items[key], [key]),
