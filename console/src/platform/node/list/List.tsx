...
import { Item } from "@/platform/node/list/Item";
import { useConnectModal } from "@/platform/node/useConnectModal";
...
export const List = ({ value, onChange, ...rest }: ListProps): ReactElement => {
  ...
  const allClusters = Session.Node.useSelectMany().sort((a, b) =>
  ...
  const handleRemove = (key: string): void => {
    ...
    dispatch(Session.Node.remove(key));
  };
      ...
      try {
        ...
        if (state.status === "connected") {
          ...
          if (state.clusterKey && state.clusterKey !== key)
            dispatch(Session.Node.changeKey({ oldKey: key, newKey: state.clusterKey }));
        } else
          addStatus({
            variant: "error",
