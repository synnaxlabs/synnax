...
describe("useSyncClusterKey", () => {
  it("should not change the key when the cluster is not connected", async () => {
    ...
    expect(Session.Node.selectState(store.getState(), "active-1")).toBeDefined();
    expect(Session.Node.selectSelectedKey(store.getState())).toEqual("active-1");
  });
  ...
  it("should not change the key when it already matches the server", async () => {
    ...
    expect(Session.Node.selectSelectedKey(store.getState())).toEqual(serverKey);
    expect(Session.Node.selectState(store.getState(), serverKey)).toBeDefined();
  });
  ...
  it("should adopt the server's cluster key once connected", async () => {
    ...
    expect(Session.Node.selectState(store.getState(), "active-1")).toBeUndefined();
    expect(Session.Node.selectState(store.getState(), serverKey)).toBeDefined();
  });
});
