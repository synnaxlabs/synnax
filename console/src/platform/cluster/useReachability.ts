// Copyright 2026 Synnax Labs, Inc.
//
// Use of this software is governed by the Business Source License included in the file
// licenses/BSL.txt.
//
// As of the Change Date specified in that file, in accordance with the Business Source
// License, use of this software will be governed by the Apache License, Version 2.0,
// included in the file licenses/APL.txt.

import { checkConnection, type connection } from "@synnaxlabs/client";
import { useAsyncEffect } from "@synnaxlabs/pluto";
import { useState } from "react";

export interface ReachabilityTarget {
  host: string;
  port: number | string;
  secure?: boolean;
}

/**
 * One-shot reachability probe against the target Core.
 * @returns null while the probe is pending or the target is absent.
 */
export const useReachability = (
  target?: ReachabilityTarget | null,
): connection.Status | null => {
  const [status, setStatus] = useState<connection.Status | null>(null);
  const { host, port, secure = false } = target ?? {};
  useAsyncEffect(
    async (signal) => {
      setStatus(null);
      if (host == null || port == null) return;
      const next = await checkConnection({
        host,
        port,
        secure,
        retry: { maxRetries: 0 },
      });
      if (!signal.aborted) setStatus(next);
    },
    [host, port, secure],
  );
  return status;
};
